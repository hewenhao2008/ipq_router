...
void print_usage(void)
{
	...
	printf("mosquitto_sub version running on libmosquitto %d.%d.%d.\n\n", major, minor, revision);
...
int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-p") || !strcmp(argv[i], "--port")){
			...
			cfg->verbose = 1;
		//ky
		}else if(!strcmp(argv[i], "--ext_topic_in")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				if(mosquitto_pub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
					fprintf(stderr, "Error: Invalid topic '%s' as publish, does it contain '+' or '#'?\n", argv[i+1]);
					return 1;
				}
				if(mosquitto_sub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
					fprintf(stderr, "Error: Invalid topic '%s' as subscription, are all '+' and '#' wildcards correct?\n", argv[i+1]);
					return 1;
				}
				cfg->ext_topic_in = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--ext_topic_out")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				...
				cfg->ext_topic_out = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--ext_topic_config")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				...
				cfg->ext_topic_config = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--ext_topic_keepalive")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				...
				cfg->ext_topic_keepalive = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--keepalive_user")){
			if(i==argc-1){
				fprintf(stderr, "Error: --keepalive argument given but no interval time specified.\n\n");
				return 1;
			}else{
				if(atoi(argv[i+1])<1 || atoi(argv[i+1])>65535){
					fprintf(stderr, "Error: Invalid keepalive_user given: %d\n", atoi(argv[i+1]));
					return 1;
				}
				cfg->keepalive_user = atoi(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--device_inform")){
			if(i==argc-1){
				fprintf(stderr, "Error: --device_inform argument given but no interval time specified.\n\n");
				return 1;
			}else{
				if(atoi(argv[i+1])<1 || atoi(argv[i+1])>65535){
					fprintf(stderr, "Error: Invalid device_inform given: %d\n", atoi(argv[i+1]));
					return 1;
				}
				cfg->device_inform = atoi(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--loc_topic_in")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				...
				cfg->loc_topic_in = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--loc_topic_out")){
			if(i==argc-1){
				fprintf(stderr, "Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				...
				cfg->loc_topic_out = strdup(argv[i+1]);
				i++;
			}
		}else if(!strcmp(argv[i], "--ext_host")){
			if(i==argc-1){
				fprintf(stderr, "Error: -h argument given but no host specified.\n\n");
				return 1;
			}else{
				cfg->ext_host = strdup(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--ext_port")){
			if(i==argc-1){
				fprintf(stderr, "Error: -p argument given but no port specified.\n\n");
				return 1;
			}else{
				...
				cfg->ext_port = atoi(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--loc_host")){
			if(i==argc-1){
				fprintf(stderr, "Error: -h argument given but no host specified.\n\n");
				return 1;
			}else{
				cfg->loc_host = strdup(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--loc_port")){
			if(i==argc-1){
				fprintf(stderr, "Error: -p argument given but no port specified.\n\n");
				return 1;
			}else{
				...
				cfg->loc_port = atoi(argv[i+1]);
			}
			i++;
		}else if(!strcmp(argv[i], "--result")){
			if(i==argc-1){
				fprintf(stderr, "Error: -p argument given but no result type specified.\n\n");
				return 1;
			}else{
				if(atoi(argv[i+1])!=0 && atoi(argv[i+1])!=1){
					fprintf(stderr, "Error: Invalid result type given: %d\n", atoi(argv[i+1]));
					return 1;
				}
				cfg->test_result = atoi(argv[i+1]);
			}
			i++;
		//end of ky
		}else{
