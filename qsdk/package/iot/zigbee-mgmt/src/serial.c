...
#if 0
int zb_send(int fd, zb_uint8 *buf, zb_uint16 bufLen)
{
	ssize_t n;
...
#if 0
int zb_read_blocking(int fd, zb_uint8 *buf, int len)
{
	int nr;
	
	nr = read(fd, buf, len);	//blocking read.
	if( nr > 0)
	{
		return nr;
	}

	ZB_DBG(">>>	zb_read_blocking error,nr=%d\n", nr);
	return nr;	
}
#endif
