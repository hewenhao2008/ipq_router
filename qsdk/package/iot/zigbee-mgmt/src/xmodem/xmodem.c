...
unsigned short halCommonCrc16( unsigned short prevResult, unsigned char newByte)
{
...
int main(int argc, char ** argv)
{
	...
	while(!feof(fin)) {
		...
		if(oldcpt < cpt) {
			...
			for(i = 0; i < PKT_LEN; i++) {
				csum.checksum = halCommonCrc16(csum.checksum, send_buf[i+HEAD]);
			}
			
			send_buf[PKT_LEN+HEAD] = (csum.checksum>>8)&0xFF;
			send_buf[PKT_LEN+HEAD+1] = csum.checksum & 0xFF;		
		...
		else 
		{
			...
			while(xmodem_running) 
			{
				...
				if( size > 0)	// size is 0 or 1.
				{
					if(recv_buf[0] == 0x15) 
					{  //NAK, retry.
						printf("ERROR: NACK, [retry = %02d]\n", retry);
						break;
					}
					else if(recv_buf[0] == 0x06)  //ACK
					{
					...
					else if(recv_buf[0] == 0x43)
					{
						printf("YZG WARN: received '%c', But we already send data.\n", recv_buf[0]);
					}
					else 
					{
