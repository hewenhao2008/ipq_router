...
$Header: //components/rel/wlanfw_halphy_tools.cnss/1.9.0/target/utf/tcmd_utf/tcmd.c#1 $
...
#endif
static void tcmd_cont_tx_data(wlan_tcmd_dev_t *dev, A_UINT32 dataRate,
                              ...
                              A_UINT32 txType, A_UINT32 shortGuard, A_UINT32 chainMask, A_BOOL disableContinuousTx,
                              A_UINT32 numPkts, A_UINT32 broadcast, A_UCHAR *srcmac, A_UCHAR *dstmac, A_UCHAR *bssid, A_UINT32 numAggregates,A_UINT32 miscFlags);
...
static void
utfTxStartInternal(wlan_tcmd_dev_t *dev, A_UINT32 dataRate, A_UINT32 txPwr,
    A_UINT32 antenna, A_UINT16 pktSz, A_UINT16 txPattern, A_UINT32 PatternLength, A_UCHAR *pPatternData, A_UINT32 aifswait,
    A_BOOL  retries , A_UINT32 txType, A_UINT32 shortGuard, A_UINT32 chainMask, A_BOOL disableContinuousTx,
    A_UINT32 numPkts,
    A_BOOL stopTxAfterDoneNumPkts,A_UINT32 broadcast, A_UCHAR *srcmac, A_UCHAR *dstmac, A_UCHAR *bssid, A_UINT32 numAggregates,A_UINT32 miscFlags);
...
//            A_PRINTF_ALWAYS("clpcpkt mode d\n");
        ...
        case TCMD_CONT_TX_FRAME:
            ...
            if (contTx->numPackets)
            {
                tcmd_cont_tx_data(dev, contTx->dataRate, adjustedTwiceTxPwr,
                    contTx->antenna, contTx->pktSz, contTx->txPattern, contTx->nPattern, contTx->dataPattern, contTx->aifsn, 1, contTx->mode, contTx->shortGuard, contTx->txChain, waitForTxComplete, contTx->numPackets, contTx->broadcast, contTx->txStation, contTx->rxStation, contTx->bssid, contTx->agg,contTx->miscFlags);
            }
            else
            {
                tcmd_cont_tx_data(dev, contTx->dataRate, adjustedTwiceTxPwr,
                    contTx->antenna, contTx->pktSz, contTx->txPattern, contTx->nPattern, contTx->dataPattern, contTx->aifsn, 1, contTx->mode, contTx->shortGuard, contTx->txChain, FALSE, 0, contTx->broadcast, contTx->txStation, contTx->rxStation, contTx->bssid, contTx->agg,contTx->miscFlags);
            }
            break;
    ...
    A_BOOL stopTxAfterDoneNumPkts, A_UINT32 broadcast, A_UCHAR *srcmac, A_UCHAR *dstmac, A_UCHAR *bssid, A_UINT32 numAggregates,A_UINT32 miscFlags)
{
    A_UINT32 rfilt;
    WHAL_TXQ_INFO qInfo;
    A_UINT32  rc_flags, param_value;
    A_UINT32 ttPkts;

    //A_PRINTF("%d, %d, %d, %d, %d, %d\n", dataRate, txPwr, antenna, pktSz, txPattern, PatternLength);
    //A_PRINTF("%d, %d, %d, %d, %d\n", pPatternData[0], aifswait, retries , txType, shortGuard);
    //A_PRINTF("0x%x, %d, %d, %d, %d\n", chainMask, disableContinuousTx,  numPkts, stopTxAfterDoneNumPkts, broadcast);
    //A_PRINTF("%d, %d, %d, %d, 0x%x\n", srcmac[5], dstmac[5], bssid[5], numAggregates, miscFlags);
    ...
    if (miscFlags & TCMD_GO_MASK)
    {
    	goto tx_go;
    }
    if (IS_MAC_NULL(srcmac))
        g_encap_cfg.sa_addr = SA_ADDR;
    else
        g_encap_cfg.sa_addr = srcmac;
    if (IS_MAC_NULL(dstmac))
        g_encap_cfg.da_addr = DA_ADDR;
    else
        g_encap_cfg.da_addr = dstmac;
    if (IS_MAC_NULL(bssid))
        g_encap_cfg.bss_addr = BSS_ADDR;
    else
        g_encap_cfg.bss_addr = bssid;

#if defined(_USE_TX_INT)
    waltest_tx(utf_dev.pdev, &utf_dev, tcmd_tx_complete_handler, NULL);
#endif

    //A_PRINTF("pktSz utfTx %d bc %d\n",pktSz,broadcast);
    g_encap_cfg.pkt_len = pktSz;
    g_encap_cfg.pattern = txPattern;
    g_encap_cfg.pattern_length = PatternLength;
    g_encap_cfg.pattern_data = pPatternData;
    g_encap_cfg.is_broadcast = broadcast ? TRUE : FALSE;
    g_encap_cfg.chain_mask = (A_UINT8) chainMask;
    g_encap_cfg.num_desc = 1;
    g_encap_cfg.num_aggr = 0;
    g_encap_cfg.data_rate = dataRate;
    if (txPattern == PN15_PATTERN)
    {
        g_encap_cfg.num_desc = 4;/*Length of PN15 is larger than MAX buffer, split it to 4 frame*/
    }
    else if (numAggregates > 1)
    {
        g_encap_cfg.num_aggr = numAggregates;
    }
    ...
    {
        param_value = miscFlags & DESC_LDPC_ENA_MASK;
...
//A_PRINTF("LDPC %d\n",param_value);
        ...
        {
//A_PRINTF("Set LDPC\n");
            rc_flags = (wal_rc_get_vdev_rc_flags(g_encap_cfg.vdev) | WHAL_RC_FLAG_LDPC);
        }
        else
        {
//A_PRINTF("Clear LDPC\n");
        ...
        param_value = miscFlags & DESC_STBC_ENA_MASK;
...
//A_PRINTF("STBC %d\n",param_value);
        ...
        {
//A_PRINTF("Set STBC\n");
            rc_flags = (wal_rc_get_vdev_rc_flags(g_encap_cfg.vdev) | WHAL_RC_FLAG_STBC);
        }
        else
        {
//A_PRINTF("Clear STBC\n");
        ...
        param_value = shortGuard;

//A_PRINTF("shortGuard %d\n",param_value);

        if (param_value)
        {
//A_PRINTF("Set SGI\n");
            rc_flags = (wal_rc_get_vdev_rc_flags(g_encap_cfg.vdev) | WHAL_RC_FLAG_SGI);
        }
        else
        {
//A_PRINTF("Clear SGI\n");
...
tx_go:
	...
	if(txType != TCMD_CONT_TX_TX99) // For Target assert issue occurred at Besra and Dakota
#endif	
        whalSetRxFilter(rfilt);
    whalEnableMibCounters(TRUE);
    ...
    if(txType == TCMD_CONT_TX_TX99) // TX99 only
     	setRxDeaf(dataRate);
...
{
    //TCMD_PRINTF("%s\n", __func__);
    utfTxStartInternal(dev, dataRate, txPwr, antenna, pktSz, txPattern, PatternLength, pPatternData, aifswait,
                                retries , txType, shortGuard, chainMask, disableContinuousTx, numPkts, TRUE, broadcast,srcmac,dstmac,bssid,numAggregates,miscFlags);
}
