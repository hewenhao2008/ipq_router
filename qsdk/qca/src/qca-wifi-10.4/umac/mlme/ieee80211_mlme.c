...
static void wlan_mlme_update_radio_priorities(struct ieee80211com *ic)
{
...
void wlan_mlme_connection_up(wlan_if_t vaphandle)
{
    ...
    && !vap_is_psta(vap)
        ...
        if (ic->ic_global_list->num_stavaps_up == 2) {
            wlan_mlme_update_radio_priorities(ic);
        }
...
#endif
            /* Do this only for non-qwrap repeater modes */
...
void wlan_mlme_connection_down(wlan_if_t vaphandle)
{
    ...
    struct ieee80211com           *tmp_ic;
    int                           i;
    u_int8_t max_stavap_up_priority = MAX_RADIO_CNT;
