    ...
    if (name[0] != '\0') {
        ...
        default: // Parent
            ...
            if ((f = fopen(name1, "r")) == NULL) {
                ...
                buff[0] = '\0';
                if(fgets(buff,sizeof(buff)/sizeof(buff[0]),f) != NULL) {
...
static void free_radiolist()
{
    ...
    while (head) {
        ...
        log_msg(WPS_DBG, "Freeing radio_ptr (%p)\n", head);
        free(head);
        head = temp;
    }
    //free(radiohead_ptr);
...
static void print_radiolist()
{
    ...
    temp = radiohead_ptr->enabled_radio_ptr;
    while (temp != NULL) {
        log_msg(WPS_DBG, "%s: p(%p) %s num_sta_vaps(%d)\n", __func__, temp,
...
static struct wps_worklist_t* insert_worklist(enum wps_conn_case conn_case)
{
    ...
    log_msg(WPS_DBG, "%s: p_newwork(%p) p_work(%p)\n", __func__, p_newwork, p_work);
    if (!workhead_ptr) {
        ...
        log_msg(WPS_DBG, "%s: first item returning %p\n", __func__, p_newwork);
        return p_newwork;
    }
    temp = workhead_ptr;
    ...
    log_msg(WPS_DBG, "%s: found(%d) workhead(%p) \n", __func__, found, workhead_ptr);
    return p_newwork;
}
...
static void print_worklist()
{
    ...
    temp = workhead_ptr;
    log_msg(WPS_DBG, "%s: workhead_p(%p)\n", __func__, temp);
    while (temp) {
        p_work = temp->work_list_ptr;
        log_msg(WPS_DBG, "%s: use-case(%d) p_work(%p)\n", __func__,
            temp->use_case, p_work);
        if (p_work)
        {
            cfg = p_work->cfg_ptr;
            log_msg(WPS_DBG, "%s: use_case(%d) p_work(%p) cfg(%p)\n", __func__,
                temp->use_case, p_work, cfg);
            while(cfg) {
                log_msg(WPS_DBG, "%s: cfg(%p) cfg->radio(%s)\n", __func__, cfg,
                    cfg->radio);
                cfg = cfg->next_ptr;
            }
        }
...
static struct wps_worklist_t* get_current_usecase(int sock_fd)
{
    ...
    while (pcur_work) {
        if (pcur_work->use_case == cur_use_case) {
            ...
            if (p_work) { // for debug
                log_msg(WPS_DBG, "cur use_case(%d) pcur_work(%p) p_work(%p) cfg(%p)\n",
...
static void free_usecase_worklist()
{
    ...
    log_msg(WPS_DBG, "Freeing workhead_p(%p)\n", workhead_ptr);
    while (workhead_ptr) {
        p_work = workhead_ptr->work_list_ptr;
        log_msg(WPS_DBG, "Freeing use-case(%d) p_work(%p)\n",
                        workhead_ptr->use_case,p_work);
        if (p_work)
        {
            head_cfg = p_work->cfg_ptr;
            while (head_cfg) {
                log_msg(WPS_DBG, "Freeing cfg(%p) cfg->radio(%s)\n", head_cfg,
                    head_cfg->radio);
                cfg = head_cfg->next_ptr;
                free(head_cfg);
                head_cfg = cfg;
            }
            log_msg(WPS_DBG, "Freeing p_work(%p)\n", p_work);
            free(p_work);
            p_work = NULL;
        }
        temp = workhead_ptr;
        ...
        if (wps_use_cases_start) {
            ...
            if ((found_case != prev_use_case) || (wps_use_cases_end)) {
                ...
                if (((p_worklist) && (head != NULL)) || (wps_use_cases_end)) {
                    log_msg(WPS_INF, "calling apply_options: p_worklist(%p), head(%p)\n",
                        p_worklist, head);
                    ...
                    if (p_worklist->work_list_ptr) {
                        ...
                        log_msg(WPS_DBG, "p_worklist->worklist_p(%p) head(%p)\n",
                ...
                if (wps_use_cases_end) {
                    ...
                    log_msg(WPS_DBG, "cur list got(%p)\n", p_curworklist);
                    break;
                }
            }
