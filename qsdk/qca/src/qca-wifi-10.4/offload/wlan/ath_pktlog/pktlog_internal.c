...
static void pktlog_txcap_add_radiotap_header(ol_pktlog_dev_t *pl_dev,  struct ath_pktlog_siginfo *siginfo, struct ath_pktlog_tx_capture *txbuf)
{
...
process_rx_info_remote(void *dev, qdf_nbuf_t amsdu)
{
    ...
    if(((struct ol_txrx_pdev_t *) pdev)->gid_flag)
    {
        ...
        qdf_mem_copy(rxstat_log.rx_desc,(u_int8_t *)&(((struct ol_txrx_pdev_t *) pdev)->gid_mgmt.member_status[0]) ,pl_hdr.size);
    }
    msdu = amsdu;
    while (msdu) {
        /* old code:
        ...
        case PKTLOG_TYPE_TX_FW_GENERATED2:
            {
                ...
                if (txrx_pdev->htt_pdev->htt_process_nondata_tx_frames) {
                    qdf_nbuf_t nbuf;
                    nbuf = (qdf_nbuf_t)(data);
                    qdf_nbuf_pull_head(data, sizeof(ath_pktlog_hdr_t));
                    txrx_pdev->htt_pdev->htt_process_nondata_tx_frames(nbuf);
                    rc = A_CONSUMED;
                }
            }
            break;
