...
wrapd_get_we_version(struct wrap_wifi_iface *aptr, char *ifname)
{
    ...
    wrapd_printf("SIOCGIWRANGE: Wireless extension version=%d\n", range->we_version_compiled);
...
static void wrapd_mpsta_connect(struct wrap_wifi_iface *parent)
{
    ...
    ifr.ifr_data = (caddr_t) &extended_cmd;
    extended_cmd.data = (caddr_t)&max_priority_radio;
    if ((ret = ioctl(wrapg->ioctl_sock, SIOCGATHEXTENDED, &ifr)) != 0) {
    ...
    is_max_priority_radio = ((os_strcmp(parent->wifi_ifname, max_priority_radio) == 0) ? 1 : 0);

    if( is_max_priority_radio && ((os_strcmp(max_priority_radio, wrapg->ethernet_extender)) != 0))
    {
        update_wired_client_parent(max_priority_radio);
    }

    if(is_max_priority_radio)
    {
        os_memset(&ifr, 0, sizeof(ifr));
        strncpy(ifr.ifr_name, wrapg->brname, IFNAMSIZ);
        ifr.ifr_hwaddr.sa_family = ARPHRD_ETHER;
        os_memcpy(ifr.ifr_hwaddr.sa_data, parent->mpsta_mac_addr, IEEE80211_ADDR_LEN);
        /* Get the MAC address */
...
static void wrapd_mpsta_disconnect(struct wrap_wifi_iface *parent)
{
    ...
    char max_priority_radio[IFNAMSIZ] = {0};
    struct wrap_wifi_iface *prim = NULL, *other = NULL;
    struct ctrl_conn *hapd;
    ...
    for(other= wrapg->wrap_wifi_list; other ; other=other->next )
    {
        if ((os_strcmp(max_priority_radio, other->wifi_ifname) == 0) && other->mpsta_conn)
        {
            prim = other;
            break;
        }
    ...
    if((os_strcmp(max_priority_radio, wrapg->ethernet_extender)) != 0)
    {
        update_wired_client_parent(max_priority_radio);
    }
    ...
    strncpy(ifr.ifr_name, wrapg->brname, IFNAMSIZ);
    ifr.ifr_hwaddr.sa_family = ARPHRD_ETHER;
    os_memcpy(ifr.ifr_hwaddr.sa_data, prim->mpsta_mac_addr, IEEE80211_ADDR_LEN);
    /* Get the MAC address */
    if(ioctl(wrapg->ioctl_sock, SIOCSIFHWADDR, &ifr) < 0)
    {
        wrapd_printf("SIOCSIFHWADDR - Unable to set bridge hw address\n");
    ...
    else
    {
        ...
        while (i < WRAP_MAX_PSTA_NUM) {
            if((parent->psta[i].added)) {
                ...
                wrapd_printf("Creating PSTA on %s flags:%x\n", prim->wifi_ifname,flags);
                retv = wrapd_psta_add(prim, prim->wifi_ifname, parent->psta[i].child, addr, flags);
