	...
	for vif in $vifs; do
		...
		case "$mode" in
			wrap|ap)
                ...
                [ -z "$extender_device" ] || {
                   wrapd -S -g $wrapd_global_ctrl_iface ap_add $extender_device $vifname $conf
                }
			;;
		esac
	done
	...
	for vif in $vifs; do
		...
		case "$mode" in
			sta)
                ...
                wrapd -S -g $wrapd_global_ctrl_iface sta_add $device $vifname $tmp_conf_file
			;;
		esac
	done
}
...
qwrap_setup() {
    ...
    wrapd_global_ctrl_iface="/var/run/wrapdglobal"
    config_get always_primary qcawifi always_primary 0
    ...
    if [ "$always_primary" -ne 0 ]; then
        aprim="-a"
    fi
	wrapd_pid="/var/run/wrapd-global.pid"
	wrapd ${iso} -P $wrapd_pid -g $wrapd_global_ctrl_iface -H $hostapd_global_ctrl_iface -w $wpa_supplicant_global_ctrl_iface $bridge_conf_file $aprim > /dev/console 2>&1 &
