...
#endif
{
    ...
    if (ieee80211_node_refcnt(ni) == 0) {
        ieee80211_note(ni->ni_vap,"node_reclaim called with 0 refcount for %s, vap: 0x%p \n",
...
ieee80211_node_unauthorize(struct ieee80211_node *ni)
{
    ...
    if (ic == NULL) {
        printk ("%s:WARN ni->ni_ic is NULL for ni(%p) ni_macddr %s\n",
...
ieee80211_atf_node_join_leave(struct ieee80211_node *ni,const u_int8_t type)
{
    ...
    if(ic->atf_commit)
    {
        ...
        {
            printk("ATM is re-allocating airtime because %s is now %s\n",
                    ether_sprintf(ni->ni_macaddr),
...
{
    if (!ieee80211_vap_deleted_is_set(vap)) {
        printk("%s: vap is not deleted by user, vap: 0x%p, vap->iv_bss: 0x%p\n",
