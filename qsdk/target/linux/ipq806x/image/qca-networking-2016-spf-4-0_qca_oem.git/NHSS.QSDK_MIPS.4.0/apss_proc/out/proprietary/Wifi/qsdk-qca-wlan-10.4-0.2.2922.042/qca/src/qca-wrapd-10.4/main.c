...
int main(int argc, char *argv[])
{
    ...
    strncpy(bridge_name,DEFAULT_BRIDGE_NAME,IFNAMSIZ);
    strncpy(bridge_ifname[0],DEFAULT_INTERFACE,IFNAMSIZ);
    ...
    for (;;) {
        c = getopt(argc, argv, "g:p:w:A:R:aBLMSITc:v:d:D:P:hb:t:i:l:r:e:H:");
        if (c < 0) {
            if(arg_count == 0)
               goto out;
            else
        ...
        {
    		strncpy(bridge_ifname[no_of_interfaces],optarg,IFNAMSIZ);
	    	no_of_interfaces++;
        }
		break;
	    case 'l':
    ...
    wrapg->always_primary = always_primary;
    wrapg->wifi_count = 0;
	...
	data->ioctl_sock=0;
	strncpy(data->brname,bridge_name,IFNAMSIZ);

        for(m=0;m<data->no_of_interfaces;m++)
        {
            strncpy(data->ifname[m],bridge_ifname[m],IFNAMSIZ);
        }
