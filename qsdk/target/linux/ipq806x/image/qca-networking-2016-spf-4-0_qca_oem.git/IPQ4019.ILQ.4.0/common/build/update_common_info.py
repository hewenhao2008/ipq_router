...
-------------------------------------------------------------------------------
  ...
  $Header: //depot/asic/ipq8066/IPQ4019.ILQ.4.0_P/meta/main/latest/common/build/update_common_info.py#7 $
  $DateTime: 2016/10/13 01:00:15 $
...
if FIT_tool:
   ...
   ipq_p_gcc5_dir = './ipq_p_gcc5'
   if not os.path.exists(ipq_p_gcc5_dir):
      os.makedirs(ipq_p_gcc5_dir,mode = 0755)
	  
   bin_dir = './bin'
   if not os.path.exists(bin_dir):
      os.makedirs(bin_dir,mode = 0755)

   ipq_premium_flash_conf_file_dir = './ipq/ipq40xx_premium'
   if not os.path.exists(ipq_premium_flash_conf_file_dir):
      os.makedirs(ipq_premium_flash_conf_file_dir,mode = 0755)

	  
   ipq_standard_flash_conf_file_dir = './ipq/ipq40xx_standard'
   if not os.path.exists(ipq_standard_flash_conf_file_dir):
      os.makedirs(ipq_standard_flash_conf_file_dir,mode = 0755)
   ...
   for flash_file in mi.get_files(attr='flash_file'):
      shutil.copy(flash_file, ipq_p_gcc5_dir)
	  


	
   for flash_file in mi.get_files(attr='flash_file_p_gcc'):
      shutil.copy(flash_file, ipq_p_gcc5_dir)
	  
   # copy all files has attribute flash_premium_conf_file to ipq/ipq40xx_premium folder
   for flash_premium_conf_file in mi.get_files(attr='flash_premium_conf_file'):
      shutil.copy(flash_premium_conf_file, ipq_premium_flash_conf_file_dir)


   # copy all files has attribute flash_standard_conf_file to ipq/ipq40xx_standard folder
   for flash_standard_conf_file in mi.get_files(attr='flash_standard_conf_file'):
      shutil.copy(flash_standard_conf_file, ipq_standard_flash_conf_file_dir)


	  # since this is premium build also copy all files has attribute flash_premium_conf_file to ipq folder
   for flash_premium_conf_file in mi.get_files(attr='flash_premium_conf_file'):
      shutil.copy(flash_premium_conf_file, ipq_dir)



  # copy all files has attribute QPST_file to uppermost meta folder
   for QPST_file in mi.get_files(attr='QPST_file'):
      shutil.copy(QPST_file, meta_dir)

   tools_dir = './ipq/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)	
    
   lg.log("update_common_info: Generate ipq single image")
   # lg.log("calling ./pack.sh <options> file1.xml file2.xml ... ")
   # os.chdir(ipq_dir)
   # #subprocess.call(['pack.sh' , 'ap148-nand-ipq806x.xml' , 'ap148-nor-ipq806x.xml'])
   # arglist = 'ap148-nor-ipq806x.xml'
   # bashCommand = '/bin/bash pack.sh ' + arglist
   # os.system(bashCommand)

   ret_nand_single = 0
   ret_nand_p_gcc5_single = 0
   ret_nor_single = 0
   ret_nor_p_gcc5_single = 0
   ret_nornand_single = 0
   ret_emmc_single = 0
   ret_norplusemmc_single = 0

   ret_nand_single = lg.log_exec(['python',pack_tool,'-t','nand','-B','-F','boardconfig_premium', '-o', bin_dir +'/'+'nand-ipq40xx-single.img',ipq_dir],verbose=0)
   print "ret_nand_single %d." % ret_nand_single
#   ret_nand_p_gcc5_single = lg.log_exec(['python',pack_tool,'-t','nand','-B','-F','boardconfig_premium', '-o', bin_dir +'/'+'nand-ipq40xx-p-gcc5-single.img',ipq_p_gcc5_dir],verbose=0)
#   print "ret_nand_p_gcc5_single %d." % ret_nand_p_gcc5_single
#   ret_nor_single = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nor-ipq40xx-single.img',ipq_dir],verbose=0)
#   print "ret_nor_single %d." % ret_nor_single
#   ret_nor_p_gcc5_single = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nor-ipq40xx-p-gcc5-single.img',ipq_p_gcc5_dir],verbose=0)
#   print "ret_nor_p_gcc5_single %d." % ret_nor_p_gcc5_single
#   ret_nornand_single = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nornand-ipq40xx-single.img',ipq_dir],verbose=0)
#   print "ret_nornand_single %d." % ret_nornand_single
#   ret_emmc_single = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'emmc-ipq40xx-single.img',ipq_dir],verbose=0)
#   print "ret_emmc_single %d." % ret_emmc_single
#   ret_norplusemmc_single = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'norplusemmc-ipq40xx-single.img',ipq_dir],verbose=0)
#   print "ret_norplusemmc_single %d." % ret_norplusemmc_single

   # Generate Single APSS Image
   ret_nor_apps = 0
   ret_nand_apps = 0
   ret_emmc_apps = 0
   ret_nornand_apps = 0
...
#   ret_nor_apps = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','appsboardconfig_premium','-o', bin_dir +'/'+'ipq40xx-nor-apps.img',ipq_dir],verbose=0)
#   print "ret_nor_apps %d." % ret_nor_apps
#   ret_nand_apps = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','appsboardconfig_premium','-o', bin_dir +'/'+'ipq40xx-nand-apps.img',ipq_dir],verbose=0)
#   print "ret_nand_apps %d." % ret_nand_apps
#   ret_emmc_apps = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','appsboardconfig_premium','-o', bin_dir +'/'+'ipq40xx-emmc-apps.img',ipq_dir],verbose=0)
#   print "ret_emmc_apps %d." % ret_emmc_apps
#   ret_nornand_apps = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','appsboardconfig_premium','-o', bin_dir +'/'+'ipq40xx-nornand-apps.img',ipq_dir],verbose=0)
#   print "ret_nornand_apps %d." % ret_nornand_apps
#   ret_noremmc_apps = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','appsboardconfig_premium','-o', bin_dir +'/'+'ipq40xx-noremmc-apps.img',ipq_dir],verbose=0)
#   print "ret_noremmc_apps %d." % ret_noremmc_apps
   
   # Streamboost3 binary generation
