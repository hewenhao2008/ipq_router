...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXWSP - Linux Bluetooth Weight Scale Profile using GATT (LE/BREDR)     */
/*             sample application.                                            */
/*                                                                            */
/*  Author:  Ryan McCord                                                      */
/*                                                                            */
...
#include "LinuxWSP.h"            /* Application Header.                       */
...
#include "SS1BTWSS.h"            /* Main SS1 WSS Service Header.              */
...
#define WSSP_PARAMETER_VALUE                        (2)  /* Denotes the value */
                                                         /* passed in on the  */
                                                         /* command line for  */
                                                         /* running with the  */
                                                         /* transport set to  */
...
#define WSP_WEIGHT_MEASUREMENT_LIST_SIZE            (5)  /* Denotes the size  */
                                                         /* of the measurement*/
                                                         /* list.             */

#define WSP_DEFAULT_WSS_SUPPORTED_FEATURES         (WSS_WEIGHT_SCALE_FEATURE_FLAG_TIME_STAMP_SUPPORTED               |  \
                                                    WSS_WEIGHT_SCALE_FEATURE_FLAG_MULTIPLE_USERS_SUPPORTED           |  \
                                                    WSS_WEIGHT_SCALE_FEATURE_FLAG_BMI_SUPPORTED                      |  \
                                                    WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_NOT_SPECIFIED        |  \
                                                    WSS_WEIGHT_SCALE_FEATURE_FLAG_HM_RESOLUTION_NOT_SPECIFIED)
                                                         /* Denotes the default   */
                                                         /* WSS Features that will*/
                                                         /* be supported by the   */
                                                         /* sample application.   */

   /* The following defines default Weight Measurement Flags for the    */
   /* sample application.                                               */
#define WSP_WEIGHT_MEASUREMENT_FLAG_DEFAULT        (WSS_WEIGHT_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL |  \
                                                    WSS_WEIGHT_MEASUREMENT_FLAG_TIME_STAMP_PRESENT         |  \
                                                    WSS_WEIGHT_MEASUREMENT_FLAG_USER_ID_PRESENT            |  \
                                                    WSS_WEIGHT_MEASUREMENT_FLAG_BMI_HEIGHT_PRESENT)

   /* The following defines the Weight Measurement Flags for an         */
   /* unsuccessful measurement.                                         */
#define WSP_UNSUCCESSFUL_WEIGHT_MEASUREMENT_FLAGS  (WSS_WEIGHT_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL |  \
                                                    WSS_WEIGHT_MEASUREMENT_FLAG_TIME_STAMP_PRESENT         |  \
                                                    WSS_WEIGHT_MEASUREMENT_FLAG_USER_ID_PRESENT)
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxWSP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxWSP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxWSP"
...
typedef enum
{
   ...
   /*          GATT Read Long procedure requests and responses.         */
typedef struct _tagUDS_Request_Data_t
{
   ...
   /* The following structure defines a Body Composition measurement    */
   /* that may be included in a list of BC Measurements.                */
typedef struct _tagWSP_Measurement_Data_t
{
   Boolean_t                      Valid;
   unsigned int                   ID;
   Boolean_t                      Indicated;
   WSS_Weight_Measurement_Data_t  Measurement;
} WSP_Measurement_Data_t;

#define WSP_MEASUREMENT_DATA_SIZE  (sizeof(WSP_Measurement_Data_t))
...
typedef struct _tagDeviceInfo_t
{
   ...
   WSS_Client_Information_t  WSSClientInfo;
   WSS_Server_Information_t  WSSServerInfo;
...
#define DEVICE_INFO_FLAGS_UDS_SERVICE_DISCOVERY_COMPLETE   0x0100
#define DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE   0x0200
...
static unsigned int        WSSInstanceID;
                                                    /* The following holds the WSS    */
                                                    /* Instance IDs that are returned  */
                                                    /* from WSS_Initialize_XXX().     */
...
static WSP_Measurement_Data_t  WeightMeasurementList[WSP_WEIGHT_MEASUREMENT_LIST_SIZE];
                                                    /* The Weigth Measurement list.    */
...
static int DiscoverAllServicesCommand(ParameterList_t *TempParam);
   ...
   /* GATT Profile Functions and Commands.                              */
...
static int ConfigureGATTCommand(ParameterList_t *TempParam);
...
static void DisplayUDSDiscoverySummary(UDS_Client_Information_t *ClientInfo);
...
static int ConfigureUDSCommand(ParameterList_t *TempParam);
...
static void DisplayConfigureUDSCommandUsage(void);
static void DisplayWriteUDSUserControlPointCommandUsage(void);

   /* Weight Scale Service Functions and Commands.                      */
static void WSSPopulateHandles(WSS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
static void DisplayWSSDiscoverySummary(WSS_Client_Information_t *ClientInfo);

static void DisplayWSSSupportedFeatures(DWord_t SupportedFeatures);
static void DecodeDisplayWeightMeasurement(unsigned int ValueLength, Byte_t *Value);
static void DisplayWeightMeasurement(WSS_Weight_Measurement_Data_t *WeightMeasurement);

static int RegisterWSSCommand(ParameterList_t *TempParam);
static int UnRegisterWSSCommand(ParameterList_t *TempParam);
static int DiscoverWSSCommand(ParameterList_t *TempParam);
static int ReadFeaturesCommand(ParameterList_t *TempParam);
static int ReadWSSConfigurationCommand(ParameterList_t *TempParam);
static int ConfigureWSSCommand(ParameterList_t *TempParam);
static int IndicateMeasurementCommand(ParameterList_t *TempParam);
static int CreateWeightMeasurementCommand(ParameterList_t *TempParam);
static int PrintWeightMeasurementsCommand(ParameterList_t *TempParam);
...
static void BTPSAPI WSS_EventCallback(unsigned int BluetoothStackID, WSS_Event_Data_t *WSS_Event_Data, unsigned long CallbackParameter);
...
static void BTPSAPI GATT_ClientEventCallback_WSS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static DeviceInfo_t *DeleteDeviceInfoEntry(DeviceInfo_t **ListHead, BD_ADDR_t BD_ADDR)
{
   ...
   /*          applications for use with WSS/WSS.                     */
static void FreeDeviceInfoEntryMemory(DeviceInfo_t *EntryToFree)
{
...
static void UserInterface(void)
{
   ...
   AddCommand("CONFIGUREUDS", ConfigureUDSCommand);
   AddCommand("WRITEUSERCP", WriteUDSUserControlPointCommand);
   AddCommand("REGISTERWSS", RegisterWSSCommand);
   AddCommand("UNREGISTERWSS", UnRegisterWSSCommand);
   AddCommand("DISCOVERWSS", DiscoverWSSCommand);
   AddCommand("READFEATURES", ReadFeaturesCommand);
   AddCommand("READWSSCONFIGURATION", ReadWSSConfigurationCommand);
   AddCommand("CONFIGUREWSS", ConfigureWSSCommand);
   AddCommand("INDICATE", IndicateMeasurementCommand);
   AddCommand("CREATE", CreateWeightMeasurementCommand);
   AddCommand("PRINT",  PrintWeightMeasurementsCommand);
   AddCommand("GETMTU", GetGATTMTU);
   AddCommand("SETMTU", SetGATTMTU);
   AddCommand("DISCOVERALL", DiscoverAllServicesCommand);
   AddCommand("CONFIGUREGATT", ConfigureGATTCommand);
      ...
      for(Index = 0; Index < Advertising_Data->Number_Data_Entries; Index++)
      {
         ...
         switch(Advertising_Data->Data_Entries[Index].AD_Type)
         {
            ...
            case HCI_LE_ADVERTISING_REPORT_DATA_TYPE_16_BIT_SERVICE_UUID_COMPLETE:
               ...
               /* We will check for the WSS Service data.  Make sure   */
               ...
               if(Advertising_Data_Entry->AD_Data_Length >= NON_ALIGNED_WORD_SIZE)
               {
                  ...
                  /* Check if this is the WSS Service data.            */
                  if(WSS_COMPARE_WSS_SERVICE_UUID_TO_UUID_16(*((UUID_16_t *)&(Advertising_Data_Entry->AD_Data_Buffer[0]))))
                  {
                     /* Decode the UUID.                                */
...
static int CloseStack(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(WSSInstanceID)
      {
         WSS_Cleanup_Service(BluetoothStackID, WSSInstanceID);
      }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options GATT:    DiscoverALL, ConfigureGATT,           *\r\n");
   ...
   printf("*                          WriteUserCP,                          *\r\n");
   printf("*                          Read,                                 *\r\n");
   printf("* Command Options WSS:                                           *\r\n");
   printf("*                Server:   RegisterWSS                           *\r\n");
   printf("*                          UnRegisterWSS,                        *\r\n");
   printf("*                          Indicate,                             *\r\n");
   printf("*                          Create,                               *\r\n");
   printf("*                          Print,                                *\r\n");
   printf("*                Client:   DiscoverWSS,                          *\r\n");
   printf("*                          ReadWSSConfiguration,                 *\r\n");
   printf("*                          ConfigureWSS,                         *\r\n");
   printf("*                Both:     ReadFeatures                          *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* Configure the flags field wssed on the Discoverability  */
            /* Mode.                                                    */
            ...
            if(WSSInstanceID)
            {
               ...
               WSS_ASSIGN_WSS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
               ...
               ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[9]), GAP_DEVICE_APPEARENCE_VALUE_GENERIC_COMPUTER);
            }

            /* Write the advertising data to the chip.                  */
            ret_val = GAP_LE_Set_Advertising_Data(BluetoothStackID, (Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[0] + Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[3] + Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[7] + 2), &(Advertisement_Data_Buffer.AdvertisingData));
            if(!ret_val)
            {
...
static int DiscoverAllServicesCommand(ParameterList_t *TempParam)
{
   ...
   if(!WSSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         printf("\r\nMust be connected to a GATT Server.\r\n");
   }
   else
      printf("\r\nOnly a GATT Client can discover GATT.\r\n");
...
static void DisplayGATTDiscoverySummary(GATT_Client_Info_t *ClientInfo)
{
   ...
   /* The following function is responsible for configuring a GATT      */
   /* Service Changed CCCD on a remote device.  This function will      */
      ...
      if(!WSSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               /* this device                                           */
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_GATT_SERVICE_DISCOVERY_COMPLETE)
               {
                  printf("\r\nAttempting to configure GATT Service Changed CCCD...\r\n");

                  /* Format the CCCD.                                   */
                  if(TempParam->Params[0].intParam == 0)
                  {
                     ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&Client_Configuration, 0);
                  }
                  else
                  {
                     if(TempParam->Params[0].intParam == 1)
                     {
                        ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&Client_Configuration, (Word_t)GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE);
                     }
                     else
                     {
                        ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&Client_Configuration, (Word_t)GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE);
                     }
                  }

                  /* Determine if the CCCD is supported (mandatory).    */
                  if(DeviceInfo->GATTClientInfo.ServiceChangedCCCD)
                  {
                     /* Store the handle.                               */
                     AttributeHandle = DeviceInfo->GATTClientInfo.ServiceChangedCCCD;

                     /* Send the write request.                         */
                     /* * NOTE * We will not save the transactionID     */
                     /*          returned by this function, which we    */
                     /*          could use to cancel the request.       */
                     if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, AttributeHandle, NON_ALIGNED_WORD_SIZE, (Byte_t *)&Client_Configuration, GATT_ClientEventCallback_WSS, AttributeHandle)) > 0)
                     {
                  ...
                  printf("\r\nService discovery has not been performed for GATT.\r\n");
            }
            else
               printf("\r\nNo Device Info.\r\n");
         }
         ...
         printf("\r\nOnly the GATT Client can configure the Service Changed CCCD.\r\n");
   }
   else
      printf("\r\nUsage: ConfigureGATT [Configuration (0 = disable, 1 = indicate, 2 = notify)]\r\n");
...
static void UDSPopulateHandles(UDS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* The following function is responsible for displaying the UDS      */
   /* service discovery summary.                                        */
static void DisplayUDSDiscoverySummary(UDS_Client_Information_t *ClientInfo)
{
   printf("\r\nUDS Service Discovery Summary\r\n\r\n");
   printf("   UDS Characteristics:\r\n");
   printf("      First Name                        : %s\r\n", (ClientInfo->UDS_Characteristic.First_Name                       ? "Supported" : "Not Supported"));
   printf("      Last Name                         : %s\r\n", (ClientInfo->UDS_Characteristic.Last_Name                        ? "Supported" : "Not Supported"));
   printf("      Email Address                     : %s\r\n", (ClientInfo->UDS_Characteristic.Email_Address                    ? "Supported" : "Not Supported"));
   printf("      Age                               : %s\r\n", (ClientInfo->UDS_Characteristic.Age                              ? "Supported" : "Not Supported"));
   printf("      Date of Birth                     : %s\r\n", (ClientInfo->UDS_Characteristic.Date_Of_Birth                    ? "Supported" : "Not Supported"));
   printf("      Gender                            : %s\r\n", (ClientInfo->UDS_Characteristic.Gender                           ? "Supported" : "Not Supported"));
   printf("      Weight                            : %s\r\n", (ClientInfo->UDS_Characteristic.Weight                           ? "Supported" : "Not Supported"));
   printf("      Height                            : %s\r\n", (ClientInfo->UDS_Characteristic.Height                           ? "Supported" : "Not Supported"));
   printf("      VO2 Max                           : %s\r\n", (ClientInfo->UDS_Characteristic.VO2_Max                          ? "Supported" : "Not Supported"));
   printf("      Heart Rate Max                    : %s\r\n", (ClientInfo->UDS_Characteristic.Heart_Rate_Max                   ? "Supported" : "Not Supported"));
   printf("      Resting Heart Rate                : %s\r\n", (ClientInfo->UDS_Characteristic.Resting_Heart_Rate               ? "Supported" : "Not Supported"));
   printf("      Maximum Recommended Heart Rate    : %s\r\n", (ClientInfo->UDS_Characteristic.Maximum_Recommended_Heart_Rate   ? "Supported" : "Not Supported"));
   printf("      Aerobic Threshold                 : %s\r\n", (ClientInfo->UDS_Characteristic.Aerobic_Threshold                ? "Supported" : "Not Supported"));
   printf("      Anaerobic Threshold               : %s\r\n", (ClientInfo->UDS_Characteristic.Anaerobic_Threshold              ? "Supported" : "Not Supported"));
   printf("      Sport Type                        : %s\r\n", (ClientInfo->UDS_Characteristic.Sport_Type                       ? "Supported" : "Not Supported"));
   printf("      Date of Threshold                 : %s\r\n", (ClientInfo->UDS_Characteristic.Date_Of_Threshold                ? "Supported" : "Not Supported"));
   printf("      Waist Circumference               : %s\r\n", (ClientInfo->UDS_Characteristic.Waist_Circumference              ? "Supported" : "Not Supported"));
   printf("      Hip Circumference                 : %s\r\n", (ClientInfo->UDS_Characteristic.Hip_Circumference                ? "Supported" : "Not Supported"));
   printf("      Fat Burn Heart Rate Lower Limit   : %s\r\n", (ClientInfo->UDS_Characteristic.Fat_Burn_Heart_Rate_Lower_Limit  ? "Supported" : "Not Supported"));
   printf("      Fat Burn Heart Rate Upper Limit   : %s\r\n", (ClientInfo->UDS_Characteristic.Fat_Burn_Heart_Rate_Upper_Limit  ? "Supported" : "Not Supported"));
   printf("      Aerobic Heart Rate Lower Limit    : %s\r\n", (ClientInfo->UDS_Characteristic.Aerobic_Heart_Rate_Lower_Limit   ? "Supported" : "Not Supported"));
   printf("      Aerobic Heart Rate Upper Limit    : %s\r\n", (ClientInfo->UDS_Characteristic.Aerobic_Heart_Rate_Upper_Limit   ? "Supported" : "Not Supported"));
   printf("      Anaerobic Heart Rate Lower Limit  : %s\r\n", (ClientInfo->UDS_Characteristic.Anaerobic_Heart_Rate_Lower_Limit ? "Supported" : "Not Supported"));
   printf("      Anaerobic Heart Rate Upper Limit  : %s\r\n", (ClientInfo->UDS_Characteristic.Anaerobic_Heart_Rate_Upper_Limit ? "Supported" : "Not Supported"));
   printf("      Five Zone Heart Rate Limits       : %s\r\n", (ClientInfo->UDS_Characteristic.Five_Zone_Heart_Rate_Limits      ? "Supported" : "Not Supported"));
   printf("      Three Zone Heart Rate Limits      : %s\r\n", (ClientInfo->UDS_Characteristic.Three_Zone_Heart_Rate_Limits     ? "Supported" : "Not Supported"));
   printf("      Two Zone Heart Rate Limit         : %s\r\n", (ClientInfo->UDS_Characteristic.Two_Zone_Heart_Rate_Limit        ? "Supported" : "Not Supported"));
   printf("      Language                          : %s\r\n", (ClientInfo->UDS_Characteristic.Language                         ? "Supported" : "Not Supported"));

   /* Remaining UDS Characteristics.               */
   printf("   UDS Database Change Increment        : %s\r\n", (ClientInfo->Database_Change_Increment      ? "Supported" : "Not Supported"));
   printf("   UDS Database Change Increment CCCD   : %s\r\n", (ClientInfo->Database_Change_Increment_CCCD ? "Supported" : "Not Supported"));
   printf("   UDS User Index                       : %s\r\n", (ClientInfo->User_Index                     ? "Supported" : "Not Supported"));
   printf("   UDS User Control Point               : %s\r\n", (ClientInfo->User_Control_Point             ? "Supported" : "Not Supported"));
   printf("   UDS User Control Point CCCD          : %s\r\n", (ClientInfo->User_Control_Point_CCCD        ? "Supported" : "Not Supported"));
}
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_UDS_SERVICE_DISCOVERY_COMPLETE)
         {
            ...
            printf("\r\nService discovery has not been performed for UDS.\r\n");
      }
      else
         printf("\r\nNo device information.\r\n");
   }
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_UDS_SERVICE_DISCOVERY_COMPLETE)
         {
            ...
            {
               case ahtUDS_Characteristic:
                  printf("\r\nUDS Characteristic CANNOT be written.\r\n");
                  break;
               case ahtDatabase_Change_Increment:
                  printf("\r\nUDS Database Change Increment CANNOT be written.\r\n");
                  break;
               case ahtDatabase_Change_Increment_CCCD:
               case ahtUser_Control_Point_CCCD:
   ...
   if(ResponseData)
   {
      ...
      switch(ResponseData->Response_Code_Value)
      {
         case rcvSuccess:
            printf("Success.\r\n");
            break;
         case rcvOpCodeNotSupported:
            printf("Op Code Not Supported.\r\n");
            break;
         case rcvInvalidParameter:
            printf("Invalid parameter.\r\n");
            break;
         case rcvOperationFailed:
            printf("Operation Failed.\r\n");
            break;
         case rcvUserNotAuthorized:
      ...
      if((ResponseData->Response_Code_Value == rcvSuccess) && (ResponseData->Request_Op_Code == ucpRegisterNewUser))
      {
...
static int ConfigureUDSCommand(ParameterList_t *TempParam)
{
   ...
   {
      printf("\r\nInvalid parameter.\r\n");
      DisplayConfigureUDSCommandUsage();
   }
...
{
   /* Display the usage.                                                */
   printf("\r\nUsage: ConfigureUDS [Type (UINT8)] [Opt. params (below)]\r\n");
   printf("\r\nWhere Type is:\r\n");
...
static void DisplayWriteUDSUserControlPointCommandUsage(void)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a WSS Client Information structure with   */
   /* the information discovered from a WSS Discovery operation.        */
static void WSSPopulateHandles(WSS_Client_Information_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   unsigned int                                  Index1;
   unsigned int                                  Index2;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((ClientInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (WSS_COMPARE_WSS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service and */
      /* populate the correct entry.                                    */
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* Check for the Weight Scale Feature UUID.                 */
            if(WSS_COMPARE_WEIGHT_SCALE_FEATURE_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
            {
               /* All WSS UUIDs are defined to be 16 bit UUIDs.         */
               if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
               {
                  ClientInfo->Weight_Scale_Feature = CurrentCharacteristic->Characteristic_Handle;

                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                    printf("Warning - Mandatory read property of WSS Weight Scale Feature characteristic not supported!\r\n");
               }
            }
            else
            {
               /* Check for the Weight Measurement UUID.                */
               if(WSS_COMPARE_WEIGHT_MEASUREMENT_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  /* All WSS UUIDs are defined to be 16 bit UUIDs.      */
                  if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
                  {
                     ClientInfo->Weight_Measurement = CurrentCharacteristic->Characteristic_Handle;

                     /* Verify that indicate is supported.              */
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_INDICATE))
                        printf("Warning - Mandatory indicate property of WSS Weight Measurement characteristic not supported!\r\n");
                     ...
                     for(Index2 = 0; Index2 < CurrentCharacteristic->NumberOfDescriptors; Index2++)
                     {
                        if(CurrentDescriptor->Characteristic_Descriptor_UUID.UUID_Type == guUUID_16)
                        {
                           ...
                           {
                              ClientInfo->Weight_Measurement_CCCD = CurrentCharacteristic->DescriptorList[Index2].Characteristic_Descriptor_Handle;
                           }
                        }
                     }
                  }
   ...
   /* The following function is responsible for displaying the WSS      */
   /* service discovery summary.                                        */
static void DisplayWSSDiscoverySummary(WSS_Client_Information_t *ClientInfo)
{
   printf("\r\nWSS Service Discovery Summary\r\n\r\n");
   printf("   WSS Feature                 : %s\r\n", (ClientInfo->Weight_Scale_Feature     ? "Supported" : "Not Supported"));
   printf("   WSS Weight Measurement      : %s\r\n", (ClientInfo->Weight_Measurement       ? "Supported" : "Not Supported"));
   printf("   WSS Weight Measurement CCCD : %s\r\n", (ClientInfo->Weight_Measurement_CCCD  ? "Supported" : "Not Supported"));
}

   /* The following function is responsible for display the supported   */
   /* features of the wss server.                                       */
static void DisplayWSSSupportedFeatures(DWord_t SupportedFeatures)
{
   printf("\r\nWSS Weight Scale Feature:\r\n");
   printf("   Value:             0x%06X.\r\n", SupportedFeatures);

   /* Display the supported features to the application.                */
   if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_TIME_STAMP_SUPPORTED)
      printf("   Time Stamp:        Supported.\r\n");
   else
      printf("   Time Stamp:        Not Supported.\r\n");

   if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_MULTIPLE_USERS_SUPPORTED)
      printf("   Multiple Users:    Supported.\r\n");
   else
      printf("   Multiple Users:    Not Supported.\r\n");

   if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_BMI_SUPPORTED)
      printf("   BMI:               Supported.\r\n");
   else
      printf("   BMI:               Not Supported.\r\n");

   /* Display the supported Weight resolution.                          */
   if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_5_KG_OR_1_LB)
      printf("   Weight Measurement Resolution:   0.5 kg or 0.01 lb.\r\n");
   else
   {
      if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_2_KG_OR_POINT_5_LB)
         printf("   Weight Measurement Resolution:   0.2 kg 0.02 lb.\r\n");
      else
      {
         if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_1_KG_OR_POINT_2_LB)
         {
            printf("   Weight Measurement Resolution:   0.1 kg or 0.05 lb.\r\n");
         }
         else
         {
            if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_05_KG_OR_POINT_1_LB)
               printf("   Weight Measurement Resolution:   0.05 kg or 0.1 lb.\r\n");
            else
            {
               if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_02_KG_OR_POINT_05_LB)
                  printf("   Weight Measurement Resolution:   0.02 kg 0.2 lb.\r\n");
               else
               {
                  if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_01_KG_OR_POINT_02_LB)
                     printf("   Weight Measurement Resolution:   0.01 kg or 0.5 lb.\r\n");
                  else
                  {
                     if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_WM_RESOLUTION_POINT_005_KG_OR_POINT_01_LB)
                        printf("   Weight Measurement Resolution:   0.005 kg or 0.01 lb.\r\n");
                     else
                        printf("   Weight Measurement Resolution:   Not specified.\r\n");
                  }
               }
            }
         }
   ...
   /* Display the supported Height resolution.                          */
   if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_HM_RESOLUTION_POINT_01_M_OR_1_INCH)
      printf("   Height Measurement Resolution:   0.01 m or 0.1 inch.\r\n");
   else
   {
      if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_HM_RESOLUTION_POINT_005_M_OR_POINT_5_INCH)
         printf("   Height Measurement Resolution:   0.005 m or 0.5 inch.\r\n");
      else
      {
         if(SupportedFeatures & WSS_WEIGHT_SCALE_FEATURE_FLAG_HM_RESOLUTION_POINT_0001_M_OR_POINT_1_INCH)
            printf("   Height Measurement Resolution:   0.0001 m or 1 inch.\r\n");
         else
            printf("   Height Measurement Resolution:   Not specified.\r\n");
      }
   }
}

   /* The following function is responsible for decoding and displaying */
   /* a Weight Measurement provided.                                    */
static void DecodeDisplayWeightMeasurement(unsigned int ValueLength, Byte_t *Value)
{
   int                       Result;
   WSS_Weight_Measurement_Data_t WeightMeasurement;
   ...
   if((ValueLength) && (Value))
   {
      ...
      /* Decode the Weight Measurement.                                 */
      if((Result = WSS_Decode_Weight_Measurement(ValueLength, Value, &WeightMeasurement)) == 0)
         DisplayWeightMeasurement(&WeightMeasurement);
      else
         DisplayFunctionError("WSS_Decode_Weight_Measurement", Result);
   }
   else
      printf("Invalid Weight Measurement Response buffer.\r\n");
}

   /* The following function is a helper function to display the Weight */
   /* Measurement.                                                      */
static void DisplayWeightMeasurement(WSS_Weight_Measurement_Data_t *WeightMeasurement)
{
   /* Display the flags.                                                */
   printf("   Flags:                0x%04X\r\n", WeightMeasurement->Flags);

   /* Display the Weight.                                               */
   printf("   Weight:               %u\r\n", WeightMeasurement->Weight);

   /* Display the Unit type.                                            */
   if(WeightMeasurement->Flags & WSS_WEIGHT_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL)
      printf("   Units:                Imperial\r\n");
   else
      printf("   Units:                SI\r\n");

   /* Display the Time Stamp if included in the measurement.            */
   if(WeightMeasurement->Flags & WSS_WEIGHT_MEASUREMENT_FLAG_TIME_STAMP_PRESENT)
   {
      printf("   Time Stamp:           %02u/%02u/%02u %02u:%02u:%02u\r\n", WeightMeasurement->Time_Stamp.Month,
                                                                           WeightMeasurement->Time_Stamp.Day,
                                                                           WeightMeasurement->Time_Stamp.Year,
                                                                           WeightMeasurement->Time_Stamp.Hours,
                                                                           WeightMeasurement->Time_Stamp.Minutes,
                                                                           WeightMeasurement->Time_Stamp.Seconds);
   }

   /* Display the User ID if included in the measurement.               */
   if(WeightMeasurement->Flags & WSS_WEIGHT_MEASUREMENT_FLAG_USER_ID_PRESENT)
   {
      printf("   User ID:              %u\r\n", WeightMeasurement->User_ID);
   }

   /* Display the Weight if included in the measurement.                */
   if(WeightMeasurement->Flags & WSS_WEIGHT_MEASUREMENT_FLAG_BMI_HEIGHT_PRESENT)
   {
      printf("   BMI:                  %u\r\n", WeightMeasurement->BMI);
      printf("   Height:               %u\r\n", WeightMeasurement->Height);
   }
}

   /* The following function is responsible for registering a WSS       */
...
static int RegisterWSSCommand(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!WSSInstanceID)
      {
         /* Register the WSS Service with GATT.                         */
         ret_val = WSS_Initialize_Service(BluetoothStackID, WSS_SERVICE_FLAGS_DUAL_MODE, WSS_EventCallback, 0, &WSSInstanceID);
         if((ret_val > 0) && (WSSInstanceID > 0))
         {
            /* Display success message.                                 */
            printf("\r\nSuccessfully registered WSS Service.\r\n");

            /* Save the ServiceID of the registered service.            */
            WSSInstanceID  = (unsigned int)ret_val;


            /* Initialize the Weight Measurement list.  This will mark  */
            /* all measurements as invalid.                             */
            BTPS_MemInitialize(WeightMeasurementList, 0, (WSS_WEIGHT_MEASUREMENT_DATA_SIZE * WSP_WEIGHT_MEASUREMENT_LIST_SIZE));

            /* Set the default features.                                */
            if((ret_val = WSS_Set_Weight_Scale_Feature(BluetoothStackID, WSSInstanceID, (DWord_t)WSP_DEFAULT_WSS_SUPPORTED_FEATURES)) != 0)
               DisplayFunctionError("WSS_Set_Weight_Scale_Feature", ret_val);
      ...
      {
         printf("\r\nWSS Service already registered.\r\n");
   ...
   /* The following function is responsible for unregistering a WSS     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnRegisterWSSCommand(ParameterList_t *TempParam)
{
   ...
   if(WSSInstanceID)
   {
      ...
      /* Unregister the WSS Service with GATT.                          */
      ret_val = WSS_Cleanup_Service(BluetoothStackID, WSSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("\r\nSuccessfully unregistered WSS Service.\r\n");

         /* Save the ServiceID of the registered service.               */
         WSSInstanceID = 0;
      }
      else
         DisplayFunctionError("WSS_Cleanup_Service", ret_val);
   }
   else
      printf("\r\nWSS Service not registered.\r\n");

   return(ret_val);
}

   /* The following function is responsible for performing a WSS Service*/
   ...
   if(!WSSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the WSS Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               WSS_ASSIGN_WSS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdWSS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  printf("GATT_Service_Discovery_Start success.\r\n");
      ...
      printf("Cannot discover WSS Services when registered as a service.\r\n");

   return(ret_val);
}

   /* The following function is responsible for reading a WSS           */
   /* Measurement CCCD on a WSS Server.  This function will return zero */
...
static int ReadFeaturesCommand(ParameterList_t *TempParam)
{
   ...
   if(WSSInstanceID)
   {
      /* Query the default features.                                    */
      if((ret_val = WSS_Query_Weight_Scale_Feature(BluetoothStackID, WSSInstanceID, &Features)) != 0)
         DisplayFunctionError("WSS_Query_Weight_Scale_Feature", ret_val);

      /* Display the features.                                          */
      DisplayWSSSupportedFeatures(Features);
   }
   else
   {
      /* Verify that there is a connection that is established.         */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Determine if service discovery has been performed on this*/
            /* device                                                   */
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Determine if the Weight Scale Feature is supported    */
               /* (mandatory).                                          */
               if(DeviceInfo->WSSClientInfo.Weight_Scale_Feature)
               {
                  /* Store the handle.                                  */
                  AttributeHandle = DeviceInfo->WSSClientInfo.Weight_Scale_Feature;

                  /* Send the GATT read request.                        */
                  /* * NOTE * We will not save the transactionID        */
                  /*          returned by this function, which we could */
                  /*          use to cancel the request.                */
                  if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, AttributeHandle, GATT_ClientEventCallback_WSS, AttributeHandle)) > 0)
                  {
                     printf("\r\nGATT Read Value Request sent:\r\n");
                     printf("   TransactionID:     %d\r\n",     ret_val);
                     printf("   Attribute Handle:  0x%04X\r\n", AttributeHandle);
               ...
               printf("\r\nService discovery has not been performed for WSS.\r\n");
         }
         else
            printf("\r\nNo Device Info.\r\n");
      }
   ...
   if(!WSSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE)
            {
               /* Determine if Weight Measurement CCCD is supported     */
               /* (mandatory).                                          */
               if(DeviceInfo->WSSClientInfo.Weight_Measurement_CCCD)
               {
                  /* Store the handle.                                  */
                  AttributeHandle = DeviceInfo->WSSClientInfo.Weight_Measurement_CCCD;
      ...
      printf("\r\nOnly the WSS Client may read the BC Measurement CCCD.\r\n");

   return(ret_val);
}

   /* The following function is responsible for configuring a WSS Weight*/
   /* Measurement CCCD on a remote device.  This function will return   */
      ...
      if(!WSSInstanceID)
      {
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE)
               {
                  printf("\r\nAttempting to configure WSS Weight Measurement CCCD...\r\n");
         ...
         printf("\r\nOnly the WSS Client can configure the BC Measurement CCCD.\r\n");
   }
   else
      printf("\r\nUsage: ConfigureWSS [Configuration (0 = disable, 1 = indicate, 2 = notify)]\r\n");

   return(ret_val);
}

   /* The following function is a command to indicate the specified     */
   /* user's                                                            */
static int IndicateMeasurementCommand(ParameterList_t *TempParam)
{
   int                            ret_val = 0;
   unsigned int                   Index;
   unsigned int                   Index2;
   WSS_Weight_Measurement_Data_t *Weight_Measurement = NULL;
   unsigned int                   ID;

   /* Verify the parameters are semi-valid.                             */
   if(BluetoothStackID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if(WSSInstanceID)
         {
            /* Find a measurement to indicate.  We will simply loop up  */
            /* to the maximum measurement ID that we have created.  This*/
            /* will enforce we always indicate the oldest measurement   */
            /* first.  This is inefficient but this sample shouldn't    */
            ...
            /*          list.                                           */
            for(Index = 0, Index2 = 0; (Index < WSP_WEIGHT_MEASUREMENT_LIST_SIZE) && (Index2 < MeasurementIDCtr); Index2++)
            {
               /* If we find an ID that matches the counter is is       */
               /* already the oldest measurement.                       */
               if(WeightMeasurementList[Index].ID == Index2)
               {
                  /* If we find a measurement that hasn't been          */
                  /* indicated.  Make sure the measurement is valid     */
                  /* first.                                             */
                  if((WeightMeasurementList[Index].Valid) && (WeightMeasurementList[Index].Indicated == FALSE))
                  {
                     /* Flag that we indicated the measurement.         */
                     WeightMeasurementList[Index].Indicated = TRUE;

                     /* Set a pointer to it.                            */
                     Weight_Measurement = &(WeightMeasurementList[Index].Measurement);

                     /* Store the ID.                                   */
                     ID = WeightMeasurementList[Index].ID;
               ...
               /* If we are at the end of the Measurement list reset to */
               /* the start of the list to keep checking ID's.          */
               if(Index == (WSP_WEIGHT_MEASUREMENT_LIST_SIZE-1))
                  Index = 0;
               else
                  Index++;
            }
            ...
            {
               /* Send the indication.                                  */
               if((ret_val = WSS_Indicate_Weight_Measurement(BluetoothStackID, WSSInstanceID, ConnectionID, Weight_Measurement)) > 0)
               {
                  printf("\r\nWeight Measurement indicated.\r\n");
                  printf("   Measurement ID:  %u\r\n", ID);
                  printf("   TransactionID:   %d\r\n", ret_val);
               }
               else
                  DisplayFunctionError("WSS_Indicate_Weight_Measurement", ret_val);

               /* Simply return success to the caller.                  */
               ret_val = 0;
            }
            else
               printf("\r\nNo measurements to indicate.\r\n");
         }
         else
            printf("\r\nOnly the WSS Server can send indications.\r\n");
      }
      else
         printf("\r\nNot connected to a remote WSS Client.\r\n");
   }
   else
      printf("\r\nInvalid BluetoothStackID.\r\n");
   ...
   /* The following function is responsible for creating a new Weight   */
   /* Measurement.                                                      */
static int CreateWeightMeasurementCommand(ParameterList_t *TempParam)
{
   int                            ret_val = 0;
   unsigned int                   Index;
   unsigned int                   Index2;
   WSS_Weight_Measurement_Data_t  Weight_Measurement;
   time_t                         CurrentTime;
   struct tm                     *TimePtr;
   ...
   if(WSSInstanceID)
   {
      ...
      BTPS_MemInitialize(&(Weight_Measurement), 0, WSS_WEIGHT_MEASUREMENT_DATA_SIZE);

      /* Set the flags.                                                 */
      Weight_Measurement.Flags |= WSS_WEIGHT_MEASUREMENT_FLAG_MEASUREMENT_UNITS_IMPERIAL;
      Weight_Measurement.Flags |= WSS_WEIGHT_MEASUREMENT_FLAG_TIME_STAMP_PRESENT;
      Weight_Measurement.Flags |= WSS_WEIGHT_MEASUREMENT_FLAG_USER_ID_PRESENT;
      Weight_Measurement.Flags |= WSS_WEIGHT_MEASUREMENT_FLAG_BMI_HEIGHT_PRESENT;

      /* Assign Measurement data.                                       */
      Weight_Measurement.User_ID = 1;
      Weight_Measurement.BMI     = rand() % 40 + 20;
      ...
      Weight_Measurement.Time_Stamp.Year    = TimePtr->tm_year + 1900;
      Weight_Measurement.Time_Stamp.Month   = TimePtr->tm_mon + 1;
      Weight_Measurement.Time_Stamp.Day     = TimePtr->tm_mday;
      Weight_Measurement.Time_Stamp.Hours   = TimePtr->tm_hour;
      Weight_Measurement.Time_Stamp.Minutes = TimePtr->tm_min;
      Weight_Measurement.Time_Stamp.Seconds = TimePtr->tm_sec;

      Weight_Measurement.Weight             = rand() % 100 + 140;
      Weight_Measurement.Height             = rand() % 10 + 55;

      /* Loop through the Weight Measurement list and replace the oldest*/
      /* measurement if necessary.                                      */
      for(Index = 0, Index2 = 0; Index < WSP_WEIGHT_MEASUREMENT_LIST_SIZE; Index2++)
      {
         /* If we haven't used all the Measurement entries in the list  */
         /* yet we are guranteed to find an entry to store the          */
         /* measurement.                                                */
         if(MeasurementIDCtr < WSP_WEIGHT_MEASUREMENT_LIST_SIZE)
         {
            /* Check if this entry has been used before.  The ID will be*/
            /* zero.                                                    */
            if(WeightMeasurementList[Index].Valid == FALSE)
            {
               /* Set the BC Measurement data fields for this index.    */
               WeightMeasurementList[Index].Measurement = Weight_Measurement;
               WeightMeasurementList[Index].Valid       = TRUE;
               WeightMeasurementList[Index].ID          = MeasurementIDCtr++;
               WeightMeasurementList[Index].Indicated   = FALSE;
         ...
         else
         {
            ...
            if(WeightMeasurementList[Index].ID == Index2)
            {
               /* Set the Weight Measurement data fields for this index.*/
               WeightMeasurementList[Index].Measurement = Weight_Measurement;
               WeightMeasurementList[Index].Valid       = TRUE;
               WeightMeasurementList[Index].ID          = MeasurementIDCtr++;
               WeightMeasurementList[Index].Indicated   = FALSE;
      ...
      printf("\r\nOnly the WSS Server can create measurements.\r\n");

   return (ret_val);
}

   /* The following function is a command to print the user's BC        */
   /* measurements.  This function returns zero for success or a        */
   /* negative integer to indicate that an error occured.               */
static int PrintWeightMeasurementsCommand(ParameterList_t *TempParam)
{
   int          ret_val = 0;
   unsigned int Index;

   /* Make sure we are the WSS Server.                                  */
   if(WSSInstanceID)
   {
      /* Loop through the Weight Measurement list and replace the oldest*/
      /* measurement if necessary.                                      */
      for(Index = 0; Index <= (unsigned int)WSP_WEIGHT_MEASUREMENT_LIST_SIZE; Index++)
      {
         /* Only print valid measurements.                              */
         if(WeightMeasurementList[Index].Valid)
         {
            printf("\r\nWeight Measurement Data:\r\n");
            printf("   ID:                   %u\r\n", WeightMeasurementList[Index].ID);
            printf("   Indicated:            %s\r\n", (WeightMeasurementList[Index].Indicated) ? "Yes" : "No");
            DisplayWeightMeasurement(&(WeightMeasurementList[Index].Measurement));
         }

      }
   }
   else
      printf("\r\nOnly the WSS Server can print measurements.\r\n");
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a WSS Server Event Callback.  This function will */
   /* be called whenever an WSS Server Profile Event occurs that is     */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the WSS Event Data   */
   /* that occurred and the WSS Event Callback Parameter that was       */
   /* specified when this Callback was The caller is free to installed. */
   /* use the contents of the WSS Event Data ONLY in the context If the */
   ...
   /* possible (this argument holds anyway because another WSS Event    */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving WSS Event Packets.  */
   /*            A Deadlock WILL occur because NO WSS Event Callbacks   */
...
static void BTPSAPI WSS_EventCallback(unsigned int BluetoothStackID, WSS_Event_Data_t *WSS_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* Verify that all parameters to this callback are Semi-Valid.       */
   if((BluetoothStackID) && (WSS_Event_Data))
   {
      /* Switch through the event type.                                 */
      switch(WSS_Event_Data->Event_Data_Type)
      {
         case etWSS_Server_Read_Weight_Measurement_CCCD_Request:
            printf("\r\netWSS_Server_Read_Weight_Measurement_CCCD_Request with size %u.\r\n", WSS_Event_Data->Event_Data_Size);
            if(WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID         = WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data->InstanceID;
               ConnectionID       = WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data->ConnectionID;
               TransactionID      = WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data->TransactionID;
               ConnectionType     = WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data->ConnectionType;
               RemoteDevice       = WSS_Event_Data->Event_Data.WSS_Read_Weight_Measurement_CCCD_Request_Data->RemoteDevice;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  ...
                  if((Result = WSS_Read_Weight_Measurement_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, WSS_ERROR_CODE_SUCCESS, DeviceInfo->WSSServerInfo.Weight_Measurement_Configuration)) != 0)
                     DisplayFunctionError("WSS_Read_Weight_Measurement_CCCD_Request_Response", Result);
               }
               else
               {
                  printf("\r\nUnknown WSS Client.\r\n");
                  ...
                  if((Result = WSS_Read_Weight_Measurement_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, WSS_ERROR_CODE_UNLIKELY_ERROR, 0)) != 0)
                     DisplayFunctionError("WSS_Read_Weight_Measurement_CCCD_Request_Response", Result);
               }
            }
            break;
         case etWSS_Server_Write_Weight_Measurement_CCCD_Request:
            printf("\r\netWSS_Server_Write_Weight_Measurement_CCCD_Request with size %u.\r\n", WSS_Event_Data->Event_Data_Size);
            if(WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data)
            {
               /* Store event information.                              */
               InstanceID          = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->InstanceID;
               ConnectionID        = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->ConnectionID;
               TransactionID       = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->TransactionID;
               ConnectionType      = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->ConnectionType;
               RemoteDevice        = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->RemoteDevice;
               ClientConfiguration = WSS_Event_Data->Event_Data.WSS_Write_Weight_Measurement_CCCD_Request_Data->ClientConfiguration;
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
               {
                  /* Make sure the WSS Client has disabled or enabled   */
                  /* indications.                                       */
                  if((!ClientConfiguration) || (ClientConfiguration == WSS_CLIENT_CHARACTERISTIC_CONFIGURATION_INDICATE_ENABLE))
                  {
                     /* Store the new configuration.                    */
                     DeviceInfo->WSSServerInfo.Weight_Measurement_Configuration = ClientConfiguration;

                     /* Send the write response.                        */
                     if((Result = WSS_Write_Weight_Measurement_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, WSS_ERROR_CODE_SUCCESS)) != 0)
                        DisplayFunctionError("WSS_Write_Weight_Measurement_CCCD_Request_Response", Result);
                  }
                  else
                  {
                     /* Send the error response.                        */
                     if((Result = WSS_Write_Weight_Measurement_CCCD_Request_Response(BluetoothStackID, InstanceID, TransactionID, WSS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED)) != 0)
                        DisplayFunctionError("WSS_Write_Weight_Measurement_CCCD_Request_Response", Result);
                  }
               }
               else
            ...
            break;
         case etWSS_Server_Confirmation_Data:
            printf("\r\netWSS_Confirmation_Data with size %u.\r\n", WSS_Event_Data->Event_Data_Size);
            if(WSS_Event_Data->Event_Data.WSS_Confirmation_Data)
            {
               /* Print event information.                              */
               InstanceID     = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->InstanceID;
               ConnectionID   = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->ConnectionID;
               TransactionID  = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->TransactionID;
               ConnectionType = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->ConnectionType;
               RemoteDevice   = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->RemoteDevice;
               Status         = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->Status;
               BytesWritten   = WSS_Event_Data->Event_Data.WSS_Confirmation_Data->BytesWritten;
   ...
   {
      /* There was an error with one or more of the input parameters.   */
      printf("\r\nWSS Callback Data: Event_Data = NULL.\r\n");
   }
...
static void BTPSAPI GATT_ClientEventCallback_WSS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* GATT WSS Client Event Callback data types.                        */
   unsigned int               ConnectionID;
   unsigned int               TransactionID;
   GATT_Connection_Type_t     ConnectionType;
   BD_ADDR_t                  RemoteDevice;
         ...
         case etGATT_Client_Error_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  switch(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorCode)
                  {
                     case WSS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED:
                        printf("   Error Mesg:       WSS_ERROR_CODE_CCCD_IMPROPERLY_CONFIGURED\r\n");
                        break;
                     default:
                        if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorCode < NUMBER_OF_ERROR_CODES)
                        {
         ...
         case etGATT_Client_Read_Response:
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if((Value) && (ValueLength != 0) && (CallbackParameter != 0))
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteDevice)) != NULL)
                  {
                     /* Check for the WSS Feature.                      */
                     if(CallbackParameter == DeviceInfo->WSSClientInfo.Weight_Scale_Feature)
                     {
                        /* Verify the length.                           */
                        if(ValueLength >= NON_ALIGNED_DWORD_SIZE)
                        {
                           /* Decode the value.                         */
                           SupportedFeatures = READ_UNALIGNED_DWORD_LITTLE_ENDIAN(Value);

                           /* Print the supported features.             */
                           DisplayWSSSupportedFeatures(SupportedFeatures);
                        }
                        else
                        {
                           printf("\r\nError - Invalid length.\r\n");
                     ...
                     {
                        /* Check for the WSS Measurement CCCD.          */
                        if(CallbackParameter == DeviceInfo->WSSClientInfo.Weight_Measurement_CCCD)
                        {
                           /* Verify the length.                        */
                           if(ValueLength >= NON_ALIGNED_WORD_SIZE)
                           {
                        ...
                        {
                           /* If the data has not been decoded and      */
                           /* displayed, then just display the raw data */
               ...
               else
               {
                  ...
                  if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
                  {
                     if(LocalDeviceIsMaster)
                     {
                        ...
                        /* supported.                                   */
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_DEFAULT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_WSS, 0);
                     }
                  }
               }
            }
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, RemoteBDADDR)) != NULL)
               {
                  ...
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_UDS_SERVICE_DISCOVERY_COMPLETE)
                  {
                     ...
                     {
                        /* Simply call the function to decode the value.*/
                        if((Result = UDS_Decode_User_Control_Point_Response(ValueLength, Value, &ResponseData)) == 0)
                        {
                           /* Display the User Control Point response.  */
                  ...
                  /* Check that service discovery has been performed.   */
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE)
                  {
                     /* Check if this is an indication for a WSS        */
                     /* Measurement.                                    */
                     if(DeviceInfo->WSSClientInfo.Weight_Measurement == AttributeHandle)
                     {
                        /* Simply call the internal function to decode  */
                        /* the Weight Measurement.                      */
                        DecodeDisplayWeightMeasurement(ValueLength, Value);
                     }
                  }
                  else
                     printf("\r\nWSS Service discovery has not been performed.\r\n");
               }
               else
               {
                  printf("\r\nUnknown UDS/WSS Server.\r\n");
               }
            }
            break;
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdUDS:
                        ...
                        break;
                     case sdWSS:
                        /* Attempt to populate the handles for the WSS  */
                        /* Service.                                     */
                        WSSPopulateHandles(&(DeviceInfo->WSSClientInfo), GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                        break;
                     case sdAll:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     ...
                     case sdUDS:
                        ...
                        /* Display the service discovery summary.       */
                        DisplayUDSDiscoverySummary(&(DeviceInfo->UDSClientInfo));
                        break;
                     case sdWSS:
                        /* Flag that service discovery has been         */
                        /* performed on for this connection.            */
                        DeviceInfo->Flags |= DEVICE_INFO_FLAGS_WSS_SERVICE_DISCOVERY_COMPLETE;

                        /* Display the service discovery summary.       */
                        DisplayWSSDiscoverySummary(&(DeviceInfo->WSSClientInfo));
                        break;
                     case sdAll:
                        /* Flag that service discovery has been         */
                        /* performed on for this connection.            */
   ...
   if(argc >= NUM_EXPECTED_PARAMETERS_USB)
   {
      ...
      switch(strtol(argv[1], &endptr, 10))
      {
         ...
         case UART_PARAMETER_VALUE:
            ...
            break;
         case WSSP_PARAMETER_VALUE:
            /* The Transport selected was WSSP, check to see if the     */
            /* number of parameters entered on the command line is      */
            /* correct.                                                 */
            if(argc == NUM_EXPECTED_PARAMETERS_UART)
            {
      ...
      else
      {
         ...
         /* invalid.                                                    */
         printf("Parameter Error (Expecting = [USB = 0, UART = 1, WSSP = 2 Flag] [IF !USB [Comm Port or Device File] [Baud Rate]])\r\n");
      }
   }
   else
   {
