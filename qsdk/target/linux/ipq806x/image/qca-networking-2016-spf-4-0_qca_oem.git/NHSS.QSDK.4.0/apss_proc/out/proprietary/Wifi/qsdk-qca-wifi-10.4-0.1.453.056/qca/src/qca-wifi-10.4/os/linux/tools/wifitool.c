...
static void tr069_chan_history(const char *ifname, int argc, char *argv[])
{
    ...
    u_int8_t act_idx=0;
    struct tm tm;
    struct timespec *ts = NULL, tstamp;
...
{
    int sock_fd, err, fips_fail = 0;
    int i, size, ret, cmd, keylen, datalen, mode;
    int en=0;
    struct iwreq iwr;
    struct ath_ioctl_fips *req=NULL;
    struct ath_fips_output *output_fips;
    u_int8_t *ptr;
    FILE *fp;
    u_int8_t key[32], data[1500], exp_op[1500], iv[16];
    int eol=0;
