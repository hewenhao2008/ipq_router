...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LinuxIPSP - Linux Bluetooth Internet Protocol Support Profile using       */
/*             (GATT based) LE application.                                   */
/*                                                                            */
/*  Author:  Ryan McCord                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   08/21/15  R. McCord      Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "LinuxIPSP.h"           /* Application Header.                       */
...
#include "SS1BTIPSP.h"           /* Main SS1 IPSP Service Header.             */
...
#define EXIT_MODE                                  (-10) /* Flags exit from   */
   ...
   /* The following constants represent the application's default IPSP  */
   /* channel parameters.                                               */
#define IPSP_CHANNEL_PARAMETERS_FLAGS              (0)
#define IPSP_CHANNEL_PARAMETERS_MAX_SDU_SIZE       ((1080) - NON_ALIGNED_WORD_SIZE)
#define IPSP_CHANNEL_PARAMETERS_MAX_PDU_SIZE       (1080)
#define IPSP_CHANNEL_PARAMETERS_PDU_QUEUE_DEPTH    (20)
#define IPSP_CHANNEL_PARAMETERS_MAX_CREDITS        (50)

   /* The following constants represent the application's default queuing*/
   /* parameters for sending packet data over the IPSP connection.      */
#define IPSP_QP_DATA_PACKET_QUEUEING_FLAGS         (L2CA_QUEUEING_FLAG_LIMIT_BY_PACKETS)
#define IPSP_QP_MAXIMUM_NUMBER_QUEUED_DATA_PACKETS (5)
#define IPSP_QP_QUEUED_DATA_PACKETS_THRESHOLD      (0)
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                "LinuxIPSP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                "LinuxIPSP_FTS.log"

   /* Determine the Name we will use for this compilation.              */
#define LE_DEMO_DEVICE_NAME                        "LinuxIPSP"
...
typedef struct _tagGAPS_Device_Appearance_Mapping_t
{
   ...
   /* structure.                                                        */
   /* * NOTE * There is no IPSP Server or Client information to store   */
   /*          service attribute handles since IPSS is only included for*/
   /*          the IPSP Node and contains only the primary service      */
   /*          attribute.                                               */
...
#define DEVICE_INFO_FLAGS_IPSP_MANUAL_CREDIT_MODE           0x10
...
static Boolean_t           NodeRegistered;          /* A boolean flag to show if the   */
                                                    /* sample is configured as a IPSP  */
                                                    /* Node so we can configure the    */
                                                    /* advertising data with IPSS.     */

static Word_t              PacketLength;            /* The size of the specified       */
                                                    /* packet data.                    */

static Byte_t             *PacketBuffer;            /* Buffer that will hold the packet*/
                                                    /* data.                           */

static Word_t              PacketBufferOffset;      /* Offset which is used to index   */
                                                    /* the packet buffer.              */

static Word_t              RemoteMTU;               /* Holds the MTU size so we can    */
                                                    /* Determine how much data we can  */
                                                    /* send for a packet.              */


static Word_t              IPSPChannelID;           /* Holds the Channel Identifier    */
                                                    /* CID for the IPSP Connection.    */
...
static BTPSCONST char *IOCapabilitiesStrings[] =
{
   ...
   /* The following string is used to send test data across the IPSP    */
   /* connection.                                                       */
static char *TestPacketData = "This is IPSP test packet data.";
...
static int SetGATTMTU(ParameterList_t *TempParam);
   ...
   /* Internet Protocol Support Profile Function Commands               */
static int RegisterNode(ParameterList_t *TempParam);
static int UnregisterNode(ParameterList_t *TempParam);
static int DiscoverIPSS(ParameterList_t *TempParam);
static int OpenNode(ParameterList_t *TempParam);
static int CloseIPSP(ParameterList_t *TempParam);
static int SendPacketCommand(ParameterList_t *TempParam);
static void SendPacket(void);
static int SetConnectionMode(ParameterList_t *TempParam);
static int GetConnectionMode(ParameterList_t *TempParam);

   /* BTPS Callback function prototypes.                                */
   /* * NOTE * There is no GATT_ClientEventCallback_IPSS() since this   */
   /*          application will not issue GATT requests for IPSS.       */
...
static void BTPSAPI IPSP_EventCallback(unsigned int BluetoothStackID, IPSP_Event_Data_t *IPSP_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI GATT_ClientEventCallback_IPSP(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTER", RegisterNode);
   AddCommand("UNREGISTER", UnregisterNode);
   AddCommand("DISCOVERIPSS", DiscoverIPSS);
   AddCommand("OPENNODE", OpenNode);
   AddCommand("CLOSE", CloseIPSP);
   AddCommand("SENDPACKET", SendPacketCommand);
   AddCommand("SETMODE", SetConnectionMode);
   AddCommand("GETMODE", GetConnectionMode);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("* Command Options IPSP:                                          *\r\n");
   printf("*                Node  :   Register,                             *\r\n");
   printf("*                          Unregister,                           *\r\n");
   printf("*                          GetMode, SetMode,                     *\r\n");
   printf("*                Router:   DiscoverIPSS,                         *\r\n");
   printf("*                          OpenNode,                             *\r\n");
   printf("*                Both:     SendPacket, Close                     *\r\n");
   printf("******************************************************************\r\n");
   ...
   if((BluetoothStackID) && (!ConnectionID))
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         else
         {
            ...
            /* Configure the flags field ipsped on the Discoverability  */
            /* Mode.                                                    */
            ...
            if(NodeRegistered)
            {
               /* Advertise the Internet Protocol Support Service (Nodes*/
               /* Only).  (1 byte type and 2 bytes UUID)                */
               ...
               IPSS_ASSIGN_IPSS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
            }
...
static int EnableDebug(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering a IPSP Node.*/
...
static int RegisterNode(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      ChannelParameters.ChannelFlags   = IPSP_CHANNEL_PARAMETERS_FLAGS;
      ChannelParameters.MaxSDUSize     = IPSP_CHANNEL_PARAMETERS_MAX_SDU_SIZE;
      ChannelParameters.MaxPDUSize     = IPSP_CHANNEL_PARAMETERS_MAX_PDU_SIZE;
      ChannelParameters.PDUQueueDepth  = IPSP_CHANNEL_PARAMETERS_PDU_QUEUE_DEPTH;
      ChannelParameters.MaximumCredits = IPSP_CHANNEL_PARAMETERS_MAX_CREDITS;

      /* Register the Node.                                             */
      /* * NOTE * This API will register the IPSS, which is required for*/
      /*          the Node.                                             */
      if(!(ret_val = IPSP_Initialize_Node_Role(BluetoothStackID, &ChannelParameters, IPSP_EventCallback, BluetoothStackID, NULL)))
      {
         /* Display success message.                                    */
         printf("Successfully registered IPSP Node.\r\n");

         /* Flag that we have registered a node.                        */
         NodeRegistered = TRUE;
      }
      else
         DisplayFunctionError("IPSP_Initialize_Node_Role", ret_val);
   }
   else
   {
   ...
   /* The following function is responsible for unregistering a IPSP    */
   /* Node.  This function will return zero on successful execution and */
...
static int UnregisterNode(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      /* Unregister the Node.                                           */
      ret_val = IPSP_Cleanup_Node_Role(BluetoothStackID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         printf("Successfully unregistered the IPSP Node.\r\n");

         /* Flag that we have unregistered a node.                      */
         NodeRegistered = FALSE;
      }
      else
         DisplayFunctionError("IPSP_Cleanup_Node_Role", ret_val);
   }
   else
   {
      printf("Connection currently active.\r\n");
   ...
   /* The following function is responsible for performing a IPSS       */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverIPSS(ParameterList_t *TempParam)
{
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            /* Configure the filter so that only the IPSS Service is    */
            /* discovered.                                              */
            UUID[0].UUID_Type = guUUID_16;
            IPSS_ASSIGN_IPSS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

            /* Start the service discovery process.                     */
            ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdIPSS);
            if(!ret_val)
            {
               /* Display success message.                              */
               printf("GATT_Service_Discovery_Start success.\r\n");
   ...
   /* The following function is responsible for establishing an IPSP    */
   /* connection to a remote Node.  This function MUST be called by the */
   /* router.  This function will return zero on successful execution   */
...
static int OpenNode(ParameterList_t *TempParam)
{
   ...
   {
      /* Verify that there is a active connection.                      */
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            {
               ChannelParameters.ChannelFlags   =  IPSP_CHANNEL_PARAMETERS_FLAGS;
               DeviceInfo->Flags               &= ~DEVICE_INFO_FLAGS_IPSP_MANUAL_CREDIT_MODE;
            }
            else
            {
            ...
            /* Connect to the IPSP Node.                                */
            /* * NOTE * We can be registered as an IPSP Node, but if we */
            /*          call this API will be acting as the IPSP Router */
            /*          role and connecting to another Node.            */
            if((ret_val = IPSP_Connect_Remote_Node(BluetoothStackID, ConnectionBD_ADDR, &ChannelParameters, IPSP_EventCallback, BluetoothStackID)) > 0)
            {
               printf("\r\nIPSP connection request sent.\r\n");
               printf("   CID:  0x%04X\r\n", (Word_t)ret_val);
               ...
               DisplayFunctionError("IPSP_Connect_Remote_Node", ret_val);
         }
         else
         {
   ...
   {
      printf("Usage: OpenNode [Credit Mode (0=Automatic,1=Manual)]\r\n");
      ret_val = INVALID_PARAMETERS_ERROR;
   }
   ...
   /* The following function is responsible for diconnecting an IPSP    */
   /* connection to a remote Node.  This function MUST be called by the */
   /* router.  This function will return zero on successful execution   */
   /* and a negative value on errors.                                   */
static int CloseIPSP(ParameterList_t *TempParam)
{
   ...
   if(ConnectionID)
   {
      /* Close the IPSP Connection for the BD_ADDR and specified role.  */
      if((ret_val = IPSP_Close_Connection(BluetoothStackID, IPSPChannelID)) != 0)
         DisplayFunctionError("IPSP_Close_Connection", ret_val);
   }
   else
      printf("No Connection Established\r\n");
   ...
   /* The following function is responsible for sending a packet to the */
   /* IPSP Router or the IPSP Node.  This function returns zero for     */
   /* success or a negative error code for errors.                      */
static int SendPacketCommand(ParameterList_t *TempParam)
{
   int           ret_val = 0;
   unsigned int  Index;
   unsigned int  TestDataSize;
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1))
   {
      ...
      if(ConnectionID)
      {
         /* Allocate enough memory for the specified packet size.       */
         PacketLength = TempParam->Params[0].intParam;
         if(PacketLength)
         {
            /* Allocate memory for the packet based on the specified    */
            /* length.                                                  */
            if((PacketBuffer = (Byte_t *)BTPS_AllocateMemory(sizeof(Byte_t)*PacketLength)) != NULL)
            {
               /* Store the test packet data string length.             */
               TestDataSize = BTPS_StringLength(TestPacketData);

               /* Reset the packet buffer offset in case it has not been*/
               /* cleaned up.                                           */
               PacketBufferOffset = 0;

               /* Fill the packet buffer with test data until we reach  */
               /* the desired length.  This loop will continually write */
               /* the test packet data string repeatedly into the packet*/
               /* buffer until we reach the desired packet length.      */
               while(PacketBufferOffset < PacketLength)
               {
                  /* Simply copy entire test string into the buffer.    */
                  /* Increment the offset of the buffer each time we    */
                  /* store a byte of the test data string.              */
                  /* * NOTE * In case the packet length is smaller then */
                  /*          the entire string we need to make sure we */
                  /*          don't go out of bounds.                   */
                  for(Index = 0; (Index < TestDataSize) && (PacketBufferOffset < PacketLength); Index++, PacketBufferOffset++)
                  {
                     PacketBuffer[PacketBufferOffset] = (Byte_t)(TestPacketData[Index]);
                  }
               }

               /* Reset the packet buffer offset so we can use it to    */
               /* index the packet buffer.                              */
               PacketBufferOffset = 0;

               /* Call the internal function to send data over the IPSP */
               /* connection.                                           */
               SendPacket();
            }
            else
            ...
            printf("Packet size specified is zero.\r\n");
      }
      else
         printf("No Connection Established\r\n");
   }
   else
      printf("\r\nUsage: SendPacket [Packet size (UINT16)]\r\n");

   return(ret_val);
}

   /* This is a helper function to send data over the IPSP connection.  */
   /* This function does not check that an IPSP connection has been     */
   /* established, but will simply try to send the data to the remote   */
   /* device.                                                           */
static void SendPacket(void)
{
   int                        ret_val = 0;
   ...
   /* Set the queuing parameters.                                       */
   QueueingParameters.Flags        = IPSP_QP_DATA_PACKET_QUEUEING_FLAGS;
   QueueingParameters.QueueLimit   = IPSP_QP_MAXIMUM_NUMBER_QUEUED_DATA_PACKETS;
   QueueingParameters.LowThreshold = IPSP_QP_QUEUED_DATA_PACKETS_THRESHOLD;

   /* While we did not receive the empty buffer space error we will     */
   /* attemp to keep sending data if we can.                            */
   while(ret_val != BTPS_ERROR_INSUFFICIENT_BUFFER_SPACE)
   {
      /* Determine the length of the buffer that has not been sent yet. */
      RemainingLength = (PacketLength - PacketBufferOffset);

      /* If the remaining length can fit in the MTU then we will simply */
      /* send the remaining data.                                       */
      DataLength = (RemainingLength <= RemoteMTU) ? RemainingLength : RemoteMTU;

      /* * NOTE * If we receive the BTPS_ERROR_INSUFFICIENT_BUFFER_SPACE*/
      /*          then we must wait until we receive the                */
      /*          IPSP_Buffer_Empty_Indication_Data before we can send  */
      /*          more data.  This response may not be received if we   */
      /*          can fit all the data we want to send in one L2CAP     */
      /*          packet.                                               */
      if((ret_val = IPSP_Send_Packet(BluetoothStackID, IPSPChannelID, &QueueingParameters, DataLength, (PacketBuffer + PacketBufferOffset))) != BTPS_ERROR_INSUFFICIENT_BUFFER_SPACE)
      {
         if(ret_val >= 0)
         {
            /* Update the packet buffer offset by the number of bytes   */
            /* successfully written.                                    */
            PacketBufferOffset += DataLength;

            /* If we have sent the total packet then we need to break   */
            /* from the loop.                                           */
            if(PacketBufferOffset >= PacketLength)
            {
               /* Make sure we sent the correct amount of data.         */
               if(PacketBufferOffset > PacketLength)
               {
                  printf("Too much data was sent.\r\n");
               }

               /* We need to free the memory allocated for the packet.  */
               BTPS_FreeMemory(PacketBuffer);
               PacketBuffer = NULL;
               PacketLength = 0;

               /* Break from the loop since there is no more data to    */
               /* send.                                                 */
               break;
            }
         }
         ...
         {
            /* Some error occured other than the                        */
            /* BTPS_ERROR_INSUFFICIENT_BUFFER_SPACE so we need to print */
            /* an error and break from the loop.                        */
            DisplayFunctionError("IPSP_Send_Packet", ret_val);
            printf("\r\nBytes sent:    %u\r\n", PacketBufferOffset);
            ...
            /* Break from the loop since an error occured.              */
            break;
         }
      }
   }
...
{
   int                          ret_val = 0;
   IPSP_Node_Connection_Mode_t  ConnectionMode;
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 2))
   {
      ...
      if(!ConnectionID)
      {
         /* Make sure this a Node.                                      */
         if(NodeRegistered)
         {
            /* Store the connection Mode.                               */
            ConnectionMode = (IPSP_Node_Connection_Mode_t)TempParam->Params[0].intParam;

            if((ret_val = IPSP_Set_Node_Connection_Mode(BluetoothStackID, ConnectionMode)) != 0)
               DisplayFunctionError("IPSP_Set_Node_Connection_Mode", ret_val);
         }
         else
         {
            printf("Node has not been initialized.\r\n");
            ret_val = FUNCTION_ERROR;
         }
      }
      else
      {
         printf("Must be disconnected.\r\n");
         ret_val = FUNCTION_ERROR;
      }
   }
   else
   {
      printf("Usage: SetMode [Node Connection Mode (0=Automatic Accept,1=Automatic Accept,2=Manual Accept)]\r\n");
      ret_val = INVALID_PARAMETERS_ERROR;
   }
...
static int GetConnectionMode(ParameterList_t *TempParam)
{
   ...
   if(NodeRegistered)
   {
      if((ret_val = IPSP_Get_Node_Connection_Mode(BluetoothStackID, &ConnectionMode)) == 0)
      {
         printf("\r\n The Node's connection mode is: %s.\r\n", ((ConnectionMode == cmAutomaticAccept) ? "Automatic accept" : ((ConnectionMode == cmAutomaticReject) ? "Automatic Reject" : "Manual Accept")));
      }
      else
         DisplayFunctionError("IPSP_Get_Node_Connection_Mode", ret_val);
   }
   else
   {
      printf("Node has not been initialized.\r\n");
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
                  ...
                  case latConfirmationRequest:
                     ...
                     switch(Authentication_Event_Data->Authentication_Event_Data.Confirmation_Request.Request_Type)
                     {
                        case crtNone:
                           ...
                           /* Handle this differently ipsped on the local*/
                           /* IO Caps.                                  */
                  ...
                  case latExtendedConfirmationRequest:
                     ...
                     switch(Authentication_Event_Data->Authentication_Event_Data.Extended_Confirmation_Request.Request_Type)
                     {
                        ...
                        case crtDisplayYesNo:
                           ...
                           {
                              /* Handle this differently ipsped on the   */
                              /* local IO Caps.                         */
                              switch(LE_Parameters.IOCapability)
                              {
                                 case licNoInputNoOutput:
   ...
   /* The following is a BAP Server Event Callback.  This function will */
   /* be called whenever an BAP Server Profile Event occurs that is     */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the IPSP Event Data  */
   /* that occurred and the IPSP Event Callback Parameter that was      */
   /* specified when this Callback was installed.  The caller is free to*/
   /* use the contents of the IPSP Event Data ONLY in the context of    */
   ...
   /* possible (this argument holds anyway because another IPSP Event   */
   /* will not be processed while this function call is outstanding).   */
   /* ** NOTE ** This function MUST NOT Block and wait for events that  */
   /*            can only be satisfied by Receiving IPSP Event Packets. */
   /*            A Deadlock WILL occur because NO IPSP Event Callbacks  */
...
static void BTPSAPI IPSP_EventCallback(unsigned int BluetoothStackID, IPSP_Event_Data_t *IPSP_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* Verify that all parameters to this callback are Semi-Valid.       */
   if((BluetoothStackID) && (IPSP_Event_Data))
   {
      switch(IPSP_Event_Data->Event_Data_Type)
      {
         case etIPSP_Open_Indication:
            printf("etIPSP_Open_Indication with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data)
            {
               /* Save event information for later use.                 */
               BD_ADDR       = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->RemoteDevice;
               Role          = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->Role;
               IPSPChannelID = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->CID;
               MaxSDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->MaxSDUSize;
               MaxPDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->MaxPDUSize;
               Credits       = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->InitialCredits;
               ...
               /* Store the Remote MTU for send packet requests.        */
               RemoteMTU = MaxSDU;
            }
            break;
         case etIPSP_Open_Request_Indication:
            printf("etIPSP_Open_Request_Indication with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data)
            {
               /* Save event information for later use.                 */
               BD_ADDR       = IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data->RemoteDevice;
               Role          = IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data->Role;
               IPSPChannelID = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->CID;
               MaxSDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data->MaxSDUSize;
               MaxPDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data->MaxPDUSize;
               Credits       = IPSP_Event_Data->Event_Data.IPSP_Open_Request_Indication_Data->InitialCredits;
               ...
               /* Since the we received this event we need to manually  */
               /* accept the connection .                               */
               if((Result = IPSP_Open_Connection_Request_Response(BluetoothStackID, IPSPChannelID, TRUE)) != 0)
                  DisplayFunctionError("IPSP_Open_Connection_Request_Response", Result);
            }
            break;
         case etIPSP_Open_Confirmation:
            printf("etIPSP_Open_Confirmation with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data)
            {
               /* Save event information for later use.                 */
               BD_ADDR       = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->RemoteDevice;
               Role          = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->Role;
               IPSPChannelID = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->CID;
               MaxSDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->MaxSDUSize;
               MaxPDU        = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->MaxPDUSize;
               Credits       = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->InitialCredits;
               Status        = IPSP_Event_Data->Event_Data.IPSP_Open_Confirmation_Data->ConnectionStatus;
               ...
               switch(Status)
               {
                  case IPSP_OPEN_STATUS_SUCCESS:
                     printf("Success.\r\n");
                     break;
                  case IPSP_OPEN_STATUS_CONNECTION_TIMEOUT:
                     printf("Connection Timeout.\r\n");
                     break;
                  case IPSP_OPEN_STATUS_CONNECTION_REFUSED:
                     printf("Connection Refused.\r\n");
                     break;
                  case IPSP_OPEN_STATUS_UNKNOWN_ERROR:
            ...
            break;
         case etIPSP_Close_Indication:
            printf("etIPSP_Close_Indication with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Close_Indication_Data)
            {
               /* Display event information.                            */
               printf("   Reason:  %u.\r\n", IPSP_Event_Data->Event_Data.IPSP_Close_Indication_Data->Reason);
               printf("   CID:     0x%04X.\r\n", IPSP_Event_Data->Event_Data.IPSP_Close_Indication_Data->CID);

               /* Reset the Channel ID.                                 */
               IPSPChannelID = 0;
            }
            break;
         case etIPSP_Close_Confirmation:
            printf("etIPSP_Close_Confirmation with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Close_Confirmation_Data)
            {
               /* Display event information.                            */
               printf("   Result:  %u.\r\n", IPSP_Event_Data->Event_Data.IPSP_Close_Confirmation_Data->Result);
               printf("   CID:     0x%04X.\r\n", IPSP_Event_Data->Event_Data.IPSP_Close_Confirmation_Data->CID);
            }
            break;
         case etIPSP_Data_Indication:
            printf("etIPSP_Data_Indication with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data)
            {
               /* Save event information for later use.                 */
               BD_ADDR       = IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data->RemoteDevice;
               Role          = IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data->Role;
               IPSPChannelID = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->CID;
               Credits       = IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data->CreditsConsumed;
               DataLength    = IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data->DataLength;
               Data          = IPSP_Event_Data->Event_Data.IPSP_Data_Indication_Data->Data;
               ...
               printf("   Role:             %s.\r\n", (Role) ? "Router" : "Node");
               printf("   CID:              0x%04X.\r\n", IPSPChannelID);
               printf("   Credits Consumed: %u.\r\n", Credits);
               printf("   Data Length:      %u.\r\n", DataLength);

               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  /* We will simply display the incoming data.          */
                  printf("   Data:\r\n\r\n");
                  ...
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_IPSP_MANUAL_CREDIT_MODE)
                  {
                     if((Result = IPSP_Grant_Credits(BluetoothStackID, IPSPChannelID, Credits)) != 0)
                        DisplayFunctionError("IPSP_Grant_Credits", Result);
                  }
               }
               else
            ...
            break;
         case etIPSP_Buffer_Empty_Indication:
            printf("etIPSP_Buffer_Empty_Indication with size %u.\r\n", IPSP_Event_Data->Event_Data_Size);
            if(IPSP_Event_Data->Event_Data.IPSP_Buffer_Empty_Indication_Data)
            {
               /* Save event information for later use.                 */
               BD_ADDR       = IPSP_Event_Data->Event_Data.IPSP_Buffer_Empty_Indication_Data->RemoteDevice;
               Role          = IPSP_Event_Data->Event_Data.IPSP_Buffer_Empty_Indication_Data->Role;
               IPSPChannelID = IPSP_Event_Data->Event_Data.IPSP_Open_Indication_Data->CID;
               ...
               /* Call the internal function to send more data.         */
               SendPacket();
            }
            break;
         default:
            printf("Unknown IPSP Event\r\n");
            break;
      }
   }
   else
   {
      ...
      printf("IPSP Callback Data: Event_Data = NULL.\r\n");
   }
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     if(!GATT_Query_Maximum_Supported_MTU(BluetoothStackID, &MTU))
                        GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, MTU, GATT_ClientEventCallback_IPSP, 0);
                  }
               }
            }
            else
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  switch((Service_Discovery_Type_t)CallbackParameter)
                  {
                     case sdGAPS:
                        printf("\r\n");
                        printf("GAPS 0x%04X - 0x%04X, UUID: ", GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle);
                     ...
                     case sdDIS:
                        printf("\r\n");
                        printf("DIS 0x%04X - 0x%04X, UUID: ", GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle);
                        DisplayUUID(&(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.UUID));
                        printf("\r\n");
                     ...
                     case sdIPSS:
                        printf("\r\n");
                        printf("IPSS 0x%04X - 0x%04X, UUID: ", GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle);
                        DisplayUUID(&(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.UUID));
                        printf("\r\n");
                        break;
                     default:
                        printf("\r\n");
                        printf("Unknown 0x%04X - 0x%04X, UUID: ", GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.Service_Handle, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.End_Group_Handle);
                        DisplayUUID(&(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data->ServiceInformation.UUID));
                        printf("\r\n");
                        break;
                  }
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  /* Check the service discovery type for IPSP          */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdIPSS)
                  {
