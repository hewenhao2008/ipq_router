...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXCSCM - Simple Linux application using Bluetopia Platform Manager     */
/*              Cycling Speed and Cadence (CSC - Client Role only) Manager    */
...
#include "LinuxCSCM_COL.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTCSCM.h"     /* CSC Manager Application Programming Interface.  */
...
#ifndef CSCS_ASSIGN_SERVICE_UUID_16
...
static unsigned int        CSCMClientEventCallbackID;/* Variable which holds the       */
                                                    /* current CSCM Server Event       */
                                                    /* Callback ID that is assigned    */
                                                    /* from the CSC Manager when the   */
                                                    /* local client registers for CSC  */
                                                    /* Server Events.                  */
...
static int RegisterCSCMEvents(ParameterList_t *TempParam);
static int UnRegisterCSCMEvents(ParameterList_t *TempParam);
...
static char *ConvertSensorLocationToString(CSCM_Sensor_Location_t Location);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM CSC Manager Callback function prototype.                     */
static void BTPSAPI CSCM_Event_Callback(CSCM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERCSCMEVENTS", RegisterCSCMEvents);
   AddCommand("UNREGISTERCSCMEVENTS", UnRegisterCSCMEvents);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      CSCS_ASSIGN_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  39)RegisterCSCMEvents                         *\r\n");
   printf("*                  40)UnRegisterCSCMEvents                       *\r\n");
...
static int RegisterCSCMEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      if(!CSCMClientEventCallbackID)
      {
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Register_Collector_Event_Callback(CSCM_Event_Callback, NULL)) > 0)
         {
            printf("CSCM_Register_Collector_Event_Callback() Success. Callback ID: %d\r\n", ret_val);

            CSCMClientEventCallbackID = (unsigned int)ret_val;
            ret_val                   = 0;
         }
         else
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Register_Collector_Event_Callback() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int UnRegisterCSCMEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Submit the Command.                                            */
         CSCM_Un_Register_Collector_Event_Callback(CSCMClientEventCallbackID);

         printf("CSCM_Un_Register_Collector_Event_Callback() Success.\r\n");

         CSCMClientEventCallbackID = 0;
         ret_val                   = 0;
      }
      else
      {
...
static int QueryConnectedSensors(ParameterList_t *TempParam)
{
   ...
   CSCM_Connected_Sensor_t *ConnectedSensorList = NULL;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         ...
         /* them                                                        */
         if((!MaximumDevices) || ((ConnectedSensorList = (CSCM_Connected_Sensor_t *)BTPS_AllocateMemory(sizeof(CSCM_Connected_Sensor_t) * MaximumDevices)) != NULL))
         {
            if((ret_val = CSCM_Query_Connected_Sensors(MaximumDevices, ConnectedSensorList, &TotalDevices)) >= 0)
            {
               printf("CSCM_Query_Connected_Sensors() Success. Result: %d. Total Devices: %u.\r\n", ret_val, TotalDevices);
               ...
               for(Index=0;Index<(unsigned int)ret_val;Index++)
               {
                  ...
                  if(ConnectedSensorList[Index].SupportedFeatures & CSCM_FEATURE_MULTIPLE_SENSOR_LOCATIONS_SUPPORTED)
            ...
            {
               /* Error occurred, inform the user and flag an error.          */
               printf("CSCM_Query_Connected_Sensors() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int ConfigureRemoteSensor(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Configure_Remote_Sensor(BD_ADDR, Flags)) == 0)
         {
            printf("CSCM_Configure_Remote_Sensor() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Configure_Remote_Sensor() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int UnConfigureRemoteSensor(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Un_Configure_Remote_Sensor(BD_ADDR)) == 0)
         {
            printf("CSCM_Un_Configure_Remote_Sensor() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Un_Configure_Remote_Sensor() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int GetConnectedSensorInfo(ParameterList_t *TempParam)
{
   int                     ret_val;
   BD_ADDR_t               BD_ADDR;
   CSCM_Connected_Sensor_t ConnectedSensor;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Get_Connected_Sensor_Info(BD_ADDR, &ConnectedSensor)) == 0)
         {
            printf("CSCM_Get_Connected_Sensor_Info() Success. \r\n");
            printf("\r\nSensor Info:\r\n");
            printf("\tConfigured:                 %s.\r\n", ConnectedSensor.Configured?"TRUE":"FALSE");
            printf("\tSuppoted Characteristics:   %08lX.\r\n", ConnectedSensor.SupportedOptionalCharateristics);
            printf("\tSupported Features:         %08lX.\r\n", ConnectedSensor.SupportedFeatures);

            if(ConnectedSensor.SupportedFeatures & CSCM_FEATURE_MULTIPLE_SENSOR_LOCATIONS_SUPPORTED)
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Get_Connected_Sensor_Info() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int GetSensorLocation(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Get_Sensor_Location(BD_ADDR)) == 0)
         {
            printf("CSCM_Get_Sensor_Location() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Get_Sensor_Location() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int UpdateCumulativeValue(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = CSCM_Update_Cumulative_Value(BD_ADDR, (DWord_t)TempParam->Params[1].intParam)) > 0)
         {
            printf("CSCM_Update_Cumulative_Value() Success. ProcedureID: %d.\r\n", ret_val);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Update_Cumulative_Value() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int UpdateSensorLocation(ParameterList_t *TempParam)
{
   ...
   {
      /* Make sure the the parameters for this function seem valid.     */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (TempParam->Params[1].intParam >= cslOther) && (TempParam->Params[1].intParam <= cslChest))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Submit the Command.                                         */
         if((ret_val = CSCM_Update_Sensor_Location(BD_ADDR, (CSCM_Sensor_Location_t)TempParam->Params[1].intParam)) > 0)
         {
            printf("CSCM_Update_Sensor_Location() Success. Procedure ID: %d\r\n", ret_val);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("CSCM_Update_Sensor_Location() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      else
      {
         ...
         for(Index=cslOther; Index <= cslChest; Index++)
         {
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case cetCSCConfigurationStatusChanged:
            ...
            break;
         case cetCSCMeasurement:
            BD_ADDRToStr(EventData->EventData.MeasurementEventData.RemoteDeviceAddress, BoardStr);
            printf("CSCM Measurement:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tFlags:                 %08lX.\r\n", EventData->EventData.MeasurementEventData.Flags);

            if(EventData->EventData.MeasurementEventData.Flags & CSCM_MEASUREMENT_FLAGS_WHEEL_REVOLUTION_DATA_PRESENT)
            {
               printf("\r\n\tWheel Revolution Data:\r\n");
               printf("\t\tCumulative Revolutions: %u.\r\n", EventData->EventData.MeasurementEventData.WheelRevolutionData.CumulativeWheelRevolutions);
               printf("\t\tLast Event Time:        %u.\r\n", EventData->EventData.MeasurementEventData.WheelRevolutionData.LastWheelEventTime);
            }

            if(EventData->EventData.MeasurementEventData.Flags & CSCM_MEASUREMENT_FLAGS_CRANK_REVOLUTION_DATA_PRESENT)
            {
               printf("\r\n\tCrank Revolution Data:\r\n");
               printf("\t\tCumulative Revolutions: %u.\r\n", EventData->EventData.MeasurementEventData.CrankRevolutionData.CumulativeCrankRevolutions);
               printf("\t\tLast Event Time:        %u.\r\n", EventData->EventData.MeasurementEventData.CrankRevolutionData.LastCrankEventTime);
            }
            break;
         case cetCSCSensorLocationResponse:
            BD_ADDRToStr(EventData->EventData.SensorLocationResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("CSCM Sensor Location Response:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
         ...
         case cetCSCProcedureComplete:
            ...
            if(EventData->EventData.ProcedureCompleteEventData.Status == CSCM_PROCEDURE_STATUS_FAILURE_ERROR_RESPONSE)
               printf("\tResponse Error Code    %u.\r\n", EventData->EventData.ProcedureCompleteEventData.ResponseErrorCode);
            break;
         default:
            printf("Unknown CSC Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nCSCM Event Data is NULL.\r\n");
