...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXGLPM_COL - Simple Linux application using Bluetopia Platform Manager */
/*                  Glucose Profile (GLP - Collector role only) Manager       */
...
/*   --------  -----------    ------------------------------------------------*/
/*   03/14/13  T. Cook        Initial creation. (Based on LinuxGLPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxGLPM_COL.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTGLPM.h"     /* GLP Manager Application Programming Interface.  */
...
#ifndef GLS_ASSIGN_SERVICE_UUID_16
...
static unsigned int        GLPEventCallbackID;      /* Variable which holds the        */
                                                    /* current GLP Event Callback ID   */
                                                    /* that is assigned from the GLP   */
                                                    /* Manager when the local client   */
                                                    /* registers for GLP Events.       */
...
static int                  FileDescriptor;         /* Variable which holds the current*/
                                                    /* file descriptor of the data log.*/

   /* The following variables are used when performing GLP Procedures to*/
   /* a remote device.                                                  */
static Boolean_t             DataTimeConfigured;
static GLPM_Date_Time_Data_t DataTime[2];
static Boolean_t             SequenceNumberConfigured;
static unsigned int          SequenceNumbers[2];
...
static char *IOCapabilitiesStrings[] =
{
   ...
   /* The following array is used to convert the measurement enumerated */
   /* type to a string.                                                 */
static char *MeasurementTypeStrings[] =
{
...
#define NUM_SUPPORTED_MEASUREMENT_TYPES                        (sizeof(MeasurementTypeStrings)/sizeof(char *))

   /* The following array is used to convert the sample location        */
   /* enumerated type to a string.                                      */
static char *SampleLocationStrings[] =
{
   "Finger",
...
#define NUM_SUPPORTED_SAMPLE_LOCATION_TYPES                    (sizeof(SampleLocationStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Procedure Type        */
   /* enumerated type to a string.                                      */
static char *ProcedureTypeStrings[] =
{
...
#define NUM_SUPPORTED_PROCEDURE_TYPES                          (sizeof(ProcedureTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Procedure Response    */
   /* Code enumerated type to a string.                                 */
static char *ProcedureResponseCodeStrings[] =
{
   "Success",
...
#define NUM_SUPPORTED_PROCEDURE_RESPONSE_CODE_TYPES            (sizeof(ProcedureResponseCodeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Measurement Error type*/
   /* enumerated type to a string.                                      */
static char *MeasurementErrorTypeStrings[] =
{
   "No error",
   "No Glucose Context",
...
#define NUM_SUPPORTED_MEASUREMENT_ERROR_TYPES                  (sizeof(MeasurementErrorTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Meal type enumerated  */
   /* type to a string.                                                 */
static char *MealTypeStrings[] =
{
   "Preprandial",
   "Postprandial",
   "Fasting",
...
#define NUM_SUPPORTED_MEAL_TYPES                               (sizeof(MealTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Tester type enumerated*/
   /* type to a string.                                                 */
static char *TesterTypeStrings[] =
{
   "Self",
...
#define NUM_SUPPORTED_TESTER_TYPES                             (sizeof(TesterTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Health type enumerated*/
   /* type to a string.                                                 */
static char *HealthTypeStrings[] =
{
   "Minor health issues",
   "Major health issues",
   "During menses",
...
#define NUM_SUPPORTED_HEALTH_TYPES                             (sizeof(HealthTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Carbohydrate type     */
   /* enumerated type to a string.                                      */
static char *CarbohydrateTypeStrings[] =
{
   "Breakfast",
   "Lunch",
   "Dinner",
...
#define NUM_SUPPORTED_CARBOHYDRATE_TYPES                       (sizeof(CarbohydrateTypeStrings)/sizeof(char *))

   /* The following array is used to convert GLPM Medication type       */
   /* enumerated type to a string.                                      */
static char *MedicationTypeStrings[] =
{
...
#define NUM_SUPPORTED_MEDICATION_TYPES                         (sizeof(MedicationTypeStrings)/sizeof(char *))
...
static int GLPRegisterCollectorEvents(ParameterList_t *TempParam);
static int GLPUnRegisterCollectorEvents(ParameterList_t *TempParam);

static int GLPStartProcedure(ParameterList_t *TempParam);
static int GLPStopProcedure(ParameterList_t *TempParam);

static int ConfigureTimeRange(ParameterList_t *TempParam);
static int ConfigureSequenceNumberRange(ParameterList_t *TempParam);

static int EnableDataLog(ParameterList_t *TempParam);
...
static void DataLogger(char *LogString);
static void GLPMFloatToStr(GLPM_Floating_Point_Data_t *Value, char *StringBuffer);
static void DisplayGlucoseMeasurement(GLPM_Glucose_Measurement_Data_t *GlucoseMeasurement, int Level);
static void DisplayGlucoseMeasurementContext(GLPM_Glucose_Measurement_Context_Data_t *GlucoseMeasurementContext, int Level);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM GLP Manager Callback function prototype.                     */
static void BTPSAPI GLPM_Event_Callback(GLPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("GLPREGISTERCOLLECTOREVENTS", GLPRegisterCollectorEvents);
   AddCommand("GLPUNREGISTERCOLLECTOREVENTS", GLPUnRegisterCollectorEvents);
   AddCommand("GLPSTARTPROCEDURE", GLPStartProcedure);
   AddCommand("GLPSTOPPROCEDURE", GLPStopProcedure);
   AddCommand("CONFIGURETIMERANGE", ConfigureTimeRange);
   AddCommand("CONFIGURESEQUENCERANGE", ConfigureSequenceNumberRange);
   AddCommand("ENABLEDATALOG", EnableDataLog);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      GLS_ASSIGN_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)GLPRegisterCollectorEvents                 *\r\n");
   printf("*                  36)GLPUnRegisterCollectorEvents               *\r\n");
   printf("*                  37)GLPStartProcedure                          *\r\n");
   printf("*                  38)GLPStopProcedure                           *\r\n");
   printf("*                  39)ConfigureTimeRange                         *\r\n");
   printf("*                  40)ConfigureSequenceRange                     *\r\n");
   printf("*                  41)EnableDataLog                              *\r\n");
   printf("*                  42)StartAdvertising                           *\r\n");
   printf("*                  43)StopAdvertising                            *\r\n");
   printf("*                  44)StartObservationScan                       *\r\n");
   printf("*                  45)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the GLP*/
   /* Manager to receive GLP Events.  This function returns zero if     */
   /* successful and a negative value if an error occurred.             */
static int GLPRegisterCollectorEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!GLPEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = GLPM_Register_Collector_Event_Callback(GLPM_Event_Callback, NULL)) > 0)
         {
            printf("GLPM_Register_Collector_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            GLPEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("GLPM_Register_Collector_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("GLP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered GLP Event Registration with the GLP Manager.*/
...
static int GLPUnRegisterCollectorEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(GLPEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         GLPM_Un_Register_Collector_Event_Callback(GLPEventCallbackID);

         printf("GLPM_Un_Register_Collector_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         GLPEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("GLP Manager Event Callback is not registered.\r\n");
   ...
   /* The following function is responsible for starting a GLP Procedure*/
   /* to a remote Glucose device.  This function returns zero if        */
   /* successful and a negative value if an error occurred.             */
static int GLPStartProcedure(ParameterList_t *TempParam)
{
   int                   ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 4))
      {
         ...
         /* Convert the procedure type.                                 */
         switch(TempParam->Params[1].intParam)
         {
            case 0:
               ProcedureData.CommandType = gptReportStoredRecords;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Procedure: Report Stored Records\r\n");
               break;
            case 1:
               ProcedureData.CommandType = gptDeleteStoredRecords;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Procedure: Delete Stored Records\r\n");
               break;
            case 2:
               ProcedureData.CommandType = gptReportNumberStoredRecords;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Procedure: Report Number of Stored Records\r\n");
               break;
            default:
               ret_val = INVALID_PARAMETERS_ERROR;
               break;
         ...
         /* Convert the operator type.                                  */
         switch(TempParam->Params[2].intParam)
         {
            case 0:
               ProcedureData.OperatorType = gotAllRecords;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: All Records\r\n");
               break;
            case 1:
               ProcedureData.OperatorType = gotLessThanOrEqualTo;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: <=\r\n");
               break;
            case 2:
               ProcedureData.OperatorType = gotGreaterThanOrEqualTo;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: >=\r\n");
               break;
            case 3:
               ProcedureData.OperatorType = gotWithinRangeOf;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: >= && <=\r\n");
               break;
            case 4:
               ProcedureData.OperatorType = gotFirstRecord;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: First Record\r\n");
               break;
            case 5:
               ProcedureData.OperatorType = gotLastRecord;
               strcpy(&(StringBuffer[strlen(StringBuffer)]), "Operator: Second Record\r\n");
               break;
            default:
               ret_val = INVALID_PARAMETERS_ERROR;
               break;
         ...
         /* Convert the filter type.                                    */
         switch(TempParam->Params[3].intParam)
         {
            case 0:
               ProcedureData.FilterType = gftSequenceNumber;
               break;
            case 1:
               ProcedureData.FilterType = gftUserFacingTime;
               break;
            default:
               ret_val = INVALID_PARAMETERS_ERROR;
               break;
         ...
         if(!ret_val)
         {
            /* Check to see if the correct filters are configured.      */
            if((ProcedureData.OperatorType == gotLessThanOrEqualTo) || (ProcedureData.OperatorType == gotGreaterThanOrEqualTo) || (ProcedureData.OperatorType == gotWithinRangeOf))
            {
               /* Check to see what type of filter is specified.        */
               if(ProcedureData.FilterType == gftSequenceNumber)
               {
                  /* Verify that a sequence number range has been       */
                  /* configured.                                        */
                  if(SequenceNumberConfigured)
                  {
                     if(ProcedureData.OperatorType == gotWithinRangeOf)
                     {
                        ProcedureData.FilterParameters.SequenceNumberRange.MinimumSequenceNumber = SequenceNumbers[0];
                        ProcedureData.FilterParameters.SequenceNumberRange.MaximumSequenceNumber = SequenceNumbers[1];
                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Min: %u\r\n", ProcedureData.FilterParameters.SequenceNumberRange.MinimumSequenceNumber);
                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Max: %u\r\n", ProcedureData.FilterParameters.SequenceNumberRange.MaximumSequenceNumber);
                     }
                     else
                     {
                        ProcedureData.FilterParameters.SequenceNumber = SequenceNumbers[0];
                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Seq #: %u\r\n", ProcedureData.FilterParameters.SequenceNumber);
                     }
                  }
                  else
                     ret_val = INVALID_PARAMETERS_ERROR;
               ...
               {
                  /* Verify that a data/time range has been configured. */
                  if(DataTimeConfigured)
                  {
                     if(ProcedureData.OperatorType == gotWithinRangeOf)
                     {
                        ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime = DataTime[0];
                        ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime = DataTime[1];

                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Min: %u/%u/%u %u:%u:%u\r\n", ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Month,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Day,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Year,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Hours,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Minutes,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MinimumDateTime.Seconds);

                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Max: %u/%u/%u %u:%u:%u\r\n", ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Month,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Day,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Year,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Hours,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Minutes,
                                                                                                     ProcedureData.FilterParameters.UserFacingTimeRange.MaximumDateTime.Seconds);
                     }
                     else
                     {
                        ProcedureData.FilterParameters.UserFacingTime = DataTime[0];
                        sprintf(&(StringBuffer[strlen(StringBuffer)]), "Date: %u/%u/%u %u:%u:%u\r\n", ProcedureData.FilterParameters.UserFacingTime.Month,
                                                                                                      ProcedureData.FilterParameters.UserFacingTime.Day,
                                                                                                      ProcedureData.FilterParameters.UserFacingTime.Year,
                                                                                                      ProcedureData.FilterParameters.UserFacingTime.Hours,
                                                                                                      ProcedureData.FilterParameters.UserFacingTime.Minutes,
                                                                                                      ProcedureData.FilterParameters.UserFacingTime.Seconds);
                     }
                  }
                  else
                     ret_val = INVALID_PARAMETERS_ERROR;
            ...
            if(!ret_val)
            {
               ...
               DataLogger(StringBuffer);

               /* Attempt to start the procedure.                       */
               if((ret_val = GLPM_Start_Procedure_Request(GLPEventCallbackID, BD_ADDR, &ProcedureData)) > 0)
               {
                  printf("GLPM_Start_Procedure_Request() Success: Procedure ID = %u.\r\n", (unsigned int)ret_val);

                  ret_val = 0;
               }
               else
               {
                  /* Error starting the procedure, inform user and flag */
                  /* an error.                                          */
                  printf("GLPM_Start_Procedure_Request() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      /* If an error occurred print the usage string.                   */
      if(ret_val)
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: GLPStartProcedure [BD_ADDR] [ProcedureType] [Operator Type] [Filter Type].\r\n");
         printf("Where: Procedure Type = \r\n");
         printf("                0 = Report Stored Records\r\n");
         printf("                1 = Delete Stored Records\r\n");
         printf("                2 = Report Number of Stored Records\r\n");
         printf("Where: Operator Type = \r\n");
         printf("                0 = All Records\r\n");
         printf("                1 = Less than or equal to\r\n");
         printf("                2 = Greater than or equal to\r\n");
         printf("                3 = Within range of\r\n");
         printf("                4 = First Record\r\n");
         printf("                5 = Last Record\r\n");
         printf("Where: Filter Type = \r\n");
         printf("                0 = Sequence Number\r\n");
         printf("                1 = User Facing Time\r\n");
   ...
   /* The following function is responsible for stopping a GLP Procedure*/
   /* to a remote Glucose device.  This function returns zero if        */
   /* successful and a negative value if an error occurred.             */
static int GLPStopProcedure(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Next check to see that the command parameters are valid.       */
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         /* Convert the parameter to a Bluetooth Device Address.        */
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         sprintf(&(StringBuffer[0]), "Stopping Procedure to %s, ID = %u\r\n", TempParam->Params[0].strParam, (unsigned int)TempParam->Params[1].intParam);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         /* Attempt to start the procedure.                             */
         if((ret_val = GLPM_Stop_Procedure_Request(GLPEventCallbackID, BD_ADDR, (unsigned int)TempParam->Params[1].intParam)) == 0)
            printf("GLPM_Stop_Procedure_Request() Success.\r\n");
         else
         {
            /* Error starting the procedure, inform user and flag an    */
            /* error.                                                   */
            printf("GLPM_Stop_Procedure_Request() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: GLPStopProcedure [BD_ADDR] [ProcedureID].\r\n");
   ...
   /* The following function is responsible for configuring the time    */
   /* range that will be used in futured Glucose Procedures.  This      */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int ConfigureTimeRange(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         /* Loop and do the necessary conversion.                       */
         for(Index=0,ret_val=0;Index<2;Index++)
         {
            if((Result = sscanf(TempParam->Params[Index].strParam, "%u/%u/%u,%u:%u:%u",  &(DataTime[Index].Month),
                                                                                         &(DataTime[Index].Day),
                                                                                         &(DataTime[Index].Year),
                                                                                         &(DataTime[Index].Hours),
                                                                                         &(DataTime[Index].Minutes),
                                                                                         &(DataTime[Index].Seconds))) != 6)
            {
               /* One or more of the necessary parameters is/are        */
               /* invalid.                                              */
               printf("Usage: ConfigureTimeRange [Minimum Time String] [Maximum Time String].\r\n");
               printf("Where: Time String = ""MM/DD/YY,HH:MM:SS"".\r\n");
               printf("Parsed: %d parameters at %u index (%s).\r\n", Result, Index, TempParam->Params[Index].strParam);
            ...
            {
               printf("Date: %u/%u/%u %u:%u:%u.\r\n", DataTime[Index].Month,
                                                        DataTime[Index].Day,
                                                        DataTime[Index].Year,
                                                        DataTime[Index].Hours,
                                                        DataTime[Index].Minutes,
                                                        DataTime[Index].Seconds);
            }
         }

         /* If the date time is configured successfully flag so.        */
         if(!ret_val)
            DataTimeConfigured = TRUE;
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
   ...
   /* The following function is responsible for configuring the sequence*/
   /* number range that will be used in futured Glucose Procedures.     */
   /* This function returns zero if successful and a negative value if  */
   /* an error occurred.                                                */
static int ConfigureSequenceNumberRange(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         if(TempParam->Params[0].intParam <= TempParam->Params[1].intParam)
         {
            /* Save the sequence range.                                 */
            SequenceNumbers[0]       = (unsigned int)TempParam->Params[0].intParam;
            SequenceNumbers[1]       = (unsigned int)TempParam->Params[1].intParam;
            SequenceNumberConfigured = TRUE;
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ConfigureSquenceRange [Minimum Sequence Number] [Maximum Sequence Number].\r\n");
   ...
   /* The following function is responsible for enabling a Data Log.    */
   /* This function returns zero if successful and a negative value if  */
   /* an error occurred.                                                */
static int EnableDataLog(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         if(TempParam->Params[0].intParam)
         {
            /* Verify that the file name is open.                       */
            if(TempParam->NumberofParameters >= 2)
            {
               /* File Name built, attempt to open it.                  */
               FileDescriptor = open(TempParam->Params[1].strParam, (O_NONBLOCK | O_APPEND | O_CREAT | O_WRONLY), (S_IRWXG | S_IRWXU | S_IRWXO));
            ...
            else
            {
               /* One or more of the necessary parameters is/are        */
               /* invalid.                                              */
               printf("Usage: EnableDataLog [0 = Disable, 1 = Enable] [FileName (Required if Enable = 1)].\r\n");
...
static void DisplayDataElement(SDP_Data_Element_t *SDPDataElement, unsigned int Level)
{
   ...
   /* The following function is a utility function which is used to log */
   /* data to a measurement file.                                       */
static void DataLogger(char *LogString)
{
   DWord_t BytesWritten;

   /* Verify that the log is currently open.                            */
   if(FileDescriptor)
   {
      /* Write the specified data to the file.                          */
      if((BytesWritten = write(FileDescriptor, LogString, strlen(LogString))) == (DWord_t)(-1))
         BytesWritten = 0;
   }
}

   /* The following function is a utility function which is used to     */
   /* convert a GLPM Floating point to a string.                        */
static void GLPMFloatToStr(GLPM_Floating_Point_Data_t *Value, char *StringBuffer)
{
   float FloatValue;

   if((Value) && (StringBuffer))
   {
      /* Convert the value to a floating point.                         */
      switch(Value->ValueType)
      {
         case gftValid:
            FloatValue = (float)(((float)Value->Mantissa)*pow((float)10, (float)Value->Exponent));

            sprintf(StringBuffer, "%f", FloatValue);
            break;
         case gftNotANumber:
            strcpy(StringBuffer, "NaN");
            break;
         case gftNotAtThisResolution:
            strcpy(StringBuffer, "NRes");
            break;
         case gftPositiveInfinity:
            strcpy(StringBuffer, "+Infinity");
            break;
         case gftNegativeInfinity:
            strcpy(StringBuffer, "-Infinity");
            break;
         case gftRFU:
         default:
   ...
   /* The following function is responsible for actually displaying an  */
   /* individual Glucose Measurement to the Display.                    */
static void DisplayGlucoseMeasurement(GLPM_Glucose_Measurement_Data_t *GlucoseMeasurement, int Level)
{
   ...
   {
      sprintf(StringBuffer, "%*s Glucose Measurement Data.\r\n", (Level*INDENT_LENGTH), "");
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      ++Level;

      /* Print the measurement flags.                                   */
      sprintf(StringBuffer, "%*s Measurement Flags:    0x%08lX.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurement->MeasurementFlags);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Print the sequence number.                                     */
      sprintf(StringBuffer, "%*s Sequence Number:      %u.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurement->SequenceNumber);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Print the date/time string.                                    */
      sprintf(StringBuffer, "%*s Base Time:            %u/%u/%u %u:%u:%u.\r\n", (Level*INDENT_LENGTH), "",
                                                                              GlucoseMeasurement->BaseTime.Month,
                                                                              GlucoseMeasurement->BaseTime.Day,
                                                                              GlucoseMeasurement->BaseTime.Year,
                                                                              GlucoseMeasurement->BaseTime.Hours,
                                                                              GlucoseMeasurement->BaseTime.Minutes,
                                                                              GlucoseMeasurement->BaseTime.Seconds);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Print the Time Offset if it is present.                        */
      if(GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_TIME_OFFSET_PRESENT)
      {
         FloatBuffer[0] = '\0';

         if(GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_TIME_OFFSET_OVERRUN)
            strcpy(FloatBuffer, " (Overrun)");
         else
         {
            if(GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_TIME_OFFSET_UNDERRUN)
               strcpy(FloatBuffer, " (Underrun)");
         }

         sprintf(StringBuffer, "%*s Time Offset:          %u%s.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurement->TimeOffset, FloatBuffer);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Print the Glucose Concentration if it is present.              */
      if(GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_CONCENTRATION_PRESENT)
      {
         GLPMFloatToStr(&(GlucoseMeasurement->Concentration.Value), FloatBuffer);

         sprintf(StringBuffer, "%*s Glucose Concentration:\r\n", (Level*INDENT_LENGTH), "");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Value:             %s %s.\r\n", ((Level+1)*INDENT_LENGTH), "", FloatBuffer, (GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_CONCENTRATION_IN_MOL_PER_LITER)?"mol/L":"kg/L");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         /* Next print the Measurement Type.                            */
         Index  = GlucoseMeasurement->Concentration.MeasurementType;
         Index -= gmtCapillaryWholeBlood;
         Index  = ((Index<NUM_SUPPORTED_MEASUREMENT_TYPES)?Index:(NUM_SUPPORTED_MEASUREMENT_TYPES-1));

         sprintf(StringBuffer, "%*s Measurement Type:  %s.\r\n", ((Level+1)*INDENT_LENGTH), "", MeasurementTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         /* Next print the sample locations.                            */
         Index  = GlucoseMeasurement->Concentration.SampleLocationType;
         Index -= gstFinger;
         Index  = ((Index<NUM_SUPPORTED_SAMPLE_LOCATION_TYPES)?Index:(NUM_SUPPORTED_SAMPLE_LOCATION_TYPES-1));

         sprintf(StringBuffer, "%*s Sample Location:   %s.\r\n", ((Level+1)*INDENT_LENGTH), "", SampleLocationStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Print the Sensor status flags if it is present.                */
      if(GlucoseMeasurement->MeasurementFlags & GLPM_MEASUREMENT_FLAGS_SENSOR_STATUS_PRESENT)
      {
         sprintf(StringBuffer, "%*s Sensor Status Flags:  0x%08lX\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurement->SensorStatusFlags);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Battery Low at Measurement:  %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_BATTERY_LOW_AT_TIME_OF_MEASUREMENT)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sensor Malfunction:          %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_MALFUNCTION)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sample Size:                 %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SAMPLE_SIZE_INSUFFICIENT)?"SUFFICIENT":"INSUFFICIENT");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Strip Insertion:             %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_STRIP_INSERTION_ERROR)?"No Error":"Error");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Strip Type:                  %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_STRIP_TYPE_INCORRECT)?"Correct Type":"Incorrect Type");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sensor Result High?:         %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_RESULT_TOO_HIGH)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sensor Result Low?:          %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_RESULT_TOO_LOW)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sensor Temperature High?:    %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_TEMPERATURE_TOO_HIGH)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Sensor Temperature Low?:     %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_TEMPERATURE_TOO_LOW)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
         ...
         sprintf(StringBuffer, "%*s Sensor Read Interrupted?:    %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_SENSOR_READ_INTERRUPTED)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s General Device Fault?:       %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_GENERAL_DEVICE_FAULT)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Time Fault?:                 %s\r\n", ((Level+1)*INDENT_LENGTH), "", (GlucoseMeasurement->SensorStatusFlags & GLPM_SENSOR_STATUS_TIME_FAULT)?"Yes":"No");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }
   }
   ...
   /* The following function is responsible for actually displaying an  */
   /* individual Glucose Measurement Context to the Display.            */
static void DisplayGlucoseMeasurementContext(GLPM_Glucose_Measurement_Context_Data_t *GlucoseMeasurementContext, int Level)
{
   char         StringBuffer[256];
   char         FloatBuffer[48];
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if(GlucoseMeasurementContext)
   {
      sprintf(StringBuffer, "%*s Glucose Measurement Context Data.\r\n", (Level*INDENT_LENGTH), "");
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);
      ...
      /* Print the measurement flags.                                   */
      sprintf(StringBuffer, "%*s Context Flags:        0x%08lX.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurementContext->ContextFlags);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Print the sequence number.                                     */
      sprintf(StringBuffer, "%*s Sequence Number:      %u.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurementContext->SequenceNumber);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Next print the carboyhydrate information if it is present.     */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_CARBOHYDRATE_DATA_PRESENT)
      {
         GLPMFloatToStr(&(GlucoseMeasurementContext->CarbohydrateData.CarbohydrateValue), FloatBuffer);

         sprintf(StringBuffer, "%*s Carbohydrate Data:\r\n", (Level*INDENT_LENGTH), "");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         Index  = GlucoseMeasurementContext->CarbohydrateData.CarbohydrateID;
         Index -= gciBreakfast;
         Index  = ((Index < NUM_SUPPORTED_CARBOHYDRATE_TYPES)?Index:(NUM_SUPPORTED_CARBOHYDRATE_TYPES-1));

         sprintf(StringBuffer, "%*s Type:              %s\r\n", ((Level+1)*INDENT_LENGTH), "", CarbohydrateTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Value:             %s kg\r\n", ((Level+1)*INDENT_LENGTH), "", FloatBuffer);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the meal information if it is present.              */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_MEAL_TYPE_PRESENT)
      {
         Index  = GlucoseMeasurementContext->MealType;
         Index -= gmtPreprandial;
         Index  = ((Index < NUM_SUPPORTED_MEAL_TYPES)?Index:(NUM_SUPPORTED_MEAL_TYPES-1));

         sprintf(StringBuffer, "%*s Meal Type:            %s.\r\n", (Level*INDENT_LENGTH), "", MealTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the tester and health information if it is present. */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_TESTER_TYPE_AND_HEALTH_TYPE_PRESENT)
      {
         Index  = GlucoseMeasurementContext->TesterType;
         Index -= gttSelf;
         Index  = ((Index < NUM_SUPPORTED_TESTER_TYPES)?Index:(NUM_SUPPORTED_TESTER_TYPES-1));

         sprintf(StringBuffer, "%*s Tester Type:          %s.\r\n", (Level*INDENT_LENGTH), "", TesterTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         Index  = GlucoseMeasurementContext->HealthType;
         Index -= ghtMinorHealthIssues;
         Index  = ((Index < NUM_SUPPORTED_HEALTH_TYPES)?Index:(NUM_SUPPORTED_HEALTH_TYPES-1));

         sprintf(StringBuffer, "%*s Health Type:          %s.\r\n", (Level*INDENT_LENGTH), "", HealthTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the exercise data information if it is present.     */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_EXERCISE_DATA_PRESENT)
      {
         sprintf(StringBuffer, "%*s Exercise Duration:    %u %s.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurementContext->ExerciseData.Duration, (GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_EXERCISE_DATA_DURATION_OVERRUN)?"(overrun)":"seconds");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Exercise Intensity:   %u %%.\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurementContext->ExerciseData.Intensity);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the medication information if it is present.        */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_MEDICATION_DATA_PRESENT)
      {
         GLPMFloatToStr(&(GlucoseMeasurementContext->MedicationData.MedicationValue), FloatBuffer);

         sprintf(StringBuffer, "%*s Medication Data:\r\n", (Level*INDENT_LENGTH), "");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         Index  = GlucoseMeasurementContext->MedicationData.MedicationID;
         Index -= gciBreakfast;
         Index  = ((Index < NUM_SUPPORTED_MEDICATION_TYPES)?Index:(NUM_SUPPORTED_MEDICATION_TYPES-1));

         sprintf(StringBuffer, "%*s Type:              %s\r\n", ((Level+1)*INDENT_LENGTH), "", MedicationTypeStrings[Index]);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         sprintf(StringBuffer, "%*s Value:             %s %s\r\n", ((Level+1)*INDENT_LENGTH), "", FloatBuffer, (GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_MEDICATION_DATA_UNITS_LITERS)?"Liters":"kilograms");
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the medication information if it is present.        */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_HBA1C_PRESENT)
      {
         GLPMFloatToStr(&(GlucoseMeasurementContext->HbA1cData), FloatBuffer);

         sprintf(StringBuffer, "%*s HbA1c Data:           %s\r\n", (Level*INDENT_LENGTH), "", FloatBuffer);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }

      /* Next print the extended flags if it is present.                */
      if(GlucoseMeasurementContext->ContextFlags & GLPM_CONTEXT_FLAGS_EXTENDED_FLAGS_PRESENT)
      {
         sprintf(StringBuffer, "%*s Extended Flags:       0x%08X\r\n", (Level*INDENT_LENGTH), "", GlucoseMeasurementContext->ExtendedFlags);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
      }
   }
...
static void BTPSAPI GLPM_Event_Callback(GLPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case getGLPMConnected:
            printf("getGLPMConnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= GLPM_CONNECTED_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:         %s\r\n", BoardStr);
               printf("   Connection Type:           %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == gctSensor) ? "Sensor" : "Collector");
               printf("   Supported Features:        0x%08X\r\n", (unsigned int)EventData->EventData.ConnectedEventData.SupportedFeatures);
            }
            break;
         case getGLPMDisconnected:
            printf("getGLPMDisconnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= GLPM_DISCONNECTED_EVENT_DATA_SIZE)
            {
            ...
            break;
         case getGLPMProcedureStarted:
            sprintf(StringBuffer, "getGLPMProcedureStarted with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= GLPM_PROCEDURE_STARTED_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.ProcedureStartedEventData.RemoteDeviceAddress, BoardStr);

               sprintf(StringBuffer, "   Bluetooth Address:     %s\r\n", BoardStr);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               sprintf(StringBuffer, "   Procedure ID:          %u\r\n", EventData->EventData.ProcedureStartedEventData.ProcedureID);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               sprintf(StringBuffer, "   Success?:              %s\r\n", EventData->EventData.ProcedureStartedEventData.Success?"YES":"NO");
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               if(EventData->EventData.ProcedureStartedEventData.Success == FALSE)
               {
                  sprintf(StringBuffer, "   Error Code:            0x%02X.\r\n", EventData->EventData.ProcedureStartedEventData.AttributeProtocolErrorCode);
                  printf("%s", StringBuffer);
                  DataLogger(StringBuffer);
               }
            }
            ...
            DataLogger(StringBuffer);
            break;
         case getGLPMProcedureStopped:
            sprintf(StringBuffer, "getGLPMProcedureStopped with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= GLPM_PROCEDURE_STOPPED_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.ProcedureStoppedEventData.RemoteDeviceAddress, BoardStr);

               sprintf(StringBuffer, "   Bluetooth Address:         %s\r\n", BoardStr);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               sprintf(StringBuffer, "   Procedure ID:              %u\r\n", EventData->EventData.ProcedureStoppedEventData.ProcedureID);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               Index  = EventData->EventData.ProcedureStoppedEventData.ProcedureType;
               Index -= gptReportStoredRecords;
               Index  = ((Index<NUM_SUPPORTED_PROCEDURE_TYPES)?Index:(NUM_SUPPORTED_PROCEDURE_TYPES-1));

               sprintf(StringBuffer, "   Procedure Type:            %s\r\n", ProcedureTypeStrings[Index]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               Index  = EventData->EventData.ProcedureStoppedEventData.ResponseCode;
               Index -= grcSuccess;
               Index  = ((Index<NUM_SUPPORTED_PROCEDURE_RESPONSE_CODE_TYPES)?Index:(NUM_SUPPORTED_PROCEDURE_RESPONSE_CODE_TYPES-1));

               sprintf(StringBuffer, "   Response Code:             %s\r\n", ProcedureResponseCodeStrings[Index]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print the Number of Stored records if applicable.     */
               if((EventData->EventData.ProcedureStoppedEventData.ProcedureType == gptReportNumberStoredRecords) && (EventData->EventData.ProcedureStoppedEventData.ResponseCode == grcSuccess))
               {
                  sprintf(StringBuffer, "   Number of Stored Records:  %u\r\n", EventData->EventData.ProcedureStoppedEventData.NumberStoredRecords);
                  printf("%s", StringBuffer);
                  DataLogger(StringBuffer);
               }
            }
            ...
            DataLogger(StringBuffer);
            break;
         case getGLPMGlucoseMeasurement:
            sprintf(StringBuffer, "getGLPMGlucoseMeasurement with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= GLPM_GLUCOSE_MEASUREMENT_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.GlucoseMeasurementEventData.RemoteDeviceAddress, BoardStr);

               sprintf(StringBuffer, "   Bluetooth Address:         %s\r\n", BoardStr);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               sprintf(StringBuffer, "   Procedure ID:              %u\r\n", EventData->EventData.GlucoseMeasurementEventData.ProcedureID);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               Index  = EventData->EventData.GlucoseMeasurementEventData.MeasurementErrorType;
               Index -= meNoError;
               Index  = ((Index<NUM_SUPPORTED_MEASUREMENT_ERROR_TYPES)?Index:(NUM_SUPPORTED_MEASUREMENT_ERROR_TYPES-1));

               sprintf(StringBuffer, "   Measurement Error Type:    %s(%u)\r\n", MeasurementErrorTypeStrings[Index], (unsigned int)EventData->EventData.GlucoseMeasurementEventData.MeasurementErrorType);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* If the Glucose Measurement is valid go ahead and      */
               /* display it.                                           */
               if(EventData->EventData.GlucoseMeasurementEventData.MeasurementFlags & GLPM_MEASUREMENT_FLAGS_GLUCOSE_MEASUREMENT_DATA_PRESENT)
                  DisplayGlucoseMeasurement(&(EventData->EventData.GlucoseMeasurementEventData.GlucoseMeasurementData), 2);

               /* If the Glucose Measurement Context is valid go ahead  */
               /* and display it.                                       */
               if(EventData->EventData.GlucoseMeasurementEventData.MeasurementFlags & GLPM_MEASUREMENT_FLAGS_GLUCOSE_MEASUREMENT_CONTEXT_DATA_PRESENT)
                  DisplayGlucoseMeasurementContext(&(EventData->EventData.GlucoseMeasurementEventData.GlucoseMeasurementContextData), 2);
            }
            ...
            break;
         default:
            printf("Unknown GLP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nGLPM Event Data is NULL.\r\n");
