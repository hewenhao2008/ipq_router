...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHTPM_COL - Simple Linux application using Bluetopia Platform Manager */
/*                  Health Thermometer Profile (HTP) Manager Application      */
/*                  Programming Interface (API) - Collector Role Only.        */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   03/14/13  T. Cook        Initial creation. (Based on LinuxHRPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxHTPM_COL.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTHTPM.h"     /* HTP Manager Application Programming Interface.  */
...
#ifndef HTS_ASSIGN_HTS_SERVICE_UUID_16
...
static unsigned int        HTPEventCallbackID;      /* Variable which holds the        */
                                                    /* current HTP Event Callback ID   */
                                                    /* that is assigned from the HTP   */
                                                    /* Manager when the local client   */
                                                    /* registers for HTP Events.       */
...
static char *TemperatureTypeToString(HTPM_Temperature_Type_t TemperatureType);
static char *TemperatureMeasurementTypeToString(HTPM_Temperature_Measurement_Type_t TemperatureMeasurementType);
...
static int HTPRegisterCollectorEvents(ParameterList_t *TempParam);
static int HTPUnRegisterCollectorEvents(ParameterList_t *TempParam);

static int HTPGetTemperatureTypeRequest(ParameterList_t *TempParam);
static int HTPGetMeasurementIntervalRequest(ParameterList_t *TempParam);
static int HTPSetMeasurementIntervalRequest(ParameterList_t *TempParam);
static int HTPGetMeasurementIntervalValidRangeRequest(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM HTP Manager Callback function prototype.                     */
static void BTPSAPI HTPM_Event_Callback(HTPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("HTPREGISTERCOLLECTOREVENTS", HTPRegisterCollectorEvents);
   AddCommand("HTPUNREGISTERCOLLECTOREVENTS", HTPUnRegisterCollectorEvents);
   AddCommand("HTPGETTEMPERATURETYPE", HTPGetTemperatureTypeRequest);
   AddCommand("HTPGETMEASUREMENTINTERVAL", HTPGetMeasurementIntervalRequest);
   AddCommand("HTPSETMEASUREMENTINTERVAL", HTPSetMeasurementIntervalRequest);
   AddCommand("HTPGETMEASUREMENTINTERVALVALIDRANGE", HTPGetMeasurementIntervalValidRangeRequest);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      HTS_ASSIGN_HTS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Temperature Type to a string.                           */
static char *TemperatureTypeToString(HTPM_Temperature_Type_t TemperatureType)
{
   char *ret_val;
   switch(TemperatureType)
   {
      ...
      case httFinger:
         ret_val = "Finger";
         break;
      case httGastroIntestinalTract:
         ret_val = "Gastro-intestinal Tract";
         break;
      case httMouth:
         ret_val = "Mouth";
         break;
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Temperature Measurement Type to a string.               */
static char *TemperatureMeasurementTypeToString(HTPM_Temperature_Measurement_Type_t TemperatureMeasurementType)
{
   char *ret_val;
   ...
   switch(TemperatureMeasurementType)
   {
      case tmtIntermediateMeasurement:
         ret_val = "Intermediate Temperature Measurement";
         break;
      case tmtTemperatureMeasurement:
         ret_val = "Temperature Measurement";
         break;
      default:
         ret_val = "Unkown";
         break;
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)HTPRegisterCollectorEvents                 *\r\n");
   printf("*                  36)HTPUnRegisterCollectorEvents               *\r\n");
   printf("*                  37)HTPGetTemperatureType                      *\r\n");
   printf("*                  38)HTPGetMeasurementInterval                  *\r\n");
   printf("*                  39)HTPSetMeasurementInterval                  *\r\n");
   printf("*                  40)HTPGetMeasurementIntervalValidRange        *\r\n");
   printf("*                  41)StartAdvertising                           *\r\n");
   printf("*                  42)StopAdvertising                            *\r\n");
   printf("*                  43)StartObservationScan                       *\r\n");
   printf("*                  44)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the HTP*/
   /* Manager to receive HTP Events.  This function returns zero if     */
   /* successful and a negative value if an error occurred.             */
static int HTPRegisterCollectorEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!HTPEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = HTPM_Register_Collector_Event_Callback(HTPM_Event_Callback, NULL)) > 0)
         {
            printf("HTPM_Register_Collector_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            HTPEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("HTPM_Register_Collector_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("HTP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered HTP Event Registration with the HTP Manager.*/
...
static int HTPUnRegisterCollectorEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(HTPEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         HTPM_Un_Register_Collector_Event_Callback(HTPEventCallbackID);

         printf("HTPM_Un_Register_Collector_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         HTPEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("HTP Manager Event Callback is not registered.\r\n");
   ...
   /* The following function is responsible for sending a HTP Get       */
   /* Temperature Type Request.  This function returns zero if          */
   /* successful and a negative value if an error occurred.             */
static int HTPGetTemperatureTypeRequest(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters>=1))
      {
         ...
         if((Result = HTPM_Get_Temperature_Type_Request(HTPEventCallbackID, BD_ADDR)) >= 0)
         {
            printf("HTPM_Get_Temperature_Type_Request() Success: Transaction ID = %d.\r\n", Result);
         ...
         else
         {
            /* Error Connecting With Remote Device, inform the user and */
            /* flag an error.                                           */
            printf("HTPM_Get_Temperature_Type_Request() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HTPGetTemperatureType [BD_ADDR].\r\n");
   ...
   /* The following function is responsible for sending a HTP Get       */
   /* Measurement Interval Request.  This function returns zero if      */
   /* successful and a negative value if an error occurred.             */
static int HTPGetMeasurementIntervalRequest(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters>=1))
      {
         ...
         if((Result = HTPM_Get_Measurement_Interval_Request(HTPEventCallbackID, BD_ADDR)) >= 0)
         {
            printf("HTPM_Get_Measurement_Interval_Request() Success: Transaction ID = %d.\r\n", Result);
         ...
         else
         {
            /* Error Connecting With Remote Device, inform the user and */
            /* flag an error.                                           */
            printf("HTPM_Get_Measurement_Interval_Request() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HTPGetMeasurementInterval [BD_ADDR].\r\n");
   ...
   /* The following function is responsible for sending a HTP Set       */
   /* Measurement Interval Request.  This function returns zero if      */
   /* successful and a negative value if an error occurred.             */
static int HTPSetMeasurementIntervalRequest(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters>=2))
      {
         ...
         if((Result = HTPM_Set_Measurement_Interval_Request(HTPEventCallbackID, BD_ADDR, (unsigned int)TempParam->Params[1].intParam)) >= 0)
         {
            printf("HTPM_Set_Measurement_Interval_Request() Success: Transaction ID = %d.\r\n", Result);
         ...
         else
         {
            /* Error Connecting With Remote Device, inform the user and */
            /* flag an error.                                           */
            printf("HTPM_Set_Measurement_Interval_Request() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HTPSetMeasurementInterval [BD_ADDR] [Measurement Interval].\r\n");
   ...
   /* The following function is responsible for sending a HTP Get       */
   /* Measurement Interval Valid Range Request.  This function returns  */
   /* zero if successful and a negative value if an error occurred.     */
static int HTPGetMeasurementIntervalValidRangeRequest(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters>=1))
      {
         ...
         if((Result = HTPM_Get_Measurement_Interval_Valid_Range_Request(HTPEventCallbackID, BD_ADDR)) >= 0)
         {
            printf("HTPM_Get_Measurement_Interval_Valid_Range_Request() Success: Transaction ID = %d.\r\n", Result);
         ...
         else
         {
            /* Error Connecting With Remote Device, inform the user and */
            /* flag an error.                                           */
            printf("HTPM_Get_Measurement_Interval_Valid_Range_Request() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HTPGetMeasurementIntervalValidRange [BD_ADDR].\r\n");
...
static void BTPSAPI HTPM_Event_Callback(HTPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case hetHTPConnected:
            printf("hetHTPConnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= HTPM_CONNECTED_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:         %s\r\n", BoardStr);
               printf("   Connection Type:           %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == httSensor) ? "Sensor" : "Collector");
               printf("   Connection Flags:          0x%08X\r\n", (unsigned int)EventData->EventData.ConnectedEventData.ConnectedFlags);
               printf("   Intermediate Temperature:  %sSupported\r\n", (EventData->EventData.ConnectedEventData.ConnectedFlags & HTPM_CONNECTED_FLAGS_INTERMEDIATE_TEMPERATURE_SUPPORTED) ? "" : "Not ");
               printf("   Get Measurement Interval:  %sSupported\r\n", (EventData->EventData.ConnectedEventData.ConnectedFlags & HTPM_CONNECTED_FLAGS_GET_MEASUREMENT_INTERVAL_SUPPORTED) ? "" : "Not ");
               printf("   Set Measurement Interval:  %sSupported\r\n", (EventData->EventData.ConnectedEventData.ConnectedFlags & HTPM_CONNECTED_FLAGS_SET_MEASUREMENT_INTERVAL_SUPPORTED) ? "" : "Not ");
               printf("   Get Temperature Type:      %sSupported\r\n", (EventData->EventData.ConnectedEventData.ConnectedFlags & HTPM_CONNECTED_FLAGS_GET_TEMPERATURE_TYPE_SUPPORTED) ? "" : "Not ");
            }
            break;
         case hetHTPDisconnected:
            printf("hetHTPDisconnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= HTPM_DISCONNECTED_EVENT_DATA_SIZE)
            {
            ...
            break;
         case hetHTPGetTemperatureTypeResponse:
            printf("hetHTPGetTemperatureTypeResponse with size %u:\r\n", EventData->EventLength);

            if(EventData->EventLength >= HTPM_GET_TEMPERATURE_TYPE_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.GetTemperatureTypeResponseEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:     %s\r\n", BoardStr);
               printf("   Transaction ID:        %u\r\n", EventData->EventData.GetTemperatureTypeResponseEventData.TransactionID);
               printf("   Success?:              %s\r\n", EventData->EventData.GetTemperatureTypeResponseEventData.Success?"YES":"NO");

               if(EventData->EventData.GetTemperatureTypeResponseEventData.Success)
                  printf("   Temperature Type:      %s\r\n", TemperatureTypeToString(EventData->EventData.GetTemperatureTypeResponseEventData.TemperatureType));
               else
                  printf("   Attribute Error Code:  0x%02X\r\n", EventData->EventData.GetTemperatureTypeResponseEventData.AttributeErrorCode);
            }
            break;
         case hetHTPGetMeasurementIntervalResponse:
            printf("hetHTPGetMeasurementIntervalResponse with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= HTPM_GET_MEASUREMENT_INTERVAL_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.GetMeasurementIntervalResponseEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:     %s\r\n", BoardStr);
               printf("   Transaction ID:        %u\r\n", EventData->EventData.GetMeasurementIntervalResponseEventData.TransactionID);
               printf("   Success?:              %s\r\n", EventData->EventData.GetMeasurementIntervalResponseEventData.Success?"YES":"NO");

               if(EventData->EventData.GetMeasurementIntervalResponseEventData.Success)
                  printf("   Measurement Interval:  %u seconds\r\n", EventData->EventData.GetMeasurementIntervalResponseEventData.MeasurementInterval);
               else
                  printf("   Attribute Error Code:  0x%02X\r\n", EventData->EventData.GetMeasurementIntervalResponseEventData.AttributeErrorCode);
            }
            break;
         case hetHTPSetMeasurementIntervalResponse:
            printf("hetHTPSetMeasurementIntervalResponse with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= HTPM_SET_MEASUREMENT_INTERVAL_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.SetMeasurementIntervalResponseEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:     %s\r\n", BoardStr);
               printf("   Transaction ID:        %u\r\n", EventData->EventData.SetMeasurementIntervalResponseEventData.TransactionID);
               printf("   Success?:              %s\r\n", EventData->EventData.SetMeasurementIntervalResponseEventData.Success?"YES":"NO");

               if(EventData->EventData.SetMeasurementIntervalResponseEventData.Success == FALSE)
                  printf("   Attribute Error Code:  0x%02X\r\n", EventData->EventData.SetMeasurementIntervalResponseEventData.AttributeErrorCode);
            }
            break;
         case hetHTPGetMeasurementIntervalValidRangeResponse:
            printf("hetHTPGetMeasurementIntervalValidRangeResponse with size %u:\r\n", EventData->EventLength);

            if(EventData->EventLength >= HTPM_GET_MEASUREMENT_INTERVAL_VALID_RANGE_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.GetMeasurementIntervalResponseEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:                 %s\r\n", BoardStr);
               printf("   Transaction ID:                    %u\r\n", EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.TransactionID);
               printf("   Success?:                          %s\r\n", EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.Success?"YES":"NO");

               if(EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.Success)
                  printf("   Measurement Interval Valid Range:  [%u - %u]\r\n", EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.LowerBounds, EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.UpperBounds);
               else
                  printf("   Attribute Error Code:              0x%02X\r\n", EventData->EventData.GetMeasurementIntervalValidRangeResponseEventData.AttributeErrorCode);
            }
            break;
         case hetHTPTemperatureMeasurement:
            printf("hetHTPTemperatureMeasurement with size %u:\r\n", EventData->EventLength);

            if(EventData->EventLength >= HTPM_TEMPERATURE_MEASUREMENT_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.TemperatureMeasurementEventData.RemoteDeviceAddress, BoardStr);

               printf("   Bluetooth Address:     %s\r\n", BoardStr);
               printf("   Measurement Flags:     0x%08X\r\n", (unsigned int)EventData->EventData.TemperatureMeasurementEventData.MeasurementFlags);
               printf("   Temperature Type:      %s\r\n", TemperatureMeasurementTypeToString(EventData->EventData.TemperatureMeasurementEventData.MeasurementType));

               if(EventData->EventData.TemperatureMeasurementEventData.MeasurementFlags & HTPM_MEASUREMENT_FLAGS_TEMPERATURE_TYPE_VALID)
                  printf("   Temperature Type:      %s\r\n", TemperatureTypeToString(EventData->EventData.TemperatureMeasurementEventData.TemperatureType));

               if(EventData->EventData.TemperatureMeasurementEventData.MeasurementFlags & HTPM_MEASUREMENT_FLAGS_TIME_STAMP_VALID)
               {
                  printf("   Time Stamp:            %u:%u:%u %u/%u/%u\r\n", EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Hours,
                                                                            EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Minutes,
                                                                            EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Seconds,
                                                                            EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Month,
                                                                            EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Day,
                                                                            EventData->EventData.TemperatureMeasurementEventData.TimeStamp.Year);
               }

               if(EventData->EventData.TemperatureMeasurementEventData.MeasurementFlags & HTPM_MEASUREMENT_FLAGS_TEMPERATURE_VALUE_NAN)
                  printf("%s", "   Measurement:           NaN\r\n");
               else
                  printf("   Measurement:           %.2f %s\r\n", (float)(((float)EventData->EventData.TemperatureMeasurementEventData.TemperatureMantissa)*pow((float)10, (float)EventData->EventData.TemperatureMeasurementEventData.TemperatureExponent)),
                                                                ((EventData->EventData.TemperatureMeasurementEventData.MeasurementFlags & HTPM_MEASUREMENT_FLAGS_TEMPERATURE_FAHRENHEIT)?"F":"C"));
            }
            break;
         default:
            printf("Unknown HTP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nHTPM Event Data is NULL.\r\n");
