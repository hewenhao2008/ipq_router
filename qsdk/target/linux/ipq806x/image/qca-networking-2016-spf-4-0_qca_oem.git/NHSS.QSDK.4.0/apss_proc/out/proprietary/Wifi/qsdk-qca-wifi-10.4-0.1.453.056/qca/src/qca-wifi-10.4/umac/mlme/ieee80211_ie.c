                             ...
                             struct ieee80211vap *vap, u_int16_t* ie_len)
{
    ...
    *frm++ = vap->iv_ic->ic_whc_root_ap_distance;

    if((vap->iv_ic->ic_whc_root_ap_distance == 0)
    ...
    else if(vap->iv_ic->ic_whc_root_ap_distance > 0)
    {
        *plen += 10;
        /* ROOTAP indicator */
        *frm++ = 0;
        ...
        *frm++ = vap->iv_ic->ic_uplink_rate & 0xff;
        *frm++ = (vap->iv_ic->ic_uplink_rate >> 8) & 0xff;
    }
...
#define IEEE80211_EXT_CAP_IE_LEN (vap->rtt_enable ? sizeof(struct ieee80211_ie_ext_cap) : (sizeof(struct ieee80211_ie_ext_cap)-1))
        OS_MEMSET(ie, 0, sizeof(struct ieee80211_ie_ext_cap));
        ie->elem_id = IEEE80211_ELEMID_XCAPS;
        ie->elem_len = IEEE80211_EXT_CAP_IE_LEN - 2;
        ie->ext_capflags = htole32(ext_capflags);
        ie->ext_capflags2 = htole32(ext_capflags2);
        ie->ext_capflags3 = ext_capflags3;
        return (frm + IEEE80211_EXT_CAP_IE_LEN);
    }
    else {
        return frm;
    }
            ...
            case IEEE80211_CWM_WIDTH160:
                if (!(ni->ni_htcap & IEEE80211_HTCAP_C_CHWIDTH40)) {
                    ...
                    if(vap->iv_cur_mode == IEEE80211_MODE_11AC_VHT160) {
                        if((ni->ni_vhtcap & IEEE80211_VHTCAP_SUP_CHAN_WIDTH_160) || (ni->ni_vhtcap & IEEE80211_VHTCAP_SUP_CHAN_WIDTH_80_160)) {
                        ...
                        if(ni->ni_vhtcap & IEEE80211_VHTCAP_SUP_CHAN_WIDTH_80_160) {
                            ni->ni_chwidth = IEEE80211_CWM_WIDTH160;
                        }
                        else {
                            ni->ni_chwidth = IEEE80211_CWM_WIDTH80;
...
ieee80211_parse_vhtop(struct ieee80211_node *ni, u_int8_t *ie)
{
    ...
    switch (vhtop->vht_op_chwidth) {
       ...
       if (IS_REVSIG_VHT160(vhtop) || IS_REVSIG_VHT80_80(vhtop)) {
               ni->ni_chwidth = IEEE80211_CWM_WIDTH160;
       }
       else {
           ni->ni_chwidth = IEEE80211_CWM_WIDTH80;
...
ieee80211_parse_opmode(struct ieee80211_node *ni, u_int8_t *ie, u_int8_t subtype)
{
    ...
    if ((opmode->ch_width <= ic_cw_width) &&
...
ieee80211_process_whc_apinfo_ie(struct ieee80211_node *ni, const u_int8_t *ie)
{
     ...
     */
    ic->ic_whc_root_ap_distance = whcAPInfoIE->whc_apinfo_root_ap_dist + 1;
}
...
ieee80211_add_mmie(struct ieee80211vap *vap, u_int8_t *bfrm, u_int32_t len)
{
    ...
    if((RSN_CIPHER_IS_CMAC(&vap->iv_rsn)) || RSN_CIPHER_IS_CMAC256(&vap->iv_rsn)){
        ...
        printk("%s[%d] CMAC / GMAC Not a valid rsn  \n",__func__,__LINE__);
        return NULL;
    }
                                             ...
                                             MU_CAP_WAR *war)
{
    ...
    if (dedicated_mac == NULL) {
        ieee80211_note(vap, "ERROR!! Memory allocation failed in %s\n",
                __func__);
        return vhtcap_info;
    }
                     ...
                     u_int8_t *sta_mac_addr)
{
    ...
    vhtcap->tx_high_data_rate = htole16(vap->iv_vhtcap_max_mcs.tx_mcs_set.data_rate);
