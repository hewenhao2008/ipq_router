...
int wireless_get_ifname_by_vapidx(int vapidx, char *ifname)
{
    ...
    sprintf(path, "uci -q get -P /var/state wireless.@wifi-iface[%d].ifname", vapidx - 1);
    f =popen(path, "r");
    if(!f)
        return -1;
...
static int opt_radidx_to_device(char *optval, char *ucival)
{
    ...
    sprintf(ucival, "wifi%d", atoi(optval) - 1);
    return 0;
}
...
static int opt_parse_standard(struct uci_context *ctx, struct wireless_s *wl, int vapidx, char *name, char *value)
{
    ...
    char ucival[UCI_MAX_STR_LEN+1];
    char device[UCI_MAX_STR_LEN+1];
    char orig_hwmode[UCI_MAX_STR_LEN + 1];
    ...
    sprintf(option, "@wifi-iface[%d].device", vapidx - 1);
    ret = uciGet(ctx, "wireless", option, device);
    if (ret || strlen(device) > IFNAMSIZ )
        return -1;

    sprintf(option, "%s.hwmode", device);
    ...
    if (strcmp(value, "a") == 0 ||
        ...
        strcmp(value, "g") == 0)
    {
        sprintf(hwmode, "11%s", value);
    }
    else if (strcmp(value, "na20") == 0 )
    {
        strcpy(hwmode, "11na");
        strcpy(htmode, "HT20");
        strcpy(wireless_htmode, "11NAHT20");
    }
    else if (strcmp(value, "na40") == 0 )
    {
        strcpy(hwmode, "11na");
        strcpy(htmode, "HT40");
        strcpy(wireless_htmode, "11NAHT40");
    }
    else if (strcmp(value, "na40plus") == 0 )
    {
        strcpy(hwmode, "11na");
        strcpy(htmode, "HT40+");
        strcpy(wireless_htmode, "11NAHT40PLUS");
    }
    else if (strcmp(value, "na40minus") == 0 )
    {
        strcpy(hwmode, "11na");
        strcpy(htmode, "HT40-");
        strcpy(wireless_htmode, "11NAHT40MINUS");
    }
    else if (strcmp(value, "ng20") == 0 )
    {
        strcpy(hwmode, "11ng");
        strcpy(htmode, "HT20");
        strcpy(wireless_htmode, "11NGHT20");
    }
    else if (strcmp(value, "ng40") == 0 )
    {
        strcpy(hwmode, "11ng");
        strcpy(htmode, "HT40");
        strcpy(wireless_htmode, "11NGHT40");
    }
    else if (strcmp(value, "ng40plus") == 0 )
    {
        strcpy(hwmode, "11ng");
        strcpy(htmode, "HT40+");
        strcpy(wireless_htmode, "11NGHT40PLUS");
    }
    else if (strcmp(value, "ng40minus") == 0 )
    {
        strcpy(hwmode, "11ng");
        strcpy(htmode, "HT40-");
        strcpy(wireless_htmode, "11NGHT40MINUS");
    }
    else if (strcmp(value, "acvht20") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT20");
        strcpy(wireless_htmode, "11ACVHT20");
    }
    else if (strcmp(value, "acvht40") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT40");
        strcpy(wireless_htmode, "11ACVHT40");
    }
    else if (strcmp(value, "acvht40plus") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT40+");
        strcpy(wireless_htmode, "11ACVHT40PLUS");
    }
    else if (strcmp(value, "acvht40minus") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT40-");
        strcpy(wireless_htmode, "11ACVHT40MINUS");
    }
    else if (strcmp(value, "acvht80") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT80");
        strcpy(wireless_htmode, "11ACVHT80");
    }
    else if (strcmp(value, "acvht160") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT160");
        strcpy(wireless_htmode, "11ACVHT160");
    }
    else if (strcmp(value, "acvht80_80") == 0 )
    {
        strcpy(hwmode, "11ac");
        strcpy(htmode, "HT80_80");
        strcpy(wireless_htmode, "11ACVHT80_80");
    }
    else if (strcmp(value, "auto") == 0 )
    {
        // When Registrar is operating in AUTO mode, keep original hwmode and
        // set htmode to AUTO.
        strcpy(hwmode, orig_hwmode);
        strcpy(htmode, "AUTO");
        strcpy(wireless_htmode, "AUTO");
    }
    else
    {
    ...
    sprintf(option, "%s.htmode", device);
...
static int opt_parse_channel(struct uci_context *ctx, struct wireless_s *wl, int radioidx, char *name, char *value)
{
    ...
    sprintf(device, "wifi%d", radioidx -1);
    sprintf(option, "%s.channel", device );
        ...
        strcmp(newval, ucival))
    {
        ...
        while (vsec)
        {
            sprintf(option, "@wifi-iface[%d].device", vsec->vapidx - 1);
...
static int opt_parse_vapind(struct uci_context *ctx, struct wireless_s *wl, int vapidx, char *name, char *value)
{
    ...
    sprintf(option, "@wifi-iface[%d].athnewind", vapidx - 1);
    ret = uciGet(ctx, "wireless", option, ucival);
...
static int wireless_apply_vapsec(void *cookie, struct package_s *pkg)
{
    ...
    while (vsec)
    {
        ...
        switch (vsec->bcn_type)
        {
            ...
            case BEACONTYPE_e11i:
                if (vsec->wpa2_auth == IEEE11IAUTHENTICATIONMODE_eEAPAuthentication)
                    ...
                    sprintf(newval, "psk2"); /*not defined in uci*/
                else
                    sprintf(newval, "psk2");

                if (vsec->wpa2_encr == IEEE11IENCRYPTIONMODES_eTKIPEncryption)
                    sprintf(newval + strlen(newval), "+tkip");
                else if (vsec->wpa2_encr == IEEE11IENCRYPTIONMODES_eAESEncryption)
                    sprintf(newval + strlen(newval), "+ccmp");
                if (vsec->wpa2_encr == IEEE11IENCRYPTIONMODES_eTKIPandAESEncryption)
                    sprintf(newval + strlen(newval), "+tkip+ccmp");

                break;
            case BEACONTYPE_eWPAand11i:
                if (vsec->wpa2_auth == IEEE11IAUTHENTICATIONMODE_eEAPAuthentication)
                    sprintf(newval, "mixed-wpa");
                else if (vsec->wpa2_auth == IEEE11IAUTHENTICATIONMODE_ePSKAuthentication)
                    sprintf(newval, "mixed-psk");
                else if (vsec->wpa2_auth == IEEE11IAUTHENTICATIONMODE_eEAPandPSKAuthentication)
                    sprintf(newval, "mixed-psk");/*not defined in uci*/
                else
                    sprintf(newval, "mixed-psk");
        ...
        sprintf(option, "@wifi-iface[%d].encryption", vsec->vapidx - 1);
        ret = uciGet(ctx, "wireless", option, ucival);
        if (ret ||
        ...
        if (vsec->psk || vsec->passphrase || vsec->wep_keyidx)
        {
            if (vsec->bcn_type == BEACONTYPE_eBasic
                && vsec->wep_keyidx )
            {
                sprintf(newval, "%d", vsec->wep_keyidx);
            }
        ...
        sprintf(option, "@wifi-iface[%d].key", vsec->vapidx - 1);
        ret = uciGet(ctx, "wireless", option, ucival);
        if (ret ||
            strcmp(ucival, newval))
        {
...
static int wireless_apply(void *cookie, struct package_s *pkg)
{
    ...
    if (strlen(radio))
    {
        sprintf(buf,"/sbin/wifi up '%s'",radio);
