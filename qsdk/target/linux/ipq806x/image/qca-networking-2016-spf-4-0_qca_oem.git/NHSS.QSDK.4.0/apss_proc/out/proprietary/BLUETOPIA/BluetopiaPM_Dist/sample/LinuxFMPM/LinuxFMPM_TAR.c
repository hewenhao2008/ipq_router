...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXFMPM_TAR - Simple Linux application using Bluetopia Platform Manager */
/*                  Find Me (FMP - Target Role only) Manager Application      */
...
/*   --------  -----------    ------------------------------------------------*/
/*   11/27/12  T. Cook        Initial creation. (Based on LinuxHRPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxFMPM_TAR.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTFMPM.h"     /* FMP Manager Application Programming Interface.  */
...
static unsigned int        FMPMTargetEventCallbackID;/* Variable which holds the       */
                                                    /* current FMPM Target Event       */
                                                    /* Callback ID that is assigned    */
                                                    /* from the FMP Manager when the   */
                                                    /* local client registers for FMP  */
                                                    /* Target Events.                  */
...
static char *AlertLevelToString(FMPM_Alert_Level_t AlertLevel);
...
static int FMPRegisterTargetEvents(ParameterList_t *TempParam);
static int FMPUnRegisterTargetEvents(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM FMP Manager Callback function prototype.                     */
static void BTPSAPI FMPM_Event_Callback(FMPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("FMPREGISTERTARGETEVENTS", FMPRegisterTargetEvents);
   AddCommand("FMPUNREGISTERTARGETEVENTS", FMPUnRegisterTargetEvents);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      IAS_ASSIGN_IAS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
   ...
   /* The following function is a utility function that is used to      */
   /* convert an FMP alert level to a string.                           */
static char *AlertLevelToString(FMPM_Alert_Level_t AlertLevel)
{
   char *ret_val;
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)FMPRegisterTargetEvents                    *\r\n");
   printf("*                  36)FMPUnRegisterTargetEvents                  *\r\n");
   printf("*                  37)StartAdvertising                           *\r\n");
   printf("*                  38)StopAdvertising                            *\r\n");
   printf("*                  39)StartObservationScan                       *\r\n");
   printf("*                  40)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the FMP*/
   /* Manager to receive FMP Target Events.  This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int FMPRegisterTargetEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!FMPMTargetEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = FMPM_Register_Target_Event_Callback(FMPM_Event_Callback, NULL)) > 0)
         {
            printf("FMPM_Register_Target_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            FMPMTargetEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("FMPM_Register_Target_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("FMP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered FMP Target Event Registration with the FMP  */
...
static int FMPUnRegisterTargetEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(FMPMTargetEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         FMPM_Un_Register_Target_Event_Callback(FMPMTargetEventCallbackID);

         printf("FMPM_Un_Register_Target_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         FMPMTargetEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("FMP Manager Event Callback is not registered.\r\n");
...
static void BTPSAPI FMPM_Event_Callback(FMPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      {
         case aetFMPAlert:
            BD_ADDRToStr(EventData->EventData.AlertEventData.RemoteDeviceAddress, BoardStr);

            printf("%s", "FMPM Alert:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Connection Type:       %s\r\n", (EventData->EventData.AlertEventData.ConnectionType == fctTarget) ? "Target" : "Monitor");
            printf("   Alert Level:           %s\r\n", AlertLevelToString(EventData->EventData.AlertEventData.AlertLevel));
            break;
         default:
            printf("Unknown FMP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nFMPM Event Data is NULL.\r\n");
