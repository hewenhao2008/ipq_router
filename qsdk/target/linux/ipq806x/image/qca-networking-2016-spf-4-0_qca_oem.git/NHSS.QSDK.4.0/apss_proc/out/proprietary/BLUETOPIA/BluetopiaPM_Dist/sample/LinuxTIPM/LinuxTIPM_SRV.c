...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXTIPM_SRV - Simple Linux application using Bluetopia Platform Manager */
/*                  Time (TIP - Server Role only) Manager Application         */
...
/*   --------  -----------    ------------------------------------------------*/
/*   11/30/12  T. Cook        Initial creation. (Based on LinuxFMPM_TAR)      */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static unsigned int        TIPMServerEventCallbackID;/* Variable which holds the       */
...
static int TIPRegisterServerEvents(ParameterList_t *TempParam);
static int TIPUnRegisterServerEvents(ParameterList_t *TempParam);
static int TIPSetLocalTimeInformation(ParameterList_t *TempParam);
static int TIPUpdateCurrentTime(ParameterList_t *TempParam);
static int TIPReferenceTimeResponse(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("TIPREGISTERSERVEREVENTS", TIPRegisterServerEvents);
   AddCommand("TIPUNREGISTERSERVEREVENTS", TIPUnRegisterServerEvents);
   AddCommand("TIPSETLOCALTIMEINFORMATION", TIPSetLocalTimeInformation);
   AddCommand("TIPUPDATECURRENTTIME", TIPUpdateCurrentTime);
   AddCommand("TIPREFERENCETIMERESPONSE", TIPReferenceTimeResponse);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)TIPRegisterServerEvents                    *\r\n");
   printf("*                  36)TIPUnRegisterServerEvents                  *\r\n");
   printf("*                  37)TIPSetLocalTimeInformation                 *\r\n");
   printf("*                  38)TIPUpdateCurrentTime                       *\r\n");
   printf("*                  39)TIPReferenceTimeResponse                   *\r\n");
   printf("*                  40)StartAdvertising                           *\r\n");
   printf("*                  41)StopAdvertising                            *\r\n");
   printf("*                  42)StartObservationScan                       *\r\n");
   printf("*                  43)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* Manager to receive TIP Server Events.  This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int TIPRegisterServerEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!TIPMServerEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = TIPM_Register_Server_Event_Callback(TIPM_Event_Callback, NULL)) > 0)
         {
            printf("TIPM_Register_Server_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            TIPMServerEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("TIPM_Register_Server_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("TIP Manager Server Event Callback already registered.\r\n");
   ...
   /* previously registered TIP Server Event Registration with the TIP  */
...
static int TIPUnRegisterServerEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMServerEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         TIPM_Un_Register_Server_Event_Callback(TIPMServerEventCallbackID);

         printf("TIPM_Un_Register_Server_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         TIPMServerEventCallbackID = 0;
...
static int TIPSetLocalTimeInformation(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMServerEventCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam >= tmzUTCMinus1200) && (TempParam->Params[0].intParam <= tmzUTCUnknown) && (TempParam->Params[1].intParam >= tmdStandardTime) && (TempParam->Params[1].intParam <= tmdUnknown))
         {
            /* Configure the local time information.                    */
            LocalTimeInformation.Time_Zone            = (TIPM_Time_Zone_Type_t)TempParam->Params[0].intParam;
            LocalTimeInformation.Daylight_Saving_Time = (TIPM_DST_Offset_Type_t)TempParam->Params[1].intParam;

            printf("Attempting to set Time Zone = %s, DST = %s.\n", TimeZoneToString(LocalTimeInformation.Time_Zone), DSTOffsetToString(LocalTimeInformation.Daylight_Saving_Time));

            /* Attempt to set the Local Time Information.               */
            if((ret_val = TIPM_Set_Local_Time_Information(TIPMServerEventCallbackID, &LocalTimeInformation)) == 0)
            {
               printf("TIPM_Set_Local_Time_Information() Success: %d.\r\n", ret_val);
            ...
            else
            {
               /* Error Disconnecting Remote Device, inform the user and*/
               /* flag an error.                                        */
               printf("TIPM_Set_Local_Time_Information() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPSetLocalTimeInformation [Time Zone] [DST Offset].\r\n");
            printf("%s", "Where Time Zone = \n");

            for(Index=tmzUTCMinus1200;Index<=tmzUTCUnknown;)
            {
               printf("%u = %s, %u = %s, %u = %s, %u = %s\n", Index, TimeZoneToString((TIPM_Time_Zone_Type_t)Index),
                                                              Index + 1, TimeZoneToString((TIPM_Time_Zone_Type_t)(Index + 1)),
                                                              Index + 2, TimeZoneToString((TIPM_Time_Zone_Type_t)(Index + 2)),
                                                              Index + 3, TimeZoneToString((TIPM_Time_Zone_Type_t)(Index + 3)));

               Index += 4;
            }

            printf("%s", "Where DST Offset = \n");
            for(Index=tmdStandardTime;Index<=tmdUnknown;Index++)
               printf("%u = %s\n", Index, DSTOffsetToString((TIPM_DST_Offset_Type_t)Index));
...
static int TIPUpdateCurrentTime(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMServerEventCallbackID)
      {
         ...
         {
            /* Attempt to set the Local Time Information.               */
            if((ret_val = TIPM_Update_Current_Time(TIPMServerEventCallbackID, (unsigned long)TempParam->Params[0].intParam)) == 0)
            {
               printf("TIPM_Update_Current_Time() Success: %d.\r\n", ret_val);
            ...
            else
            {
               /* Error Disconnecting Remote Device, inform the user and*/
               /* flag an error.                                        */
               printf("TIPM_Update_Current_Time() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPUpdateCurrentTime [Update Mask].\r\n");
            printf("%s", "Update Mask = \n");
            printf("0x%08X = Manual Update\n", TIPM_CURRENT_TIME_ADJUST_REASON_FLAGS_MANUAL_TIME_UPDATE);
            printf("0x%08X = External Reference Time Update\n", TIPM_CURRENT_TIME_ADJUST_REASON_FLAGS_EXTERNAL_REFERENCE_TIME_UPDATE);
            printf("0x%08X = Time Zone Change\n", TIPM_CURRENT_TIME_ADJUST_REASON_FLAGS_CHANGE_OF_TIMEZONE);
            printf("0x%08X = DST Change\n", TIPM_CURRENT_TIME_ADJUST_REASON_FLAGS_CHANGE_OF_DST);
   ...
   /* reference time request.  This function returns zero if successful */
   /* and a negative value if an error occurred.                        */
static int TIPReferenceTimeResponse(ParameterList_t *TempParam)
{
   int                                    ret_val;
   ...
   if(Initialized)
   {
      ...
      if(TIPMServerEventCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 4) && (TempParam->Params[0].intParam >= tmsUnknown) && (TempParam->Params[0].intParam <= tmsCellularNetwork))
         {
            /* Configure the Reference Time Information.                */
            ReferenceTimeInformation.Flags              = (TIPM_REFERENCE_TIME_INFORMATION_FLAGS_REFERENCE_TIME_KNOWN | TIPM_REFERENCE_TIME_INFORMATION_FLAGS_ACCURACY_KNOWN);
            ReferenceTimeInformation.Source             = (TIPM_Time_Source_Type_t)TempParam->Params[0].intParam;
            ReferenceTimeInformation.Accuracy           = TempParam->Params[1].intParam;
            ReferenceTimeInformation.Days_Since_Update  = TempParam->Params[2].intParam;
            ReferenceTimeInformation.Hours_Since_Update = TempParam->Params[3].intParam;

            /* Attempt to set the Reference Time Information.           */
            if((ret_val = TIPM_Reference_Time_Response(TIPMServerEventCallbackID, &ReferenceTimeInformation)) == 0)
            {
               printf("TIPM_Reference_Time_Response() Success: %d.\r\n", ret_val);
            ...
            else
            {
               /* Error Disconnecting Remote Device, inform the user and*/
               /* flag an error.                                        */
               printf("TIPM_Reference_Time_Response() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPReferenceTimeResponse [Time Source] [Accuracy] [Days since update] [Hours since update].\r\n");
            printf("%s", "Time Source = \n");

            for(Index=tmsUnknown;Index<=tmsCellularNetwork;Index++)
               printf("%u = %s", Index, TimeSourceToString((TIPM_Time_Source_Type_t)Index));
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case aetTIPDisconnected:
            ...
            printf("%s", "TIPM Disconnected:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Connection Type:       %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == tctServer) ? "Server" : "Client");
            break;
         case aetTIPGetReferenceTimeRequest:
            printf("%s", "TIPM Get Reference Time Request, Respond with TIPReferenceTimeResponse\r\n");
            break;
         default:
