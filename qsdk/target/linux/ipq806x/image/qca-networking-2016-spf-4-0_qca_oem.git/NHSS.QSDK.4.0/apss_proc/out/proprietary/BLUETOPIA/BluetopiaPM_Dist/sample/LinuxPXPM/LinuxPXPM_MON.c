...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXPXPM_MON - Simple Linux application using Bluetopia Platform Manager */
/*                  Proximity Profile (PXP - Monitor Role Only) Manager       */
...
/*   --------  -----------    ------------------------------------------------*/
/*   11/26/12  T. Cook       Initial creation. (Based on LinuxHRPM)           */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxPXPM_MON.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTPXPM.h"     /* PXP Manager Application Programming Interface.  */
...
#define PLATFORM_MANAGER_NOT_INITIALIZED_ERROR     (-7)  /* Denotes that an   */
...
#ifndef IAS_ASSIGN_IAS_SERVICE_UUID_16
   ...
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#ifndef LLS_ASSIGN_LLS_SERVICE_UUID_16
   ...
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#ifndef TPS_ASSIGN_TPS_SERVICE_UUID_16
...
static unsigned int        PXPMEventCallbackID;     /* Variable which holds the        */
                                                    /* current PXPM Event Callback ID  */
                                                    /* that is assigned from the PXPM  */
                                                    /* Manager when the local client   */
                                                    /* registers for PXPM Events.      */
...
static char *AlertLevelToString(PXPM_Alert_Level_t AlertLevel);
...
static int PXPRegisterMonitorEvents(ParameterList_t *TempParam);
static int PXPUnRegisterMonitorEvents(ParameterList_t *TempParam);
static int PXPSetPathLossRefreshTime(ParameterList_t *TempParam);
static int PXPSetPathLossThreshold(ParameterList_t *TempParam);
static int PXPQueryCurrentPathLoss(ParameterList_t *TempParam);
static int PXPSetPathLossAlertLevel(ParameterList_t *TempParam);
static int PXPSetLinkLossAlertLevel(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM PXP Manager Callback function prototype.                     */
static void BTPSAPI PXPM_Event_Callback(PXPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("PXPREGISTERMONITOREVENTS", PXPRegisterMonitorEvents);
   AddCommand("PXPUNREGISTERMONITOREVENTS", PXPUnRegisterMonitorEvents);
   AddCommand("PXPSETPATHLOSSREFRESHTIME", PXPSetPathLossRefreshTime);
   AddCommand("PXPSETPATHLOSSTHRESHOLD", PXPSetPathLossThreshold);
   AddCommand("PXPQUERYCURRENTPATHLOSS", PXPQueryCurrentPathLoss);
   AddCommand("PXPSETPATHLOSSALERTLEVEL", PXPSetPathLossAlertLevel);
   AddCommand("PXPSETLINKLOSSALERTLEVEL", PXPSetLinkLossAlertLevel);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   {
      /* Format the data.                                               */
      Length = ((UUID_16_SIZE*3) + NON_ALIGNED_BYTE_SIZE);
      ...
      LLS_ASSIGN_LLS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
      IAS_ASSIGN_IAS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE*2]));
      TPS_ASSIGN_TPS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE*3]));
   ...
   /* The following function is a utility function that is used to      */
   /* convert an PXP alert level to a string.                           */
static char *AlertLevelToString(PXPM_Alert_Level_t AlertLevel)
{
   char *ret_val;
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)PXPRegisterMonitorEvents                   *\r\n");
   printf("*                  36)PXPUnRegisterMonitorEvents                 *\r\n");
   printf("*                  37)PXPSetPathLossRefreshTime                  *\r\n");
   printf("*                  38)PXPSetPathLossThreshold                    *\r\n");
   printf("*                  39)PXPQueryCurrentPathLoss                    *\r\n");
   printf("*                  40)PXPSetPathLossAlertLevel                   *\r\n");
   printf("*                  41)PXPSetLinkLossAlertLevel                   *\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the PXP*/
   /* Manager to receive PXP Monitor Events.  This function returns zero*/
   /* if successful and a negative value if an error occurred.          */
static int PXPRegisterMonitorEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!PXPMEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = PXPM_Register_Monitor_Event_Callback(PXPM_Event_Callback, NULL)) > 0)
         {
            printf("PXPM_Register_Monitor_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            PXPMEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("PXPM_Register_Monitor_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("PXP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered PXP Event Registration with the PXP Manager.*/
...
static int PXPUnRegisterMonitorEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PXPMEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         PXPM_Un_Register_Monitor_Event_Callback(PXPMEventCallbackID);

         printf("PXPM_Un_Register_Monitor_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         PXPMEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("PXP Manager Event Callback is not registered.\r\n");
   ...
   /* The following function is responsible for setting the Path Loss   */
   /* Refresh Time.  This function returns zero if successful and a     */
   /* negative value if an error occurred.                              */
static int PXPSetPathLossRefreshTime(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         printf("Attempting to set Path Loss Refresh Time: %d (ms).\r\n", (int)TempParam->Params[0].intParam);

         if((Result = PXPM_Set_Path_Loss_Refresh_Time(PXPMEventCallbackID, (unsigned int)TempParam->Params[0].intParam)) >= 0)
         {
            printf("PXPM_Set_Path_Loss_Refresh_Time() Success: %d.\r\n", Result);
         ...
         {
            /* Error setting the Refresh Time, inform the user and flag */
            /* an error.                                                */
            printf("PXPM_Set_Path_Loss_Refresh_Time() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PXPSetPathLossRefreshTime [Refresh Time (milliseconds)].\r\n");
   ...
   /* The following function is responsible for setting the Path Loss   */
   /* Threshold.  This function returns zero if successful and a        */
   /* negative value if an error occurred.                              */
static int PXPSetPathLossThreshold(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         ...
         printf("Attempting to set Path Loss Threshold: %s, %d.\r\n", TempParam->Params[0].strParam, (int)TempParam->Params[1].intParam);

         if((Result = PXPM_Set_Path_Loss_Threshold(PXPMEventCallbackID, BD_ADDR, (int)TempParam->Params[1].intParam)) >= 0)
         {
            printf("PXPM_Set_Path_Loss_Threshold() Success: %d.\r\n", Result);
         ...
         {
            /* Error setting the Path Loss Threshold, inform the user   */
            /* and flag an error.                                       */
            printf("PXPM_Set_Path_Loss_Threshold() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PXPSetPathLossThreshold [BD_ADDR] [Threshold (dBm)].\r\n");
   ...
   /* The following function is responsible for querying the current    */
   /* Path Loss.  This function returns zero if successful and a        */
   /* negative value if an error occurred.                              */
static int PXPQueryCurrentPathLoss(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         ...
         if((Result = PXPM_Query_Current_Path_Loss(PXPMEventCallbackID, BD_ADDR, &PathLoss)) >= 0)
         {
            printf("PXPM_Query_Current_Path_Loss() Success: %d, Path Loss = %d dBm.\r\n", Result, PathLoss);
         ...
         {
            /* Error quering the current Path Loss, inform the user and */
            /* flag an error.                                           */
            printf("PXPM_Query_Current_Path_Loss() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PXPQueryCurrentPathLoss [BD_ADDR].\r\n");
   ...
   /* The following function is responsible for setting the Path Loss   */
   /* Alert Level.  This function returns zero if successful and a      */
   /* negative value if an error occurred.                              */
static int PXPSetPathLossAlertLevel(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         ...
         printf("Attempting to set Path Loss Alert Level: %s, %s.\r\n", TempParam->Params[0].strParam, AlertLevelToString((PXPM_Alert_Level_t)TempParam->Params[1].intParam));

         if((Result = PXPM_Set_Path_Loss_Alert_Level(PXPMEventCallbackID, BD_ADDR, (PXPM_Alert_Level_t)TempParam->Params[1].intParam)) >= 0)
         {
            printf("PXPM_Set_Path_Loss_Alert_Level() Success: %d.\r\n", Result);
         ...
         {
            /* Error setting the Path Loss Alert Level, inform the user */
            /* and flag an error.                                       */
            printf("PXPM_Set_Path_Loss_Alert_Level() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PXPSetPathLossAlertLevel [BD_ADDR] [Alert Level (0 = No Alert, 1 = Mild Alert, 2 = High Alert))].\r\n");
   ...
   /* The following function is responsible for setting the Link Loss   */
   /* Alert Level.  This function returns zero if successful and a      */
   /* negative value if an error occurred.                              */
static int PXPSetLinkLossAlertLevel(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         ...
         printf("Attempting to set Link Loss Alert Level: %s, %s.\r\n", TempParam->Params[0].strParam, AlertLevelToString((PXPM_Alert_Level_t)TempParam->Params[1].intParam));

         if((Result = PXPM_Set_Link_Loss_Alert_Level(PXPMEventCallbackID, BD_ADDR, (PXPM_Alert_Level_t)TempParam->Params[1].intParam)) >= 0)
         {
            printf("PXPM_Set_Link_Loss_Alert_Level() Success: %d.\r\n", Result);
         ...
         {
            /* Error setting the Link Loss Alert Level, inform the user */
            /* and flag an error.                                       */
            printf("PXPM_Set_Link_Loss_Alert_Level() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PXPSetLinkLossAlertLevel [BD_ADDR] [Alert Level (0 = No Alert, 1 = Mild Alert, 2 = High Alert))].\r\n");
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* The following function is the Proximity Manager Event Callback    */
   /* function that is Registered with the Proximity Profile Manager.   */
   /* This callback is responsible for processing all Proximity Manager */
   /* Events.                                                           */
static void BTPSAPI PXPM_Event_Callback(PXPM_Event_Data_t *EventData, void *CallbackParameter)
{
   char BoardStr[16];
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case etPXPConnected:
            ...
            printf("%s", "PXPM Connected:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type:   %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == pctPXPMonitor)?"Monitor":"Reporter");
            printf("   Link Loss Alert:   %sSupported\r\n", (EventData->EventData.ConnectedEventData.SupportedFeatures & PXPM_SUPPORTED_FEATURES_FLAGS_LINK_LOSS_ALERT) ? "" : "Not ");
            printf("   Path Loss Alert:   %sSupported\r\n", (EventData->EventData.ConnectedEventData.SupportedFeatures & PXPM_SUPPORTED_FEATURES_FLAGS_PATH_LOSS_ALERT) ? "" : "Not ");
            break;
         case etPXPDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);

            printf("%s", "PXPM Disconnected:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type:   %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == pctPXPMonitor)?"Monitor":"Reporter");
            break;
         case etPXPPathLossAlert:
            BD_ADDRToStr(EventData->EventData.PathLossAlertEventData.RemoteDeviceAddress, BoardStr);

            printf("%s", "PXPM Path Loss Alert:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type:   %s\r\n", (EventData->EventData.PathLossAlertEventData.ConnectionType == pctPXPMonitor)?"Monitor":"Reporter");
            printf("   Alert Level:       %s\r\n", AlertLevelToString(EventData->EventData.PathLossAlertEventData.AlertLevel));
            break;
         case etPXPLinkLossAlert:
            BD_ADDRToStr(EventData->EventData.LinkLossAlertEventData.RemoteDeviceAddress, BoardStr);

            printf("%s", "PXPM Link Loss Alert:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type:   %s\r\n", (EventData->EventData.LinkLossAlertEventData.ConnectionType == pctPXPMonitor)?"Monitor":"Reporter");
            printf("   Alert Level:       %s\r\n", AlertLevelToString(EventData->EventData.LinkLossAlertEventData.AlertLevel));
            break;
         default:
            printf("Unknown PXP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nPXPM Event Data is NULL.\r\n");
