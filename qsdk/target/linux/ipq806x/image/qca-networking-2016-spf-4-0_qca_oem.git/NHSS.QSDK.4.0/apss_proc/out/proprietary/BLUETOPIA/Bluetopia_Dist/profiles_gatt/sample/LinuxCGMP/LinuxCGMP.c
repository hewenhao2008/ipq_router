...
static void DecodeDisplayRACPResponse(Word_t BufferLength, Byte_t *Buffer)
{
   ...
   if((BufferLength) && (Buffer) )
   {
      ...
      if(!Result)
      {
         if(RACPResponseData.ResponseType == rarResponseCode)
         {
         ...
         {
            if(RACPResponseData.ResponseType == rarNumberOfStoredRecords)
            {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(CheckForBond())
            {
               ...
               if((RACPRequestData.CommandType >= racReportStoredRecordsRequest) && (RACPRequestData.CommandType <= racNumberOfStoredRecordsRequest))
               {
                  /* Determine the operator type.                       */
                  if(RACPRequestData.CommandType == racAbortOperationRequest)
                     RACPRequestData.OperatorType = raoNull;
                  else
                     RACPRequestData.OperatorType = (Byte_t)TempParam->Params[1].intParam;

                  /* Make sure the operator is semi-valid.              */
                  if(((RACPRequestData.OperatorType >= raoAllRecords) && (RACPRequestData.OperatorType <= raoLastRecord)) || ((RACPRequestData.CommandType == racAbortOperationRequest) && (RACPRequestData.OperatorType == raoNull)))
                  {
                     /* Determine the parameters.                       */
                     switch(RACPRequestData.OperatorType)
                     {
                        case raoGreaterThanOrEqualTo:
                        case raoLessThanOrEqualTo:
               ...
               while((ret_val == 0) && (Index != NULL))
               {
                  ...
                  if((ret_val == 0) && (MeasurementFound == TRUE))
                  {
                     ...
                     if((NewMeasurement = (Measurement_t *)BTPS_AllocateMemory(sizeof(Measurement_t))) != NULL)
                     {
                        ...
                        if((RACPData.OperatorType == raoFirstRecord) || (RACPData.OperatorType == raoLastRecord))
                           break;
                     }
                     else
                     {
      ...
      if((SensorMeasurementList.Head != NULL) && (SensorMeasurementList.Tail != NULL) && (SensorMeasurementList.NumberOfMeasurements >= 1))
      {
         switch(RACPData.OperatorType)
         {
            ...
            case raoAllRecords:
               ...
               if(RACPData.OperatorType == raoAllRecords)
            ...
            case raoWithinRangeOf:
               ...
               while(Index != NULL)
               {
                  ...
                  if((RACPData.OperatorType == raoLessThanOrEqualTo) && (Index->TimeOffset <= RACPData.ParameterData.TimeOffset))
                     (*NumberOfMeasurementsFound)++;

                  /* If the time offset is greater than or equal to.    */
                  if((RACPData.OperatorType == raoGreaterThanOrEqualTo) && (Index->TimeOffset >= RACPData.ParameterData.TimeOffset))
                     (*NumberOfMeasurementsFound)++;

                  /* If the time offset is in range.                    */
                  if((RACPData.OperatorType == raoWithinRangeOf) && (Index->TimeOffset >= RACPData.ParameterData.Range.Minimum) && (Index->TimeOffset <= RACPData.ParameterData.Range.Maximum))
                     (*NumberOfMeasurementsFound)++;
                  ...
                  if(CGMS_Event_Data->Event_Data.CGMS_Read_Client_Configuration_Data->InstanceID == CGMSInstanceID)
                  {
                     ...
                     switch(CGMS_Event_Data->Event_Data.CGMS_Read_Client_Configuration_Data->ClientConfigurationType)
                     {
                         case ctCGMSMeasurement:
                            ...
                            break;
                         case ctRecordAccessControlPoint:
                            printf("   Config Type:      ctRecordAccessControlPoint.\r\n");
                            ...
                            break;
                        case ctSpecificOpsControlPoint:
                            printf("   Config Type:      ctSpecificOpsControlPoint.\r\n");
            ...
            if(CGMS_Event_Data->Event_Data.CGMS_Record_Access_Control_Point_Command_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  if(DeviceInfo->ServerInfo.RACP_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_INDICATE_ENABLE)
                  {
                     ...
                     /* procedure.                                      */
                     if((!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_RACP_OUTSTANDING)) || ((DeviceInfo->Flags & DEVICE_INFO_FLAGS_RACP_OUTSTANDING) && (CGMS_Event_Data->Event_Data.CGMS_Record_Access_Control_Point_Command_Data->FormatData.CommandType == racAbortOperationRequest)))
                     {
                        ...
                        switch(CommandType)
                        {
                           case racReportStoredRecordsRequest:
                              printf("OpCode:           racReportStoredRecordsRequest.\r\n");
                              break;
                           case racDeleteStoredRecordsRequest:
                              printf("OpCode:           racDeleteStoredRecordsRequest.\r\n");
                              break;
                           case racAbortOperationRequest:
                              printf("OpCode:           racAbortOperationRequest.\r\n");
                              break;
                           case racNumberOfStoredRecordsRequest:
                              printf("OpCode:           racNumberOfStoredRecordsRequest.\r\n");
                              break;
                           default:
                        ...
                        {
                           case raoNull:
                              printf("Operator:         raoNull.\r\n");
                              break;
                           case raoAllRecords:
                              printf("Operator:         raoAllRecords.\r\n");
                              break;
                           case raoLessThanOrEqualTo:
                           case raoGreaterThanOrEqualTo:
                              /* Determine the operator type.           */
                              if((OperatorType == raoLessThanOrEqualTo))
                                 printf("Operator:         raoLessThanOrEqualTo.\r\n");
                              else
                                 printf("Operator:         raoGreaterThanOrEqualTo.\r\n");
                              ...
                              break;
                           case raoWithinRangeOf:
                              printf("Operator:         raoWithinRangeOf.\r\n");
                              ...
                              break;
                           case raoFirstRecord:
                              printf("Operator:         raoFirstRecord.\r\n");
                              break;
                           case raoLastRecord:
                              printf("Operator:         raoLastRecord.\r\n");
                              break;
                           default:
                        ...
                        if((RACPResult == CGMS_RACP_RESPONSE_CODE_SUCCESS) && (CommandType == racNumberOfStoredRecordsRequest))
                        {
