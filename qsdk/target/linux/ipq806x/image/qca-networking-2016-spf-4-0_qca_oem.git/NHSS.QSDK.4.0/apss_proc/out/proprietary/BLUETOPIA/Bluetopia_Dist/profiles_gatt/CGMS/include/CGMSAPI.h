...
#define CGMS_ERROR_BUFFER_NOT_EMPTY                             (-1015)
   ...
   /* *** NOTE *** The CGMS Client can ignore a CRC sent by the CGMS    */
   /*            Server.                                                */
...
#define CGMS_E2E_CRC_SUPPORTED                                  (0x01)
   ...
   /* *** NOTE *** If the Session Start Time Characteristic or the      */
   /*            Specific Ops Control Point Characteristic is written,  */
   /*            we cannot internally determine if a CRC is present and */
   /*            valid with the incoming data from the CGMS Client.  In */
   /*            order to handle this situation, the CGMS Server will be*/
   /*            notified if a CRC is present and valid by either the   */
   /*            generated                                              */
   /*            etCGMS_Server_Write_Session_Start_Time_Request or      */
   /*            etCGMS_Server_Specific_Ops_Control_Point_Command CGMS  */
   /*            events, in the CGMS_Event_Callback().  These events    */
   /*            will contain the following structures that use these   */
   /*            bit values: CGMS_Write_Session_Start_Time_Data_t (Flags*/
   /*            field), and                                            */
   ...
   /*            decoded and some contain a CRC while others do not,    */
   /*            they can still be decoded.                             */
...
#define CGMS_SERVER_INFORMATION_DATA_SIZE                       (sizeof(CGMS_Server_Information_t))
   ...
   /* *** NOTE *** If CGMS_MEASUREMENT_FLAG_TREND_INFORMATION_PRESENT   */
...
typedef enum
{
   rarNumberOfStoredRecords = CGMS_RACP_OPCODE_NUMBER_OF_STORED_RECORDS_RESPONSE,
   rarResponseCode          = CGMS_RACP_OPCODE_RESPONSE_CODE
...
#define CGMS_RACP_RESPONSE_DATA_SIZE                            (sizeof(CGMS_RACP_Response_Data_t))
   ...
   /* value to notify.                                                  */
   /* *** NOTE *** For each event it is up to the application to return */
   /*            (or write) the correct Client Configuration descriptor */
   /*            based on this value.                                   */
typedef enum
{
   ctCGMSMeasurement,
   ctRecordAccessControlPoint,
   ctSpecificOpsControlPoint
...
#define CGMS_READ_SESSION_START_TIME_DATA_SIZE                  (sizeof(CGMS_Read_Session_Start_Time_Data_t))
   ...
   /*            cannot internally determine if a CRC is present and    */
   /*            valid with the incoming data from the CGMS Client.  In */
   /*            order to handle this situation, the CGMS Server will be*/
   /*            notified if a CRC is present and valid by the generated*/
   /*            etCGMS_Server_Write_Session_Start_Time_Request event,  */
   /*            sent to the CGMS_Event_Callback().  This event will    */
   /*            contain the following structure that uses the bit      */
   /*            values (below): CGMS_Write_Session_Start_Time_Data_t   */
   /*            (Flags field).  This way the CGMS Server will be able  */
   /*            to determine if a CRC is present and valid, and send   */
   /*            the appropriate response to the CGMS Client.           */
   /* *** NOTE *** Valid values for the Flags field are                 */
   /*            CGMS_E2E_CRC_PRESENT and CGMS_E2E_CRC_VALID.           */
...
typedef enum
{
   racReportStoredRecordsRequest   = CGMS_RACP_OPCODE_REPORT_STORED_RECORDS,
   racDeleteStoredRecordsRequest   = CGMS_RACP_OPCODE_DELETE_STORED_RECORDS,
   racAbortOperationRequest        = CGMS_RACP_OPCODE_ABORT_OPERATION,
   racNumberOfStoredRecordsRequest = CGMS_RACP_OPCODE_REPORT_NUMBER_OF_STORED_RECORDS
...
typedef enum
{
   raoNull                 = CGMS_RACP_OPERATOR_NULL,
   raoAllRecords           = CGMS_RACP_OPERATOR_ALL_RECORDS,
   raoLessThanOrEqualTo    = CGMS_RACP_OPERATOR_LESS_THAN_OR_EQUAL_TO,
   raoGreaterThanOrEqualTo = CGMS_RACP_OPERATOR_GREATER_THAN_OR_EQUAL_TO,
   raoWithinRangeOf        = CGMS_RACP_OPERATOR_WITHIN_RANGE_OF,
   raoFirstRecord          = CGMS_RACP_OPERATOR_FIRST_RECORD,
   raoLastRecord           = CGMS_RACP_OPERATOR_LAST_RECORD
...
#define CGMS_TIME_OFFSET_RANGE_DATA_SIZE                        (sizeof(CGMS_Time_Offset_Range_Data_t))
   ...
   /* *** NOTE *** The CommandType member is an enumerated type that    */
   /*            must be of the form CGMS_RACP_OPCODE_XXX.              */
   /* *** NOTE *** The OperatorType member is an enumerated type that   */
   /*            must be of the form CGMS_RACP_OPERATOR_XXX.            */
   /* *** NOTE *** The FilterType member is an enumerated type that must*/
   /*            be of the form CGMS_RACP_FILTER_TYPE_XXX.              */
...
#define CGMS_SPECIFIC_OPS_CONTROL_POINT_RESPONSE_DATA_SIZE      (sizeof(CGMS_Specific_Ops_Control_Point_Response_Data_t))
   ...
   /*            Server will be notified if a CRC is present and valid  */
   /*            by the generated                                       */
...
#define CGMS_SPECIFIC_OPS_CONTROL_POINT_COMMAND_DATA_SIZE       (sizeof(CGMS_Specific_Ops_Control_Point_Command_Data_t))
   ...
   /* Indication                                                        */
   /* *** NOTE *** The Characteristic_Type parameter will NEVER be set  */
   /*            to ctIntermediateTemperature for this event.           */
   /* *** NOTE ***The Status member is set to one of the following      */
   /*            values: GATT_CONFIRMATION_STATUS_SUCCESS               */
...
#define CGMS_EVENT_DATA_SIZE                                    (sizeof(CGMS_Event_Data_t))
   ...
   /*            can only be satisfied by Receiving CGMS Profile Event  */
...
typedef void (BTPSAPI *CGMS_Event_Callback_t)(unsigned int BluetoothStackID, CGMS_Event_Data_t *CGMS_Event_Data, unsigned long CallbackParameter);
   ...
   /* *** NOTE *** Only 1 CGMS Server may be open at a time, per        */
   /*            Bluetooth Stack ID.                                    */
   /* *** NOTE *** All Client Requests will be dispatch to the          */
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Read_Client_Configuration_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, Word_t ClientConfiguration);
   ...
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with each CGMS Measurement.                   */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Notify_CGMS_Measurements(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, unsigned int Flags, unsigned int NumberOfMeasurements, CGMS_Measurement_Data_t **MeasurementData);
   ...
   /*            function will respond to the Request successfully with */
   /*            a valid value in the CGMSFeature parameter.            */
   /* *** NOTE *** If the ErrorCode is non-zero then an error response  */
   /*            will be sent to the remote device and CGMSFeature      */
   /*            parameter will be ignored.                             */
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Feature.                        */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Feature_Read_Request_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, unsigned int Flags, Byte_t ErrorCode, CGMS_Feature_Data_t *CGMSFeature);
   ...
   /* *** NOTE *** If the ErrorCode parameter is set to 0x00 then this  */
   /*            function will respond to the Request successfully with */
   /*            a valid value in the CGMSStatus parameter.             */
   /* *** NOTE *** If the ErrorCode is non-zero then an error response  */
   /*            will be sent to the remote device and CGMSStatus       */
   /*            parameter will be ignored.                             */
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Status.                         */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Status_Read_Request_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, unsigned int Flags, Byte_t ErrorCode, CGMS_Status_Data_t *CGMSStatus);
   ...
   /* *** NOTE *** If the ErrorCode parameter is set to 0x00 then this  */
   /*            function will respond to the Request successfully with */
   /*            a valid value in the SessionStartTime parameter.       */
   /* *** NOTE *** If the ErrorCode is non-zero then an error response  */
   /*            will be sent to the remote device and SessionStartTime */
   /*            parameter will be ignored.                             */
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Session Start Time.             */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Session_Start_Time_Read_Request_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, unsigned int Flags, Byte_t ErrorCode, CGMS_Session_Start_Time_Data_t *SessionStartTime);
   ...
   /* *** NOTE *** If the ErrorCode is non-zero then an error response  */
   /*            will be sent to the remote device.                     */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Session_Start_Time_Write_Request_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, Byte_t ErrorCode);
   ...
   /* *** NOTE *** If the ErrorCode parameter is set to 0x00 then this  */
   /*            function will respond to the Request successfully with */
   /*            a valid value in the SessionRunTime parameter.         */
   /* *** NOTE *** If the ErrorCode is non-zero then an error response  */
   /*            will be sent to the remote device and SessionRunTime   */
   /*            parameter will be ignored.                             */
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Session Run Time.               */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Session_Run_Time_Read_Request_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, unsigned int Flags, Byte_t ErrorCode, CGMS_Session_Run_Time_Data_t *SessionRunTime);
   ...
   /* error occurs.                                                     */
   /* *** NOTE *** If the ErrorCode parameter is set to 0x00 the        */
   /*            Procedure Request will be accepted.                    */
   /* *** NOTE *** If the ErrorCode is non-zero than an error response  */
   /*            will be sent to the remote device.                     */
   ...
   /*            operation, the Client does not have proper             */
   /*            authentication to write to the RACP characteristic, or */
   /*            a RACP procedure with the Client is already in         */
   /*            progress.  All other reasons should return ZERO for the*/
   /*            ErrorCode and then send RACP Result indication to      */
   /*            indicate any other errors.  For Example: If the Operand*/
   /*            in the Request is not supported by the Server this API */
   /*            should be called with ErrorCode set to ZERO and then   */
   /*            the CGMS_Indicate_Record_Access_Control_Point_Result() */
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Indicate_Record_Access_Control_Point_Result(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, Byte_t RequestOpCode, Byte_t ResponseCode);
   ...
   /*            will be sent to the remote device.                     */
   /* *** NOTE *** This function is primarily provided to allow a way to*/
   /*            reject CGMS Specific Ops Control Point commands when   */
   /*            the Server has not been configured properly for CGMS   */
   /*            SOCP operation, the Client does not have proper        */
   /*            authentication to write to the CGMS SOCP               */
   /*            characteristic, a CGMS SOCP procedure with the Client  */
   /*            is already in progress or when Invalid CRC is found or */
   /*            when CRC is Missing.  All other reasons should return  */
   /*            ZERO for the ErrorCode and then send CGMS SOCP Result  */
   /*            indication to indicate any other errors.  For Example: */
   /*            If the Operand in the Request is not supported by the  */
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Specific_Ops_Control_Point_Response(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int TransactionID, Byte_t ErrorCode);
   ...
   /* *** NOTE *** Only 1 CGMS SOCP Request indication may be           */
   /*            outstanding per CGMS Instance.                         */
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Specific Ops Control Point      */
   /*            indication.                                            */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Indicate_CGMS_Specific_Ops_Control_Point_Result(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, unsigned int Flags, Byte_t RequestOpCode, Byte_t ResponseCodeValue);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Communication Interval          */
   /*            indication.                                            */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Indicate_Communication_Interval(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, unsigned int Flags, Byte_t CommunicationIntervalMinutes);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Calibration Data indication.    */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Indicate_Calibration_Data(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, unsigned int Flags, CGMS_Calibration_Data_Record_t *CalibrationData);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated and      */
   /*            included with the CGMS Alert Level indication.         */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Indicate_Alert_Level(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned int ConnectionID, unsigned int Flags, CGMS_SOCP_Response_Type_t ResponseOpCode, Word_t AlertLevel);
   ...
   /* *** NOTE *** If this function fails for any reason during the     */
   /*            decoding of a measurement, then any following          */
   /*            measurements will not be decoded.                      */
   /* *** NOTE *** If a CRC is attached to a decoded CGMS Measurement   */
   /*            then the CRCFlags field of the CGMS_Measurement_Data_t */
   /*            structure will be marked that there is a CRC present   */
   /*            and if it is valid.  See CGMS_E2E_CRC_PRESENT and      */
   /*            CGMS_E2E_CRC_VALID for more information.  This way the */
   /*            CGMS client can determine if a CRC has been included   */
   /*            and is valid for a CGMS Measurement.                   */
   /* *** NOTE *** Since the Flags field of a CGMS Measurement does not */
   /*            indicate whether a CRC is present as an optional field */
   /*            (Size indicates this), the CGMS Measurements, when     */
   /*            decoded, will each be marked in the (CRCFlags field) of*/
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Decode_CGMS_Measurement(unsigned int BufferLength, Byte_t *Buffer, unsigned int NumberOfMeasurements, CGMS_Measurement_Data_t **MeasurementData);
   ...
   /*            incoming CRC to the CGMS_E2E_NOT_SUPPORTED_VALUE found */
   /*            in CGMSTypes.h.  If the incoming CRC is not set to this*/
   /*            value then the CRC will be verified for the CGMS       */
   /*            Feature.                                               */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Decode_CGMS_Feature(unsigned int ValueLength, Byte_t *Value, CGMS_Feature_Data_t *CGMSFeature);
   ...
   /* *** NOTE *** The Flags parameter can either be                    */
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated for      */
   /*            verification of the CGMS Status.                       */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Decode_CGMS_Status(unsigned int Flags, unsigned int ValueLength, Byte_t *Value, CGMS_Status_Data_t *CGMSStatus);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated for      */
   /*            verification of the CGMS Session Start Time.           */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Decode_Session_Start_Time(unsigned int Flags, unsigned int ValueLength, Byte_t *Value, CGMS_Session_Start_Time_Data_t *SessionTime);
   ...
   /* negative return error code if an error occurs.                    */
   /* *** NOTE *** If either the third parameter is 0 or the fourth     */
   /*            parameter is null, then the calculated buffer length   */
   /*            will simply be returned.  This will allow the caller to*/
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Format_Session_Start_Time(unsigned int Flags, CGMS_Session_Start_Time_Data_t *SessionStartTime, unsigned int BufferLength, Byte_t *Buffer);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated for      */
   /*            verification of the CGMS Session Run Time.             */
...
BTPSAPI_DECLARATION int BTPSAPI CGMS_Decode_Record_Access_Control_Point_Response(unsigned int ValueLength, Byte_t *Value, CGMS_RACP_Response_Data_t *RACPData);
   ...
   /*            request and the same will hold the size of output      */
   /*            Buffer after formatting.                               */
BTPSAPI_DECLARATION int BTPSAPI CGMS_Format_Record_Access_Control_Point_Command(CGMS_RACP_Format_Data_t *FormatData, unsigned int *BufferLength, Byte_t *Buffer);
   ...
   /*            CGMS_E2E_CRC_SUPPORTED or CGMS_E2E_CRC_NOT_SUPPORTED to*/
   /*            indicate whether a CRC needs to be calculated for      */
   /*            verification of the CGMS Specific Ops Control Point    */
   /*            Result.                                                */
