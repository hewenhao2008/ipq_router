...
/*   --------  -----------    ------------------------------------------------*/
/*   12/19/11  M. Seabold     Initial creation. (Based on LinuxSPPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxPBAM_PCE.h" /* Main Application Prototypes and Constants.      */
...
static int ConnectRemoteDevice(ParameterList_t *TempParam);
static int DisconnectDevice(ParameterList_t *TempParam);
static int Abort(ParameterList_t *TempParam);
static int PullPhoneBook(ParameterList_t *TempParam);
static int PullPhoneBookSize(ParameterList_t *TempParam);
static int SetPhoneBook(ParameterList_t *TempParam);
static int SetPhoneBookAbsolute(ParameterList_t *TempParam);
static int PullvCardListing(ParameterList_t *TempParam);
static int PullvCard(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("CONNECTREMOTEDEVICE", ConnectRemoteDevice);
   AddCommand("DISCONNECTDEVICE", DisconnectDevice);
   AddCommand("ABORT", Abort);
   AddCommand("PULLPHONEBOOK", PullPhoneBook);
   AddCommand("PULLPHONEBOOKSIZE", PullPhoneBookSize);
   AddCommand("SETPHONEBOOK", SetPhoneBook);
   AddCommand("SETPHONEBOOKABSOLUTE", SetPhoneBookAbsolute);
   AddCommand("PULLVCARDLISTING", PullvCardListing);
   AddCommand("PULLVCARD", PullvCard);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  32)ConnectRemoteDevice                        *\r\n");
   printf("*                  33)DisconnectDevice                           *\r\n");
   printf("*                  34)Abort                                      *\r\n");
   printf("*                  35)PullPhoneBook                              *\r\n");
   printf("*                  36)PullPhoneBookSize                          *\r\n");
   printf("*                  37)SetPhoneBook                               *\r\n");
   printf("*                  38)SetPhoneBookAbsolute                       *\r\n");
   printf("*                  39)PullvCardListing                           *\r\n");
   printf("*                  40)PullvCard                                  *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for creating a connection   */
   /* to remote device with the PhoneBook Access Manager Profile.       */
   /* Any events associated with this connection will be sent to the    */
   /* PBAM_Event_Callback function. This function returns zero if       */
   /* successful and a negative error code if it fails.                 */
static int ConnectRemoteDevice(ParameterList_t *TempParam)
{
   int          Result;
   int          ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         ...
         /* Make the actual ConnectRemoteDevice Call.                   */
         if(!(Result = PBAM_Connect_Remote_Device(BD_ADDR, TempParam->Params[1].intParam, (TempParam->NumberofParameters > 2)?TempParam->Params[2].intParam:0, PBAM_Event_Callback, NULL, NULL)))
         {
            printf("PBAM_Connect_Remote_Device(%s, %u) Success. \r\n", TempParam->Params[0].strParam, TempParam->Params[1].intParam);
         ...
         {
            /* Error connecting to the remote PBAM Device, inform the   */
            /* user and flag an error.                                  */
            printf("PBAM_Connect_Remote_Device(%s, %u) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].intParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ConnectRemoteDevice [BD_ADDR] [Server Port] [Flags (Optional - 1:Authentication, 2:Encryption)]\r\n");
   ...
   /* This function is responsible for disconnecting the PBA Manager    */
   /* connection to a previously connected remote device. This function */
   /* returns zero if successful and a negative error code if it fails. */
static int DisconnectDevice(ParameterList_t *TempParam)
{
   int          Result;
   int          ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         ...
         /* Make the actual DisconnectDevice Call.                      */
         if(!(Result = PBAM_Disconnect_Device(BD_ADDR)))
         {
            printf("PBAM_Disconnect_Device(%s) Success.\r\n", TempParam->Params[0].strParam);
         ...
         {
            /* Error disconnecting from the remote PBAM Device, inform  */
            /* the user and flag an error.                              */
            printf("PBAM_Disconnect_Device(%s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: DisconnectDevice [BD_ADDR]\r\n");
   ...
   /* This function is responsible for aborting any current PBAM        */
   /* operations. Since only one PBA Request can be processed at a time,*/
   /* this function must be called to interrupt a current request if    */
   /* another is to be sent. This function returns zero if successful   */
   /* and a negative error code if it fails.                            */
static int Abort(ParameterList_t *TempParam)
{
   int          Result;
   int          ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         ...
         /* Make the actual Abort Call.                                 */
         if(!(Result = PBAM_Abort(BD_ADDR)))
         {
            printf("PBAM_Abort(%s) Success.\r\n", TempParam->Params[0].strParam);
         ...
         {
            /* Error aborting the current operation, inform the user and*/
            /* flag an error.                                           */
            printf("PBAM_Abort(%s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, Result, ERR_ConvertErrorCodeToString(Result));
   ...
   /* This function is responsible for doing a PhoneBook pull on a      */
   /* connected remote device. The data will be sent back to the        */
   /* user via a series of VCardData Events to the callback function    */
   /* registered when the connection was opened. This function will     */
   /* reurns zero if successful and a negative error value if it fails. */
static int PullPhoneBook(ParameterList_t *TempParam)
{
   int          Result;
   int          ret_val;
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 7) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Make the actual PullPhoneBook Call.                         */
         if(!(Result = PBAM_Pull_Phone_Book(BD_ADDR, TempParam->Params[1].strParam, (DWord_t)TempParam->Params[2].intParam, (DWord_t)TempParam->Params[3].intParam, (TempParam->Params[4].intParam == 0)?pmvCard21:(TempParam->Params[4].intParam ==1)?pmvCard30:pmDefault, (Word_t)TempParam->Params[5].intParam, (Word_t)TempParam->Params[6].intParam)))
         {
            printf("PBAM_Pull_Phone_Book(%s) Success.\r\n", TempParam->Params[0].strParam);
         ...
         {
            /* Error pulling the Phone Book, inform the user and flag an*/
            /* error.                                                   */
            printf("PBAM_Pull_Phone_Book(%s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PullPhoneBook [BD_ADDR] [Phonebook Name/Path] [Low Filter] [High Filter] [Card Format (0 = v2.1; 1 = v3.0; 2 = Default)] [Max List Count] [List Offset]\r\n");
   ...
   /* This function is responsible for pulling the PhoneBook Size from a*/
   /* connected remote device. The size is returned in a PhoneBookSize  */
   /* event. This function returns zero if successful and a negative    */
...
static int PullPhoneBookSize(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         ...
         /* Make the actual PullPhoneBookSize Call.                     */
         if(!(Result = PBAM_Pull_Phone_Book_Size(BD_ADDR)))
         {
            printf("PBAM_Pull_Phone_Book_Size(%s) Success.\r\n", TempParam->Params[0].strParam);
         ...
         {
            /* Error pulling the Phone Book Size, inform the user and   */
            /* flag an error.                                           */
            printf("PBAM_Pull_Phone_Book_Size(%s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PullPhoneBookSize [BD_ADDR]\r\n");
   ...
   /* This function is responsible for sending a SetPhoneBook request   */
   /* to a connected remote device. This function returns zero if       */
   /* successful and a negative error value if it fails.                */
static int SetPhoneBook(ParameterList_t *TempParam)
{
   int          Result;
   int          ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         ...
         /* Make the actual SetPhoneBook Call.                          */
         if(!(Result = PBAM_Set_Phone_Book(BD_ADDR, (TempParam->Params[1].intParam == 0)?pspRoot:((TempParam->Params[1].intParam == 1)?pspDown:pspUp), (TempParam->NumberofParameters > 2 && TempParam->Params[2].strParam)?TempParam->Params[2].strParam:NULL)))
         {
            printf("PBAM_Set_Phone_Book(%s, %d) Success.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].intParam);
         ...
         {
            /* Error setting the PhoneBook, inform the user and flag an */
            /* error.                                                   */
            printf("PBAM_Set_Phone_Book(%s,%d) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].intParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: SetPhoneBook [BD_ADDR] [PathOption (0 - Root, 1 - Down, 2- Up)], [FolderName (Optional)]\r\n");
...
static int SetPhoneBookAbsolute(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Make the actual SetPhoneBookAbsolute Call.                  */
         if(!(Result = PBAM_Set_Phone_Book_Absolute(BD_ADDR, TempParam->Params[1].strParam)))
         {
            printf("PBAM_Set_Phone_Book_Absolute(%s, %s) Success.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam);
         ...
         else
         {
            /* Error setting the PhoneBook, inform the user and flag an */
            /* error.                                                   */
            printf("PBAM_Set_Phone_Book(%s,%s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: SetPhoneBookAbsolute [BD_ADDR] [Path]\r\n");
...
static int PullvCardListing(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && ((TempParam->NumberofParameters >= 6) || ((TempParam->NumberofParameters >= 7) && (TempParam->Params[6].strParam) && (strlen(TempParam->Params[6].strParam)))) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Make the actual PullvCardListing Call.                      */
         //if(!(Result = PBAM_Pull_vCard_Listing(BD_ADDR, TempParam->Params[1].strParam, order, search, NULL, (Word_t)(TempParam->Params[5].intParam), (Word_t)(TempParam->Params[6].intParam))))
         if(!(Result = PBAM_Pull_vCard_Listing(BD_ADDR, TempParam->Params[1].strParam, (TempParam->Params[2].intParam == 0)?ploDefault:(TempParam->Params[2].intParam == 1)?ploIndexed:(TempParam->Params[2].intParam == 2)?ploAlphabetical:ploPhonetical, (TempParam->Params[3].intParam == 0)?psaDefault:(TempParam->Params[3].intParam == 1)?psaName:(TempParam->Params[3].intParam == 2)?psaNumber:psaSound, (TempParam->NumberofParameters >= 7)?TempParam->Params[6].strParam:NULL, (Word_t)(TempParam->Params[4].intParam), (Word_t)(TempParam->Params[5].intParam))))
         {
            printf("PBAM_PullvCardListing(%s, %s, %d, %d, %d, %d, %s) Success.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam, TempParam->Params[2].intParam, TempParam->Params[3].intParam,  TempParam->Params[4].intParam, TempParam->Params[5].intParam, (TempParam->NumberofParameters >= 7)?TempParam->Params[6].strParam:"NULL");
         ...
         {
            /* Error pulling the vcard list, inform the user and flag an*/
            /* error.                                                   */
            printf("PBAM_PullvCardListing(%s, %s, %d, %d, %d, %d, %s) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam, TempParam->Params[2].intParam, TempParam->Params[3].intParam,  TempParam->Params[4].intParam, TempParam->Params[5].intParam, (TempParam->NumberofParameters >= 7)?TempParam->Params[6].strParam:"NULL", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PBAM_PullvCardListing [BD ADDR] [Phonebook Name] [List Option] [Search Attribute] [Max List Count] [List Start Offset] [Search Value (Optional)\r\n");
...
static int PullvCard(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 5) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Make the actual PullvCard Call.                             */
         if(!(Result = PBAM_Pull_vCard(BD_ADDR, TempParam->Params[1].strParam, (DWord_t)(TempParam->Params[2].intParam), (DWord_t)(TempParam->Params[3].intParam), (TempParam->Params[4].intParam == 0)?pmvCard21:(TempParam->Params[4].intParam == 1)?pmvCard30:pmDefault)))
         {
            printf("PBAM_Pull_vCard(%s, %s, %d, %d, %d) Success.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam, TempParam->Params[2].intParam, TempParam->Params[3].intParam, TempParam->Params[4].intParam);
         ...
         {
            /* Error pulling the vCard, inform the user and flag an     */
            /* error.                                                   */
            printf("PBAM_Pull_vCard(%s, %s, %d, %d, %d) Failure: %d, %s.\r\n", TempParam->Params[0].strParam, TempParam->Params[1].strParam, TempParam->Params[2].intParam, TempParam->Params[3].intParam, TempParam->Params[4].intParam, Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PBAM_Pull_vCard [BD_ADDR] [vCard Name] [Filter Low] [Filter High] [Format (0 = v2.1; 1 = v3.0; 2 = Default)]\r\n");
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case petConnectionStatus:
            ...
            break;
         case petVCardData:
            printf("PBAM VCard Event Data.\r\n");

            BD_ADDRToStr(EventData->EventData.VCardEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address:   %s\r\n", Buffer);
            printf("    Status:           %d\r\n", EventData->EventData.VCardEventData.Status);
            printf("    Final:            %s\r\n", (EventData->EventData.VCardEventData.Final)?"TRUE":"FALSE");
            printf("    New Missed Calls: %u\r\n", EventData->EventData.VCardEventData.NewMissedCalls);
            printf("    Buffer Size       %u\r\n", EventData->EventData.VCardEventData.BufferSize);
            printf("    VCard Format:     %s\r\n", (EventData->EventData.VCardEventData.VCardFormat == pmvCard21)?"Card21":(EventData->EventData.VCardEventData.VCardFormat == pmvCard30)?"Card30":"Default");
            break;
         case petPhoneBookSize:
            printf("PBAM Phone Book Size.\r\n");

            BD_ADDRToStr(EventData->EventData.PhoneBookSizeEventData.RemoteDeviceAddress, Buffer);
         ...
         case petPhoneBookSet:
            printf("PBAM Phone Book Set.\r\n");

            BD_ADDRToStr(EventData->EventData.PhoneBookSetEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Status:         %d\r\n", EventData->EventData.PhoneBookSetEventData.Status);
            printf("    CurrentPath:    %s\r\n", EventData->EventData.PhoneBookSetEventData.CurrentPath);
            break;
         case petVCardListing:
            printf("PBAM VCard Listing.\r\n");

            BD_ADDRToStr(EventData->EventData.VCardListingEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address:   %s\r\n", Buffer);
            printf("    Status:           %d\r\n", EventData->EventData.VCardListingEventData.Status);
            printf("    Final:            %d\r\n", EventData->EventData.VCardListingEventData.Final);
            printf("    New Missed Calls: %d\r\n", EventData->EventData.VCardListingEventData.NewMissedCalls);
            printf("    Buffer Size:      %d\r\n", EventData->EventData.VCardListingEventData.BufferSize);
            printf("    Buffer Data:\r\n  %s\r\n", EventData->EventData.VCardListingEventData.Buffer);                
