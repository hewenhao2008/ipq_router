...
#include "LinuxSPPLE.h"    /* Main Application Prototypes and Constants.      */
...
#define LE_DEMO_DEVICE_NAME                        "LinuxSPPLE"

   /* Constants for configuration parameters context indices.           */
#define CONFIG_CONNECTION_PARAMETERS               (1)
#define CONFIG_ADVERTISING_PARAMETERS              (2)
#define CONFIG_CONNECTABILITY_PARAMETERS           (3)
#define CONFIG_SCAN_PARAMETERS                     (4)
...
#define GAPLE_PARAMETERS_DATA_SIZE                       (sizeof(GAPLE_Parameters_t))
   ...
   /* The following structure holds the scan parameters in Raw Data     */
   /* format.                                                           */
typedef struct _tagScan_Parameters_t
{
   GAP_LE_Scan_Type_t     ScanType;
   unsigned int           ScanInterval;
   unsigned int           ScanWindow;
   GAP_LE_Address_Type_t  LocalAddressType;
   GAP_LE_Filter_Policy_t FilterPolicy;
   Boolean_t              FilterDuplicates;
} Scan_Parameters_t;

   /* The following structure holds the configurable parameters for     */
   /* connections, advertising, and scanning.                           */
typedef struct _tagParam_Config_t
{
   Scan_Parameters_t                  ScanParams;
   GAP_LE_Connection_Parameters_t     ConnectParams;
   GAP_LE_Advertising_Parameters_t    AdvertiseParams;
   GAP_LE_Connectability_Parameters_t ConnectabilityParams;
...
#define PARAMETER_CONFIGURATION_DATA_SIZE                       (sizeof(Parameter_Configuration_t))
...
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_DIS             0x20
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_GAPS            0x40
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_SPPLE           0x80
#define DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_IDLE            0xE8
...
static Param_Config_t      ParameterConfiguration;  /* Variable which holds the states */
                                                    /* of all the given advertising,   */
                                                    /* scanning, and connecting options*/
                                                    /* to be used by those commands.   */

static Range_t ConnectParamRange[] =
{
   { MINIMUM_MINIMUM_CONNECTION_INTERVAL, MAXIMUM_MINIMUM_CONNECTION_INTERVAL},
   { MINIMUM_MAXIMUM_CONNECTION_INTERVAL, MAXIMUM_MAXIMUM_CONNECTION_INTERVAL},
   { MINIMUM_SLAVE_LATENCY,               MAXIMUM_SLAVE_LATENCY              },
   { MINIMUM_LINK_SUPERVISION_TIMEOUT,    MAXIMUM_LINK_SUPERVISION_TIMEOUT   },
   { MINIMUM_CONNECTION_EVENT_LENGTH,     MAXIMUM_CONNECTION_EVENT_LENGTH    },
   { MINIMUM_CONNECTION_EVENT_LENGTH,     MAXIMUM_CONNECTION_EVENT_LENGTH    }
};
...
static void DisplayFilterPolicy(GAP_LE_Filter_Policy_t FilterPolicy);
static void DisplayAddressType(GAP_LE_Address_Type_t AddressType);
static void DisplayLEParams(int Context);
static void DisplayLEParamsUsage(int Context);
...
static int GetLEParams(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("GETLEPARAMS", GetLEParams);
...
static void DisplayFilterPolicy(GAP_LE_Filter_Policy_t FilterPolicy)
{
   switch(FilterPolicy)
   {
      ...
      case fpWhiteList:
         printf("Whitelist\n");
         break;
      case fpNoWhiteListDirectedRPA:
         printf("Directed RPA\n");
         break;
      case fpWhiteListDirectedRPA:
         printf("Whitelist Directed RPA\n");
         break;
      default:
         break;
   }
...
static void DisplayAddressType(GAP_LE_Address_Type_t AddressType)
{
   switch(AddressType)
   {
      ...
      case latPublicIdentity:
         printf("Public Identity\n");
         break;
      case latRandomIdentity:
         printf("Random Identity\n");
         break;
      case latResolvableFallbackPublic:
         printf("Resolvable Public\n");
         break;
      case latResolvableFallbackPrivate:
         printf("Resolvable Private\n");
         break;
   }
}
...
static void DisplayLEParams(int Context)
{
   ...
   switch(Context)
   {
      case CONFIG_CONNECTION_PARAMETERS:
         printf("Current Connect Params\n");
         printf("   Connect Interval Min: %d\n", ParameterConfiguration.ConnectParams.Connection_Interval_Min);
         printf("   Connect Interval Max: %d\n", ParameterConfiguration.ConnectParams.Connection_Interval_Max);
         printf("          Slave Latency: %d\n", ParameterConfiguration.ConnectParams.Slave_Latency);
         printf("    Supervision Timeout: %d\n", ParameterConfiguration.ConnectParams.Supervision_Timeout);
         printf("  Connection Length Min: %d\n", ParameterConfiguration.ConnectParams.Minimum_Connection_Length);
         printf("  Connection Length Max: %d\n", ParameterConfiguration.ConnectParams.Maximum_Connection_Length);
         break;
      case CONFIG_ADVERTISING_PARAMETERS:
         printf("Current Advertising Params\n");
         printf("   Advertising Interval Min: %d\n", ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min);
         printf("   Advertising Interval Max: %d\n", ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max);
         printf("    Advertising Channel Map: ");
         switch(ParameterConfiguration.AdvertiseParams.Advertising_Channel_Map)
         {
            case GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_37:
               printf("Channel 37\n");
               break;
            case GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_38:
               printf("Channel 38\n");
               break;
            case GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_39:
               printf("Channel 39\n");
               break;
            case GAP_LE_ADVERTISING_CHANNEL_MAP_USE_ALL_CHANNELS:
               printf("All Channels\n");
               break;
            default:
         ...
         printf("        Scan Request Filter: ");
         DisplayFilterPolicy(ParameterConfiguration.AdvertiseParams.Scan_Request_Filter);
         printf("  Connection Request Filter: ");
         DisplayFilterPolicy(ParameterConfiguration.AdvertiseParams.Connect_Request_Filter);
         break;
      case CONFIG_CONNECTABILITY_PARAMETERS:
         printf("Current Advertising Connectability Params\n");
         printf("   Connectability Mode: %s\n", ParameterConfiguration.ConnectabilityParams.Connectability_Mode?"Connectable":"Non-Connectable");
         printf("      Own Address Type: ");
         DisplayAddressType(ParameterConfiguration.ConnectabilityParams.Own_Address_Type);
         printf("   Direct Address Type: ");
         DisplayAddressType(ParameterConfiguration.ConnectabilityParams.Direct_Address_Type);

         BD_ADDRToStr(ParameterConfiguration.ConnectabilityParams.Direct_Address, BoardStr);
         printf("        Direct Address: %s\n", BoardStr);
         printf("   Enable Scan Response: %s\n", ParameterConfiguration.EnableScanResponse ? "True" : "False");
         break;
      case CONFIG_SCAN_PARAMETERS:
         printf("Current Scan Params\n");
         printf("            Scan Type: %s\n", ParameterConfiguration.ScanParams.ScanType?"Active":"Passive");
         printf("        Scan Interval: %d\n", ParameterConfiguration.ScanParams.ScanInterval);
         printf("          Scan Window: %d\n", ParameterConfiguration.ScanParams.ScanWindow);
         printf("   Local Address Type: ");
         DisplayAddressType(ParameterConfiguration.ScanParams.LocalAddressType);
         printf("        Filter Policy: ");
         DisplayFilterPolicy(ParameterConfiguration.ScanParams.FilterPolicy);
         printf("    Filter Duplicates: %s\n", ParameterConfiguration.ScanParams.FilterDuplicates?"True":"False");
         break;
      default:
         break;
   }
...
static void DisplayLEParamsUsage(int Context)
{
   if(Context == CONFIG_CONNECTION_PARAMETERS)
   {
      printf("Usage: SetLEParams [Context] [Index] [Value].\n");
      printf("   Context 1: Connection Parameters\n");
      printf("         Index 1. Connect Int Min (8ms-4000ms)\n");
      printf("         Index 2. Connect Int Max (8ms-4000ms)\n");
      printf("         Index 3. Slave Latency   (0-500)\n");
      printf("         Index 4. Supervision TO  (100ms-32000ms)\n");
      printf("         Index 5. Connect Len Min (0ms-40959ms)\n");
      printf("         Index 6. Connect Len Max (0ms-40959ms)\n");
   }
   else
   {
      if(Context == CONFIG_ADVERTISING_PARAMETERS)
      {
         printf("Usage: SetLEParams [Context] [Index] [Value].\n");
         printf("   Context 2: Advertising Parameters\n");
         printf("         Index 1. Adv Int Min (20ms-10240ms)\n");
         printf("         Index 2. Adv Int Max (20ms-10240ms)\n");
         printf("         Index 3. Adv Channel Map (Channel)\n");
         printf("             [0 = All Channels]\n");
         printf("             [1 = Channel 37]\n");
         printf("             [2 = Channel 38]\n");
         printf("             [3 = Channel 39]\n");
         printf("         Index 4. Scan Req Filter (Filter).\n");
         printf("             [0 = None]\n");
         printf("             [1 = WhiteList]\n");
         printf("         Index 5. Connect Req Filter (Filter).\n");
         printf("             [0 = None]\n");
         printf("             [1 = WhiteList]\n");
      }
      else
      {
         if(Context == CONFIG_CONNECTABILITY_PARAMETERS)
         {
            printf("Usage: SetLEParams [Context] [Index] [Value].\n");
            printf("   Context 3: Connectability Parameters\n");
            printf("         Index 1. Connectability Mode (Mode).\n");
            printf("             [0 = Non-Connectable]\n");
            printf("             [1 = Connectable]\n");
            printf("             [2 = Direct]\n");
            printf("             [3 = LowDutyCycleDirect]\n");
            printf("         Index 2. Own Addr Type (Type).\n");
            printf("             [0 = Public]\n");
            printf("             [1 = Random]\n");
            printf("             [2 = ResolvablePublic\n");
            printf("             [3 = ResolvablePrivate]\n");
            printf("         Index 3. Direct Address and Type [(BD_ADDR, 0 = Null)] [Type].\n");
            printf("             [0 = Public]\n");
            printf("             [1 = Random]\n");
            printf("             [2 = PublicIdentity]\n");
            printf("             [3 = RandomIdentity]\n");
            printf("         Index 4. Enable Scan Response (0 = False, 1 = True)\n");
         }
         else
         {
            if(Context == CONFIG_SCAN_PARAMETERS)
            {
               printf("Usage: SetLEParams [Context] [Index] [Value].\n");
               printf("   Context 4: Scan Parameters\n");
               printf("         Index 1. Scan Type (Type)\n");
               printf("             [0 = Passive]\n");
               printf("             [1 = Active]\n");
               printf("         Index 2. Scan Interval (3ms-10240ms)\n");
               printf("         Index 3. Scan Window   (3ms-10240ms)\n");
               printf("         Index 4. Local Address Type (Type)\n");
               printf("             [0 = Public]\n");
               printf("             [1 = Random]\n");
               printf("             [2 = ResolvablePublic\n");
               printf("             [3 = ResolvablePrivate]\n");
               printf("         Index 5. Filter Policy (Filter)\n");
               printf("             [0 = None]\n");
               printf("             [1 = WhiteList]\n");
               printf("             [2 = RPA]\n");
               printf("             [3 = RPA Whitelist]\n");
               printf("         Index 6. Filter Duplicates (0 = False, 1 = True)\n");
            }
            else
            {
               printf("Usage: SetLEParams [Context] [Index] [Value].\n");
   ...
   if(!BluetoothStackID)
   {
      ...
      if(HCI_DriverInformation)
      {
         ...
         if(Result > 0)
         {
            ...
            ParameterConfiguration.ConnectParams.Connection_Interval_Min    = 50;
            ParameterConfiguration.ConnectParams.Connection_Interval_Max    = 100;
            ParameterConfiguration.ConnectParams.Slave_Latency              = 30;
            ParameterConfiguration.ConnectParams.Maximum_Connection_Length  = 10000;
            ParameterConfiguration.ConnectParams.Minimum_Connection_Length  = 0;
            ParameterConfiguration.ConnectParams.Supervision_Timeout        = 20000;

            ParameterConfiguration.EnableScanResponse                       = TRUE;

            /* Set the default advertising parameters.               */
            ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min = 100;
            ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max = 200;
            ParameterConfiguration.AdvertiseParams.Advertising_Channel_Map  = GAP_LE_ADVERTISING_CHANNEL_MAP_USE_ALL_CHANNELS;
            ParameterConfiguration.AdvertiseParams.Scan_Request_Filter      = fpNoFilter;
            ParameterConfiguration.AdvertiseParams.Connect_Request_Filter   = fpNoFilter;

            /* Set the default advertising connectability            */
            /* parameters.                                           */
            ParameterConfiguration.ConnectabilityParams.Connectability_Mode = lcmConnectable;
            ParameterConfiguration.ConnectabilityParams.Own_Address_Type    = latPublic;
            ParameterConfiguration.ConnectabilityParams.Direct_Address_Type = latPublic;
            ASSIGN_BD_ADDR(ParameterConfiguration.ConnectabilityParams.Direct_Address, 0, 0, 0, 0, 0, 0);

            ParameterConfiguration.ScanParams.ScanType                      = stActive;
            ParameterConfiguration.ScanParams.ScanInterval                  = 10;
            ParameterConfiguration.ScanParams.ScanWindow                    = 10;
            ParameterConfiguration.ScanParams.LocalAddressType              = latPublic;
            ParameterConfiguration.ScanParams.FilterPolicy                  = fpNoFilter;
            ParameterConfiguration.ScanParams.FilterDuplicates              = TRUE;
         ...
         if((DataLength) && (Data))
         {
            ...
            if(DisplayRawData)
            {
               ...
               if((DataLength) && (Data))
               {
                  ...
                  printf("Data CANNOT be displayed.\r\n");
            }
         }
      }
      else
...
static int StartScan(unsigned int BluetoothStackID)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* scan.                                                          */
      Result = GAP_LE_Perform_Scan(BluetoothStackID, ParameterConfiguration.ScanParams.ScanType, ParameterConfiguration.ScanParams.ScanInterval, ParameterConfiguration.ScanParams.ScanWindow, ParameterConfiguration.ScanParams.LocalAddressType, ParameterConfiguration.ScanParams.FilterPolicy, ParameterConfiguration.ScanParams.FilterDuplicates, GAP_LE_Event_Callback, 0);
   ...
   if((BluetoothStackID) && (!COMPARE_NULL_BD_ADDR(BD_ADDR)))
   {
      if(COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR))
      {
         ...
         if(Result >= 0)
         {
            ...
            /* the connection.                                          */
            Result = GAP_LE_Create_Connection(BluetoothStackID, ParameterConfiguration.ScanParams.ScanInterval, ParameterConfiguration.ScanParams.ScanWindow, Result ? fpNoFilter : fpWhiteList, AddressType, Result ? &BD_ADDR : NULL, latPublic, &ParameterConfiguration.ConnectParams, GAP_LE_Event_Callback, 0);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                          GetLEParams, SetLEParams              *\r\n");
   printf("* Command Options SPPLE:   DiscoverSPPLE, RegisterSPPLE,         *\r\n");
   printf("*                          UnregisterSPPLE, Send,                *\r\n");
   printf("*                          ConfigureSPPLE, Read, Loopback,       *\r\n");
...
static int SetLEParams(ParameterList_t *TempParam)
{
   ...
   int       context = -1;
   BD_ADDR_t BD_ADDR;
   Boolean_t DisplayValues = FALSE;

   /* First, check that valid Bluetooth Stack ID exists.                */
   if(BluetoothStackID)
   {
      /* Next, make sure that a valid device address exists.            */
      if((TempParam) && (TempParam->NumberofParameters >= 3) && ((TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= 4)))
      {
         /* Verify and assign the new value.                            */
         context = TempParam->Params[0].intParam;
         index   = TempParam->Params[1].intParam - 1;

         switch(context)
         {
            case CONFIG_CONNECTION_PARAMETERS:
               if((TempParam->Params[2].intParam >= ConnectParamRange[index].Low) && ((TempParam->Params[2].intParam <= ConnectParamRange[index].High)))
               {
                  switch(index)
                  {
                     case 0:
                        if(ParameterConfiguration.ConnectParams.Connection_Interval_Max >= (Word_t)TempParam->Params[2].intParam)
                           ParameterConfiguration.ConnectParams.Connection_Interval_Min = (Word_t)TempParam->Params[2].intParam;
                        else
                        {
                           printf("Connection Interval Min must be <= Connection Interval Max.\n");
                           printf("Desired Min: %d, Current Max: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ConnectParams.Connection_Interval_Max);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                        break;
                     case 1:
                        if(ParameterConfiguration.ConnectParams.Connection_Interval_Min <= (Word_t)TempParam->Params[2].intParam)
                           ParameterConfiguration.ConnectParams.Connection_Interval_Max = (Word_t)TempParam->Params[2].intParam;
                        else
                        {
                           printf("Connection Interval Max must be >= Connection Interval Min.\n");
                           printf("Desired Max: %d, Current Min: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ConnectParams.Connection_Interval_Min);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                        break;
                     case 2:
                        ParameterConfiguration.ConnectParams.Slave_Latency = (Word_t)TempParam->Params[2].intParam;
                        break;
                     case 3:
                        ParameterConfiguration.ConnectParams.Supervision_Timeout = (Word_t)TempParam->Params[2].intParam;
                        break;
                     case 4:
                        if(ParameterConfiguration.ConnectParams.Maximum_Connection_Length >= (Word_t)TempParam->Params[2].intParam)
                           ParameterConfiguration.ConnectParams.Minimum_Connection_Length = (Word_t)TempParam->Params[2].intParam;
                        else
                        {
                           printf("Maximum Connection Length must be >= Minimum Connection Length.\n");
                           printf("Desired Min: %d, Current Max: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ConnectParams.Maximum_Connection_Length);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                        break;
                     case 5:
                        if(ParameterConfiguration.ConnectParams.Minimum_Connection_Length <= (Word_t)TempParam->Params[2].intParam)
                           ParameterConfiguration.ConnectParams.Maximum_Connection_Length = (Word_t)TempParam->Params[2].intParam;
                        else
                        {
                           printf("Minimum Connection Length must be <= Maximum Connection Length.\n");
                           printf("Desired Max: %d, Current Min: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ConnectParams.Minimum_Connection_Length);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                        break;
                  }
            ...
            case CONFIG_ADVERTISING_PARAMETERS:
               ...
               {
                  case 0:
                     if(TempParam->Params[2].intParam <= ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max)
                     {
                        if((TempParam->Params[2].intParam >= MINIMUM_ADVERTISING_INTERVAL) && (TempParam->Params[2].intParam <= MAXIMUM_ADVERTISING_INTERVAL))
                           ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min = (Word_t)TempParam->Params[2].intParam;
                        else
                           ret_val = INVALID_PARAMETERS_ERROR;
                     }
                     else
                     {
                        printf("Advertising Interval Min must be <= Advertising Interval Max.\n");
                        printf("Desired Min: %d, Current Max: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max);
                        ret_val = INVALID_PARAMETERS_ERROR;
                     }
                     break;
                  case 1:
                     if(TempParam->Params[2].intParam >= ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min)
                     {
                        if((TempParam->Params[2].intParam >= MINIMUM_ADVERTISING_INTERVAL) && (TempParam->Params[2].intParam <= MAXIMUM_ADVERTISING_INTERVAL))
                           ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max = (Word_t)TempParam->Params[2].intParam;
                        else
                           ret_val = INVALID_PARAMETERS_ERROR;
                     }
                     else
                     {
                        printf("Advertising Interval Min must be <= Advertising Interval Max.\n");
                        printf("Desired Max: %d, Current Min: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min);
                        ret_val = INVALID_PARAMETERS_ERROR;
                     }
                     break;
                  case 2:
                     switch(TempParam->Params[2].intParam)
                     {
                        case 0:
                           ...
                           break;
                        case 1:
                           ParameterConfiguration.AdvertiseParams.Advertising_Channel_Map = GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_37;
                           break;
                        case 2:
                           ParameterConfiguration.AdvertiseParams.Advertising_Channel_Map = GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_38;
                           break;
                        case 3:
                           ParameterConfiguration.AdvertiseParams.Advertising_Channel_Map = GAP_LE_ADVERTISING_CHANNEL_MAP_USE_CHANNEL_39;
                           break;
                        default:
                           ret_val = INVALID_PARAMETERS_ERROR;
                           break;
                     ...
                     break;
                  case 3:
                     if(((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam >= fpNoFilter) && ((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam <= fpWhiteListDirectedRPA))
                        ParameterConfiguration.AdvertiseParams.Scan_Request_Filter = (GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 4:
                     if(((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam >= fpNoFilter) && ((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam <= fpWhiteListDirectedRPA))
                        ParameterConfiguration.AdvertiseParams.Connect_Request_Filter = (GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
               }
            ...
            case CONFIG_CONNECTABILITY_PARAMETERS:
               ...
               {
                  case 0:
                     if(((GAP_LE_Connectability_Mode_t)TempParam->Params[2].intParam >= lcmNonConnectable) && ((GAP_LE_Connectability_Mode_t)TempParam->Params[2].intParam <= lcmLowDutyCycleDirectConnectable))
                     {
                        /* If Connectability Mode is set to             */
                        /* Non-Connectable, a check must be done to see */
                        /* if the Advertising Intervals are long enough */
                        /* for non-connectable mode.                    */
                        if((GAP_LE_Connectability_Mode_t)TempParam->Params[2].intParam == lcmNonConnectable)
                        {
                           if(ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min < MINIMUM_NONCONNECTABLE_ADVERTISING_INTERVAL)
                           {
                              printf("Setting Advertising Interval Min to Minimum Non-Connectable Advertising Interval\n");
                              ParameterConfiguration.AdvertiseParams.Advertising_Interval_Min = MINIMUM_NONCONNECTABLE_ADVERTISING_INTERVAL;

                              if(ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max < MINIMUM_NONCONNECTABLE_ADVERTISING_INTERVAL)
                              {
                                 printf("Setting Advertising Interval Max to Minimum Non-Connectable Advertising Interval\n");
                                 ParameterConfiguration.AdvertiseParams.Advertising_Interval_Max = MINIMUM_NONCONNECTABLE_ADVERTISING_INTERVAL;
                              }
                           }
                        }

                        ParameterConfiguration.ConnectabilityParams.Connectability_Mode = (GAP_LE_Connectability_Mode_t)TempParam->Params[2].intParam;
                     }
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 1:
                     if(((GAP_LE_Address_Type_t)TempParam->Params[2].intParam >= latPublic) && ((GAP_LE_Address_Type_t)TempParam->Params[2].intParam <= latResolvableFallbackPrivate))
                        ParameterConfiguration.ConnectabilityParams.Own_Address_Type = (GAP_LE_Address_Type_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 2:
                     if((TempParam->NumberofParameters >= 4) && (TempParam->Params[2].intParam != 0) && (TempParam->Params[2].strParam != NULL))
                     {
                        StrToBD_ADDR(TempParam->Params[2].strParam, &BD_ADDR);
                        ParameterConfiguration.ConnectabilityParams.Direct_Address_Type = TempParam->Params[3].intParam;
                        ParameterConfiguration.ConnectabilityParams.Direct_Address      = BD_ADDR;
                     }
                     else
                     {
                        /* Reset the Direct Address to NULL.            */
                  ...
                  case 3:
                     if((TempParam->Params[2].intParam >= 0) && (TempParam->Params[2].intParam <= 1))
                        ParameterConfiguration.EnableScanResponse = (Boolean_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  default:
            ...
            case CONFIG_SCAN_PARAMETERS:
               ...
               {
                  case 0:
                     if(((GAP_LE_Scan_Type_t)TempParam->Params[2].intParam >= stPassive) && ((GAP_LE_Scan_Type_t)TempParam->Params[2].intParam <= stActive))
                        ParameterConfiguration.ScanParams.ScanType = (GAP_LE_Address_Type_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 1:
                     if((TempParam->Params[2].intParam >= MINIMUM_LE_SCAN_INTERVAL) && (TempParam->Params[2].intParam <= MAXIMUM_LE_SCAN_INTERVAL))
                     {
                        if((unsigned int)TempParam->Params[2].intParam >= ParameterConfiguration.ScanParams.ScanWindow)
                           ParameterConfiguration.ScanParams.ScanInterval = TempParam->Params[2].intParam;
                        else
                        {
                           printf("Scan Window must be <= Scan Interval.\n");
                           printf("Desired Scan Interval: %d, Current Scan Window: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ScanParams.ScanWindow);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                     }
                     else
                     ...
                     break;
                  case 2:
                     if((TempParam->Params[2].intParam >= MINIMUM_LE_SCAN_WINDOW) && (TempParam->Params[2].intParam <= MAXIMUM_LE_SCAN_WINDOW))
                     {
                        if((unsigned int)TempParam->Params[2].intParam <= ParameterConfiguration.ScanParams.ScanInterval)
                           ParameterConfiguration.ScanParams.ScanWindow = TempParam->Params[2].intParam;
                        else
                        {
                           printf("Scan Window must be <= Scan Interval.\n");
                           printf("Desired Scan Window: %d, Current Scan Interval: %d\n", (Word_t)TempParam->Params[2].intParam, ParameterConfiguration.ScanParams.ScanInterval);
                           ret_val = INVALID_PARAMETERS_ERROR;
                        }
                     }
                     else
                  ...
                  case 3:
                     if(((GAP_LE_Address_Type_t)TempParam->Params[2].intParam >= latPublic) && ((GAP_LE_Address_Type_t)TempParam->Params[2].intParam <= latResolvableFallbackPrivate))
                        ParameterConfiguration.ScanParams.LocalAddressType = (GAP_LE_Address_Type_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 4:
                     if(((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam >= fpNoFilter) && ((GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam <= fpWhiteListDirectedRPA))
                        ParameterConfiguration.ScanParams.FilterPolicy = (GAP_LE_Filter_Policy_t)TempParam->Params[2].intParam;
                     else
                        ret_val = INVALID_PARAMETERS_ERROR;
                     break;
                  case 5:
                     ParameterConfiguration.ScanParams.FilterDuplicates = (Boolean_t)TempParam->Params[2].intParam;
                     break;
                  default:
                     ret_val = INVALID_PARAMETERS_ERROR;
                     break;
...
static int GetLEParams(ParameterList_t *TempParam)
{
   ...
   if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 1) && (TempParam->Params[0].intParam <= 4))
   {
      context = TempParam->Params[0].intParam;
      DisplayLEParams(context);
   }
   ...
   {
      printf("Usage: GetLEParams [Context].\n");
      printf("   Context 1: Connection Parameters\n");
      printf("   Context 2: Advertising Parameters\n");
      printf("   Context 3: Connectability Parameters\n");
      printf("   Context 4: Scan Parameters\n");
         ...
         else
         {
            ...
            if(!ret_val)
            {
               ...
               if(!ret_val)
               {
                  ...
                  ret_val = GAP_LE_Advertising_Enable(BluetoothStackID, ParameterConfiguration.EnableScanResponse, &ParameterConfiguration.AdvertiseParams, &ParameterConfiguration.ConnectabilityParams, GAP_LE_Event_Callback, 0);
                  if(!ret_val)
                  {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam >= dtLogFile) && (TempParam->Params[0].intParam <= dtFTS))
      {
         ...
         else
         {
            ...
            if(!DebugID)
            {
               ...
               switch((BTPS_Debug_Type_t)TempParam->Params[1].intParam)
               {
                  case dtLogFile:
               ...
               /* Make sure the log file name is valid.                 */
               if(LogFileName)
               {
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(!Handle)
               {
                  ShowPrompt                  = TRUE;
                  DeviceInfo->DISHandleIndex  = 0;
                  DeviceInfo->Flags          &= (Byte_t)(~DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_IDLE);
                  printf("\nDIS Search Complete\n");
               }
               else
               {
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
         ...
         case etGATT_Connection_Device_Disconnection:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Disconnection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Disconnection_Data->RemoteDevice)) != NULL)
               {
