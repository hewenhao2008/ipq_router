...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXPASM_SRV - Simple Linux application using Bluetopia Platform Manager */
/*                  Phone Alert Status (PAS - Server Role Only) Manager       */
...
/*   --------  -----------    ------------------------------------------------*/
/*   11/28/12  D. Lange       Initial creation. (Based on LinuxPXPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxPASM_SRV.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTPASM.h"     /* PAS Manager Application Programming Interface.  */
...
#ifndef PASS_ASSIGN_PASS_SERVICE_UUID_16
...
static unsigned int        PASMEventCallbackID;     /* Variable which holds the        */
                                                    /* current PASM Event Callback ID  */
                                                    /* that is assigned from the PASM  */
                                                    /* Manager when the local client   */
                                                    /* registers for PASM Events.      */
...
static int RegisterServerEvents(ParameterList_t *TempParam);
static int UnRegisterServerEvents(ParameterList_t *TempParam);
static int SetAlertStatus(ParameterList_t *TempParam);
static int QueryAlertStatus(ParameterList_t *TempParam);
static int SetRingerSetting(ParameterList_t *TempParam);
static int QueryRingerSetting(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM PAS Manager Callback function prototype.                     */
static void BTPSAPI PASM_Event_Callback(PASM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERSERVEREVENTS", RegisterServerEvents);
   AddCommand("UNREGISTERSERVEREVENTS", UnRegisterServerEvents);
   AddCommand("SETALERTSTATUS", SetAlertStatus);
   AddCommand("QUERYALERTSTATUS", QueryAlertStatus);
   AddCommand("SETRINGERSETTING", SetRingerSetting);
   AddCommand("QUERYRINGERSETTING", QueryRingerSetting);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      PASS_ASSIGN_PASS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)RegisterServerEvents                       *\r\n");
   printf("*                  36)UnRegisterServerEvents                     *\r\n");
   printf("*                  37)SetAlertStatus                             *\r\n");
   printf("*                  38)QueryAlertStatus                           *\r\n");
   printf("*                  39)SetRingerSetting                           *\r\n");
   printf("*                  40)QueryRingerSetting                         *\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the PAS*/
   /* Manager to receive PAS Server Events.  This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int RegisterServerEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!PASMEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = PASM_Register_Server_Event_Callback(PASM_Event_Callback, NULL)) > 0)
         {
            printf("PASM_Register_Server_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            PASMEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("PASM_Register_Server_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("PAS Manager Event Callback already registered.\r\n");
   ...
   /* previously registered PAS Event Registration with the PAS Manager.*/
...
static int UnRegisterServerEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PASMEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         PASM_Un_Register_Server_Event_Callback(PASMEventCallbackID);

         printf("PASM_Un_Register_Server_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         PASMEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("PAS Manager Event Callback is not registered.\r\n");
   ...
   /* The following function is responsible for issuing a Set Alert     */
   /* Status over a registered PAS Event Registration with the PAS      */
...
static int SetAlertStatus(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PASMEventCallbackID)
      {
         /* Next, go ahead and verify that we have all the required     */
         /* parameters.                                                 */
         if((TempParam) && (TempParam->NumberofParameters >= 3))
         {
            /* Parameters appear to be active, let's attempt to set the */
            /* Alert Status.                                            */
            AlertStatus.RingerStateActive  = (Boolean_t)(TempParam->Params[0].intParam?TRUE:FALSE);
            AlertStatus.VibrateStateActive = (Boolean_t)(TempParam->Params[1].intParam?TRUE:FALSE);
            AlertStatus.DisplayStateActive = (Boolean_t)(TempParam->Params[2].intParam?TRUE:FALSE);

            /* Alert Status built, go ahead and submit it to the PAS    */
            /* Manager.                                                 */
            Result = PASM_Set_Alert_Status(PASMEventCallbackID, &AlertStatus);

            if(!Result)
            {
               printf("PASM_Set_Alert_Status(%s %s %s) Success: %d.\r\n", AlertStatus.RingerStateActive?"TRUE":"FALSE", AlertStatus.VibrateStateActive?"TRUE":"FALSE", AlertStatus.DisplayStateActive?"TRUE":"FALSE", Result);
            ...
            {
               printf("PASM_Set_Alert_Status(%s %s %s) Failure: %d, %s.\r\n", AlertStatus.RingerStateActive?"TRUE":"FALSE", AlertStatus.VibrateStateActive?"TRUE":"FALSE", AlertStatus.DisplayStateActive?"TRUE":"FALSE", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            printf("Usage: SetAlertStatus [0/1 - Ringer State] [0/1 - Vibrate State] [0/1 - Display State].\r\n");
   ...
   /* The following function is responsible for issuing a Query Alert   */
   /* Status over a registered PAS Event Registration with the PAS      */
   /* Manager.  This function returns zero if successful and a negative */
   /* value if an error occurred.                                       */
static int QueryAlertStatus(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PASMEventCallbackID)
      {
         /* Attempt to query the Alert Status.                          */
         Result = PASM_Query_Alert_Status(PASMEventCallbackID, &AlertStatus);

         if(!Result)
         {
            printf("PASM_Query_Alert_Status() Success: %d.\r\n", Result);
            printf("   Ringer State  : %s:\r\n", AlertStatus.RingerStateActive?"TRUE":"FALSE");
            printf("   Vibrate State : %s:\r\n", AlertStatus.VibrateStateActive?"TRUE":"FALSE");
            printf("   Display State : %s:\r\n", AlertStatus.DisplayStateActive?"TRUE":"FALSE");
         ...
         {
            printf("PASM_Query_Alert_Status() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
   ...
   /* The following function is responsible for issuing a Set Ringer    */
   /* Setting over a registered PAS Event Registration with the PAS     */
   /* Manager.  This function returns zero if successful and a negative */
   /* value if an error occurred.                                       */
static int SetRingerSetting(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PASMEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            /* Parameters appear to be active, let's attempt to set the */
            /* Ringer Setting.                                          */
            RingerSetting = (PASM_Ringer_Setting_t)(TempParam->Params[0].intParam?prsNormal:prsSilent);

            /* Ringer Setting built, go ahead and submit it to the PAS  */
            /* Manager.                                                 */
            Result = PASM_Set_Ringer_Setting(PASMEventCallbackID, RingerSetting);

            if(!Result)
            {
               printf("PASM_Set_Ringer_Setting(%s) Success: %d.\r\n", (RingerSetting == prsSilent)?"Silent":"Normal", Result);
            ...
            {
               printf("PASM_Set_Ringer_Setting(%s) Failure: %d, %s.\r\n", (RingerSetting == prsSilent)?"Silent":"Normal", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            printf("Usage: SetRingerSetting [0 - Silent, 1 - Normal].\r\n");
   ...
   /* The following function is responsible for issuing a Query Ringer  */
   /* Setting over a registered PAS Event Registration with the PAS     */
   /* Manager.  This function returns zero if successful and a negative */
   /* value if an error occurred.                                       */
static int QueryRingerSetting(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(PASMEventCallbackID)
      {
         /* Attempt to query the Ringer Setting.                        */
         Result = PASM_Query_Ringer_Setting(PASMEventCallbackID, &RingerSetting);

         if(!Result)
         {
            printf("PASM_Query_Ringer_Setting() Success: %d.\r\n", Result);
            printf("   Ringer Setting  : %s:\r\n", (RingerSetting == prsSilent)?"Silent":"Normal");
         ...
         {
            printf("PASM_Query_Ringer_Setting() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* The following function is the Phone Alert Status Manager Event    */
   /* Callback function that is Registered with the Phone Alert Status  */
   /* Manager.  This callback is responsible for processing all Phone   */
   /* Alert Status Manager Events.                                      */
static void BTPSAPI PASM_Event_Callback(PASM_Event_Data_t *EventData, void *CallbackParameter)
{
   char BoardStr[16];
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case etPASConnected:
            ...
            printf("PASM Connected:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type  : %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == pctPASServer)?"Server":"Client");
            break;
         case etPASDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);

            printf("PASM Disconnected:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Connection Type  : %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == pctPASServer)?"Server":"Client");
            break;
         case etPASRingerControlPointCommand:
            BD_ADDRToStr(EventData->EventData.RingerControlPointCommandEventData.RemoteDeviceAddress, BoardStr);

            printf("PASM Ringer Control Point Command:\r\n");
            printf("   Bluetooth Address: %s\r\n", BoardStr);
            printf("   Ringer Command   : %s\r\n", (EventData->EventData.RingerControlPointCommandEventData.RingerControlCommand == prcSilent)?"Silent":((EventData->EventData.RingerControlPointCommandEventData.RingerControlCommand == prcMuteOnce)?"Mute Once":"Cancel Silent"));
            break;
         default:
            printf("Unknown PAS Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nPASM Event Data is NULL.\r\n");
