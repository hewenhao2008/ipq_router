...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXANPM - Simple Linux application using Bluetopia Platform Manager     */
/*              Alert Notification Profile (ANP) Manager Application          */
...
/*   --------  -----------    ------------------------------------------------*/
/*   06/15/15  M. Seabold     Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxANPM_CLT.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTANPM.h"     /* ANP Manager Application Programming Interface.  */
...
#ifndef ANS_ASSIGN_ANS_SERVICE_UUID_16

   #define ANS_ASSIGN_ANS_SERVICE_UUID_16(_x)      ASSIGN_BLUETOOTH_UUID_16(*((UUID_16_t *)(_x)), 0x18, 0x11)

#endif

   /* The following MACRO is a utility function is used to determine if */
   /* a Category ID value is valid.                                     */
#define ANPM_IS_VALID_CATEGORY_IDENTIFICATION(_x)  (((_x) >= cmSimpleAlert) && ((_x) <= cmInstantMessage))
...
static unsigned int        ANPEventCallbackID;      /* Variable which holds the        */
                                                    /* current ANP Event Callback ID   */
                                                    /* that is assigned from the ANP   */
                                                    /* Manager when the local client   */
                                                    /* registers for ANP Events.       */
...
static char *AlertCategoryToString(ANPM_Category_Identification_t AlertCategory);
...
static int ANPMRegisterEventCallback(ParameterList_t *TempParam);
static int ANPMUnRegisterEventCallback(ParameterList_t *TempParam);
static int ANPMGetSupportedNewAlertCategories(ParameterList_t *TempParam);
static int ANPMGetSupportedUnreadStatusCategories(ParameterList_t *TempParam);
static int ANPMEnableNewAlertNotifications(ParameterList_t *TempParam);
static int ANPMEnableUnreadStatusNotifications(ParameterList_t *TempParam);
static int ANPMEnableNewAlertCategory(ParameterList_t *TempParam);
static int ANPMEnableUnreadStatusCategory(ParameterList_t *TempParam);
static int ANPMRequestNewAlertNotification(ParameterList_t *TempParam);
static int ANPMRequestUnreadStatusNotification(ParameterList_t *TempParam);
...
static void DisplayCategoryUsage(void);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM ANP Manager Callback function prototype.                     */
static void BTPSAPI ANPM_Event_Callback(ANPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("ANPMREGISTEREVENTCALLBACK", ANPMRegisterEventCallback);
   AddCommand("ANPMUNREGISTEREVENTCALLBACK", ANPMUnRegisterEventCallback);
   AddCommand("ANPMGETSUPPORTEDNEWALERTCATEGORIES", ANPMGetSupportedNewAlertCategories);
   AddCommand("ANPMGETSUPPORTEDUNREADSTATUSCATEGORIES", ANPMGetSupportedUnreadStatusCategories);
   AddCommand("ANPMENABLENEWALERTNOTIFICATIONS", ANPMEnableNewAlertNotifications);
   AddCommand("ANPMENABLEUNREADSTATUSNOTIFICATIONS", ANPMEnableUnreadStatusNotifications);
   AddCommand("ANPMENABLENEWALERTCATEGORY", ANPMEnableNewAlertCategory);
   AddCommand("ANPMENABLEUNREADSTATUSCATEGORY", ANPMEnableUnreadStatusCategory);
   AddCommand("ANPMREQUESTNEWALERTNOTIFICATION", ANPMRequestNewAlertNotification);
   AddCommand("ANPMREQUESTUNREADSTATUSNOTIFICATION", ANPMRequestUnreadStatusNotification);
   ...
   /* The following function takes a parameter list, a start offset in  */
   /* the parameter list, and a pointer to a char pointer that will     */
   /* return an allocated buffer on success.  The function searches for */
   /* quote characters, and it concatenates the param strings without   */
   /* the quotes into the ReturnString.  The function returns a negative*/
   /* error code on failure and the offset of the next unused parameter */
   /* on success.                                                       */
static int FormatMultiStringParam(ParameterList_t *TempParam, unsigned int StringParameterOffset, char **ReturnString)
{
   ...
   /* Initialize the return string to NULL so that the caller doesn't   */
   /* try to free on failure.                                           */
   *ReturnString = NULL;

   /* Verify that the given offset is valid.                            */
   if((TempParam->NumberofParameters - StringParameterOffset) > 0)
   {
      /* Verify that the first character is the opening quote.          */
      if((BTPS_StringLength(TempParam->Params[StringParameterOffset].strParam) > 0) && (TempParam->Params[StringParameterOffset].strParam[0] == '\"'))
      {
         /* Calculate the buffer size needed to concatenate all strings.*/
         /* Exit when the closing quote is found or an error occurs.    */
         for(BufferSize = 0, i = StringParameterOffset; i < TempParam->NumberofParameters; i++)
         {
            /* Calculate the string length.                             */
            if((j = BTPS_StringLength(TempParam->Params[i].strParam)))
            {
               /* Check for the exit condition.                         */
               if(TempParam->Params[i].strParam[j-1] == '\"')
               {
                  /* Subtract two characters for the beginning and      */
                  /* ending quote.                                      */
                  BufferSize += (j - 2);
                  break;
               }
               ...
               {
                  /* Add an extra character for the space in-between    */
                  /* this string and the next string.                   */
                  BufferSize += j + 1;
               }
            }
            else
            {
               /* The string length is invalid.  Just add a space       */
               /* character.                                            */
               BufferSize += 1;
            }
         }

         if(i != TempParam->NumberofParameters)
         {
            /* Allocate a buffer with two extra spaces (one for the null*/
            /* terminator, and another for the trailing quote that is   */
            /* temporarily copied over.                                 */
            if((*ReturnString = BTPS_AllocateMemory((BufferSize + 2) * sizeof(char))))
            {
               /* Copy the first param string without the beginning     */
               /* quote.                                                */
               BTPS_StringCopy(*ReturnString, &(TempParam->Params[StringParameterOffset].strParam[1]));

               /* If there was only one parameter to evaluate, then     */
               /* don't continue.                                       */
               if(i != StringParameterOffset)
               {
                  /* Copy over the rest of the parameters with a space  */
                  /* in-between.                                        */
                  for(j = StringParameterOffset + 1; j <= i; j++)
                  {
                     BTPS_StringCopy(&((*ReturnString)[BTPS_StringLength(*ReturnString)]), " ");
                     if(BTPS_StringLength(TempParam->Params[j].strParam))
                        BTPS_StringCopy(&((*ReturnString)[BTPS_StringLength(*ReturnString)]), TempParam->Params[j].strParam);
                  }
               }

               /* Null-terminate the last quote character and set the   */
               /* return value to the next unused parameter.            */
               (*ReturnString)[BufferSize] = '\0';
               ret_val                     = i + 1;
            }
            else
...
static void StrToBD_ADDR(char *BoardStr, BD_ADDR_t *Board_Address)
{
   ...
   /* convert an ANPM Category to a string.                             */
static char *AlertCategoryToString(ANPM_Category_Identification_t AlertCategory)
{
   char *ret_val;
   ...
   switch(AlertCategory)
   {
      ...
      case cmHighPriorityAlert:
         ret_val = "High Priority";
         break;
      case cmInstantMessage:
         ret_val = "Instant Message";
         break;
      case cmAllSupportedCategories:
         ret_val = "All Supported Categories";
         break;
      default:
         ret_val = "Unknown Category";
         break;
   }
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      ANS_ASSIGN_ANS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  37)StartObservationScan                       *\r\n");
   printf("*                  38)StopObservationScan                        *\r\n");
   printf("*                  39)ANPMRegisterEventCallback                  *\r\n");
   printf("*                  40)ANPMUnRegisterEventCallback                *\r\n");
   printf("*                  41)ANPMGetSupportedNewAlertCategories         *\r\n");
   printf("*                  42)ANPMGetSupportedUnreadStatusCategories     *\r\n");
   printf("*                  43)ANPMEnableNewAlertNotifications            *\r\n");
   printf("*                  44)ANPMEnableUnreadStatusNotifications        *\r\n");
   printf("*                  45)ANPMEnableNewAlertCategory                 *\r\n");
   printf("*                  46)ANPMEnableUnreadStatusCategory             *\r\n");
   printf("*                  47)ANPMRequestNewAlertNotification            *\r\n");
   printf("*                  48)ANPMRequestUnreadStatusNotification        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ANPMRegisterEventCallback(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(!ANPEventCallbackID)
      {
         /* Attempt to register for events.                             */
         if((ret_val = ANPM_Register_Client_Event_Callback(ANPM_Event_Callback, NULL)) > 0)
         {
            printf("ANPM_Register_Server_Event_Callback() Success: %d.\r\n", ret_val);

            /* Note the Callback ID and flag success.                   */
            ANPEventCallbackID = (unsigned int)ret_val;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("ANPM_Register_Server_Event_Callback() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         printf("ANPM Callback Already Registered.\r\n");
...
static int ANPMUnRegisterEventCallback(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         ANPM_Un_Register_Server_Event_Callback(ANPEventCallbackID);

         printf("ANPM_Un_Register_Server_Event_Callback() Success.\r\n");
      ...
      else
      {
         ...
         printf("ANP Manager Event Callback is not registered.\r\n");
...
static int ANPMGetSupportedNewAlertCategories(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure the user has registered a callback. */
      if(ANPEventCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Make the API call.                                       */
            if((Result = ANPM_Get_Supported_New_Alert_Categories(ANPEventCallbackID, BD_ADDR)) > 0)
            {
               printf("ANPM_Get_Supported_New_Alert_Categories() Success. Transaction ID: %d.\r\n", Result);
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_Get_Supported_New_Alert_Categories() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMGetSupportedNewAlertCategories  [BD_ADDR].\r\n");
      ...
      {
         printf("Register an ANPM callback using ANPMRegisterEventCallabck.\r\n");
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Make the API call.                                       */
            if((Result = ANPM_Get_Supported_Unread_Status_Categories(ANPEventCallbackID, BD_ADDR)) > 0)
            {
               printf("ANPM_Get_Supported_Unread_Status_Categories() Success. Transaction ID: %d.\r\n", Result);
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_Get_Supported_Unread_Status_Categories() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMGetSupportedUnreadStatusCategories [BD_ADDR].\r\n");
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            ...
            if(TempParam->Params[1].intParam)
               Result = ANPM_Enable_New_Alert_Notifications(ANPEventCallbackID, BD_ADDR);
            else
               Result = ANPM_Disable_New_Alert_Notifications(ANPEventCallbackID, BD_ADDR);

            /* Make the API call.                                       */
            if(Result >= 0)
            {
               printf("ANPM_%s_New_Alert_Notifications() Success.\r\n", TempParam->Params[1].intParam?"Enable":"Disable");
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_%s_New_Alert_Notifications() Failure: %d, %s.\r\n", TempParam->Params[1].intParam?"Enable":"Disable", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMEnableNewAlertNotifications [BD_ADDR] [Enable (1 - Enable, 0 - Disable)].\r\n");
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            ...
            /* Check if we are enabling or disabling.                   */
            if(TempParam->Params[1].intParam)
               Result = ANPM_Enable_Unread_Status_Notifications(ANPEventCallbackID, BD_ADDR);
            else
               Result = ANPM_Disable_Unread_Status_Notifications(ANPEventCallbackID, BD_ADDR);

            /* Make the API call.                                       */
            if(Result >= 0)
            {
               printf("ANPM_%s_Unread_Status_Notifications() Success.\r\n", TempParam->Params[1].intParam?"Enable":"Disable");
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_%s_Unread_Status_Notifications() Failure: %d, %s.\r\n", TempParam->Params[1].intParam?"Enable":"Disable", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMEnableUnreadStatusNotifications [BD_ADDR] [Enable (1 - Enable, 0 - Disable)].\r\n");
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].strParam))
         {
            ...
            /* Check if we are enabling or disabling.                   */
            if(TempParam->Params[2].intParam)
               Result = ANPM_Enable_New_Alert_Category(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam);
            else
               Result = ANPM_Disable_New_Alert_Category(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam);

            /* Make the API call.                                       */
            if(Result >= 0)
            {
               printf("ANPM_%s_New_Alert_Category() Success.\r\n", TempParam->Params[2].intParam?"Enable":"Disable");
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_%s_New_Alert_Category() Failure: %d, %s.\r\n", TempParam->Params[2].intParam?"Enable":"Disable", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMEnableNewAlertCategory [BD_ADDR] [Category] [Enable (1 - Enable, 0 - Disable)].\r\n");
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].strParam))
         {
            ...
            /* Check if we are enabling or disabling.                   */
            if(TempParam->Params[2].intParam)
               Result = ANPM_Enable_Unread_Status_Category(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam);
            else
               Result = ANPM_Disable_Unread_Status_Category(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam);

            /* Make the API call.                                       */
            if(Result >= 0)
            {
               printf("ANPM_%s_Unread_Status_Category() Success.\r\n", TempParam->Params[2].intParam?"Enable":"Disable");
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_%s_Unread_Status_Category() Failure: %d, %s.\r\n", TempParam->Params[2].intParam?"Enable":"Disable", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMEnableUnreadStatusCategory [BD_ADDR] [Category] [Enable (1 - Enable, 0 - Disable)].\r\n");
            DisplayCategoryUsage();
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            ...
            /* Make the API call.                                       */
            if((Result = ANPM_Request_New_Alert_Notification(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam)) > 0)
            {
               printf("ANPM_Request_New_Alert_Notification() Success. Transaction ID: %d.\r\n", Result);
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_Request_New_Alert_Notification() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMRequestNewAlertNotification [BD_ADDR] [Category].\r\n");
            DisplayCategoryUsage();
   ...
   if(Initialized)
   {
      ...
      if(ANPEventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            ...
            /* Make the API call.                                       */
            if((Result = ANPM_Request_Unread_Status_Notification(ANPEventCallbackID, BD_ADDR, TempParam->Params[1].intParam)) > 0)
            {
               printf("ANPM_Request_Unread_Status_Notification() Success. Transaction ID: %d.\r\n", Result);
            ...
            {
               /* Error occurred, so inform the user.                   */
               printf("ANPM_Request_Unread_Status_Notification() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ANPMRequestUnreadStatusNotification [BD_ADDR] [Category].\r\n");
            DisplayCategoryUsage();
...
static void DisplayRemoteDeviceProperties(unsigned long UpdateMask, DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   /* The following function is responsible for displaying the available*/
   /* categories for usage of an ANP command.                         */
static void DisplayCategoryUsage()
{
   unsigned int i;

   printf("   Where Category is one of the corresponding numerical values:\r\n");
...
static void BTPSAPI ANPM_Event_Callback(ANPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case aetANPConnected:
            BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Connected: %s, %s\r\n", Buffer, (EventData->EventData.ConnectedEventData.ConnectionType == actServer)?"Server Connection":"Client Connection");
            break;
         case aetANPDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Disconnected: %s, %s\r\n", Buffer, (EventData->EventData.DisconnectedEventData.ConnectionType == actServer)?"Server Connection":"Client Connection");
            break;
         case aetANPSupportedNewAlertCategoriesResult:
            BD_ADDRToStr(EventData->EventData.SupportedNewAlertCategoriesEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Supported New Alert Categories Result.\r\n");
            printf("\tRemoteDeviceAddress:  %s.\r\n", Buffer);
            printf("\tTransactionID:        %u.\r\n", EventData->EventData.SupportedNewAlertCategoriesEventData.TransactionID);
            printf("\tStatus:               %u.\r\n", EventData->EventData.SupportedNewAlertCategoriesEventData.Status);

            if(EventData->EventData.SupportedNewAlertCategoriesEventData.Status == ANPM_OPERATION_STATUS_ATT_PROTOCOL_ERROR)
               printf("\tProtocol Error:       %u.\r\n", EventData->EventData.SupportedNewAlertCategoriesEventData.AttProtocolErrorCode);

            if(EventData->EventData.SupportedNewAlertCategoriesEventData.Status == ANPM_OPERATION_STATUS_SUCCESS)
               printf("\tSupported Categories: %08lX.\r\n", EventData->EventData.SupportedNewAlertCategoriesEventData.SupportedCategories);

            break;
         case aetANPSupportedUnreadCategoriesResult:
            BD_ADDRToStr(EventData->EventData.SupportedUnreadCategoriesEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Supported Unread Status Categories Result.\r\n");
            printf("\tRemoteDeviceAddress:  %s.\r\n", Buffer);
            printf("\tTransactionID:        %u.\r\n", EventData->EventData.SupportedUnreadCategoriesEventData.TransactionID);
            printf("\tStatus:               %u.\r\n", EventData->EventData.SupportedUnreadCategoriesEventData.Status);

            if(EventData->EventData.SupportedUnreadCategoriesEventData.Status == ANPM_OPERATION_STATUS_ATT_PROTOCOL_ERROR)
               printf("\tProtocol Error:       %u.\r\n", EventData->EventData.SupportedUnreadCategoriesEventData.AttProtocolErrorCode);

            if(EventData->EventData.SupportedUnreadCategoriesEventData.Status == ANPM_OPERATION_STATUS_SUCCESS)
               printf("\tSupported Categories: %08lX.\r\n", EventData->EventData.SupportedUnreadCategoriesEventData.SupportedCategories);

            break;
         case aetANPNewAlertNotification:
            BD_ADDRToStr(EventData->EventData.NewAlertNotificationEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM New Alert Notification.\r\n");
            printf("\tRemoteDeviceAddress: %s.\r\n", Buffer);
            printf("\tCategory:            %s.\r\n", AlertCategoryToString(EventData->EventData.NewAlertNotificationEventData.CategoryID));
            printf("\tNumber Alerts        %u.\r\n", EventData->EventData.NewAlertNotificationEventData.NumberNewAlerts);
            
            if(EventData->EventData.NewAlertNotificationEventData.LastAlertText)
               printf("\tLast Alert Text:     %s.\r\n", EventData->EventData.NewAlertNotificationEventData.LastAlertText);

            break;
         case aetANPUnreadStatusNotification:
            BD_ADDRToStr(EventData->EventData.UnreadStatusNotificationEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Unread Status Notification.\r\n");
            printf("\tRemoteDeviceAddress: %s.\r\n", Buffer);
            printf("\tCategory:            %s.\r\n", AlertCategoryToString(EventData->EventData.UnreadStatusNotificationEventData.CategoryID));
            printf("\tNumber Alerts        %u.\r\n", EventData->EventData.UnreadStatusNotificationEventData.NumberUnreadAlerts);

            break;
         case aetANPCommandResult:
            BD_ADDRToStr(EventData->EventData.CommandResultEventData.RemoteDeviceAddress, Buffer);

            printf("ANPM Command Result.\r\n");
            printf("\tRemoteDeviceAddress:  %s.\r\n", Buffer);
            printf("\tTransaction ID:       %u.\r\n", EventData->EventData.CommandResultEventData.TransactionID);
            printf("\tStatus:               %u.\r\n", EventData->EventData.CommandResultEventData.Status);

            if(EventData->EventData.CommandResultEventData.Status == ANPM_OPERATION_STATUS_ATT_PROTOCOL_ERROR)
               printf("\tProtocol Error:       %u.\r\n", EventData->EventData.CommandResultEventData.AttProtocolErrorCode);

            break;
         default:
            printf("ANPM: Unknown ANP Manager Event Received: 0x%08X, Length: 0x%08X.\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nANPM Event Data is NULL.\r\n");
