...
#include "LinuxDEVM.h"     /* Main Application Prototypes and Constants.      */
...
static int QueryRemoteDeviceEIRData(ParameterList_t *TempParam);
...
static int ScheduleInterleavedAdvertisement(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("QUERYREMOTEDEVICEEIRDATA", QueryRemoteDeviceEIRData);
   ...
   AddCommand("SCHEDULEINTERLEAVEDADVERTISEMENT", ScheduleInterleavedAdvertisement);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  21)QueryRemoteDeviceEIRData                   *\r\n");
   printf("*                  22)AddRemoteDevice                            *\r\n");
   printf("*                  23)DeleteRemoteDevice                         *\r\n");
   printf("*                  24)UpdateRemoteDeviceAppData                  *\r\n");
   printf("*                  25)DeleteRemoteDevices                        *\r\n");
   printf("*                  26)PairWithRemoteDevice                       *\r\n");
   printf("*                  27)CancelPairWithRemoteDevice                 *\r\n");
   printf("*                  28)UnPairRemoteDevice                         *\r\n");
   printf("*                  29)QueryRemoteDeviceServices                  *\r\n");
   printf("*                  30)QueryRemoteDeviceServiceSupported          *\r\n");
   printf("*                  31)QueryRemoteDevicesForService               *\r\n");
   printf("*                  32)QueryRemoteDeviceServiceClasses            *\r\n");
   printf("*                  33)AuthenticateRemoteDevice                   *\r\n");
   printf("*                  34)EncryptRemoteDevice                        *\r\n");
   printf("*                  35)ConnectWithRemoteDevice                    *\r\n");
   printf("*                  36)DisconnectRemoteDevice                     *\r\n");
   printf("*                  37)SetRemoteDeviceLinkActive                  *\r\n");
   printf("*                  38)CreateSDPRecord                            *\r\n");
   printf("*                  39)DeleteSDPRecord                            *\r\n");
   printf("*                  40)AddSDPAttribute                            *\r\n");
   printf("*                  41)DeleteSDPAttribute                         *\r\n");
   printf("*                  42)EnableBluetoothDebug                       *\r\n");
   printf("*                  43)RegisterAuthentication                     *\r\n");
   printf("*                  44)UnRegisterAuthentication                   *\r\n");
   printf("*                  45)PINCodeResponse                            *\r\n");
   printf("*                  46)PassKeyResponse                            *\r\n");
   printf("*                  47)UserConfirmationResponse                   *\r\n");
   printf("*                  48)ChangeSimplePairingParameters              *\r\n");
   printf("*                  49)EnableFeature                              *\r\n");
   printf("*                  50)DisableFeature                             *\r\n");
   printf("*                  51)QueryActiveFeatures                        *\r\n");
   printf("*                  52)SendRawHCICommand                          *\r\n");
   printf("*                  53)ScheduleInterleavedAdvertisement           *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int QueryRemoteDeviceProperties(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for Querying the Remote     */
   /* Device EIR Data of a specific Remote Device.  This function       */
   /* returns zero if successful and a negative value if an error       */
   /* occurred.                                                         */
static int QueryRemoteDeviceEIRData(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters))
      {
         ...
         printf("Attempting to Query Device EIR Data: %s.\r\n", TempParam->Params[0].strParam);

         if((Result = DEVM_QueryRemoteDeviceEIRData(BD_ADDR, &EIRData)) >= 0)
         {
            printf("DEVM_QueryRemoteDeviceProperties() Success: %d.\r\n", Result);
            ...
            if(Result == 0)
            {
               printf("\r\nNo EIR Data available for device.\r\n");
            }
            else
            {
               printf("\t\t ");
               for(Index = 0; Index < EXTENDED_INQUIRY_RESPONSE_DATA_MAXIMUM_SIZE; Index++)
               {
                  printf(" %02X", EIRData.Extended_Inquiry_Response_Data[Index]);
               ...
               {
                  TotalEntries = (unsigned int)Result;

                  if((TLVList = (DEVM_Tag_Length_Value_t*)BTPS_AllocateMemory(TotalEntries * sizeof(DEVM_Tag_Length_Value_t))) != NULL)
                  {
                     if((Result = DEVM_ConvertRawEIRDataToParsedEIRData(&EIRData, TotalEntries, TLVList)) > 0)
                     {
                        TotalEntries = (unsigned int)Result;

                        printf("\r\nParsed:\r\n");

                        for(TLVIndex = 0; TLVIndex < TotalEntries; TLVIndex++)
                        {
                           switch(TLVList[TLVIndex].DataType)
                           {
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_FLAGS:
                                 printf("\tFLAGS:\r\n");

                                 if(TLVList[TLVIndex].DataLength == 1)
                                    printf("\t\t0x%02X\r\n", TLVList[TLVIndex].DataBuffer[0]);
                                 else
                                    printf("\t\t*** Invalid Data\r\n");

                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_16_BIT_SERVICE_CLASS_UUID_PARTIAL:
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_16_BIT_SERVICE_CLASS_UUID_COMPLETE:
                                 printf("\t16-Bit UUID List (%s):\r\n", (TLVList[TLVIndex].DataType == HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_16_BIT_SERVICE_CLASS_UUID_PARTIAL ? "Partial" : "Complete"));

                                 for(Index = 0; (TLVList[TLVIndex].DataLength - Index) >= 2; Index += 2)
                                    printf("\t\t0x%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[Index+1], TLVList[TLVIndex].DataBuffer[Index]);

                                 if(Index != TLVList[TLVIndex].DataLength)
                                    printf("\t\t*** Invalid Data\r\n");

                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_32_BIT_SERVICE_CLASS_UUID_PARTIAL:
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_32_BIT_SERVICE_CLASS_UUID_COMPLETE:
                                 printf("\t32-Bit UUID List (%s):\r\n", (TLVList[TLVIndex].DataType == HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_32_BIT_SERVICE_CLASS_UUID_PARTIAL ? "Partial" : "Complete"));

                                 for(Index = 0; (TLVList[TLVIndex].DataLength - Index) >= 4; Index += 4)
                                    printf("\t\t0x%02X%02X%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[Index+3], TLVList[TLVIndex].DataBuffer[Index+2], TLVList[TLVIndex].DataBuffer[Index+1], TLVList[TLVIndex].DataBuffer[Index]);
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_128_BIT_SERVICE_CLASS_UUID_PARTIAL:
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_128_BIT_SERVICE_CLASS_UUID_COMPLETE:
                                 printf("\t128-Bit UUID List (%s):\r\n", (TLVList[TLVIndex].DataType == HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_128_BIT_SERVICE_CLASS_UUID_PARTIAL ? "Partial" : "Complete"));

                                 for(Index = 0; (TLVList[TLVIndex].DataLength - Index) >= 16; Index += 16)
                                    printf("\t\t%02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[Index+15], TLVList[TLVIndex].DataBuffer[Index+14], TLVList[TLVIndex].DataBuffer[Index+13], TLVList[TLVIndex].DataBuffer[Index+12], TLVList[TLVIndex].DataBuffer[Index+11], TLVList[TLVIndex].DataBuffer[Index+10], TLVList[TLVIndex].DataBuffer[Index+9], TLVList[TLVIndex].DataBuffer[Index+8], TLVList[TLVIndex].DataBuffer[Index+7], TLVList[TLVIndex].DataBuffer[Index+6], TLVList[TLVIndex].DataBuffer[Index+5], TLVList[TLVIndex].DataBuffer[Index+4], TLVList[TLVIndex].DataBuffer[Index+3], TLVList[TLVIndex].DataBuffer[Index+2], TLVList[TLVIndex].DataBuffer[Index+1], TLVList[TLVIndex].DataBuffer[Index]);
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_LOCAL_NAME_SHORTENED:
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_LOCAL_NAME_COMPLETE:
                                 printf("\tLocal Name%s:\r\n", ((TLVList[TLVIndex].DataType == HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_LOCAL_NAME_SHORTENED) ? " (Shortened)" : " (Complete)"));
                                 printf("\t\t%.*s\r\n", TLVList[TLVIndex].DataLength, (char *)(TLVList[TLVIndex].DataBuffer));
                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_TX_POWER_LEVEL:
                                 printf("\tTX Power Level:\r\n");

                                 if(TLVList[TLVIndex].DataLength == 1)
                                    printf("\t\t%d dBm\r\n", (int)(TLVList[TLVIndex].DataBuffer[0]));
                                 else
                                    printf("\t\t*** Invalid Data\r\n");

                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_CLASS_OF_DEVICE:
                                 printf("\tClass of Device:\r\n");

                                 if(TLVList[TLVIndex].DataLength == 3)
                                    printf("\t\t0x%02X%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[2], TLVList[TLVIndex].DataBuffer[1], TLVList[TLVIndex].DataBuffer[0]);
                                 else
                                    printf("\t\t*** Invalid Data\r\n");

                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_SIMPLE_PAIRING_HASH_C:
                                 printf("\tSimple Pairing Hash C-192:\r\n");

                                 if(TLVList[TLVIndex].DataLength == 16)
                                    printf("\t\t0x%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[0], TLVList[TLVIndex].DataBuffer[1], TLVList[TLVIndex].DataBuffer[2], TLVList[TLVIndex].DataBuffer[3], TLVList[TLVIndex].DataBuffer[4], TLVList[TLVIndex].DataBuffer[5], TLVList[TLVIndex].DataBuffer[6], TLVList[TLVIndex].DataBuffer[7], TLVList[TLVIndex].DataBuffer[8], TLVList[TLVIndex].DataBuffer[9], TLVList[TLVIndex].DataBuffer[10], TLVList[TLVIndex].DataBuffer[11], TLVList[TLVIndex].DataBuffer[12], TLVList[TLVIndex].DataBuffer[13], TLVList[TLVIndex].DataBuffer[14], TLVList[TLVIndex].DataBuffer[15]);
                                 else
                                    printf("\t\t*** Invalid Data\r\n");

                                 break;

                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_SIMPLE_PAIRING_RANDOMIZER_R:
                                 printf("\tSimple Pairing Randomizer R-192:\r\n");
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_DEVICE_ID:
                                 printf("\tDevice ID:\r\n");

                                 if(TLVList[TLVIndex].DataLength == 8)
                                 {
                                    printf("\t\tVendor ID Source: 0x%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[1], TLVList[TLVIndex].DataBuffer[0]);
                                    printf("\t\tVendor ID:        0x%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[3], TLVList[TLVIndex].DataBuffer[2]);
                                    printf("\t\tProduct ID:       0x%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[5], TLVList[TLVIndex].DataBuffer[4]);
                                    printf("\t\tVersion:          0x%02X%02X\r\n", TLVList[TLVIndex].DataBuffer[7], TLVList[TLVIndex].DataBuffer[6]);
                                 }
                                 else
                                    printf("\t\t*** Invalid Data\r\n");
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_SIMPLE_PAIRING_HASH_C_256:
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_SIMPLE_PAIRING_RANDOMIZER_R_256:
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_3D_INFORMATION_DATA:
                                 printf("\t3D Information Data:\r\n");

                                 if(TLVList[TLVIndex].DataLength >= 2)
                                 {
                                    printf("\t\tAssociation Notification:       %s\r\n", ((TLVList[TLVIndex].DataBuffer[0] & 0x01) ? "Supported" : "Not Supported"));
                                    printf("\t\tBattery Level Reporting:        %s\r\n", ((TLVList[TLVIndex].DataBuffer[0] & 0x02) ? "Supported" : "Not Supported"));
                                    printf("\t\tSend Bat Lvl Report on Startup: %s\r\n", ((TLVList[TLVIndex].DataBuffer[0] & 0x04) ? "Send" : "Do Not Send"));
                                    printf("\t\tFactory Test Mode:              %s\r\n", ((TLVList[TLVIndex].DataBuffer[0] & 0x80) ? "Factory Test Mode" : "Normal Mode"));
                                    printf("\t\tPath Loss Threshold:            %u dB\r\n", TLVList[TLVIndex].DataBuffer[1]);
                                 }
                                 else
                                    printf("\t\t*** Invalid Data\r\n");
                              ...
                              case HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_MANUFACTURER_SPECIFIC:
                                 ...
                                 if(TLVList[TLVIndex].DataLength >= 2)
                                 {
                                    ...
                                    for(Index = 1; Index < TLVList[TLVIndex].DataLength; Index++)
                                    {
                                       printf(" %02X", TLVList[TLVIndex].DataBuffer[Index]);
         ...
         else
         {
            /* Error Querying Remote Device, inform the user and flag an*/
            /* error.                                                   */
            printf("DEVM_QueryRemoteDeviceEIRData() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: QueryRemoteDeviceEIRData [BD_ADDR].\r\n");
...
static int QueryActiveLocalDeviceFeatures(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for scheduling interleaved  */
   /* advertising.  This function returns zero if successful and a      */
   /* negative value if an error occurred.                              */
static int ScheduleInterleavedAdvertisement(ParameterList_t *TempParam)
{
   int           ret_val;
   ...
   unsigned int  Index;
   unsigned int  Index2;
   unsigned long NumberInterleavedAdvertisements;
   ...
   if(Initialized)
   {
      ...
      {
         /* Get the number of advertisements to schedule.               */
         NumberInterleavedAdvertisements = (unsigned long)TempParam->Params[0].intParam;

         /* Schedule the requested number of advertisements.            */
         Index                           = 0;
         while(Index++ < NumberInterleavedAdvertisements)
         {
            /* Get a random data length for advertisement from 1 - 255. */
            DataLength = (rand() % 255) + 1;

            /* Calculate a random duration for the packet from 20 ms -  */
            /* 1019 ms.                                                 */
            Duration   = (rand() % 1000) + 20;

            /* Create random data of the requested size.                */
            for(Index2=0;Index2<DataLength;Index2++)
               Buffer[Index2] = (rand() % 255) + 1;

            /* Generate a random address.                               */
            RandomAddress.BD_ADDR0 = (rand() % 255);
            RandomAddress.BD_ADDR1 = (rand() % 255);
            RandomAddress.BD_ADDR2 = (rand() % 255);
            RandomAddress.BD_ADDR3 = (rand() % 255);
            RandomAddress.BD_ADDR4 = (rand() % 255);
            RandomAddress.BD_ADDR5 = (rand() % 255);

            /* Attempt to schedule the interleaved advertisement.       */
            if((Result = DEVM_ScheduleInterleavedAdvertisement(DEVMCallbackID, 0, Duration, RandomAddress, DataLength, Buffer)) > 0)
            {
               /* Scheduled the advertisement so go ahead and print     */
               /* information on the advertisement.                     */
               printf("DEVM_ScheduleInterleavedAdvertisement() SUCCESS:.\r\n");
               printf("Interleaved Advertising ID                     : %u.\r\n", (unsigned int)Result);
               printf("Duration                                       : %u.\r\n", Duration);
               BD_ADDRToStr(RandomAddress, (char *)Buffer);
            ...
            {
               /* Error attempting to schedule interleaved              */
               /* advertisement, inform the user and flag an error.     */
               printf("DEVM_ScheduleInterleavedAdvertisement() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ScheduleInterleavedAdvertisement [NumberToSchedule].\r\n");
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case detRemoteDeviceConnectionStatus:
            ...
            break;
         case detInterleavedAdvertisementComplete:
            printf("Interleaved Advertisement Complete:\r\n");
            printf("Interleaved Advertisement ID: %u\r\n", EventData->EventData.InterleavedAdvertisementCompleteEventData.InterleavedAdvertisementID);
            printf("DEVM Callback ID            : %u (%u)\r\n", EventData->EventData.InterleavedAdvertisementCompleteEventData.DeviceManagerCallbackID, DEVMCallbackID);
            printf("Status                      : %d (%s)\r\n", EventData->EventData.InterleavedAdvertisementCompleteEventData.Status, (EventData->EventData.InterleavedAdvertisementCompleteEventData.Status)?ERR_ConvertErrorCodeToString(EventData->EventData.InterleavedAdvertisementCompleteEventData.Status):"SUCCESS");
            break;
         case detInterleavedAdvertisingSuspended:
            printf("Interleaved Advertisement Suspended:\r\n");
           break;
         case detInterleavedAdvertisingResumed:
            printf("Interleaved Advertisement Resumed:\r\n");
           break;
         default:
