...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXANCM - Simple Linux application using Bluetopia Platform Manager     */
/*              Apple Notification Center Service Manager (ANCM) Application  */
...
/*   --------  -----------    ------------------------------------------------*/
/*   10/14/13  M. Buckley     Initial creation. (Based on LinuxGLPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxANCM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTPM.h"       /* BTPM Application Programming Interface.         */
#include "SS1BTANCM.h"     /* ANCS Manager Application Programming Interface. */
...
#define MAX_NUM_OF_PARAMETERS                      (20)  /* Denotes the max   */
                                                         /* number of         */
                                                         /* parameters a      */
                                                         /* command can have. */
...
static unsigned int        ANCMEventCallbackID;     /* Variable which holds the        */
                                                    /* current ANCM Event Callback ID  */
                                                    /* that is assigned from the ANCS  */
                                                    /* Manager when the local client   */
                                                    /* registers for ANCM Events.      */
...
static char *IOCapabilitiesStrings[] =
{
   ...
   /* The following string table is used to map the Event ID values to  */
   /* an easily displayable string.                                     */
static char *EventIDStrings[] =
{
   "Notification Added",
...
#define NUM_SUPPORTED_EVENT_ID_TYPES                     (sizeof(EventIDStrings)/sizeof(char *))

   /* The following string table is used to map the Category ID values  */
   /* to an easily displayable string.                                  */
static char *CategoryIDStrings[] =
{
   "Other",
   "Incoming Call",
   "Missed Call",
...
#define NUM_SUPPORTED_CATEGORY_ID_TYPES                  (sizeof(CategoryIDStrings)/sizeof(char *))

   /* The following string table is used to map the Notification        */
   /* Attribute ID values to an easily displayable string.              */
static char *NotificationAttributeIDStrings[] =
{
   "App Identifier",
   "Title",
   "Subtitle",
   ...
   "Message Size",
   "Date",
   "Unknown Notification Attribute ID"
};

#define NUM_SUPPORTED_NOTIFICATION_ATTRIBUTE_ID_TYPES    ((sizeof(NotificationAttributeIDStrings)/sizeof(char *)) - 1)

   /* The following string table is used to map the App Attribute ID    */
   /* values to an easily displayable string.                           */
static char *AppAttributeIDStrings[] =
{
   "Display Name",
   "Unknown App Attribute ID"
};

#define NUM_SUPPORTED_APP_ATTRIBUTE_ID_TYPES             ((sizeof(AppAttributeIDStrings)/sizeof(char *)) - 1)
...
static int ANCMQueryConnectedDevices(ParameterList_t *TempParam);
static int ANCMRegisterConsumerEvents(ParameterList_t *TempParam);
static int ANCMUnRegisterConsumerEvents(ParameterList_t *TempParam);

static int ANCMGetNotificationAttributes(ParameterList_t *TempParam);
static int ANCMGetAppAttributes(ParameterList_t *TempParam);
static int ANCMQueryNotifications(ParameterList_t *TempParam);
static int PrintAttributeIDs(ParameterList_t *TempParam);
...
static void DisplayGetNotificationAttributesResponse(ANCM_Get_Notification_Attributes_Response_Event_Data_t *EventData);
static void DisplayGetAppAttributesResponse(ANCM_Get_App_Attributes_Response_Event_Data_t *EventData);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM ANCS Manager Callback function prototype.                    */
static void BTPSAPI ANCM_Event_Callback(ANCM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("ANCMQUERYCONNECTEDDEVICES", ANCMQueryConnectedDevices);
   AddCommand("ANCMREGISTERCONSUMEREVENTS", ANCMRegisterConsumerEvents);
   AddCommand("ANCMUNREGISTERCONSUMEREVENTS", ANCMUnRegisterConsumerEvents);
   AddCommand("ANCMGETNOTIFICATIONATTRIBUTES", ANCMGetNotificationAttributes);
   AddCommand("ANCMGETAPPATTRIBUTES", ANCMGetAppAttributes);
   AddCommand("ANCMQUERYNOTIFICATIONS", ANCMQueryNotifications);
   AddCommand("PRINTATTRIBUTEIDS", PrintAttributeIDs);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      ANCS_ASSIGN_ANCS_SERVICE_UUID_128(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)ANCMQueryConnectedDevices                  *\r\n");
   printf("*                  36)ANCMRegisterConsumerEvents                 *\r\n");
   printf("*                  37)ANCMUnRegisterConsumerEvents               *\r\n");
   printf("*                  38)ANCMGetNotificationAttributes              *\r\n");
   printf("*                  39)ANCMGetAppAttributes                       *\r\n");
   printf("*                  40)ANCMQueryNotifications                     *\r\n");
   printf("*                  41)PrintAttributeIDs                          *\r\n");
   printf("*                  42)EnableDataLog                              *\r\n");
   printf("*                  43)StartAdvertising                           *\r\n");
   printf("*                  44)StopAdvertising                            *\r\n");
   printf("*                  45)StartObservationScan                       *\r\n");
   printf("*                  46)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for querying the ANCS       */
   /* Manager to get the list of connected devices that support ANCS.   */
...
static int ANCMQueryConnectedDevices(ParameterList_t *TempParam)
{
   ...
   int           Result;
   char          BoardStr[16];
   BD_ADDR_t    *RemoteDeviceList;
   unsigned int  MaxNumberDevices;
   unsigned int  TotalNumberDevices;
   ...
   if(Initialized)
   {
      ...
      {
         /* Get the Maximum Number of Devices to query from the first   */
         /* parameter.                                                  */
         MaxNumberDevices = TempParam->Params[0].intParam;

         if((RemoteDeviceList = BTPS_AllocateMemory(MaxNumberDevices * sizeof(BD_ADDR_t))) != NULL)
         {
            if((Result = ANCM_Query_Connected_Devices(MaxNumberDevices, RemoteDeviceList, &TotalNumberDevices)) > 0)
            {
               printf("ANCM_Query_Connected_Devices() Success: %d.\r\n", Result);

               /* Print out the total number of devices.                */
               printf("   Total Number of Devices:            %u\r\n", TotalNumberDevices);

               /* Print out the number of devices copied into the       */
               /* buffer.                                               */
               printf("   Number of Devices Queried:          %u\r\n", Result);
               printf("   Device List:\r\n");

               /* Print out each BD_ADDR received.                      */
               for(Index = 0; Index < Result; Index++)
               {
                  BD_ADDRToStr(RemoteDeviceList[Index], BoardStr);
            ...
            else
            {
               ...
               /* ANCM_Query_Connected_Devices().                       */
               printf("ANCM_Query_Connected_Devices() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

               ret_val = FUNCTION_ERROR;
            }

            BTPS_FreeMemory(RemoteDeviceList);
         }
         else
         {
            /* Allocation failed.  Flag an error and report it to the   */
            /* user.                                                    */
            printf("Unable to allocate memory for ANCMQueryConnectedDevices Remote Device List.\r\n");
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ANCMQueryConnectedDevices [Max Number of Devices].\r\n");
   ...
   /* The following function is responsible for registering with the    */
   /* ANCS Manager to receive ANCM Events.  This function returns zero  */
   /* if successful and a negative value if an error occurred.          */
static int ANCMRegisterConsumerEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!ANCMEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = ANCM_Register_Consumer_Event_Callback(ANCM_Event_Callback, NULL)) > 0)
         {
            printf("ANCM_Register_Consumer_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            ANCMEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("ANCM_Register_Consumer_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("ANCS Manager Event Callback already registered.\r\n");
   ...
   /* previously registered ANCM Event Registration with the ANCS       */
...
static int ANCMUnRegisterConsumerEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(ANCMEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         ANCM_Un_Register_Consumer_Event_Callback(ANCMEventCallbackID);

         printf("ANCM_Un_Register_Consumer_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         ANCMEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("ANCS Manager Event Callback is not registered.\r\n");
   ...
   /* Notification Attributes command to the ANCS Manager.  This        */
   /* function returns zero if successful or a negative value if an     */
   /* error occurred.                                                   */
static int ANCMGetNotificationAttributes(ParameterList_t *TempParam)
{
   int                                         Index;
   ...
   ANCM_Notification_Attribute_ID_t            TempAttributeID;
   ANCM_Notification_Attribute_Request_Data_t *AttributeRequestData;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 3))
      {
         ...
         /* Cast the second parameter to a DWord_t.                     */
         NotificationUID = (DWord_t)(TempParam->Params[1].intParam);

         /* Iterate through the parameter list to count the number of   */
         /* attributes that we are requesting and verify that a max     */
         /* length parameter is provided for each one.                  */
         for(Index = 2; Index < TempParam->NumberofParameters; Index++)
         {
            /* Get the next Notification Attribute ID.                  */
            TempAttributeID = (ANCM_Notification_Attribute_ID_t)(TempParam->Params[Index].intParam);
            /* If a Max Length parameter is required, make sure it was  */
            /* included in the parameters.                              */
            if(MaxLengthParameterRequired(adtNotification, TempAttributeID, 0))
            {
               Index++;

               if(Index == TempParam->NumberofParameters)
               {
                  /* Flag an error and inform the user.                 */
                  printf("Invalid command data: missing Max Length parameter for Attribute ID %d.\r\n", TempAttributeID);

                  ret_val = INVALID_PARAMETERS_ERROR;
               }
            }

            /* Increment the number of attributes that we are           */
            /* requesting.                                              */
            NumberOfAttributes++;
         }
         ...
         if(!ret_val)
         {
            /* Allocate enough memory for each Attribute ID and its     */
            /* length.                                                  */
            if((AttributeRequestData = (ANCM_Notification_Attribute_Request_Data_t *)BTPS_AllocateMemory(sizeof(ANCM_Notification_Attribute_Request_Data_t) * NumberOfAttributes)) != NULL)
            {
               /* Create an entry in AttributeRequestData for each      */
               /* attribute being requested.                            */
               for(Index = 0, Index1 = 2; Index < NumberOfAttributes; Index++)
               {
                  /* Copy over the Notification Attribute ID.           */
                  TempAttributeID                                     = (ANCM_Notification_Attribute_ID_t)(TempParam->Params[Index1].intParam);
                  AttributeRequestData[Index].NotificationAttributeID = TempAttributeID;

                  /* Check to see if a Max Length parameter is required.*/
                  if(MaxLengthParameterRequired(adtNotification, TempAttributeID, 0))
                  {
                     /* Advance the index of the Parameter List to get  */
                     /* the Max Length parameter for the attribute.     */
                     Index1++;

                     /* Copy over the Max Length parameter.             */
                     AttributeRequestData[Index].AttributeMaxLength = TempParam->Params[Index1].intParam;
                  }

                  /* Increment the index of the Parameter List.         */
                  Index1++;
               }
            }
            else
            {
               /* Flag an error and inform the user.                    */
               printf("Failed to allocate buffer to hold Attribute Request Data.\r\n");
         ...
         if(!ret_val)
         {
            ...
            if((ret_val = ANCM_Get_Notification_Attributes(ANCMEventCallbackID, BD_ADDR, NotificationUID, NumberOfAttributes, AttributeRequestData)) > 0)
            {
               printf("ANCM_Get_Notification_Attributes() Success.  Transaction ID: %d.\r\n", ret_val);
            ...
            {
               /* Error sending the command, inform user and flag an    */
               /* error.                                                */
               printf("ANCM_Get_Notification_Attributes() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));

               ret_val = FUNCTION_ERROR;
            }

            /* Free any Attribute Request Data memory we may have       */
            /* allocated.                                               */
            if(AttributeRequestData)
               BTPS_FreeMemory(AttributeRequestData);
         }
      }
      else
         ret_val = INVALID_PARAMETERS_ERROR;

      /* If an Invalid Parameters Error occurred, print the usage       */
      /* string.                                                        */
      if(ret_val == INVALID_PARAMETERS_ERROR)
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ANCMGetNotificationAttributes [BD_ADDR] [NotificationUID] [Attribute Request Data].\r\n");
         printf("Where: Attribute Request Data = \r\n");
         printf("                A list of Notification Attribute IDs, each followed by a max length parameter (if required)");
         printf("                (Note: Use PrintAttributeIDs 0 to obtain a list of valid Notification Attribute IDs.)\r\n");
      }
   }
   else
   {
   ...
   /* The following function is responsible for sending a Get App       */
   /* Attributes command to the ANCS Manager.  This function returns    */
   /* zero if successful and a negative value if an error occurred.     */
static int ANCMGetAppAttributes(ParameterList_t *TempParam)
{
   int                                Index;
   ...
   char                              *AppIdentifier;
   unsigned int                       NumberOfAttributes;
   ANCM_App_Attribute_ID_t            TempAttributeID;
   ANCM_App_Attribute_Request_Data_t *AttributeRequestData;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 3))
      {
         ...
         printf("App Identifier: %s", TempParam->Params[1].strParam);

         /* Allocate memory for the App Identifier.                     */
         if((AppIdentifier = BTPS_AllocateMemory(strlen(TempParam->Params[1].strParam) + 1)) != NULL)
         {
            /* Get the App Identifier.                                  */
            strcpy(AppIdentifier, TempParam->Params[1].strParam);

            /* Iterate through the parameter list to count the number   */
            /* of attributes that we are requesting and verify that a   */
            /* max length parameter is provided for each one.           */
            for(Index = 2; Index < TempParam->NumberofParameters; Index++)
            {
               /* Get the next App Attribute ID.                        */
               TempAttributeID = (ANCM_App_Attribute_ID_t)(TempParam->Params[Index].intParam);

               /* If a Max Length parameter is required, make sure it   */
               /* was included in the parameters.                       */
               if(MaxLengthParameterRequired(adtApplication, 0, TempAttributeID))
               {
                  Index++;
         ...
         {
            /* Flag an error and inform the user.                       */
            printf("Failed to allocate memory for the App Identifier.\r\n");
         ...
         if(!ret_val)
         {
            /* Allocate enough memory for each Attribute ID and its     */
            /* length.                                                  */
            if((AttributeRequestData = (ANCM_App_Attribute_Request_Data_t *)BTPS_AllocateMemory(sizeof(ANCM_App_Attribute_Request_Data_t) * NumberOfAttributes)) != NULL)
            {
               /* Create an entry in AttributeRequestData for each      */
               /* attribute being requested.                            */
               for(Index = 0, Index1 = 2; Index < NumberOfAttributes; Index++)
               {
                  /* Copy over the Notification Attribute ID.           */
                  TempAttributeID                            = (ANCM_App_Attribute_ID_t)(TempParam->Params[Index1].intParam);
                  AttributeRequestData[Index].AppAttributeID = TempAttributeID;
         ...
         {
            /* Attempt to send the command.                             */
            if((ret_val = ANCM_Get_App_Attributes(ANCMEventCallbackID, BD_ADDR, AppIdentifier, NumberOfAttributes, AttributeRequestData)) > 0)
            {
               printf("ANCM_Get_App_Attributes() Success.  Transaction ID: %d.\r\n", ret_val);
            ...
            else
            {
               /* Error sending the command, inform user and flag an    */
               /* error.                                                */
               printf("ANCM_Get_App_Attributes() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
            ...
            /* Free the App Identifier.                                 */
            if(AppIdentifier)
               BTPS_FreeMemory(AppIdentifier);
         }
      }
      else
         ret_val = INVALID_PARAMETERS_ERROR;
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ANCMGetAppAttributes [BD_ADDR] [AppIdentifier] [Attribute Request Data].\r\n");
         printf("Where: Attribute Request Data = \r\n");
         printf("                A list of App Attribute IDs, each followed by a max length parameter (if required)");
         printf("                (Note: Use PrintAttributeIDs 1 to obtain a list of valid App Attribute IDs.)\r\n");
      }
   }
   else
   {
   ...
   /* The following function is responsible for querying the current    */
   /* notifications on the remote notification source.  This function   */
   /* returns zero if successful and a negative value if an error       */
   /* occurred.                                                         */
static int ANCMQueryNotifications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         if((ret_val = ANCM_Query_Current_Notifications(ANCMEventCallbackID, BD_ADDR)) == 0)
         {
            printf("ANCM_Query_Current_Notifications() Success: %d.\r\n", ret_val);

            ret_val            = 0;
         }
         else
         {
            /* Error refreshing, inform user and flag an error.         */
            printf("ANCM_Query_Current_Notifications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("Usage: ANCMRefreshNotifications [BD_ADDR].\r\n");
   ...
   /* The following function prints out all available options for       */
   /* Attribute IDs for either a Get Notification Attributes command or */
   /* a Get App Attributes command.  It takes as its sole parameter the */
   /* Command ID of the command whose Attribute IDs are to be printed.  */
...
static int PrintAttributeIDs(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1))
   {
      if(TempParam->Params[0].intParam == 0)
      {
         /* Print out the available Notification Attribute IDs.         */
         printf("******************************************************************\r\n");
         printf("**                  Notification Attribute IDs                  **\r\n");
         printf("******************************************************************\r\n");
         printf("*  ID  Value           Max Length   Return Type                  *\r\n");
         printf("*                      Param Reqd?                               *\r\n");
         printf("*  0   App Identifier      No       UTF-8 String                 *\r\n");
         printf("*  1   Title               Yes      UTF-8 String                 *\r\n");
         printf("*  2   Subtitle            Yes      UTF-8 String                 *\r\n");
         printf("*  3   Message             Yes      UTF-8 String                 *\r\n");
         printf("*  4   MessageSize         No       String (see ANCS Spec)       *\r\n");
         printf("*  5   Date                No       UTS #35 yyyyMMdd\'T\'HHmmSS    *\r\n");
         printf("******************************************************************\r\n");
      }
      else
      {
         if(TempParam->Params[0].intParam == 1)
         {
            /* Print out the available App Attribute IDs.               */
            printf("******************************************************************\r\n");
            printf("**                      App Attribute IDs                       **\r\n");
            printf("******************************************************************\r\n");
            printf("*  ID  Value           Max Length   Return Type                  *\r\n");
            printf("*                      Param Reqd?                               *\r\n");
            printf("*  0   Display Name        No       UTF-8 String                 *\r\n");
            printf("******************************************************************\r\n");
         }
         else
            ret_val = INVALID_PARAMETERS_ERROR;
   ...
   /* If an Invalid Parameters Error occurred, print the usage string.  */
   if(ret_val == INVALID_PARAMETERS_ERROR)
   {
      /* One or more of the necessary parameters is/are invalid.        */
      printf("Usage: PrintAttributeIDs [Command ID].\r\n");
      printf("Where: Command ID = \r\n");
      printf("                0 = Get Notification Attributes\r\n");
      printf("                1 = Get App Attributes\r\n");
   }
...
static void DisplayLocalDeviceProperties(unsigned long UpdateMask, DEVM_Local_Device_Properties_t *LocalDeviceProperties)
{
   ...
   if(LocalDeviceProperties)
   {
      ...
      /* If changed, print out the discoverable mode.                   */
      ...
      /* If changed, print out the connectable mode.                    */
      ...
      /* If changed, print out the pairable mode.                       */
      ...
      /* If changed, print out the device flags.                        */
...
static void DisplayRemoteDeviceProperties(unsigned long UpdateMask, DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if(RemoteDeviceProperties)
   {
      ...
      /* If changed, print out whether the device supports low energy.  */
      if(LEFlags & DEVM_REMOTE_DEVICE_FLAGS_DEVICE_SUPPORTS_LOW_ENERGY)
         printf("LE Type:             %s\r\n", (!SingleMode)?"Dual Mode":"Single Mode");

      /* If changed, print out the remote device flags.                 */
...
static void DataLogger(char *LogString)
{
   ...
   /* display a response to a Get Notification Attributes Request       */
   /* transcation.                                                      */
static void DisplayGetNotificationAttributesResponse(ANCM_Get_Notification_Attributes_Response_Event_Data_t *EventData)
{
   int                                 Result;
   ...
   ANCM_Parsed_Attribute_Data_t        ParsedAttributeData;
   ANCM_Notification_Attribute_ID_t    TempAttributeID;
   ANCM_Notification_Attribute_Data_t *AttributePtr;

   /* Verify that the input parameters are semi-valid.                  */
   if(EventData)
   {
      /* Print out the remote device's Bluetooth Address.               */
      BD_ADDRToStr(EventData->RemoteDeviceAddress, BoardStr);
      ...
      sprintf(StringBuffer, "   Status:                    0x%X\r\n", EventData->Status);
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);

      /* Only print out the rest of the response data if no error       */
      /* occurred.                                                      */
      if(!(EventData->Status))
      {
         /* Print out the Notification UID.                             */
         sprintf(StringBuffer, "   Notification UID:          %lu\r\n", (unsigned long)EventData->NotificationUID);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         /* Print out the Attribute Data Length.                        */
         sprintf(StringBuffer, "   Attribute Data Length:     %u\r\n", EventData->AttributeDataLength);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);

         /* Try to convert the raw attribute data to parsed attribute   */
         /* data.                                                       */
         if(!(Result = ANCM_Convert_Raw_Attribute_Data_To_Parsed_Attribute_Data(adtNotification, EventData->AttributeDataLength, EventData->AttributeData, &ParsedAttributeData)))
         {
            sprintf(StringBuffer, "\r\nDecoded Notification Attributes:\r\n");
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            /* Print out the Attribute Data Type (for verification).    */
            sprintf(StringBuffer, "   Attribute Data Type:       %s\r\n", (ParsedAttributeData.Type == adtNotification) ? "adtNotification" : "adtApplication");
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            /* Print out the number of attributes.                      */
            sprintf(StringBuffer, "   Number of Attributes:      %u\r\n", ParsedAttributeData.NumberOfAttributes);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            /* Print out the data for each attribute.                   */
            AttributePtr = ParsedAttributeData.AttributeData.NotificationAttributeData;

            for(Index = 0; Index < ParsedAttributeData.NumberOfAttributes; Index++)
            {
               /* Print out the Attribute ID and its description.       */
               TempAttributeID = AttributePtr[Index].AttributeID;
               Index1          = (TempAttributeID < NUM_SUPPORTED_NOTIFICATION_ATTRIBUTE_ID_TYPES) ? TempAttributeID : NUM_SUPPORTED_NOTIFICATION_ATTRIBUTE_ID_TYPES;

               sprintf(StringBuffer, "   Attribute ID:              %u (%s)\r\n", TempAttributeID, NotificationAttributeIDStrings[Index1]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the length of the Attribute Data.           */
               sprintf(StringBuffer, "   Data Length:               %u\r\n", AttributePtr[Index].AttributeLength);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the header for the attribute data.          */
               sprintf(StringBuffer, "   Attribute Data:\r\n");
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Set a temporary pointer to the attribute data.        */
               TempDataPtr = (char *)AttributePtr[Index].AttributeData;

               /* Verify that the attribute data pointer is valid.      */
               if(TempDataPtr)
               {
                  /* Calculate the maximum length of the string that we */
                  /* can copy.                                          */
                  MaxLength = (AttributePtr[Index].AttributeLength < (sizeof(StringBuffer2)-1)) ? AttributePtr[Index].AttributeLength : (sizeof(StringBuffer2)-1);

                  /* Check to see if this is the Message Size attribute */
                  /* (which must be 1 byte).                            */
                  if(TempAttributeID == ANCS_NOTIFICATION_ATTRIBUTE_ID_MESSAGE_SIZE)
                  {
                     /* Print out the Message Size.                     */
                     strncpy(StringBuffer2, TempDataPtr, MaxLength);
                     StringBuffer2[MaxLength] = '\0';

                     sprintf(StringBuffer, "      Message Size:           %s\r\n", StringBuffer2);
                     printf("%s", StringBuffer);
                     DataLogger(StringBuffer);
                  }
                  else
                  {
                     /* Print out the Attribute Data as a string.       */
                     sprintf(StringBuffer, "      Message String:         ");
                     printf("%s", StringBuffer);
                     DataLogger(StringBuffer);

                     /* Print the string directly to the output device  */
                     /* (we assume here that the output device supports */
                     /* UTF-8 strings).                                 */
                     strncpy(StringBuffer2, TempDataPtr, MaxLength);
                     StringBuffer2[MaxLength] = '\0';

                     puts(StringBuffer2);
                     DataLogger(StringBuffer2);
                  }
               }
            ...
            /* Tell the module to free the memory for the Parsed        */
            /* Attribute Data.                                          */
            ANCM_Free_Parsed_Attribute_Data(&ParsedAttributeData);
         }
         else
         {
            /* Error sending the command; inform the user and flag an   */
            /* error.                                                   */
            sprintf(StringBuffer, "ANCM_Convert_Raw_Attribute_Data_To_Parsed_Attribute_Data() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);
         }
      }
   ...
   /* The following function is a utility function which is used to     */
   /* display a response to a Get App Attributes Request transcation.   */
static void DisplayGetAppAttributesResponse(ANCM_Get_App_Attributes_Response_Event_Data_t *EventData)
{
   int                           Result;
   int                           Index;
   ...
   ANCM_App_Attribute_ID_t       TempAttributeID;
   ANCM_App_Attribute_Data_t    *AttributePtr;
   ANCM_Parsed_Attribute_Data_t  ParsedAttributeData;

   /* Verify that the input parameters are semi-valid.                  */
   if(EventData)
   {
      sprintf(StringBuffer, "Get App Attributes Response on Data Source Characteristic.\r\n");
      printf("%s", StringBuffer);
      DataLogger(StringBuffer);
      ...
      if(!(EventData->Status))
      {
         /* Print out the App Identifier.                               */
         sprintf(StringBuffer, "   App Identifier:            %s\r\n", EventData->AppIdentifier);
         printf("%s", StringBuffer);
         DataLogger(StringBuffer);
         ...
         /* Try to convert the raw attribute data to parsed attribute   */
         /* data.                                                       */
         if(!(Result = ANCM_Convert_Raw_Attribute_Data_To_Parsed_Attribute_Data(adtApplication, EventData->AttributeDataLength, EventData->AttributeData, &ParsedAttributeData)))
         {
            sprintf(StringBuffer, "\r\nDecoded App Attributes:\r\n");
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);
            ...
            /* Print out the data for each attribute.                   */
            AttributePtr = ParsedAttributeData.AttributeData.AppAttributeData;

            for(Index = 0; Index < ParsedAttributeData.NumberOfAttributes; Index++)
            {
               /* Print out the Attribute ID and its description.       */
               TempAttributeID = AttributePtr[Index].AttributeID;
               Index1 = (TempAttributeID < NUM_SUPPORTED_APP_ATTRIBUTE_ID_TYPES) ? TempAttributeID : NUM_SUPPORTED_APP_ATTRIBUTE_ID_TYPES;

               sprintf(StringBuffer, "   Attribute ID:              %u (%s)\r\n", TempAttributeID, AppAttributeIDStrings[Index1]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);
               ...
               /* Print out the Attribute Data as a string.             */
               /* * NOTE * This Sample Application assumes that all     */
               /*          received Attribute Data lies within the set  */
               /*          of ASCII characters.  Received data is       */
               /*          typically in UTF-8 form and the ANCM Module  */
               /*          does not perform conversions of any kind.    */
               sprintf(StringBuffer, "   Attribute Data:            ");
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);
               ...
               for(Index1 = 0; Index1 < AttributePtr[Index].AttributeLength; Index1++)
               {
                  sprintf(StringBuffer, "%c", TempDataPtr[Index1]);
                  printf("%s", StringBuffer);
                  DataLogger(StringBuffer);
               }
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* The following function is the ANCS Manager Event Callback         */
   /* function that is Registered with the ANCS Manager.  This callback */
   /* is responsible for processing all ANCS Manager Events.            */
static void BTPSAPI ANCM_Event_Callback(ANCM_Event_Data_t *EventData, void *CallbackParameter)
{
   char         BoardStr[16];
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case etANCMConnected:
            sprintf(StringBuffer, "etANCMConnected with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= ANCM_CONNECTED_EVENT_DATA_SIZE)
            {
               /* Print out the remote device's Bluetooth Address.      */
            ...
            break;
         case etANCMDisconnected:
            sprintf(StringBuffer, "etANCMDisconnected with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= ANCM_DISCONNECTED_EVENT_DATA_SIZE)
            {
               /* Print out the remote device's Bluetooth Address.      */
            ...
            break;
         case etANCMNotificationReceived:
            sprintf(StringBuffer, "etANCMNotificationReceived with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= ANCM_NOTIFICATION_RECEIVED_EVENT_DATA_SIZE)
            {
               /* Print out the remote device's Bluetooth Address.      */
               BD_ADDRToStr(EventData->EventData.NotificationReceivedEventData.RemoteDeviceAddress, BoardStr);

               sprintf(StringBuffer, "   Bluetooth Address:         %s\r\n", BoardStr);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the Notification's Event ID.                */
               Index = EventData->EventData.NotificationReceivedEventData.EventID;
               Index = ((Index < NUM_SUPPORTED_EVENT_ID_TYPES)?Index:(NUM_SUPPORTED_EVENT_ID_TYPES - 1));

               sprintf(StringBuffer, "   Event ID:                  %u (%s)\r\n", EventData->EventData.NotificationReceivedEventData.EventID, EventIDStrings[Index]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the Notification's Event Flags.             */
               sprintf(StringBuffer, "   Event Flag Silent:         %s\r\n", (EventData->EventData.NotificationReceivedEventData.EventFlags & ANCS_EVENT_FLAG_SILENT) ? "Yes" : "No");
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               sprintf(StringBuffer, "   Event Flag Silent:         %s\r\n", (EventData->EventData.NotificationReceivedEventData.EventFlags & ANCS_EVENT_FLAG_IMPORTANT) ? "Yes" : "No");
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the Notification's Category ID.             */
               Index = EventData->EventData.NotificationReceivedEventData.CategoryID;
               Index = ((Index < NUM_SUPPORTED_CATEGORY_ID_TYPES)?Index:(NUM_SUPPORTED_CATEGORY_ID_TYPES - 1));

               sprintf(StringBuffer, "   Category ID:               %u (%s)\r\n", EventData->EventData.NotificationReceivedEventData.CategoryID, CategoryIDStrings[Index]);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the Notification's Category Count.          */
               sprintf(StringBuffer, "   Category Count:            %u\r\n", EventData->EventData.NotificationReceivedEventData.CategoryCount);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);

               /* Print out the Notification's Notification UID.        */
               sprintf(StringBuffer, "   Notification UID:          %lu\r\n", (unsigned long)EventData->EventData.NotificationReceivedEventData.NotificationUID);
               printf("%s", StringBuffer);
               DataLogger(StringBuffer);
            }
            ...
            DataLogger(StringBuffer);
            break;
         case etANCMGetNotificationAttributesResponse:
            sprintf(StringBuffer, "etANCMGetNotificationAttributesResponse with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= ANCM_GET_NOTIFICATION_ATTRIBUTES_RESPONSE_EVENT_DATA_SIZE)
            {
               /* Go ahead and display the response data.               */
               DisplayGetNotificationAttributesResponse(&(EventData->EventData.GetNotificationAttributesResponseEventData));
            }

            strcpy(StringBuffer, "\r\n");
            DataLogger(StringBuffer);
            break;
         case etANCMGetAppAttributesResponse:
            sprintf(StringBuffer, "etANCMGetAppAttributesResponse with size %u:\r\n", EventData->EventLength);
            printf("%s", StringBuffer);
            DataLogger(StringBuffer);

            if(EventData->EventLength >= ANCM_GET_NOTIFICATION_ATTRIBUTES_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.GetAppAttributesResponseEventData.RemoteDeviceAddress, BoardStr);

               /* Go ahead and display the response data.               */
               DisplayGetAppAttributesResponse(&(EventData->EventData.GetAppAttributesResponseEventData));
            }
            break;
         default:
            printf("Unknown ANCS Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nANCM Event Data is NULL.\r\n");
