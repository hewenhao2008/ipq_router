...
/*                  (API) Interface - Server Role only.                       */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
#include "LinuxFTPM_SRV.h" /* Main Application Prototypes and Constants.      */
...
static unsigned int        FTPConnectionID;         /* Variable used to hold the       */
                                                    /* FTP Server ID of the currently  */
                                                    /* opened FTP Server.              */
...
static DWord_t             FTPServerSDPHandle;      /* Variable used to hold the SDP   */
                                                    /* Service Record of the FTP Server*/
                                                    /* SDP Service Record.             */
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* FTPM Server Event Callback function prototype.                    */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  49)OpenServer                                 *\r\n");
   printf("*                  50)CloseServer                                *\r\n");
   printf("*                  51)OpenServerPortRequestResponse              *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int SendRawHCICommand(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for opening an FTP Server on*/
   /* the Local Device.  This function opens an FTP Server in either the*/
   /* specified directory or the Current Directory of the Local Machine.*/
   /* This function accepts the RFCOMM Server Channel to open the server*/
   /* on as well.  This function returns zero if successful, or a       */
...
static int OpenServer(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Next, let's make sure that there is not a FTP Server already   */
      /* open.                                                          */
      if(!FTPConnectionID)
      {
         /* Let's check to see if a Remote RFCOMM Port has been         */
         /* specified.                                                  */
         if((TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && ((TempParam->Params[0].intParam >= SPP_PORT_NUMBER_MINIMUM) && (TempParam->Params[0].intParam <= SPP_PORT_NUMBER_MAXIMUM)))
         {
            /* RFCOMM Port appears to be valid, so now let's check to   */
            /* see if an optional directory was specified.              */
            if((TempParam->NumberofParameters > 2) && (TempParam->Params[2].strParam) && (strlen(TempParam->Params[2].strParam)))
               strcpy(RootPath, TempParam->Params[2].strParam);
            else
            {
               /* Let's figure out the current working directory of the */
               /* program so we can use that for our Root Directory.    */
            ...
            /* Simply attempt to open an FTP Server, on RFCOMM Server   */
            /* Port specified, with the correct directory, with all the */
            /* permissions.                                             */
            ret_val = FTPM_Open_Server(TempParam->Params[0].intParam, (unsigned long)TempParam->Params[1].intParam, RootPath, (FTPM_SERVER_PERMISSION_MASK_READ | FTPM_SERVER_PERMISSION_MASK_WRITE | FTPM_SERVER_PERMISSION_MASK_DELETE), FTPM_Event_Callback, NULL);

            /* If the Open was successful, then note the FTP Server ID. */
            if(ret_val > 0)
            {
               /* Note the FTP Server ID of the opened FTP Server.      */
               FTPConnectionID = ret_val;

               /* The Server was opened successfully, now register a SDP*/
               /* Record indicating that an FTP Server exists.  Do this */
               /* by first creating a Service Name.                     */
               sprintf(ServiceName, "FTP Server Port %u", TempParam->Params[0].intParam);

               /* Now that a Service Name has been created try to       */
               /* Register the SDP Record.                              */
               ret_val = FTPM_Register_Server_SDP_Record(FTPConnectionID, ServiceName, &FTPServerSDPHandle);

               /* If there was an error creating the FTP Server's SDP   */
               /* Service Record then go ahead an close down the server */
               /* an flag an error.                                     */
               if(ret_val < 0)
               {
                  printf("Unable to Register FTP Server SDP Record, Error = %d.\r\n", ret_val);

                  FTPM_Close_Server(FTPConnectionID);

                  /* Flag that there is no longer an FTP Server Open.   */
                  FTPConnectionID = 0;
               ...
               else
               {
                  ...
                  /* correctly.                                         */
                  printf("FTP Server Opened (Channel = %u, Root Directory = %s).\r\n", TempParam->Params[0].intParam, RootPath);
            ...
            {
               printf("Unable to Open FTP Server, Error = %d.\r\n", ret_val);
         ...
         {
            printf("Usage: OpenServer [RFCOMM Server Port] [Open Flags: Bit Mask (1: Authorization, 2: Authentication, 4: Encryption)] [Root Directory Path (optional)].\r\n");
      ...
      {
         printf("Unable to open FTP Server.\r\nFTP Server is already open.\r\n");
   ...
   /* The following function is responsible for closing an FTP Server   */
...
static int CloseServer(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* If an FTP Server is already opened, then simply close it.      */
      if(FTPConnectionID)
      {
         /* Finally close the FTP Server.                               */
         ret_val = FTPM_Close_Server(FTPConnectionID);

         if(ret_val < 0)
         {
            printf("FTPM_Close_Server(): Error %d\r\n", ret_val);
         ...
         /* Flag that there is no FTP Server currently open.            */
         FTPConnectionID = 0;
      ...
      {
         printf("There is NO FTP Server currently open.\r\n");
...
static int OpenServerPortRequestResponse(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Verify that an FTP Server Port is currently opened.            */
      if(FTPConnectionID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            if((Result = FTPM_Server_Connect_Request_Response(FTPConnectionID, (Boolean_t)TempParam->Params[1].intParam)) == 0)
            {
               printf("FTPM_Server_Connect_Request_Response(%s) Success: %d.\r\n", TempParam->Params[1].intParam?"Accept":"Reject", Result);
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* The following function is the FTPM Server Event Callback function */
   /* that is Registered with the FTP Manager Module.  This callback is */
   /* responsible for processing all FTPM Server Events.                */
static void BTPSAPI FTPM_Event_Callback(FTPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      switch(EventData->EventType)
      {
         case etFTPMIncomingConnectionRequest:
            /* A Connection Request Indication was received, this means */
            /* that the Client is requesting to connect to the Server.  */
            /* Display the appropriate message.                         */
            printf("\r\n");
            BD_ADDRToStr(EventData->EventData.ServerIncomingConnectionRequestEventData.RemoteDeviceAddress, BoardStr);
            printf("etFTPMIncomingConnectRequestIndication, FTPConnectionID: 0x%04X.", EventData->EventData.ServerIncomingConnectionRequestEventData.FTPConnectionID);
            printf("\r\nCall OpenServerPortRequestResponse.");
            printf("\r\nServer>");
            break;
         case etFTPMConnectIndication:
            /* A Client has connected to the Server, display the BD_ADDR*/
            /* of the connecting Device.                                */
            printf("\r\n");
            printf("etFTPMConnectIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerConnectIndicationEventData.FTPConnectionID);
            BD_ADDRToStr(EventData->EventData.ServerConnectIndicationEventData.RemoteDeviceAddress, BoardStr);
            printf("BD_ADDR: %s", BoardStr);
            printf("\r\nFTPM>");
         ...
         case etFTPMDisconnectIndication:
            /* A Disconnect Indication was received, this means that the*/
            /* Client has disconnected from the Server.  Display the    */
            /* appropriate message.                                     */
            printf("\r\n");
            printf("etFTPMDisconnectIndication, FTPConnectionID: 0x%04X.", EventData->EventData.DisconnectIndicationEventData.FTPConnectionID);
            printf("\r\nFTPM>");

            /* Flag that we are no longer connected.                    */
            Connected = FALSE;
            break;
         case etFTPMDirectoryRequestIndication:
            /* The Client requested a list of the contents of the       */
            /* currently occupied Directory.  Display a message         */
            /* indicating this and the Current Directory occupied by the*/
            /* Client.                                                  */
            printf("\r\n");
            printf("etFTPMDirectoryRequestIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerDirectoryRequestIndicationEventData.FTPConnectionID);
            printf("   Directory: ");
            printf("%s", EventData->EventData.ServerDirectoryRequestIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMChangeDirectoryIndication:
            /* The Client requested to change to a New Directory.       */
            /* Display a message indicating this and the Current        */
            /* Directory after the change.                              */
            printf("\r\n");
            printf("etFTPMChangeDirectoryIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerChangeDirectoryIndicationEventData.FTPConnectionID);
            printf("   Directory: ");
            printf("%s", EventData->EventData.ServerChangeDirectoryIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMDirectoryDeleteIndication:
            /* The Client requested to Delete a Directory.  Display a   */
            /* message indicating this, the Directory to be Deleted, and*/
            /* the Current Directory.                                   */
            printf("\r\n");
            printf("etFTPMDirectoryDeleteIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerDirectoryDeleteIndicationEventData.FTPConnectionID);
            printf("   Deleted Directory: ");
            printf("%s", EventData->EventData.ServerDirectoryDeleteIndicationEventData.DeletedDirectoryName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerDirectoryDeleteIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMDirectoryCreateIndication:
            /* The Client requested to Create a Directory.  Display a   */
            /* message indicating this, the Directory to be created, and*/
            /* the Current Directory.                                   */
            printf("\r\n");
            printf("etFTPMDirectoryCreateIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerDirectoryCreateIndicationEventData.FTPConnectionID);
            printf("   Created Directory: ");
            printf("%s", EventData->EventData.ServerDirectoryCreateIndicationEventData.CreatedDirectoryName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerDirectoryCreateIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMFileDeleteIndication:
            /* The Client requested to delete a file.  Display a message*/
            /* indicating this, the name of the file being deleted, and */
            /* the current directory.                                   */
            printf("\r\n");
            printf("etFTPMFileDeleteIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerFileDeleteIndicationEventData.FTPConnectionID);
            printf("   Deleted File: ");
            printf("%s", EventData->EventData.ServerFileDeleteIndicationEventData.DeletedFileName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerFileDeleteIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMFileCreateIndication:
            /* The Client requested to Create a File.  Display a message*/
            /* indicating this, the Create File to be created, and the  */
            /* Current Directory.                                       */
            printf("\r\n");
            printf("etFTPMFileCreateIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerFileCreateIndicationEventData.FTPConnectionID);
            printf("   Created File: ");
            printf("%s", EventData->EventData.ServerFileCreateIndicationEventData.CreatedFileName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerFileCreateIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMFilePutIndication:
            /* The Client requested the transfer of a File to the       */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etFTPMFilePutIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ServerFilePutIndicationEventData.FTPConnectionID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (EventData->EventData.ServerFilePutIndicationEventData.TransferComplete?"TRUE":"FALSE"),
                                                                                                     EventData->EventData.ServerFilePutIndicationEventData.TotalLength,
                                                                                                     EventData->EventData.ServerFilePutIndicationEventData.TransferredLength);
            printf("   File: ");
            printf("%s", EventData->EventData.ServerFilePutIndicationEventData.FileName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerFilePutIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         case etFTPMFileGetIndication:
            /* The Client requested the transfer of a File from the     */
            /* Server.  Display all relevant information.               */
            printf("\r\n");
            printf("etFTPMFileGetIndication, FTPConnectionID: 0x%08X\r\n", EventData->EventData.ServerFileGetIndicationEventData.FTPConnectionID);
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (EventData->EventData.ServerFileGetIndicationEventData.TransferComplete?"TRUE":"FALSE"),
                                                                                                     EventData->EventData.ServerFileGetIndicationEventData.TotalLength,
                                                                                                     EventData->EventData.ServerFileGetIndicationEventData.TransferredLength);
            printf("   File: ");
            printf("%s", EventData->EventData.ServerFileGetIndicationEventData.FileName);
            printf("\r\n");
            printf("   Current Directory: ");
            printf("%s", EventData->EventData.ServerFileGetIndicationEventData.DirectoryName);
            printf("\r\nFTPM>");
            break;
         default:
