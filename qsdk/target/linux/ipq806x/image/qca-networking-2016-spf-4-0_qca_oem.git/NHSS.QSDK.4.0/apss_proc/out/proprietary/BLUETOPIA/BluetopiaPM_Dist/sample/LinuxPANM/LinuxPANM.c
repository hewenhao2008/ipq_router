...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXPANM -  Simple Linux Application using Bluetooth Platform Manager    */
/*               Personal Area Network Manager API.                           */
/*                                                                            */
/*  Author:  Matt Seabold                                                     */
/*                                                                            */
...
#include "LinuxPANM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTPANM.h"     /* PAN Manager Application Programming Interface.  */
...
static int QueryConnectedDevices(ParameterList_t *TempParam);
static int QueryCurrentConfiguration(ParameterList_t *TempParam);
static int ChangeIncomingConnectionFlags(ParameterList_t *TempParam);
static int PANRegisterEventCallback(ParameterList_t *TempParam);
static int PANUnRegisterEventCallback(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM Personal Area Network Manager Callback function prototype.   */
static void BTPSAPI PANM_Event_Callback(PANM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("QUERYCONNECTEDDEVICES", QueryConnectedDevices);
   AddCommand("QUERYCURRENTCONFIGURATION", QueryCurrentConfiguration);
   AddCommand("CHANGEINCOMINGCONNECTIONFLAGS", ChangeIncomingConnectionFlags);
   AddCommand("PANREGISTEREVENTCALLBACK", PANRegisterEventCallback);
   AddCommand("PANUNREGISTEREVENTCALLBACK", PANUnRegisterEventCallback);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  34)ConnectRemoteServer                        *\r\n");
   printf("*                  35)CloseConnection                            *\r\n");
   printf("*                  36)QueryConnectedDevices                      *\r\n");
   printf("*                  37)QueryCurrentConfiguration                  *\r\n");
   printf("*                  38)ChangeIncomingConnectionFlags              *\r\n");
   printf("*                  39)PANRegisterEventCallback                   *\r\n");
   printf("*                  40)PANUnRegisterEventCallback                 *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* PAN Connection Request.  This function returns zero on successful */
   /* execution and a negative value on all errors.                     */
static int ConnectionRequestResponse(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
      {
         ...
         printf("Attempting to respond %s to Open Request.\r\n", (TempParam->Params[1].intParam)?"TRUE":"FALSE");

         /* Attempt to submit our response.                             */
         if(!(Result = PANM_Connection_Request_Response(BD_ADDR, (Boolean_t)TempParam->Params[1].intParam)))
         {
            /* Open Request Response was successful, go ahead and inform*/
            /* the User.                                                */
            printf("PANM_Connection_Request_Response() Success: %d.\r\n", Result);
         ...
         {
            /* Error submitting the response, inform the user.          */
            printf("PANM_Connection_Request_Response() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: ConnectionRequestResponse [BD_ADDR] [Reject/Accept - 0/1].\r\n");
   ...
   /* a remote PAN Server.  This function returns zero on successful    */
   /* execution and a negative value on all errors.                     */
static int ConnectRemoteServer(ParameterList_t *TempParam)
{
   int       Result;
   ...
   {
      /* Make sure the incoming parameters are semi-valid.              */
      if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].strParam) && (TempParam->Params[1].intParam) && (TempParam->Params[2].intParam))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         printf("Attempting to Open Remote Server.\r\n");

         /* Attempt to open the remote server.                          */
         if(!(Result = PANM_Connect_Remote_Device(BD_ADDR, (PAN_Service_Type_t)(TempParam->Params[1].intParam - 1), (PAN_Service_Type_t)(TempParam->Params[2].intParam - 1), (TempParam->NumberofParameters > 3)?TempParam->Params[3].intParam:0, PANM_Event_Callback, NULL, NULL)))
         {
            /* Open successful, go ahead and inform the User.           */
            printf("PANM_Connect_Remote_Device() Success: %d.\r\n", Result);
         ...
         {
            /* Error opening the remote server, inform the user.        */
            printf("PANM_Connect_Remote_Device() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: ConnectRemoteServer [BD_ADDR] [Local Service Type (User = 1, NAP = 2, Group Ad Hoc = 3)] [Remote Service Type (User = 1, NAP = 2, Group Ad Hoc = 3)] [Open Flags (1 = Authenticate, 2 = Encrypt) (Optional)].\r\n");
   ...
   /* opened PAN Connection. This function returns zero on successful   */
   /* execution and a negative value on all errors.                     */
static int CloseConnection(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Attempt to open the remote server.                          */
         if(!(Result = PANM_Close_Connection(BD_ADDR)))
         {
            /* Close successful, go ahead and inform the User.          */
            printf("PANM_Close_Connection() Success: %d.\r\n", Result);
         ...
         {
            /* Error closing the remote server, inform the user.        */
            printf("PANM_Close_Connection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: CloseConnection [BD_ADDR].\r\n");
   ...
   /* connected PAN devices.  This function returns zero on successful  */
   /* execution and a negative value on all errors.                     */
static int QueryConnectedDevices(ParameterList_t *TempParam)
{
   int          Index;
   ...
   if(Initialized)
   {
      ...
      if(TempParam)
      {
         /* Attempt to query the number of connected devices.           */
         if(!(Result = PANM_Query_Connected_Devices(0, NULL, &TotalNumberConnectedDevices)))
         {
            /* Check that there is at least one connected devices.      */
            if(TotalNumberConnectedDevices)
            {
               /* Allocate storage for the device list.                 */
               if((ConnectedDevices = (BD_ADDR_t *)BTPS_AllocateMemory(TotalNumberConnectedDevices * sizeof(BD_ADDR_t))) != NULL)
               {
                  /* Query the actual list of connected devices.        */
                  if((Result = PANM_Query_Connected_Devices(TotalNumberConnectedDevices, ConnectedDevices, NULL)) >= 0)
                  {
                     /* Query successful, go ahead and inform the User. */
                     printf("PANM_Query_Connected_Devices() Success: %d.\r\n", Result);
                     printf("\r\n");
                     printf("Connected Devices:\r\n");

                     for(Index = 0; Index < Result; Index++)
                     {
                        BD_ADDRToStr(ConnectedDevices[Index], Buffer);
                        printf("     %s\r\n", Buffer);
                     }
                  ...
                  {
                     /* Error querying the actual list of devices,      */
                     /* inform the user.                                */
                     printf("PANM_Query_Connected_Devices() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

                     ret_val = FUNCTION_ERROR;
                  }

                  BTPS_FreeMemory(ConnectedDevices);
               }
               else
               {
                  /* Unable to allocate memory for List.                */
            ...
            {
               /* Querying the number of connected devices was          */
               /* successful, but no devices are connected. Inform the  */
               /* user.                                                 */
               printf("PANM_Query_Connected_Devices() Success: %d, No devices connected.\r\n", Result);
         ...
         {
            /* Error querying the number of connected devices, inform   */
            /* the user.                                                */
            printf("PANM_Query_Connected_Devices() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: QueryConnectedDevices.\r\n");
   ...
   /* The following function is responsible for querying the current PAN*/
   /* Manager Configuration.  This function returns zero on successful  */
   /* execution and a negative value on all errors.                     */
static int QueryCurrentConfiguration(ParameterList_t *TempParam)
{
   int                          Result;
   int                          ret_val;
   PANM_Current_Configuration_t CurrentConfiguration;
   ...
   if(Initialized)
   {
      ...
      {
         /* Attempt to open the remote server.                          */
         if(!(Result = PANM_Query_Current_Configuration(&CurrentConfiguration)))
         {
                     /* Query successful, go ahead and inform the User. */
                     printf("PANM_Query_Current_Configuration() Success: %d.\r\n", Result);
                     printf("\r\n");
                     printf("Service Types: 0x%08lx (", CurrentConfiguration.ServiceTypeFlags);

                     if(CurrentConfiguration.ServiceTypeFlags & PAN_PERSONAL_AREA_NETWORK_USER_SERVICE)
                        printf("PAN User");

                     if(CurrentConfiguration.ServiceTypeFlags & PAN_NETWORK_ACCESS_POINT_SERVICE)
                     {
                        if(CurrentConfiguration.ServiceTypeFlags & PAN_PERSONAL_AREA_NETWORK_USER_SERVICE)
                           printf(", ");
                     ...
                     if(CurrentConfiguration.ServiceTypeFlags & PAN_GROUP_ADHOC_NETWORK_SERVICE)
                     {
                        if((CurrentConfiguration.ServiceTypeFlags & PAN_PERSONAL_AREA_NETWORK_USER_SERVICE) || (CurrentConfiguration.ServiceTypeFlags & PAN_GROUP_ADHOC_NETWORK_SERVICE))
                           printf(", ");

                        printf("Group Adhoc Network");
                     }
                     
                     printf(")\r\n");

                     printf("Incoming Connection Flags: 0x%08lx (", CurrentConfiguration.IncomingConnectionFlags);

                     if(CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_AUTHORIZATION)
                        printf("Authorization");

                     if(CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_AUTHENTICATION)
                     {
                        if(CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_AUTHORIZATION)
                           printf(", ");
                     ...
                     if(CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_ENCRYPTION)
                     {
                        if((CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_AUTHORIZATION) || (CurrentConfiguration.IncomingConnectionFlags & PANM_INCOMING_CONNECTION_FLAGS_REQUIRE_ENCRYPTION))
                           printf(", ");
         ...
         {
            /* Error querying the configuration, inform the user.       */
            printf("PANM_Query_Current_Configuration() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: QueryCurrentConfiguration.\r\n");
...
//xxx Implement
   ...
   /* Connection Flags that control how incoming PAN Open Requests are  */
   /* handled. This function returns zero on successful execution and a */
...
static int ChangeIncomingConnectionFlags(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         printf("Change Incoming Connection Flags to %d.\r\n", TempParam->Params[0].intParam);

         /* Attempt to change the Connection Flags.                     */
         if(!(Result = PANM_Change_Incoming_Connection_Flags(TempParam->Params[0].intParam)))
         {
            /* Change successful, go ahead and inform the User.         */
            printf("PANM_Change_Incoming_Connection_Flags() Success: %d.\r\n", Result);
         ...
         {
            /* Error changing the flags, inform the user.               */
            printf("PANM_Change_Incoming_Connection_Flags() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         printf("Usage: ChangeIncomingConnectionFlags [Flags (1 = Authorization, 2 = Authentication, 4 = Encryption)].\r\n");
   ...
   /* The following function is responsible for registering the local   */
   /* module to receive PAN events. This function returns zero on       */
...
static int PANRegisterEventCallback(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      printf("Attempting to Register for Events.\r\n");

      /* Attempt to open the remote server.                             */
      if((Result = PANM_Register_Event_Callback(PANM_Event_Callback, NULL)) > 0)
      {
         /* Registration successful, go ahead and inform the User.      */
         printf("PANM_Register_Event_Callback() Success. Callback ID: %d.\r\n", Result);
      ...
      else
      {
         ...
         printf("PANM_Register_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
   ...
   /* The following function is responsible for un-registering the local*/
   /* module from receiving PAN Events. This function returns zero on   */
   /* successful execution and a negative value on all errors.          */
static int PANUnRegisterEventCallback(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         ...
         PANM_Un_Register_Event_Callback(TempParam->Params[0].intParam);

         printf("PANM_Un_Register_Event_Callback() Success.\r\n");
      ...
      {
         printf("Usage: UnRegisterEvents [Callback ID].\r\n");
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter)
{
   ...
   /* The following function is the Personal Area Network Event Callback*/
   /* function that is Registered with the PAN Manager. This callback is*/
   /* responsible for processing all Personal Area Network Events.      */
static void BTPSAPI PANM_Event_Callback(PANM_Event_Data_t *EventData, void *CallbackParameter)
{
   char Buffer[32];
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case petPANMIncomingConnectionRequest:
            ...
            printf("PANM: Incoming Connection Request Received\r\n");

            BD_ADDRToStr(EventData->EventData.IncomingConnectionReqeustEventData.RemoteDeviceAddress, Buffer);
            printf("    BD_ADDR: %s\r\n", Buffer);

            printf("Respond with the command: ConnectionRequestResponse\r\n");
            break;
         case petPANMConnected:
            printf("PANM: Device Connected\r\n");

            BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, Buffer);
            printf("    BD_ADDR     : %s\r\n", Buffer);
            printf("    Service Type: %s\r\n", (EventData->EventData.ConnectedEventData.ServiceType == pstPersonalAreaNetworkUser)?"PAN User":(EventData->EventData.ConnectedEventData.ServiceType == pstNetworkAccessPoint)?"Network Access Point":"Group Ad Hoc");
            break;
         case petPANMConnectionStatus:
            printf("PANM: Connection Status\r\n");

            BD_ADDRToStr(EventData->EventData.ConnectionStatusEventData.RemoteDeviceAddress, Buffer);
            printf("    BD_ADDR     : %s\r\n", Buffer);
            printf("    Service Type: %s\r\n", (EventData->EventData.ConnectionStatusEventData.ServiceType == pstPersonalAreaNetworkUser)?"PAN User":(EventData->EventData.ConnectionStatusEventData.ServiceType == pstNetworkAccessPoint)?"Network Access Point":"Group Ad Hoc");
            printf("    Status      : %u\r\n", EventData->EventData.ConnectionStatusEventData.Status);
            break;
         case petPANMDisconnected:
            printf("PANM: Device Disconnected\r\n");

            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, Buffer);
            printf("    BD_ADDR     : %s\r\n", Buffer);
            printf("    Service Type: %s\r\n", (EventData->EventData.DisconnectedEventData.ServiceType == pstPersonalAreaNetworkUser)?"PAN User":(EventData->EventData.DisconnectedEventData.ServiceType == pstNetworkAccessPoint)?"Network Access Point":"Group Ad Hoc");
            break;
         default:
            printf("Unhandled PANM Event Received.\r\n");
            break;
      }
   }
   else
      printf("PANM Event Data is NULL.\r\n");
