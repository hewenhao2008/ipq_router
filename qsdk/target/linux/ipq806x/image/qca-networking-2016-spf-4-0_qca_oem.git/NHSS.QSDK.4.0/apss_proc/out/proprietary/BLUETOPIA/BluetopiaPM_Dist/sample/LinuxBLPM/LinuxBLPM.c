...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBLPM - Simple Linux application using Bluetopia Platform Manager     */
/*              Blood Pressure Profile (BLP) Manager Application Programming  */
/*              Interface (API).                                              */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   01/08/13  R. Byrne       Initial creation. (Based on LinuxBASM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxBLPM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTBLPM.h"     /* BLP Manager Application Programming Interface.  */
...
#ifndef BLS_ASSIGN_BLS_SERVICE_UUID_16
...
static int BLPRegisterCollectorEvents(ParameterList_t *TempParam);
static int BLPUnRegisterCollectorEvents(ParameterList_t *TempParam);
static int BLPEnableBPMIndications(ParameterList_t *TempParam);
static int BLPDisableBPMIndications(ParameterList_t *TempParam);
static int BLPEnableICPNotifications(ParameterList_t *TempParam);
static int BLPDisableICPNotifications(ParameterList_t *TempParam);
static int BLPGetBloodPressureFeature(ParameterList_t *TempParam);
static int BLPCancelTransaction(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM BLP Manager Callback function prototype.                     */
static void BTPSAPI BLPM_Event_Callback(BLPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("BLPREGISTERCOLLECTOREVENTS", BLPRegisterCollectorEvents);
   AddCommand("BLPUNREGISTERCOLLECTOREVENTS", BLPUnRegisterCollectorEvents);
   AddCommand("BLPENABLEBPMINDICATIONS", BLPEnableBPMIndications);
   AddCommand("BLPDISABLEBPMINDICATIONS", BLPDisableBPMIndications);
   AddCommand("BLPENABLEICPNOTIFICATIONS", BLPEnableICPNotifications);
   AddCommand("BLPDISABLEICPNOTIFICATIONS", BLPDisableICPNotifications);
   AddCommand("BLPGETBLOODPRESSUREFEATURE", BLPGetBloodPressureFeature);
   AddCommand("BLPCANCELTRANSACTION", BLPCancelTransaction);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      BLS_ASSIGN_BLS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)BLPRegisterCollectorEvents                 *\r\n");
   printf("*                  36)BLPUnRegisterCollectorEvents               *\r\n");
   printf("*                  37)BLPEnableBPMIndications                    *\r\n");
   printf("*                  38)BLPDisableBPMIndications                   *\r\n");
   printf("*                  39)BLPEnableICPNotifications                  *\r\n");
   printf("*                  40)BLPDisableICPNotifications                 *\r\n");
   printf("*                  41)BLPGetBloodPressureFeature                 *\r\n");
   printf("*                  42)BLPCancelTransaction                       *\r\n");
...
static int QueryRemoteDeviceServices(ParameterList_t *TempParam)
{
   ...
   /* Collector to receive (and process) Authentication Events.  This   */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int RegisterAuthentication(ParameterList_t *TempParam)
{
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the BLP*/
   /* Manager to receive BLP Events.  This function returns zero if     */
   /* successful and a negative value if an error occurred.             */
static int BLPRegisterCollectorEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if(!EventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = BLPM_Register_Collector_Event_Callback(BLPM_Event_Callback, NULL)) > 0)
         {
            printf("BLPM_Register_Collector_Event_Callback() Success: %d.\r\n", Result);
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("BLPM_Register_Collector_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("BLP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered BLP Event Registration with the BLP Manager.*/
...
static int BLPUnRegisterCollectorEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         BLPM_Un_Register_Collector_Event_Callback(EventCallbackID);

         printf("BLPM_Un_Register_Collector_Event_Callback() Success.\r\n");
      ...
      else
      {
         ...
         printf("BLP Manager Event Callback is not registered.\r\n");
   ...
   /* on a remote Battery Service Instance.  A callback must be         */
   /* registered with the BLP Module before calling this command.  This */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int BLPEnableBPMIndications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BLPM_Enable_Blood_Pressure_Indications(EventCallbackID, BluetoothAddress)) == 0)
            {
               printf("BLPM_Enable_Blood_Pressure_Indications() Success.\r\n");
            }
            else
            {
               printf("BLPM_Enable_Blood_Pressure_Indications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPEnableBPMIndications [BD_ADDR].\r\n");
         }
      }
      else
      {
         ...
         printf("An event callback must be registered with the BLPM Manager. Use BLPRegisterCollectorEvents.\r\n");
      }
   }
   else
   {
   ...
   /* The following function is responsible for disabling notifications */
   /* on a remote Battery Service Instance.  A callback must be         */
   /* registered with the BLP Module before calling this command.  This */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int BLPDisableBPMIndications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BLPM_Disable_Blood_Pressure_Indications(EventCallbackID, BluetoothAddress)) == 0)
            {
               printf("BLPM_Disable_Blood_Pressure_Indications() Success.\r\n");
            }
            else
            {
               printf("BLPM_Disable_Blood_Pressure_Indications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPDisableBPMIndications [BD_ADDR].\r\n");
         }
      }
      else
      {
   ...
   /* Level from a remote Battery Service Instance.  A callback must be */
   /* registered with the BLP Module before calling this command.  This */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int BLPEnableICPNotifications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BLPM_Enable_Intermediate_Cuff_Pressure_Notifications(EventCallbackID, BluetoothAddress)) == 0)
            {
               printf("BLPM_Enable_Intermediate_Cuff_Pressure_Notifications() Success.\r\n\n");

               ret_val = 0;
            }
            else
            {
               printf("BLPM_Enable_Intermediate_Cuff_Pressure_Notifications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPEnableICPNotifications [BD_ADDR].\r\n");
         }
      }
      else
      {
   ...
   /* The following function is responsible for reading the Battery     */
   /* Identification from a remote Battery Service Instance.  A callback*/
   /* must be registered with the BLP Module before calling this        */
   /* command.  This function returns zero if successful and a negative */
   /* value if an error occurred.                                       */
static int BLPDisableICPNotifications(ParameterList_t *TempParam)
{
   BD_ADDR_t BluetoothAddress;
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BLPM_Disable_Intermediate_Cuff_Pressure_Notifications(EventCallbackID, BluetoothAddress)) == 0)
            {
               printf("BLPM_Disable_Intermediate_Cuff_Pressure_Notifications() Success.\r\n\n");

               ret_val = 0;
            }
            else
            {
               printf("BLPM_Disable_Intermediate_Cuff_Pressure_Notifications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPDisableICPNotifications [BD_ADDR].\r\n");
         }
      }
      else
      {
...
static int BLPGetBloodPressureFeature(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BLPM_Get_Blood_Pressure_Feature(EventCallbackID, BluetoothAddress)) > 0)
            {
               printf("BLPM_Get_Blood_Pressure_Feature() Success.\r\n\n");
            ...
            {
               printf("BLPM_Get_Blood_Pressure_Feature() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPGetBloodPressureFeature [BD_ADDR].\r\n");
         }
      }
      else
      {
   ...
   /* The following function is responsible for canceling a transaction */
   /* with a remote Battery Service Instance.  A callback must be       */
   /* registered with the BLP Module before calling this command.  This */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int BLPCancelTransaction(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         {
            /* Submit the command.                                      */
            if((ret_val = BLPM_Cancel_Transaction(EventCallbackID, TempParam->Params[0].intParam)) == 0)
            {
               printf("BLPM_Cancel_Transaction() Success.\r\n");
            }
            else
            {
               printf("BLPM_Cancel_Transaction() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BLPCancelTransaction [TransactionID].\r\n");
         }
      }
      else
      {
...
static void DisplayBloodPressureEventData(BLPM_Blood_Pressure_Measurement_Event_Data_t * EventData)
{
   printf("   Compound Pressure Value\r\n");
   printf("      Systolic:      %u %s\r\n", EventData->SystolicPressure, EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_UNITS_KPA ? "kPa" : "mmHg");
   printf("      Diastolic:     %u %s\r\n", EventData->DiastolicPressure, EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_UNITS_KPA ? "kPa" : "mmHg");
   printf("      Mean Arterial: %u %s\r\n\n", EventData->MeanArterialPressure, EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_UNITS_KPA ? "kPa" : "mmHg");

   printf("   Flags:              0x%02X\r\n", EventData->MeasurementFlags);
   if(EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_TIME_STAMP_PRESENT)
      printf("   Time Stamp:         %02u/%02u/%02u, %02u:%02u:%02u\r\n", EventData->TimeStamp.Day, EventData->TimeStamp.Month, EventData->TimeStamp.Year, EventData->TimeStamp.Hours, EventData->TimeStamp.Minutes, EventData->TimeStamp.Seconds);
   else
      printf("   Time Stamp:         Not Present\r\n");

   if(EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_PULSE_RATE_PRESENT)
      printf("   Pulse Rate:         %u\r\n", EventData->PulseRate);
   else
      printf("   Pulse Rate:         Not Present\r\n");

   if(EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_USER_ID_PRESENT)
      printf("   User ID:            %u\r\n", EventData->UserID);
   else
      printf("   User ID:            Not Present\r\n");

   if(EventData->MeasurementFlags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_MEASUREMENT_STATUS_PRESENT)
      printf("   Measurement Status: %u\r\n", EventData->MeasurementStatus);
   else
      printf("   Measurement Status: Not Present\r\n");
}

static void DisplayCuffPressureEventData(BLPM_Intermediate_Cuff_Pressure_Event_Data_t * EventData)
{
   printf("   Cuff Pressure:      %u %s\r\n", EventData->IntermediateCuffPressure, EventData->Flags & BLPM_INTERMEDIATE_CUFF_PRESSURE_FLAGS_UNITS_KPA ? "kPa" : "mmHg");
   printf("   Flags:              0x%02X\r\n", EventData->Flags);

   if(EventData->Flags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_TIME_STAMP_PRESENT)
      printf("   Time Stamp:         %02u/%02u/%02u, %02u:%02u:%02u\r\n", EventData->TimeStamp.Day, EventData->TimeStamp.Month, EventData->TimeStamp.Year, EventData->TimeStamp.Hours, EventData->TimeStamp.Minutes, EventData->TimeStamp.Seconds);
   else
      printf("   Time Stamp:         Not Present\r\n");

   if(EventData->Flags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_PULSE_RATE_PRESENT)
      printf("   Pulse Rate:         %u\r\n", EventData->PulseRate);
   else
      printf("   Pulse Rate:         Not Present\r\n");

   if(EventData->Flags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_USER_ID_PRESENT)
      printf("   User ID:            %u\r\n", EventData->UserID);
   else
      printf("   User ID:            Not Present\r\n");

   if(EventData->Flags & BLPM_BLOOD_PRESSURE_MEASUREMENT_FLAGS_MEASUREMENT_STATUS_PRESENT)
      printf("   Measurement Status: %u\r\n", EventData->MeasurementStatus);
   else
      printf("   Measurement Status: Not Present\r\n");
}
...
static void BTPSAPI BLPM_Event_Callback(BLPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case betBLPConnected:
            printf("betBLPConnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BLPM_CONNECTED_EVENT_DATA_SIZE)
            {
               ...
               printf("   Connection Type:     %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == bctSensor) ? "Sensor" : "Collector");
            }
            break;
         case betBLPDisconnected:
            printf("betBLPDisconnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BLPM_DISCONNECTED_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID:  %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address:  %s\r\n", BoardStr);
               printf("   Disconnected Flags: 0x%08X\r\n", (unsigned int)EventData->EventData.DisconnectedEventData.DisconnectedFlags);
               printf("   Connection Type:    %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == bctSensor) ? "Sensor" : "Collector");
            }
            break;
         case betBLPBloodPressureMeasurement:
            printf("betBLPBloodPressureMeasurement with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BLPM_BLOOD_PRESSURE_MEASUREMENT_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.BloodPressureMeasurementEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n\n", BoardStr);

               DisplayBloodPressureEventData(&(EventData->EventData.BloodPressureMeasurementEventData));
            }
            break;
         case betBLPIntermediateCuffPressure:
            printf("betBLPIntermediateCuffPressure with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BLPM_INTERMEDIATE_CUFF_PRESSURE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.IntermediateCuffPressureEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n\n", BoardStr);

               DisplayCuffPressureEventData(&(EventData->EventData.IntermediateCuffPressureEventData));
            }
            break;
         case betBLPBloodPressureFeatureResponse:
            printf("betBLPBloodPressureFeatureResponse with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BLPM_BLOOD_PRESSURE_FEATURE_RESPONSE_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.BloodPressureFeatureResponseEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n", BoardStr);
               printf("   TransactionID:     %u\r\n", EventData->EventData.BloodPressureFeatureResponseEventData.TransactionID);
               printf("   Status:            %d\r\n", EventData->EventData.BloodPressureFeatureResponseEventData.Status);

               /* Check for a successful status.                        */
               if(!EventData->EventData.BloodPressureFeatureResponseEventData.Status)
               {
                  printf("\n");
                  printf("   Feature:         0x%04X\r\n", EventData->EventData.BloodPressureFeatureResponseEventData.Feature);
               }
            }
            break;
         default:
            printf("Unknown BLP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nBLPM Event Data is NULL.\r\n");
