...
#define DEVM_ASSIGN_ADD_REMOTE_DEVICE_DUAL_MODE_CLASS_OF_DEVICE(_x) \
   ...
   /* The following is a type definition of the User Supplied Device    */
   /* Manager Callback function that can be registered to be called back*/
   /* from the Local Device Manager when the Power for the actual device*/
   /* needs to be changed.  This function is called in an arbitrary     */
   /* thread context and is useful for implementations that require an  */
   /* external mechanism to control power to the device.  This function */
   /* passes to the state that is required (TRUE or FALSE), followed by */
   /* the Callback parameter that was registered when the Callback was  */
   /* installed.  If the State parameter is TRUE, then this callback is */
   /* instructing the caller to Physically Power On the device so that  */
   /* the Bluetooth Stack can communicate with the device.  if the State*/
   /* parameter is FALSE, then this callback is instructing the caller  */
   /* to Physically Power Down the device because communication is no   */
   /* longer required.                                                  */
   /* * NOTE * This is a VERY low level callback.  As such NO other     */
   /*          Bluetopia Platform Manager calls should be made !!!!!!!! */
   /* * NOTE * This callback can ONLY be specified in the Initialization*/
   /*          Data parameters when the Device Manager is initialized.  */
   /*          Once it has been set, it cannot be changed.              */
typedef void (BTPSAPI *DEVM_Power_Device_Callback_t)(Boolean_t State, void *CallbackParameter);

   /* The following structure is used with the                          */
   /* DEVM_Default_Initialization_Data_t to specify the minimum minimum */
   /* connection interval and minimum maximum connection interval that  */
   /* we will accept in an LE Connection Parameter Update Request.      */
   /* allowable                                                         */
   /* * NOTE * If the                                                   */
   /*     DEVM_DEFAULT_ALLOWABLE_LE_PARAMETERS_MINIMUM_INTERVALS_VALID  */
   /*          flag is set then both the Minimum_Minimum_Interval and   */
   /*          Minimum_Maximum_Interval must contain valid values.      */
typedef struct _tagDEVM_Default_Allowable_LE_Parameters_Data_t
{
   Word_t Flags;
   Word_t Minimum_Minimum_Interval;
   Word_t Minimum_Maximum_Interval;
} DEVM_Default_Allowable_LE_Parameters_Data_t;

#define DEVM_DEFAULT_ALLOWABLE_LE_PARAMETERS_DATA_SIZE                  (sizeof(DEVM_Default_Allowable_LE_Parameters_Data_t))

   /* The following represents the valid bits that may be set in the    */
   /* Flags member of the DEVM_Default_Allowable_LE_Parameters_Data_t   */
   /* structure.                                                        */
#define DEVM_DEFAULT_ALLOWABLE_LE_PARAMETERS_MINIMUM_INTERVALS_VALID    0x0001

   /* The following structure is used with the                          */
   /* DEVM_Initialization_Data_t structure to specify default           */
   /* initiailization override values.  These values, if specified (note*/
   /* the Bitmask) inform the Local Device Manager which values should  */
   /* be used to override the compiled default values.                  */
   /* * NOTE * These values are only the default values.  If there is   */
   /*          stored configuration data that has any of these values   */
   /*          specified then the stored configuration values are used. */
   /* * NOTE * The manufacturing data (DefaultAdvertisingData), if      */
   /*          specified, must be a raw blob with the following data:   */
   /*                2 byte Company Identifier (Little Endian)          */
   /*                Manufacturer Specific Data.                        */
   /* * NOTE * The advertising data and scan response                   */
   /*          data(DefaultAdvertisingData and DefaultScanResponseData  */
   /*          respectively), if specified, must a binary blob made up  */
   /*          of Length (1 byte) - Tag (1 byte) - Value (arbitrary)    */
   /*          (LTV) structures.                                        */
   /* * NOTE * If the DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_NON_   */
   /*          INTERLEAVED_TIMING flag is set then the                  */
   /*          DefaultMinLENonInterleavedAdvertisingDurationMS and      */
   /*          DefaultMinLENonInterleavedAdvertisingPeriodMs values must*/
   /*          be valid.                                                */
typedef struct _tagDEVM_Default_Initialization_Data_t
{
   unsigned long                                InitializationOverrideFlags;
   char                                        *DefaultDeviceName;
   Class_of_Device_t                            DefaultClassOfDevice;
   Boolean_t                                    DefaultConnectableMode;
   Boolean_t                                    DefaultDiscoverableMode;
   Boolean_t                                    DefaultPairableMode;
   Boolean_t                                    DefaultDeviceIDInformationValid;
   ...
   Word_t                                       DefaultLEAdvertisingIntervalMax;
   unsigned int                                 DefaultLEConnectionTimeout;
   unsigned int                                 DefaultManufacturingDataLength;
   Byte_t                                      *DefaultManufacturingData;
   unsigned int                                 DefaultAdvertisingDataLength;
   Advertising_Data_t                           DefaultAdvertisingData;
   unsigned int                                 DefaultScanResponseDataLength;
   Scan_Response_Data_t                         DefaultScanResponseData;
   DEVM_Default_Allowable_LE_Parameters_Data_t  DefaultLEAllowableParameters;
   unsigned int                                 DefaultMinLENonInterleavedAdvertisingDurationMS;
   unsigned int                                 DefaultMinLENonInterleavedAdvertisingPeriodMS;
} DEVM_Default_Initialization_Data_t;

#define DEVM_DEFAULT_INITIALIZATION_DATA_SIZE                  (sizeof(DEVM_Default_Initialization_Data_t))

   /* The following parameters are used with the                        */
   /* InitializationOverrideFlags member of the                         */
...
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_PAIRABLE_MODE             0x00000010
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_DEVICE_ID_INFO_VALID      0x00000020
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_DEVICE_ID_INFO            0x00000040
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_DEVICE_APPEARANCE         0x00000080
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_CONNECT_PARAMS         0x00000100
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_SCAN_INTERVAL          0x00000200
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_SCAN_WINDOW            0x00000400
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_CONNECT_TIMEOUT        0x00000800
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_ADVERTISING_INTV       0x00001000
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_MANUFACTURER_DATA         0x00002000
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_ADVERTISING_DATA          0x00004000
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_DEVM_ALLOW_LE_PARAM_DATA  0x00008000
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_SET_DEVICE_ID_IN_EIR_DATA 0x00010000
#define DEVM_INITIALIZATION_DATA_OVERRIDE_FLAGS_LE_NON_INTERLEAVED_TIMING 0x00020000

   /* The following structure is the structure that is used to pass     */
   /* initialization information when the Message Sub-system is         */
   /* initialized.                                                      */
typedef struct _tagDEVM_Initialization_Data_t
{
   HCI_DriverInformation_t            *HCI_DriverInformation;
   DEVM_Power_Device_Callback_t        PowerDeviceCallback;
   void                               *PowerDeviceCallbackParameter;
   DEVM_Default_Initialization_Data_t *DefaultInitializationData;
} DEVM_Initialization_Data_t;

#define DEVM_INITIALIZATION_DATA_SIZE                          (sizeof(DEVM_Initialization_Data_t))

   /* The following enum is used to request certain properties that     */
   /* are maintained internally within DEVM. This properties are only   */
   /* accessible to applications that are linked into the PM server.    */
typedef enum
{
   dptSecurityInformation,
   dptLESecurityInformation,
   dptDriverInformation,
   dptLocalIRK
} DEVM_Extended_Property_Type_t;

typedef struct _tagDEVM_Ext_Prop_Security_Information_t
{
   Link_Key_t LinkKey;
} DEVM_Ext_Prop_Security_Information_t;

typedef struct _tagDEVM_Ext_Prop_LE_Security_Information_t
{
   BD_ADDR_t             IdentityAddress;
   GAP_LE_Address_Type_t IdentityAddressType;
   Long_Term_Key_t       LTK;
   Random_Number_t       Rand;
   Encryption_Key_t      IRK;
   Word_t                EDIV;
} DEVM_Ext_Prop_LE_Security_Information_t;

typedef struct _tagDEVM_Extended_Property_t
{
   DEVM_Extended_Property_Type_t Type;
   union
   {
      DEVM_Ext_Prop_Security_Information_t SecurityInformation;
      DEVM_Ext_Prop_LE_Security_Information_t LESecurityInformation;
      HCI_DriverInformation_t *DriverInformation;
      Encryption_Key_t IRK;
   } Property;
} DEVM_Extended_Property_t;
...
BTPSAPI_DECLARATION int BTPSAPI DEVM_PowerOffDevice(void);
   ...
   /* device.  This function returns BOOLEAN TRUE if the local device   */
   /* is currently powered on, or FALSE if the local device is not      */
   /* currently powered on.                                             */
BTPSAPI_DECLARATION Boolean_t BTPSAPI DEVM_QueryDevicePowerState(void);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef Boolean_t (BTPSAPI *PFN_DEVM_QueryDevicePowerState_t)(void);
#endif
...
BTPSAPI_DECLARATION void BTPSAPI DEVM_AcknowledgeDevicePoweringDown(unsigned int DeviceManagerCallbackID);
   ...
   /* The following function is provided to allow a mechansim for local */
   /* modules to determine the current Bluetooth Stack ID of the        */
   /* currently opened Bluetooth Stack.  This function returns a        */
   /* positive, non-zero value if successful (which represents the      */
   /* current Bluetooth Stack ID of the currently opened Bluetooth      */
   /* Stack) or a negative return value if there was an error.          */
BTPSAPI_DECLARATION int BTPSAPI DEVM_QueryDeviceBluetoothStackID(void);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef void (BTPSAPI *PFN_DEVM_QueryDeviceBluetoothStackID_t)(void);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI DEVM_SetRemoteDeviceLinkActive(BD_ADDR_t RemoteDevice);
   ...
   /* * NOTE * The PersistClient parameter is only used when linking    */
   /*          against the client libraries. Any call using this server */
   /*          API will ignore this parameter.                          */
...
BTPSAPI_DECLARATION int BTPSAPI DEVM_AddLocalEIRData(unsigned int DataLength, unsigned char *Data);
   ...
   /* The following function is a utilty to request properties that     */
   /* are internally managed by DEVM. The Type parameter indicates      */
   /* what property is being requested. The Property parameter is       */
   /* the structure in which the property will be returned. The         */
   /* RequestParameter parameter is a void pointer to any data needed   */
   /* as a filter for the property request (e.g. a BD_ADDR for a remote */
   /* device's property). This function returns zero if successful and a*/
   /* negative error code if there was an error.                        */
   /* * NOTE * This function is only available to applications that are */
   /*          linked into the PM Server. This information is not       */
   /*          exposed to any client applications.                      */
BTPSAPI_DECLARATION int BTPSAPI DEVM_GetExtendedProperty(DEVM_Extended_Property_Type_t Type, DEVM_Extended_Property_t *Property, void *RequestParameter);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_DEVM_GetExtendedProperty_t)(DEVM_Extended_Property_Type_t Type, DEVM_Extended_Property_t *Property, void *RequestParameter);
#endif
