...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXOPPM - Simple Linux application using Bluetopia Platform Manager     */
/*              Object Push Application Programming (API) Interface.          */
/*                                                                            */
/*  Author:  Matt Seabold                                                     */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   12/17/13  M. Seabold     Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxOPPM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTOPPM.h"     /* OPP Manager Application Programming Interface.  */
...
static unsigned char      *CurrentBuffer;           /* variables are used by both the  */
                                                    /* client and server when an       */
...
static const char vCARDData[] =
{
   ...
   "ADR;POSTAL;WORK:9960 Corporate Campus Drive Suite 3300;Louisville, KY;;;;40223;USA\x0D\x0A" \
   "END:VCARD\x0D\x0A"
} ;
...
static int ParseRemoteObjectPushServices(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM OPP Manager Callback function prototype.                     */
static void BTPSAPI OPPM_Event_Callback(OPPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("PARSEREMOTEOBJECTPUSHSERVICES", ParseRemoteObjectPushServices);
...
static void WriteReceivedData(unsigned int BufferLength, unsigned char *Buffer)
{
   ...
   printf("%lu bytes written to file :%s\r\n", (unsigned long)BytesWritten, CurrentFileName);
}
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  33)RegisterServer                             *\r\n");
   printf("*                  34)UnRegisterServer                           *\r\n");
   printf("*                  35)ConnectionRequestResponse                  *\r\n");
   printf("*                  36)ParseRemoteObjectPushServices              *\r\n");
   printf("*                  37)ConnectRemoteDevice                        *\r\n");
   printf("*                  38)Disconnect                                 *\r\n");
   printf("*                  39)Abort                                      *\r\n");
   printf("*                  40)PushObject                                 *\r\n");
   printf("*                  41)PullBusinessCard                           *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static void DisplayRemoteDeviceProperties(unsigned long UpdateMask, DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   /* The following function is responsible for register a OPP Server.  */
...
static int RegisterServer(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Register the Server.                                        */
         ret_val = OPPM_Register_Server((unsigned int)TempParam->Params[0].intParam, (unsigned long)TempParam->Params[1].intParam, (unsigned long)TempParam->Params[2].intParam, (char *)TempParam->Params[3].strParam, OPPM_Event_Callback, 0);

         /* Now check to see if the command was submitted successfully. */
         if(ret_val > 0)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Register_Server: Function Successful. ServerID: %d\r\n", ret_val);
         ...
         else
         {
            ...
            printf("OPPM_Register_Server() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: RegisterServer [ServerPort] [SupportedObjectTypes] [IncomingConnectionFlags] [ServiceName].\r\n");
   ...
   /* The following function is responsible for un-register a OPP       */
   /* Server.  This function returns zero if successful and a negative  */
   /* value if an error occurred.                                       */
static int UnRegisterServer(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         ...
         ret_val = OPPM_Un_Register_Server((unsigned int)TempParam->Params[0].intParam);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Un_Register_Server: Function Successful.\r\n");
         }
         else
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Un_Register_Server() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
...
static int ConnectionRequestResponse(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         ...
         ret_val = OPPM_Connection_Request_Response((unsigned int)TempParam->Params[1].intParam, (Boolean_t)TempParam->Params[2].intParam);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Connection_Request_Response: Function Successful.\r\n");
         }
         else
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Connection_Request_Response() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ConnectionRequestResponse [ServerID] [Accept (1 = Accept, 0 = Reject)].\r\n");
   ...
   /* The following function is responsible for parsing the service     */
   /* information of a remote device for Object Push services. This     */
   /* fuction returns zero if successful and a negative value if there  */
   /* was an error.                                                     */
static int ParseRemoteObjectPushServices(ParameterList_t *TempParam)
{
   int                        ret_val;
   unsigned int               Index;
   BD_ADDR_t                  BD_ADDR;
   OPPM_Parsed_Service_Info_t ParsedServiceInfo;
   ...
   if(Initialized)
   {
      ...
      {
         /* Convert the BD_ADDR to a string.                            */
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Send the connection request response.                       */
         ret_val = OPPM_Parse_Remote_Object_Push_Services(BD_ADDR, &ParsedServiceInfo);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Parse_Remote_Object_Push_Services: Function Successful.\r\n");

            for(Index=0;Index<ParsedServiceInfo.NumberServices;Index++)
            {
               printf("   Service %u:\r\n", Index);
               printf("      Server Port:           %u\r\n", ParsedServiceInfo.ServiceDetails[Index].ServerPort);
               printf("      Supported Object Mask: 0x%08X\r\n", (unsigned int)ParsedServiceInfo.ServiceDetails[Index].SupportedObjectTypes);
               printf("      Service Name:          %s\r\n", ParsedServiceInfo.ServiceDetails[Index].ServiceName);
            }

            OPPM_Free_Parsed_Object_Push_Service_Info(&ParsedServiceInfo);
         }
         else
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Parse_Remote_Object_Push_Services() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ParseRemoteObjectPushServices [BD_ADDR].\r\n");
   ...
   /* The following function is responsible for sending a Connect Remote*/
   /* Device request. This function returns zero if successful and a    */
...
static int ConnectRemoteDevice(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].strParam))
      {
         ...
         /* Send the connection request response.                       */
         ret_val = OPPM_Connect_Remote_Device(BD_ADDR, TempParam->Params[1].intParam, TempParam->Params[2].intParam, OPPM_Event_Callback, NULL, NULL);

         /* Now check to see if the command was submitted successfully. */
         if(ret_val > 0)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Connect_Remote_Device: Function Successful. ClientID: %d\r\n", ret_val);
         ...
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Connect_Remote_Device() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ConnectRemoteDevice [BD_ADDR] [Server Port] [Connection Flags (1 - Authenticate, 2 - Encrypt)].\r\n");
...
static int Disconnect(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Send the connection request response.                       */
         ret_val = OPPM_Disconnect(TempParam->Params[0].intParam);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Disconnect: Function Successful.\r\n");
         ...
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Disconnect() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: Disconnect [Client/Server ID].\r\n");
   ...
   /* The following function is responsible for sending a Abort request.*/
   /* This function returns zero if successful and a negative value if  */
   /* there was an error.                                               */
static int Abort(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Send the connection request response.                       */
         ret_val = OPPM_Abort(TempParam->Params[0].intParam);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Abort: Function Successful.\r\n");
         ...
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Abort() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: Abort [Client ID].\r\n");
...
static int PushObject(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[2].strParam))
      {
         /* Everything appears to be semi-valid, go ahead and read all  */
         /* of the file object that was specified into the Current      */
         /* Buffer.                                                     */
         if(!ReadFileIntoCurrentBuffer(TempParam->Params[2].strParam))
         {
            /* Send the push object request.                            */
            ret_val = OPPM_Push_Object_Request(TempParam->Params[0].intParam, (OPPM_Object_Type_t)TempParam->Params[1].intParam, TempParam->Params[2].strParam, CurrentBufferSize, CurrentBufferSize, CurrentBuffer, TRUE);
            ...
            {
               /* The function was submitted successfully.              */
               printf("OPPM_Push_Object_Request: Function Successful.\r\n");
            ...
            {
               /* There was an error submitting the function.           */
               printf("OPPM_Push_Object_Request() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
            }
         }
         else
         {
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PushObject [Client ID] [Object Type] [Object File Name].\r\n");
...
static int PullBusinessCard(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         /* Send the pull business card request.                        */
         ret_val = OPPM_Pull_Business_Card_Request(TempParam->Params[0].intParam);

         /* Now check to see if the command was submitted successfully. */
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
            printf("OPPM_Pull_Business_Card_Request: Function Successful.\r\n");
         ...
         {
            /* There was an error submitting the function.              */
            printf("OPPM_Pull_Business_Card_Request() Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         }
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: PullBusinessCard [Client ID] [Filename to save card].\r\n");
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter)
{
   ...
   /* The following function is for an OPP Event Callback.  This        */
   /* function will be called whenever a OPP Event occurs that is       */
   /* associated with the Bluetooth Stack.  This function passes to the */
   /* caller the OPP Event Data that occurred and the OPP Event Callback*/
   /* Parameter that was specified when this Callback was installed.    */
   /* The caller is free to use the contents of the OPP Event Data ONLY */
   ...
   /* another OPP Event will not be processed while this function call  */
   /* is outstanding).                                                  */
   /* * NOTE * This function MUST NOT Block and wait for events that can*/
   /*          only be satisfied by Receiving OPP Event Packets.  A     */
...
static void BTPSAPI OPPM_Event_Callback(OPPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData != NULL)
   {
      ...
      switch(EventData->EventType)
      {
         case oetIncomingConnectionRequest:
            BD_ADDRToStr(EventData->EventData.IncomingConnectionRequestEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Incoming Connection Request:\r\n");
            printf("   Server ID: %u\r\n", EventData->EventData.IncomingConnectionRequestEventData.ServerPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            break;
         case oetConnected:
            BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Device Connected:\r\n");
            printf("   Server ID: %u\r\n", EventData->EventData.ConnectedEventData.ServerPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            break;
         case oetConnectionStatus:
            BD_ADDRToStr(EventData->EventData.ConnectionStatusEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Connection Status:\r\n");
            printf("   Client ID: %u\r\n", EventData->EventData.ConnectionStatusEventData.ClientPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            printf("   Status: %u\r\n", EventData->EventData.ConnectionStatusEventData.Status);
            break;
         case oetDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Disconnected:\r\n");
            printf("   ID: %u\r\n", EventData->EventData.DisconnectedEventData.PortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            break;
         case oetPushObjectRequest:
            BD_ADDRToStr(EventData->EventData.PushObjectRequestEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Push Object Request:\r\n");
            printf("   Server ID: %u\r\n", EventData->EventData.PushObjectRequestEventData.ServerPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            ...
            {
               if(EventData->EventData.PushObjectRequestEventData.Final)
                  CurrentOperation = coNone;

               ResponseCode = OPPM_RESPONSE_STATUS_CODE_SUCCESS;
            }
            else
            {
               ...
               {
                  /* Set the filename.                                  */
                  if(EventData->EventData.PushObjectRequestEventData.ObjectName)
                     strcpy(CurrentFileName, EventData->EventData.PushObjectRequestEventData.ObjectName);

                  unlink(CurrentFileName);

                  switch(EventData->EventData.PushObjectRequestEventData.ObjectType)
                  {
                     case obtvCard:
                  ...
                  printf("   Total Length: %lu\r\n", EventData->EventData.PushObjectRequestEventData.ObjectTotalLength);
                  ...
                  ResponseCode = OPPM_RESPONSE_STATUS_CODE_BAD_REQUEST;
            }

            printf("   Final: %s\r\n", (EventData->EventData.PushObjectRequestEventData.Final)?"TRUE":"FALSE");
            printf("   Data Length: %u\r\n", EventData->EventData.PushObjectRequestEventData.DataLength);

            if(EventData->EventData.PushObjectRequestEventData.DataBuffer)
            {
               printf("   Data:\r\n");

               for(Index=0,Buffer[0]='\0';(Index<EventData->EventData.PushObjectRequestEventData.DataLength) && (Index < (sizeof(Buffer) - 1));Index++)
                  printf("%c", EventData->EventData.PushObjectRequestEventData.DataBuffer[Index]);

               printf("\r\n");

               WriteReceivedData(EventData->EventData.PushObjectRequestEventData.DataLength, EventData->EventData.PushObjectRequestEventData.DataBuffer);
            }

            /* Only respond if this is the last request.                */
            if(EventData->EventData.PushObjectRequestEventData.Final)
            {
               Result = OPPM_Push_Object_Response(EventData->EventData.PushObjectRequestEventData.ServerPortID, ResponseCode);
            ...
            break;
         case oetPushObjectResponse:
            BD_ADDRToStr(EventData->EventData.PushObjectResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Push Object Response:\r\n");
            printf("   Client ID: %u\r\n", EventData->EventData.PushObjectResponseEventData.ClientPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            printf("   Response Code: %u\r\n", EventData->EventData.PushObjectResponseEventData.ResponseCode);
            break;
         case oetPullBusinessCardRequest:
            BD_ADDRToStr(EventData->EventData.PullBusinessCardRequestEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Pull Business Card Request:\r\n");
            printf("   Server ID: %u\r\n", EventData->EventData.PullBusinessCardRequestEventData.ServerPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            Result = OPPM_Pull_Business_Card_Response(EventData->EventData.PullBusinessCardRequestEventData.ServerPortID, OPPM_RESPONSE_STATUS_CODE_SUCCESS, strlen(vCARDData), strlen(vCARDData), (unsigned char *)vCARDData, TRUE);

            if(!Result)
               printf("\r\nOPPM_Pull_Business_Card_Response() Successful.\r\n");
            else
               printf("\r\nOPPM_Pull_Business_Card_Response() Failure %d.\r\n", Result);
            break;
         case oetPullBusinessCardResponse:
            BD_ADDRToStr(EventData->EventData.PullBusinessCardResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("OPPM Pull Business Card Response:\r\n");
            printf("   Client ID: %u\r\n", EventData->EventData.PullBusinessCardResponseEventData.ClientPortID);
            printf("   Remote Device Address: %s\r\n", BoardStr);
            printf("   Response Code: %u\r\n", EventData->EventData.PullBusinessCardResponseEventData.ResponseCode);
            printf("   Total Length: %lu\r\n", EventData->EventData.PullBusinessCardResponseEventData.ObjectTotalLength);
            printf("   Data Length: %u\r\n", EventData->EventData.PullBusinessCardResponseEventData.DataLength);
            printf("   Final: %s\r\n", (EventData->EventData.PullBusinessCardResponseEventData.Final)?"TRUE":"FALSE");

            if(EventData->EventData.PullBusinessCardResponseEventData.DataLength)
            {
               printf("Data:\r\n");

               for(Index=0,Buffer[0]='\0';(Index<EventData->EventData.PullBusinessCardResponseEventData.DataLength) && (Index < (sizeof(Buffer) - 1));Index++)
                  printf("%c", EventData->EventData.PullBusinessCardResponseEventData.DataBuffer[Index]);
            ...
            if((EventData->EventData.PullBusinessCardResponseEventData.ResponseCode == OPPM_RESPONSE_STATUS_CODE_SUCCESS) && (EventData->EventData.PullBusinessCardResponseEventData.DataLength))
               WriteReceivedData(EventData->EventData.PullBusinessCardResponseEventData.DataLength, EventData->EventData.PullBusinessCardResponseEventData.DataBuffer);
   ...
   else
   {
      ...
      printf("\r\nOPP callback data: Event_Data = NULL.\r\n");
   }
