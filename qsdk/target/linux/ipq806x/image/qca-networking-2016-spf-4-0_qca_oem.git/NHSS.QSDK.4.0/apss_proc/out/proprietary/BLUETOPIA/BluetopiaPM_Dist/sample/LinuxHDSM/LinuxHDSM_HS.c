...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHDSM_HS - Simple Linux application using Bluetopia Platform Manager  */
/*                 Headset Profile (HDSET) Manager Application Programming    */
/*                 Interface (API) - Headset Role only.                       */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
#include "LinuxHDSM_HS.h"  /* Main Application Prototypes and Constants.      */
...
static int HDSMSetupAudioConnection(BD_ADDR_t BD_ADDR);
...
static int HDSMQuerySCOConnectionHandle(ParameterList_t *TempParam);

static int HDSMSendButtonPress(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("QUERYSCOCONNECTIONHANDLE", HDSMQuerySCOConnectionHandle);
   AddCommand("SENDBUTTONPRESS", HDSMSendButtonPress);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  41)QuerySCOConnectionHandle                   *\r\n");
   printf("*                  42)SendButtonPress                            *\r\n");
   printf("*                  43)HDSRegisterEventCallback                   *\r\n");
   printf("*                  44)HDSUnRegisterEventCallback                 *\r\n");
   printf("*                  45)HDSRegisterDataCallback                    *\r\n");
   printf("*                  46)HDSUnRegisterDataCallback                  *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
   ...
   if(HDSventCallbackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 1))
      {
         ...
         if(TempParam->Params[1].intParam)
         {
            ...
            ret_val = HDSMSetupAudioConnection(BD_ADDR);
         }
         else
         {
...
static int HDSMSetupAudioConnection(BD_ADDR_t BD_ADDR)
{
   ...
   if(Initialized)
   {
      ...
      /* command.                                                       */
      Result  = HDSM_Setup_Audio_Connection(HDSventCallbackID, sctHeadset, BD_ADDR, FALSE);
...
static int HDSMReleaseAudioConnection(BD_ADDR_t BD_ADDR)
{
   ...
   if(Initialized)
   {
      /* The Port ID appears to be a semi-valid value.  Now submit the  */
      /* command.                                                       */
      Result  = HDSM_Release_Audio_Connection(HDSventCallbackID, sctHeadset, BD_ADDR);
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            ...
            if(!Result)
            {
               ...
               Result = HDSM_Connect_Remote_Device(HDSventCallbackID, sctHeadset, BD_ADDR, ServerPortNumber, ConnectionFlags, HDSM_Event_Callback, NULL, NULL);
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            ...
            Result = HDSM_Disconnect_Device(HDSventCallbackID, sctHeadset, BD_ADDR);
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1))
         {
            ...
            Result = HDSM_Connection_Request_Response(HDSventCallbackID, sctHeadset, BD_ADDR, (Boolean_t)(TempParam->Params[1].intParam));
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[1].intParam <= HDSET_SPEAKER_GAIN_MAXIMUM))
         {
            ...
            /* command.                                                 */
            Result  = HDSM_Set_Remote_Speaker_Gain(HDSventCallbackID, sctHeadset, BD_ADDR, TempParam->Params[1].intParam);
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[1].intParam <= HDSET_MICROPHONE_GAIN_MAXIMUM))
         {
            ...
            /* command.                                                 */
            Result  = HDSM_Set_Remote_Microphone_Gain(HDSventCallbackID, sctHeadset, BD_ADDR, TempParam->Params[1].intParam);
...
static int QueryHeadsetServices(ParameterList_t *TempParam)
{
   int                        ret_val;
   BD_ADDR_t                  BD_ADDR;
   HDSM_Service_Information_t ServiceInformation;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         ...
         ret_val = HDSM_Query_Remote_Device_Service_Information(BD_ADDR, &ServiceInformation);

         if(!ret_val)
         {
            printf("HDSM_Query_Remote_Device_Service_Information(): Sucess\r\n");
            
            if(ServiceInformation.Flags & HDSM_SERVICE_INFORMATION_FLAGS_AUDIO_GATEWAY_DATA_VALID)
            {
               printf("   Port:  %u\r\n", ServiceInformation.AudioGatewayInformation.PortNumber);
               printf("   Flags: %08lX\r\n", ServiceInformation.AudioGatewayInformation.Flags);
            }
            else
            {
               printf("   No Audio Gateway record present\r\n");
            }
         }
         else
            printf("HDSM_Query_Remote_Device_Service_Information(): Failure: %d, %s\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      }
      else
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: QueryHeadsetServices [BD_ADDR].\r\n");
...
static int ChangeIncomingConnectionFlags(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0))
      {
         ...
         ret_val = HDSM_Change_Incoming_Connection_Flags(sctHeadset, (unsigned long)TempParam->Params[0].intParam);
         if(!ret_val)
         {
            /* The function was submitted successfully.                 */
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            ...
            /* the command.                                             */
            Result  = HDSM_Send_Button_Press(HDSventCallbackID, BD_ADDR);
            ...
            {
               /* The function was submitted successfully.              */
               printf("HDSM_Send_Button_Press: Function Successful.\r\n");
            }
            else
            {
               /* There was an error submitting the function.           */
               printf("HDSM_Send_Button_Press() Failure: %d (%s).\r\n", Result, ERR_ConvertErrorCodeToString(Result));
            }
         }
         else
         {
            /* The Memory Location parameter is invalid.                */
            printf("Usage: SendButtonPress [BD_ADDR].\r\n");
   ...
   if(Initialized)
   {
      ...
      if(HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam))
         {
            ...
            /* submit the command.                                      */
            Result = HDSM_Query_SCO_Connection_Handle(HDSventCallbackID, sctHeadset, BD_ADDR, &SCOHandle);
            ...
            {
               /* The function was submitted successfully.              */
               printf("HDSM_Query_SCO_Connection_Handle(): Function Successful. Handle: %u.\r\n", (unsigned int)SCOHandle);
            }
            else
            {
               /* There was an error submitting the function.           */
               printf("HDSM_Query_SCO_Connection_Handle() Failure: %d (%s).\r\n", Result, ERR_ConvertErrorCodeToString(Result));
            }
         }
         else
         {
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("HDS Control Event Callback MUST be registered before making this call.\r\n");
   ...
   if(Initialized)
   {
      ...
      if(!HDSventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0))
         {
            /* Callback has not been registered, go ahead and attempt to*/
            /* register it.                                             */
            Result = HDSM_Register_Event_Callback(sctHeadset, (Boolean_t)TempParam->Params[0].intParam, HDSM_Event_Callback, NULL);
            if(Result > 0)
            {
...
static int HDSRegisterDataCallback(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(!HDSDataCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         Result = HDSM_Register_Data_Event_Callback(sctHeadset, HDSM_Event_Callback, NULL);
         if(Result > 0)
         {
...
static Boolean_t ParseAudioGatewayServiceClass(SDP_Record_t *SDPRecordPtr)
{
   ...
   if(SDPRecordPtr)
   {
      ...
      while((Index1--) && (!ret_val))
      {
         ...
         if(*ServiceClass == HEADSET_PROFILE_AUDIO_GATEWAY_UUID)
            ret_val = TRUE;
   ...
   if(EventData != NULL)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case hetHDSRingIndication:
            ...
            /* information.                                             */
            printf("hetHDSRingIndication, BD_ADDR: %s.\r\n", BoardStr);
            break;
