...
#include "LinuxANPM_SRV.h"     /* Main Application Prototypes and Constants.      */
...
static int ANPSetNewAlert(ParameterList_t *TempParam);
static int ANPSetUnReadAlert(ParameterList_t *TempParam);
static int ANPRegisterServerEventCallback(ParameterList_t *TempParam);
static int ANPUnRegisterServerEventCallback(ParameterList_t *TempParam);
...
static void DisplaySetAlertUsage(char *AlertType, Boolean_t LastAlertText);
...
static void UserInterface(void)
{
   ...
   AddCommand("ANPSETNEWALERT", ANPSetNewAlert);
   AddCommand("ANPSETUNREADALERT", ANPSetUnReadAlert);
   AddCommand("ANPREGISTERSERVEREVENTCALLBACK", ANPRegisterServerEventCallback);
   AddCommand("ANPUNREGISTERSERVEREVENTCALLBACK", ANPUnRegisterServerEventCallback);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)ANPSetNewAlert                             *\r\n");
   printf("*                  36)ANPSetUnReadAlert                          *\r\n");
   printf("*                  37)ANPRegisterServerEventCallback             *\r\n");
   printf("*                  38)ANPUnRegisterServerEventCallback           *\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for setting the new alert   */
   /* status on the ANP server.  This function returns zero on          */
...
static int ANPSetNewAlert(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (ANPM_IS_VALID_CATEGORY_IDENTIFICATION(TempParam->Params[0].intParam)) && (TempParam->Params[1].intParam >= 0))
      {
         /* If it the parameters indicate that the optional alert text  */
         /* is available, then try to parse it.                         */
         if((TempParam->NumberofParameters >= 3))
         {
            if(FormatMultiStringParam(TempParam, 2, &LastAlertText) >= 2)
            {
               if((ret_val = ANPM_Set_New_Alert((ANPM_Category_Identification_t)TempParam->Params[0].intParam, (Byte_t)TempParam->Params[1].intParam, LastAlertText)) == 0)
                  printf("ANPM_Set_New_Alert() Success.\r\n");
               else
               {
                  printf("ANPM_Set_New_Alert() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));

                  ret_val = FUNCTION_ERROR;
               }

               /* Free the alert text now that we are done with it.     */
               BTPS_FreeMemory(LastAlertText);
            }
            else
            {
               /* One or more of the necessary parameters is/are        */
               /* invalid.                                              */
               DisplaySetAlertUsage("New", TRUE);
         ...
         {
            /* There is no alert text, just pass NULL.                  */
            if((ret_val = ANPM_Set_New_Alert((ANPM_Category_Identification_t)TempParam->Params[0].intParam, (Byte_t)TempParam->Params[1].intParam, NULL)) == 0)
               printf("ANPM_Set_New_Alert() Success.\r\n");
            else
            {
               printf("ANPM_Set_New_Alert() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
   ...
   /* The following function is responsible for setting the un-read     */
   /* alert status on the ANP server.  This function returns zero on    */
   /* successful execution and a negative value on all errors.          */
static int ANPSetUnReadAlert(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (ANPM_IS_VALID_CATEGORY_IDENTIFICATION(TempParam->Params[0].intParam)) && (TempParam->Params[1].intParam >= 0))
      {
         if((ret_val = ANPM_Set_Un_Read_Alert((ANPM_Category_Identification_t)TempParam->Params[0].intParam, (Byte_t)TempParam->Params[1].intParam)) == 0)
            printf("ANPM_Set_Un_Read_Alert() Success.\r\n");
         else
         {
            printf("ANPM_Set_Un_Read_Alert() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         DisplaySetAlertUsage("UnRead", FALSE);
   ...
   /* The following function is responsible for registering with the ANP*/
   /* Manager to receive ANP Events.  This function returns zero if     */
   /* successful and a negative value if an error occurred.             */
static int ANPRegisterServerEventCallback(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if(!ANPEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = ANPM_Register_Server_Event_Callback(ANPM_Event_Callback, NULL)) > 0)
         {
            printf("ANPM_Register_Server_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            ANPEventCallbackID = (unsigned int)Result;
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("ANP Manager Event Callback already registered.\r\n");
   ...
   /* previously registered ANP Event Registration with the ANP Manager.*/
...
static void DisplayRemoteDeviceProperties(unsigned long UpdateMask, DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   /* The following function is responsible for displaying the usage    */
   /* information for the ANPSet*Alert commands.                        */
static void DisplaySetAlertUsage(char *AlertType, Boolean_t LastAlertText)
{
   unsigned int i;
   ...
   if(AlertType)
   {
      printf("Usage: ANPSet%sAlert [Category] [%sAlertCount (0-255)]%s.\r\n", AlertType, AlertType, LastAlertText ? " \"[LastAlertText String (Optional)]\"" : "");
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case aetANPDisconnected:
            ...
            break;
         case aetANPNewAlertCategoryEnabled:
            BD_ADDRToStr(EventData->EventData.NewAlertCategoryEnabledEventData.RemoteDeviceAddress, Buffer);

            printf("ANMP New Alert Category Enabled: %s.\r\n", Buffer);
            printf("Enabled Category:                %s.\r\n", AlertCategoryToString(EventData->EventData.NewAlertCategoryEnabledEventData.CategoryEnabled));
            printf("Currently Enabled Categories:    0x%04X.\r\n", EventData->EventData.NewAlertCategoryEnabledEventData.EnabledCategories);
            break;
         case aetANPNewAlertCategoryDisabled:
            BD_ADDRToStr(EventData->EventData.NewAlertCategoryDisabledEventData.RemoteDeviceAddress, Buffer);

            printf("ANMP New Alert Category Disabled: %s.\r\n", Buffer);
            printf("Disabled Category:                %s.\r\n", AlertCategoryToString(EventData->EventData.NewAlertCategoryDisabledEventData.CategoryDisabled));
            printf("Currently Enabled Categories:     0x%04X.\r\n", EventData->EventData.NewAlertCategoryDisabledEventData.EnabledCategories);
            break;
         case aetANPUnReadAlertCategoryEnabled:
            BD_ADDRToStr(EventData->EventData.UnReadAlertCategoryEnabledEventData.RemoteDeviceAddress, Buffer);

            printf("ANMP Un-Read Alert Category Enabled: %s.\r\n", Buffer);
            printf("Enabled Category:                    %s.\r\n", AlertCategoryToString(EventData->EventData.UnReadAlertCategoryEnabledEventData.CategoryEnabled));
            printf("Currently Enabled Categories:        0x%04X.\r\n", EventData->EventData.UnReadAlertCategoryEnabledEventData.EnabledCategories);
            break;
         case aetANPUnReadAlertCategoryDisabled:
            BD_ADDRToStr(EventData->EventData.UnReadAlertCategoryDisabledEventData.RemoteDeviceAddress, Buffer);

            printf("ANMP Un-Read Alert Category Disabled: %s.\r\n", Buffer);
            printf("Disabled Category:                    %s.\r\n", AlertCategoryToString(EventData->EventData.UnReadAlertCategoryDisabledEventData.CategoryDisabled));
            printf("Currently Enabled Categories:         0x%04X.\r\n", EventData->EventData.UnReadAlertCategoryDisabledEventData.EnabledCategories);
            break;
         default:
