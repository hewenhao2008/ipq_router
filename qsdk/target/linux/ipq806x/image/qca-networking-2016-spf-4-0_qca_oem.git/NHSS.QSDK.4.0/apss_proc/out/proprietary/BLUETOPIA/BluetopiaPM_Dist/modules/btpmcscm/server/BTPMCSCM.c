...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  BTPMCSCM - Cycling Speed and Cadence Manager for Stonestreet One Bluetooth*/
...
#include "SS1BTCSC.h"           /* Bluetooth CSC Service Prototypes/Constants.*/

#include "SS1BTPM.h"            /* BTPM Main Prototypes and Constants.        */
#include "BTPMERR.h"            /* BTPM Error Prototypes/Constants.           */
#include "BTPMCFG.h"            /* BTPM Configuration Settings/Constants.     */

   /* The following defines the CSCM LE Configuration File Section Name.*/
#define CSCM_LE_CONFIGURATION_FILE_SECTION_NAME                   "CSCM-Collector"

   /* The following defines the Maximum Key Size that is used in the    */
   /* CSCM LE Configuration File.                                       */
#define CSCM_LE_CONFIGURATION_FILE_MAXIMUM_KEY_LENGTH             (4+(BD_ADDR_SIZE*2))

   /* The following defines the CSCM LE Configuration File Maximum Line */
   /* Length.                                                           */
#define CSCM_LE_CONFIGURATION_FILE_MAXIMUM_LINE_LENGTH            ((BTPM_CONFIGURATION_SETTINGS_MAXIMUM_FILE_LINE_LENGTH/2)-CSCM_LE_CONFIGURATION_FILE_MAXIMUM_KEY_LENGTH)

   /* The following define the Key Names that are used with the CSCM LE */
   /* Configuration File.                                               */
#define CSCM_LE_KEY_NAME_DEVICE_INFORMATION                       "DI-%02X%02X%02X%02X%02X%02X"

#define CSCM_PROCEDURE_TIMEOUT                                    30000
...
#define CSCM_LE_DEVICE_INFORMATION_VALUE_SIZE                  (sizeof(Stored_Device_Information_t))
...
static Callback_Entry_t *CallbackEntryList;
   ...
   /* Information List (which holds connected CSC LE devices).          */
...
static void DispatchCSCEvent(Callback_Info_t *CallbackInfo, CSCM_Event_Data_t *EventData, BTPM_Message_t *Message, Boolean_t LockHeld);
static void DispatchCSCBroadcastEvent(CSCM_Event_Data_t *EventData, BTPM_Message_t *Message, Boolean_t LocalOnly);
...
static void DispatchMeasurementEvent(Device_Entry_t *Device, unsigned long Flags, CSCM_Wheel_Revolution_Data_t *WheelData, CSCM_Crank_Revolution_Data_t *CrankData);
static void DispatchSensorLocationResponseEvent(unsigned int ClientID, Device_Entry_t *Device, unsigned int Status, CSCM_Sensor_Location_t Location);
...
static Boolean_t ReloadCSCDeviceFromFile(Device_Entry_t *Device, BD_ADDR_t BluetoothAddress);
static void StoreCSCMDeviceToFile(Device_Entry_t *Device);
static void ClearCSCMDeviceFromFile(BD_ADDR_t BluetoothAddress);

static void DeleteTransactionsForDevice(BD_ADDR_t RemoteDeviceAddress);

static unsigned long ConvertLocationListToMask(CSCS_SCCP_Supported_Sensor_Locations_t *SupportedSensorLocations);

static CSC_UUID_t DetermineCSCUUID(GATT_UUID_t UUID);
static Word_t FindCCCD(GATT_Characteristic_Information_t *Characteristic);
static Device_Entry_t *DiscoverCSCSensor(BD_ADDR_t *BluetoothAddress);

static int ProcessRegisterCollectorEvents(unsigned int ClientID, CSCM_Event_Callback_t CallbackFunction, void *CallbackParameter);
static void ProcessUnRegisterCollectorEvents(unsigned int ClientID, unsigned int CollectorCallbackID);
static int ProcessQueryConnectedSensors(unsigned int ClientID, unsigned int MaximumRemoteDeviceListEntries, CSCM_Connected_Sensor_t *ConnectedDeviceList, unsigned int *TotalNumberConnectedDevices);
...
static int ProcessGetConnectedSensorInfo(unsigned int ClientID, BD_ADDR_t RemoteDeviceAddress, CSCM_Connected_Sensor_t *DeviceInfo);
...
static int ProcessUpdateSensorLocation(unsigned int ClientID, BD_ADDR_t RemoteDeviceAddress, CSCM_Sensor_Location_t Location);

static void ProcessRegisterCollectorEventsMessage(CSCM_Register_Collector_Events_Request_t *Message);
static void ProcessUnRegisterCollectorEventsMessage(CSCM_Un_Register_Collector_Events_Request_t *Message);
static void ProcessQueryConnectedSensorsMessage(CSCM_Query_Connected_Sensors_Request_t *Message);
static void ProcessConfigureRemoteSensorMessage(CSCM_Configure_Remote_Sensor_Request_t *Message);
static void ProcessUnConfigureRemoteSensorMessage(CSCM_Un_Configure_Remote_Sensor_Request_t *Message);
static void ProcessGetConnectedSensorInfoMessage(CSCM_Get_Connected_Sensor_Info_Request_t *Message);
static void ProcessGetSensorLocationMessage(CSCM_Get_Sensor_Location_Request_t *Message);
static void ProcessUpdateCumulativeValueMessage(CSCM_Update_Cumulative_Value_Request_t *Message);
static void ProcessUpdateSensorLocationMessage(CSCM_Update_Sensor_Location_Request_t *Message);

static void ProcessReceivedMessage(BTPM_Message_t *Message);
static void ProcessClientUnRegister(unsigned int ClientID);

static void ProcessCSCMConnection(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties);
static void ProcessCSCMDisconnection(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties);
static void ProcessCSCMDevicePaired(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties);
static void ProcessCSCMDeviceUnPaired(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties);
...
static Device_Entry_t *SearchDeviceEntryByProcedureID(Device_Entry_t **ListHead, unsigned int ProcedureID)
{
   return((Device_Entry_t *)BSC_SearchGenericListEntry(ekUnsignedInteger, (void *)&ProcedureID, STRUCTURE_OFFSET(Device_Entry_t, CurrentProcedureID), STRUCTURE_OFFSET(Device_Entry_t, NextDeviceEntryPtr), (void *)ListHead));
}
...
static void FreeTransactionEntryList(Transaction_Entry_t **ListHead)
{
   ...
   /* CSC event to the specified callback.                              */
...
static void DispatchCSCEvent(Callback_Info_t *CallbackInfo, CSCM_Event_Data_t *EventData, BTPM_Message_t *Message, Boolean_t LockHeld)
{
   ...
   /* The following function is a utility function that exists to       */
   /* dispatch the specified CSC Data event to the correct registered   */
   /* CSC Data Event Callback.                                          */
   /* * NOTE * This function should be called with the CSC Manager Lock */
         ...
         if(CallbackInfoArrayPtr)
         {
            ...
            while((Index < NumberCallbacks) && (Initialized))
            {
               if(CallbackInfoArrayPtr[Index].ClientID != ServerID)
               {
                  ...
                  if(Index == Index1)
                     DispatchCSCEvent(&CallbackInfoArray[Index], EventData, Message, FALSE);
               }
               else
                  DispatchCSCEvent(&CallbackInfoArray[Index], EventData, Message, FALSE);
...
static void DispatchConnectedEvent(Device_Entry_t *Device)
{
   ...
   CSCM_Connected_Message_t Message;
   ...
   if(Device)
   {
      ...
      BTPS_MemInitialize(&Message, 0, CSCM_CONNECTED_MESSAGE_SIZE);

      Message.MessageHeader.AddressID         = 0;
      Message.MessageHeader.MessageID         = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup      = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction   = CSCM_MESSAGE_FUNCTION_CONNECTED;
      Message.MessageHeader.MessageLength     = CSCM_CONNECTED_MESSAGE_SIZE;
      ...
      DispatchCSCBroadcastEvent(&EventData, (BTPM_Message_t *)&Message, FALSE);
   }
}
...
{
   CSCM_Event_Data_t        EventData;
   CSCM_Disconnected_Message_t Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_DISCONNECTED_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_DISCONNECTED;
      Message.MessageHeader.MessageLength   = CSCM_DISCONNECTED_MESSAGE_SIZE;
...
{
   CSCM_Event_Data_t                           EventData;
   CSCM_Configuration_Status_Changed_Message_t Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_CONFIGURATION_STATUS_CHANGED_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_CONFIGURATION_STATUS_CHANGED;
      Message.MessageHeader.MessageLength   = CSCM_CONFIGURATION_STATUS_CHANGED_MESSAGE_SIZE;
...
{
   CSCM_Event_Data_t           EventData;
   CSCM_Measurement_Message_t  Message;
   ...
   if(Device)
   {
      ...
      EventData.EventData.MeasurementEventData.Flags               = Flags;
      EventData.EventData.MeasurementEventData.WheelRevolutionData = *WheelData;
      EventData.EventData.MeasurementEventData.CrankRevolutionData = *CrankData;

      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_MEASUREMENT_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_MEASUREMENT;
      Message.MessageHeader.MessageLength   = CSCM_MEASUREMENT_MESSAGE_SIZE;

      Message.RemoteDeviceAddress           = Device->BluetoothAddress;
      Message.Flags                         = Flags;
      Message.WheelRevolutionData           = *WheelData;
      Message.CrankRevolutionData           = *CrankData;
...
static void DispatchSensorLocationResponseEvent(unsigned int ClientID, Device_Entry_t *Device, unsigned int Status, CSCM_Sensor_Location_t Location)
{
   Callback_Entry_t                        *CallbackEntry;
   CSCM_Event_Data_t                        EventData;
   CSCM_Sensor_Location_Response_Message_t  Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                 */
      BTPS_MemInitialize(&Message, 0, CSCM_SENSOR_LOCATION_RESPONSE_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_SENSOR_LOCATION_RESPONSE;
      Message.MessageHeader.MessageLength   = CSCM_SENSOR_LOCATION_RESPONSE_MESSAGE_SIZE;
      ...
      if(ClientID == MSG_GetServerAddressID())
         DispatchCSCBroadcastEvent(&EventData, (BTPM_Message_t *)&Message, TRUE);
      else
      {
         /* Make sure this client has registered.                       */
         if((CallbackEntry = SearchCallbackEntryByClientID(&CallbackEntryList, ClientID)) != NULL)
            DispatchCSCEvent(&CallbackEntry->CallbackInfo, &EventData, (BTPM_Message_t *)&Message, TRUE);
      }
   }
...
{
   CSCM_Event_Data_t                       EventData;
   CSCM_Cumulative_Value_Updated_Message_t Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_CUMULATIVE_VALUE_UPDATED_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_CUMULATIVE_VALUE_UPDATED;
      Message.MessageHeader.MessageLength   = CSCM_CUMULATIVE_VALUE_UPDATED_MESSAGE_SIZE;
...
{
   CSCM_Event_Data_t                      EventData;
   CSCM_Sensor_Location_Updated_Message_t Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_SENSOR_LOCATION_UPDATED_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_SENSOR_LOCATION_UPDATED;
      Message.MessageHeader.MessageLength   = CSCM_SENSOR_LOCATION_UPDATED_MESSAGE_SIZE;
...
{
   CSCM_Event_Data_t           EventData;
   CSCM_Procedure_Complete_Message_t  Message;
   ...
   if(Device)
   {
      ...
      /* Now format the IPC Message.                                    */
      BTPS_MemInitialize(&Message, 0, CSCM_PROCEDURE_COMPLETE_MESSAGE_SIZE);

      Message.MessageHeader.AddressID       = 0;
      Message.MessageHeader.MessageID       = MSG_GetNextMessageID();
      Message.MessageHeader.MessageGroup    = BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER;
      Message.MessageHeader.MessageFunction = CSCM_MESSAGE_FUNCTION_PROCEDURE_COMPLETE;
      Message.MessageHeader.MessageLength   = CSCM_PROCEDURE_COMPLETE_MESSAGE_SIZE;
...
static Boolean_t ReloadCSCDeviceFromFile(Device_Entry_t *Device, BD_ADDR_t BluetoothAddress)
{
   char                        KeyName[CSCM_LE_CONFIGURATION_FILE_MAXIMUM_KEY_LENGTH];
   ...
   {
      sprintf(KeyName, CSCM_LE_KEY_NAME_DEVICE_INFORMATION, BluetoothAddress.BD_ADDR5, BluetoothAddress.BD_ADDR4, BluetoothAddress.BD_ADDR3, BluetoothAddress.BD_ADDR2, BluetoothAddress.BD_ADDR1, BluetoothAddress.BD_ADDR0);

      if(SET_ReadBinaryData(CSCM_LE_CONFIGURATION_FILE_SECTION_NAME, KeyName, (unsigned char *)&StoredDeviceInformation, CSCM_LE_DEVICE_INFORMATION_VALUE_SIZE, BTPM_CONFIGURATION_LOW_ENERGY_CONFIGURATION_FILE_NAME) == CSCM_LE_DEVICE_INFORMATION_VALUE_SIZE)
      {
         DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Loaded: %s, %08lX, %08lX\n", StoredDeviceInformation.Configured?"TRUE":"FALSE",StoredDeviceInformation.SupportedFeatures, StoredDeviceInformation.SupportedLocations));
...
static void StoreCSCMInformation(BD_ADDR_t BD_ADDR, Stored_Device_Information_t *StoredDeviceInformation)
{
   char KeyName[CSCM_LE_CONFIGURATION_FILE_MAXIMUM_KEY_LENGTH];

   if(StoredDeviceInformation)
   {
      DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Storing Info: %s, %08lX, %08lX\n", StoredDeviceInformation->Configured?"TRUE":"FALSE", StoredDeviceInformation->SupportedFeatures, StoredDeviceInformation->SupportedLocations));
      sprintf(KeyName, CSCM_LE_KEY_NAME_DEVICE_INFORMATION, BD_ADDR.BD_ADDR5, BD_ADDR.BD_ADDR4, BD_ADDR.BD_ADDR3, BD_ADDR.BD_ADDR2, BD_ADDR.BD_ADDR1, BD_ADDR.BD_ADDR0);

      /* Write out the information structure.                           */
      SET_WriteBinaryData(CSCM_LE_CONFIGURATION_FILE_SECTION_NAME, KeyName, (unsigned char *)StoredDeviceInformation, CSCM_LE_DEVICE_INFORMATION_VALUE_SIZE, BTPM_CONFIGURATION_LOW_ENERGY_CONFIGURATION_FILE_NAME);
   }
}
...
static void StoreCSCMDeviceToFile(Device_Entry_t *Device)
{
   ...
   if((Device) && (Device->Flags & DEVICE_ENTRY_FLAGS_STORE_STATE) && (!COMPARE_NULL_BD_ADDR(Device->BluetoothAddress)))
   {
      ...
      StoreCSCMInformation(Device->BluetoothAddress, &StoredDeviceInformation);
   }
...
static void ClearCSCMDeviceFromFile(BD_ADDR_t BluetoothAddress)
{
   ...
   if(!COMPARE_NULL_BD_ADDR(BluetoothAddress))
   {
      ...
      SET_WriteBinaryData(CSCM_LE_CONFIGURATION_FILE_SECTION_NAME, KeyName, NULL, 0, BTPM_CONFIGURATION_LOW_ENERGY_CONFIGURATION_FILE_NAME);
   }
...
static unsigned long ConvertLocationListToMask(CSCS_SCCP_Supported_Sensor_Locations_t *SupportedSensorLocations)
{
   ...
   for(Index=0;Index<SupportedSensorLocations->NumberOfSensorLocations;Index++)
   {
      ...
      {
         case CSCS_SENSOR_LOCATION_TOP_OF_SHOE:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_TOP_OF_SHOE;
            break;
         case CSCS_SENSOR_LOCATION_IN_SHOE:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_IN_SHOE;
            break;
         case CSCS_SENSOR_LOCATION_HIP:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_HIP;
            break;
         case CSCS_SENSOR_LOCATION_FRONT_WHEEL:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_FRONT_WHEEL;
            break;
         case CSCS_SENSOR_LOCATION_LEFT_CRANK:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_LEFT_CRANK;
            break;
         case CSCS_SENSOR_LOCATION_RIGHT_CRANK:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_RIGHT_CRANK;
            break;
         case CSCS_SENSOR_LOCATION_LEFT_PEDAL:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_LEFT_PEDAL;
            break;
         case CSCS_SENSOR_LOCATION_RIGHT_PEDAL:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_RIGHT_PEDAL;
            break;
         case CSCS_SENSOR_LOCATION_FRONT_HUB:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_FRONT_HUB;
            break;
         case CSCS_SENSOR_LOCATION_REAR_DROPOUT:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_REAR_DROPOUT;
            break;
         case CSCS_SENSOR_LOCATION_CHAINSTAY:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_CHAINSTAY;
            break;
         case CSCS_SENSOR_LOCATION_REAR_WHEEL:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_REAR_WHEEL;
            break;
         case CSCS_SENSOR_LOCATION_REAR_HUB:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_REAR_HUB;
            break;
         case CSCS_SENSOR_LOCATION_CHEST:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_CHEST;
            break;
         default:
         case CSCS_SENSOR_LOCATION_OTHER:
            Mask |= CSCM_SUPPORTED_SENSOR_LOCATION_OTHER;
            break;
      }
   }
   ...
   /* This function will take a GATT UUID from a discovered CSC service */
   /* and attempt to determine the characteristic the UUID represents.  */
static CSC_UUID_t DetermineCSCUUID(GATT_UUID_t UUID)
{
   CSC_UUID_t Type = cuUnknown;

   DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Enter\n"));

   if(UUID.UUID_Type == guUUID_16)
   {
      if(CSCS_COMPARE_CSC_MEASUREMENT_UUID_TO_UUID_16(UUID.UUID.UUID_16))
         Type = cuMeasurements;
      else
      {
         if(CSCS_COMPARE_CSC_FEATURE_UUID_TO_UUID_16(UUID.UUID.UUID_16))
            Type = cuFeatures;
         else
         {
            if(CSCS_COMPARE_SENSOR_LOCATION_UUID_TO_UUID_16(UUID.UUID.UUID_16))
               Type = cuLocation;
            else
            {
               if(CSCS_COMPARE_SC_CONTROL_POINT_UUID_TO_UUID_16(UUID.UUID.UUID_16))
                  Type = cuControlPoint;
            }
         }
      }
...
static Word_t FindCCCD(GATT_Characteristic_Information_t *Characteristic)
{
   ...
   /* This function is used to discover the CSC Sensor service on a     */
...
static Device_Entry_t *DiscoverCSCSensor(BD_ADDR_t *BluetoothAddress)
{
   ...
   if((Result = DEVM_QueryRemoteDeviceServices(*BluetoothAddress, DEVM_QUERY_REMOTE_DEVICE_SERVICES_FLAGS_LOW_ENERGY, 0, NULL, &ServiceDataSize)) == 0)
   {
         ...
         if((Result = DEVM_QueryRemoteDeviceServices(*BluetoothAddress, DEVM_QUERY_REMOTE_DEVICE_SERVICES_FLAGS_LOW_ENERGY, ServiceDataSize, ServiceData, NULL)) > 0)
         {
            ...
            if((Result = DEVM_ConvertRawServicesStreamToParsedServicesData(ServiceDataSize, ServiceData, &ParsedGATTData)) == 0)
            {
               ...
               for(Index=0;Index<ParsedGATTData.NumberServices;Index++)
               {
                  ...
                  /* Check for CSC.                                     */
                  if((UUID.UUID_Type == guUUID_16) && (CSCS_COMPARE_SERVICE_UUID_TO_UUID_16(UUID.UUID.UUID_16)))
                  {
                     ...
                     for(CharacteristicIndex=0;CharacteristicIndex<GATTServiceDiscoveryIndicationData->NumberOfCharacteristics;CharacteristicIndex++)
                     {
                        ...
                        switch(DetermineCSCUUID(UUID))
                        {
                           case cuMeasurements:
                              if((DeviceEntry.ClientInformation.CSC_Measurement_Client_Configuration = FindCCCD(CharacteristicInformation)) > 0)
                                 DeviceEntry.ClientInformation.CSC_Measurement = CharacteristicInformation->Characteristic_Handle;
                              break;
                           case cuFeatures:
                              DeviceEntry.ClientInformation.CSC_Feature = CharacteristicInformation->Characteristic_Handle;
                              break;
                           case cuLocation:
                              DeviceEntry.ClientInformation.Sensor_Location = CharacteristicInformation->Characteristic_Handle;
                              DeviceEntry.ExtraCharacteristics             |= CSCM_SUPPORTED_CHARACTERISTIC_SENSOR_LOCATION;
                              break;
                           case cuControlPoint:
                              if((DeviceEntry.ClientInformation.SC_Control_Point_Client_Configuration = FindCCCD(CharacteristicInformation)) > 0)
                              {
                                 ...
                                 DeviceEntry.ExtraCharacteristics              |= CSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT;
                              }
                              break;
                           case cuUnknown:
                           default:
                              DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unknown characteristic in CSC Service\n"));
                        }
                     }

                     /* The Measurement and Features characteristics    */
                     /* MUST be present.                                */
                     if((DeviceEntry.ClientInformation.CSC_Measurement) && (DeviceEntry.ClientInformation.CSC_Feature))
                     {
                        ...
                        if((DeviceEntryPtr = AddDeviceEntry(&DeviceEntryList, &DeviceEntry)) != NULL)
                        {
                           ...
                           /* about this device.                        */
                           ReloadCSCDeviceFromFile(DeviceEntryPtr, DeviceEntryPtr->BluetoothAddress);
                     ...
                     /* Exit loop after parsing CSC information.        */
                     break;
                  }
               }
            ...
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FAILURE), ("DEVM_ConvertRawServicesStreamToParsedServicesData returned %d.\n", Result));
            }
         }
         else
         {
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FAILURE), ("DEVM_QueryRemoteDeviceServices returned %d.\n", Result));
         }
      ...
      {
         DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FAILURE), ("Allocation request for Service Data failed, size = %u.\n", ServiceDataSize));
      }
   }
   else
   {
   ...
   DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Exit 0x%X\n", DeviceEntry));
      ...
      if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &RemoteDeviceAddress)) != NULL)
      {
         ...
         if(DeviceEntry->ConfigurationState == csNotConfigured)
         {
            ...
            if((TransactionEntryPtr = AddTransactionEntry(&TransactionEntryList, &TransactionEntry)) != NULL)
            {
               ...
               if((ret_val = GATM_WriteValue(GATMCallbackID, RemoteDeviceAddress, DeviceEntry->ClientInformation.CSC_Measurement_Client_Configuration, NON_ALIGNED_WORD_SIZE, (Byte_t *)&Buffer)) > 0)
               {
                  ...
                  if(Flags & CSCM_CONFIGURE_FLAG_SKIP_SUPPORTED_SENSOR_LOCATIONS)
                     DeviceEntry->Flags |= DEVICE_ENTRY_FLAGS_SKIP_SUPPORTED_LOCATIONS;
                  else
                     DeviceEntry->Flags &= ~DEVICE_ENTRY_FLAGS_SKIP_SUPPORTED_LOCATIONS;
         ...
         else
         {
            ...
            if(DeviceEntry->ConfigurationState == csConfigured)
               ret_val = BTPM_ERROR_CODE_CSC_ALREADY_CONFIGURED;
            else
            {
               /* We could also be un-configuring.                      */
               if(DeviceEntry->ConfigurationState > csConfigured)
                  ret_val = BTPM_ERROR_CODE_CSC_CURRENTLY_UN_CONFIGURING;
               else
               {
                  DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Already Configuring: %d\n", DeviceEntry->ConfigurationState));
                  ret_val = 0;
               }
            }
         }
         ...
         ret_val = BTPM_ERROR_CODE_CSC_NOT_CONNECTED;
   }
   else
      ret_val = BTPM_ERROR_CODE_INVALID_PARAMETER;
   ...
   if(!COMPARE_NULL_BD_ADDR(RemoteDeviceAddress))
   {
      ...
      if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &RemoteDeviceAddress)) != NULL)
      {
         ...
         if(DeviceEntry->ConfigurationState == csConfigured)
         {
            ...
            TransactionEntry.TransactionID    = GetNextTransactionID();
            TransactionEntry.BluetoothAddress = RemoteDeviceAddress;
            TransactionEntry.TransactionType  = (DeviceEntry->ExtraCharacteristics & CSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT)?ttDisableControlCCCD:ttDisableMeasurementCCCD;

            Handle                            = (DeviceEntry->ExtraCharacteristics & CSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT)?DeviceEntry->ClientInformation.SC_Control_Point_Client_Configuration:DeviceEntry->ClientInformation.CSC_Measurement_Client_Configuration;
         ...
         else
         {
            ...
            /* need an error.                                           */
            if(DeviceEntry->ConfigurationState == csNotConfigured)
               ret_val = BTPM_ERROR_CODE_CSC_NOT_CONFIGURED;
            else
            {
               /* If we are configuring, throw in error.                */
               if(DeviceEntry->ConfigurationState < csConfigured)
                  ret_val = BTPM_ERROR_CODE_CSC_CURRENTLY_CONFIGURING;
               else
                  ret_val = 0;
            }
         }
...
static int ProcessGetSensorLocation(unsigned int ClientID, BD_ADDR_t RemoteDeviceAddress)
{
   ...
   if(!COMPARE_NULL_BD_ADDR(RemoteDeviceAddress))
   {
      if(((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &RemoteDeviceAddress)) != NULL) && (DeviceEntry->ConfigurationState == csConfigured))
      {
         ...
         if(DeviceEntry->ExtraCharacteristics & CSCM_SUPPORTED_CHARACTERISTIC_SENSOR_LOCATION)
         {
            ...
            if((SearchTransactionEntry(&TransactionEntryList, &DeviceEntry->BluetoothAddress, ttReadLocation, ClientID)) == NULL)
            {
               ...
               ret_val = BTPM_ERROR_CODE_CSC_SAME_REQUEST_ALREADY_OUTSTANDING;
         }
         else
            ret_val = BTPM_ERROR_CODE_CSC_REMOTE_FEATURE_NOT_SUPPORTED;
      }
      else
      {
...
static int ProcessUpdateCumulativeValue(unsigned int ClientID, BD_ADDR_t RemoteDeviceAddress, DWord_t CumulativeValue)
{
   ...
   Transaction_Entry_t                  TransactionEntry;
   Transaction_Entry_t                 *TransactionEntryPtr;
   CSCS_SC_Control_Point_Format_Data_t  CommandData;
   ...
   if(!COMPARE_NULL_BD_ADDR(RemoteDeviceAddress))
   {
      ...
      {
         /* Make sure the device supports this operation.               */
         if(DeviceEntry->ExtraCharacteristics & CSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT)
         {
            ...
            if(DeviceEntry->CurrentProcedure == cpNone)
            {
               ...
               if((TransactionEntryPtr = AddTransactionEntry(&TransactionEntryList, &TransactionEntry)) != NULL)
               {
                  ...
                  CommandData.CommandType                  = cpcSetCumulativeValueRequest;
                  CommandData.CmdParameter.CumulativeValue = CumulativeValue;

                  BufferLength = sizeof(Buffer);

                  if((ret_val = CSCS_Format_SC_Control_Point_Command(&CommandData, &BufferLength, Buffer)) == 0)
                  {
                     ...
                     if((ret_val = GATM_WriteValue(GATMCallbackID, DeviceEntry->BluetoothAddress, DeviceEntry->ClientInformation.SC_Control_Point, BufferLength, Buffer)) > 0)
                     {
                        ...
                        if((ret_val = TMR_StartTimer((void *)ProcedureID, ProcedureTimerCallback, CSCM_PROCEDURE_TIMEOUT)) > 0)
                        {
               ...
               ret_val = BTPM_ERROR_CODE_CSC_CONTROL_POINT_PROCEDURE_IN_PROGRESS;
         }
         else
            ret_val = BTPM_ERROR_CODE_CSC_REMOTE_FEATURE_NOT_SUPPORTED;
      }
...
static int ProcessUpdateSensorLocation(unsigned int ClientID, BD_ADDR_t RemoteDeviceAddress, CSCM_Sensor_Location_t Location)
{
      ...
      if(((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &RemoteDeviceAddress)) != NULL) && (DeviceEntry->ConfigurationState == csConfigured))
      {
         ...
         if(DeviceEntry->ExtraCharacteristics & CSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT)
         {
            ...
            if(DeviceEntry->CurrentProcedure == cpNone)
            {
               ...
               {
                  /* Format the Control Point Command.                  */
                  CommandData.CommandType                 = cpcUpdateSensorLocationRequest;
                  CommandData.CmdParameter.SensorLocation = Location;
   ...
   /* * NOTE * This function *MUST* be called with the CSC Manager Lock */
   /*          held.                                                    */
static void ProcessRegisterCollectorEventsMessage(CSCM_Register_Collector_Events_Request_t *Message)
{
   int                                       Result;
   CSCM_Register_Collector_Events_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_REGISTER_COLLECTOR_EVENTS_RESPONSE_SIZE);
      ...
      ResponseMessage.MessageHeader.MessageLength  = CSCM_REGISTER_COLLECTOR_EVENTS_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
...
static void ProcessUnRegisterCollectorEventsMessage(CSCM_Un_Register_Collector_Events_Request_t *Message)
{
   CSCM_Un_Register_Collector_Events_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_UN_REGISTER_COLLECTOR_EVENTS_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_UN_REGISTER_COLLECTOR_EVENTS_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = 0;
...
static void ProcessQueryConnectedSensorsMessage(CSCM_Query_Connected_Sensors_Request_t *Message)
{
   ...
   CSCM_Query_Connected_Sensors_Response_t  StaticMessage;
   CSCM_Query_Connected_Sensors_Response_t *ResponseMessage;
   ...
   if(Message)
   {
      ...
      if((Result = ProcessQueryConnectedSensors(Message->MessageHeader.AddressID, 0, NULL, &TotalDevices)) == 0)
      {
         ...
         {
            BTPS_MemInitialize(&StaticMessage, 0, CSCM_QUERY_CONNECTED_SENSORS_RESPONSE_SIZE(0));
            ...
            StaticMessage.MessageHeader.MessageLength = CSCM_QUERY_CONNECTED_SENSORS_RESPONSE_SIZE(0) - BTPM_MESSAGE_HEADER_SIZE;
         ...
         else
         {
            ...
            if((ResponseMessage = (CSCM_Query_Connected_Sensors_Response_t *)BTPS_AllocateMemory(CSCM_QUERY_CONNECTED_SENSORS_RESPONSE_SIZE(NumberDevices))) != NULL)
            {
               BTPS_MemInitialize(ResponseMessage, 0, CSCM_QUERY_CONNECTED_SENSORS_RESPONSE_SIZE(NumberDevices));
               ...
               if((Result = ProcessQueryConnectedSensors(Message->MessageHeader.AddressID, NumberDevices, ResponseMessage->ConnectedSensors, &ResponseMessage->TotalNumberConnectedDevices)) >= 0)
               {
                  ...
                  ResponseMessage->MessageHeader               = Message->MessageHeader;
                  ResponseMessage->MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
                  ResponseMessage->MessageHeader.MessageLength = CSCM_QUERY_CONNECTED_SENSORS_RESPONSE_SIZE(NumberDevices) - BTPM_MESSAGE_HEADER_SIZE;
                  ResponseMessage->Status                      = 0;
...
{
   int                                     Result;
   CSCM_Configure_Remote_Sensor_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_CONFIGURE_REMOTE_SENSOR_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_CONFIGURE_REMOTE_SENSOR_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
...
{
   int                                        Result;
   CSCM_Un_Configure_Remote_Sensor_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_UN_CONFIGURE_REMOTE_SENSOR_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_UN_CONFIGURE_REMOTE_SENSOR_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
...
{
   int                                       Result;
   CSCM_Connected_Sensor_t                   Sensor;
   CSCM_Get_Connected_Sensor_Info_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_GET_CONNECTED_SENSOR_INFO_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_GET_CONNECTED_SENSOR_INFO_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
...
{
   int                                 Result;
   CSCM_Get_Sensor_Location_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_GET_SENSOR_LOCATION_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_GET_SENSOR_LOCATION_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
...
{
   int                                     Result;
   CSCM_Update_Cumulative_Value_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_UPDATE_CUMULATIVE_VALUE_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_UPDATE_CUMULATIVE_VALUE_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
...
{
   int                                    Result;
   CSCM_Update_Sensor_Location_Response_t ResponseMessage;
   ...
   if(Message)
   {
      ...
      /* Format and send the response message.                          */
      BTPS_MemInitialize(&ResponseMessage, 0, CSCM_UPDATE_SENSOR_LOCATION_RESPONSE_SIZE);

      ResponseMessage.MessageHeader               = Message->MessageHeader;
      ResponseMessage.MessageHeader.MessageID    |= BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK;
      ResponseMessage.MessageHeader.MessageLength = CSCM_UPDATE_SENSOR_LOCATION_RESPONSE_SIZE - BTPM_MESSAGE_HEADER_SIZE;
      ResponseMessage.Status                      = Result;
   ...
   /* circumstances.                                                    */
   /* * NOTE * This function *MUST* be called with the CSC Manager      */
...
static void ProcessReceivedMessage(BTPM_Message_t *Message)
{
   ...
   if((Message) && (!(Message->MessageHeader.MessageID & BTPM_MESSAGE_MESSAGE_ID_RESPONSE_MASK)))
   {
      ...
      {
         case CSCM_MESSAGE_FUNCTION_REGISTER_COLLECTOR_EVENTS:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Register Events Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_REGISTER_COLLECTOR_EVENTS_REQUEST_SIZE)
               ProcessRegisterCollectorEventsMessage((CSCM_Register_Collector_Events_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_UN_REGISTER_COLLECTOR_EVENTS:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Un Register Events Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_UN_REGISTER_COLLECTOR_EVENTS_REQUEST_SIZE)
               ProcessUnRegisterCollectorEventsMessage((CSCM_Un_Register_Collector_Events_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_QUERY_CONNECTED_SENSORS:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Query Connected Sensors Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_QUERY_CONNECTED_SENSORS_REQUEST_SIZE)
               ProcessQueryConnectedSensorsMessage((CSCM_Query_Connected_Sensors_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_CONFIGURE_REMOTE_SENSOR:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Configure Remote Sensor Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_CONFIGURE_REMOTE_SENSOR_REQUEST_SIZE)
               ProcessConfigureRemoteSensorMessage((CSCM_Configure_Remote_Sensor_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_UN_CONFIGURE_REMOTE_SENSOR:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Un Configure Remote Sensor Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_UN_CONFIGURE_REMOTE_SENSOR_REQUEST_SIZE)
               ProcessUnConfigureRemoteSensorMessage((CSCM_Un_Configure_Remote_Sensor_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_GET_CONNECTED_SENSOR_INFO:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Get Connected Sensor Info Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_GET_CONNECTED_SENSOR_INFO_REQUEST_SIZE)
               ProcessGetConnectedSensorInfoMessage((CSCM_Get_Connected_Sensor_Info_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_GET_SENSOR_LOCATION:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Get Sensor Location Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_GET_SENSOR_LOCATION_REQUEST_SIZE)
               ProcessGetSensorLocationMessage((CSCM_Get_Sensor_Location_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_UPDATE_CUMULATIVE_VALUE:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Update Cumulative Value Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_UPDATE_CUMULATIVE_VALUE_REQUEST_SIZE)
               ProcessUpdateCumulativeValueMessage((CSCM_Update_Cumulative_Value_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
            break;
         case CSCM_MESSAGE_FUNCTION_UPDATE_SENSOR_LOCATION:
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("aaa Message\n"));

            if(BTPM_MESSAGE_SIZE(Message->MessageHeader.MessageLength) >= CSCM_UPDATE_SENSOR_LOCATION_REQUEST_SIZE)
               ProcessUpdateSensorLocationMessage((CSCM_Update_Sensor_Location_Request_t *)Message);
            else
            {
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Invalid Message Length\n"));
            }
...
static void ProcessClientUnRegister(unsigned int ClientID)
{
   ...
   DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Enter: %u\n", ClientID));
   ...
   /* The following function is a utility function that is used to      */
   /* process a possible CSCM Connection.                               */
...
static void ProcessCSCMConnection(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if(RemoteDeviceProperties)
   {
      ...
      if((DeviceEntry = DiscoverCSCSensor(&(RemoteDeviceProperties->BD_ADDR))) != NULL)
      {
   ...
   /* The following function is a utility function that is used to      */
   /* process a possible CSCM Connection.                               */
   /* * NOTE * This function *MUST* be called with the CSCM Manager Lock*/
   /*          held.                                                    */
static void ProcessCSCMDisconnection(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   Device_Entry_t *DeviceEntry;
   ...
   /* The following function is a utility function that is used to      */
   /* process a possible CSCM Device Paired Event.                      */
   /* * NOTE * This function *MUST* be called with the CSCM Manager Lock*/
   /*          held.                                                    */
static void ProcessCSCMDevicePaired(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if(RemoteDeviceProperties)
   {
      if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &(RemoteDeviceProperties->BD_ADDR))) != NULL)
      {
         ...
         StoreCSCMDeviceToFile(DeviceEntry);
      }
   }
   ...
   /* The following function is a utility function that is used to      */
   /* process a possible CSCM Device Un-Paired Event.                   */
   /* * NOTE * This function *MUST* be called with the CSCM Manager Lock*/
   /*          held.                                                    */
static void ProcessCSCMDeviceUnPaired(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if(RemoteDeviceProperties)
   {
      ...
      if(ReloadCSCDeviceFromFile(NULL, RemoteDeviceProperties->BD_ADDR))
         ClearCSCMDeviceFromFile(RemoteDeviceProperties->BD_ADDR);
   }
...
static void ProcessLowEnergyAddressChangeEvent(DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if(RemoteDeviceProperties)
   {
      ...
      if(ReloadCSCDeviceFromFile(NULL, RemoteDeviceProperties->PriorResolvableBD_ADDR))
         ClearCSCMDeviceFromFile(RemoteDeviceProperties->PriorResolvableBD_ADDR);
...
static void ProcessRemoteDeviceDeletedEvent(BD_ADDR_t RemoteDeviceAddress)
{
   ...
   if(!COMPARE_NULL_BD_ADDR(RemoteDeviceAddress))
   {
      ...
      if(ReloadCSCDeviceFromFile(NULL, RemoteDeviceAddress))
         ClearCSCMDeviceFromFile(RemoteDeviceAddress);
   }
...
static void ProcessRemoteDevicePropertiesChangedEvent(unsigned long ChangedMemberMask, DEVM_Remote_Device_Properties_t *RemoteDeviceProperties)
{
   ...
   if((ChangedMemberMask) && (RemoteDeviceProperties) && (RemoteDeviceProperties->RemoteDeviceFlags & DEVM_REMOTE_DEVICE_FLAGS_DEVICE_SUPPORTS_LOW_ENERGY))
   {
      ...
      {
         DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Device Flags: 0x%08X\n", RemoteDeviceProperties->RemoteDeviceFlags));
         ...
         if(ChangedMemberMask & (DEVM_REMOTE_DEVICE_PROPERTIES_CHANGED_LE_CONNECTION_STATE | DEVM_REMOTE_DEVICE_PROPERTIES_CHANGED_LE_SERVICES_STATE))
         {
            ...
            {
               /* Process a possible CSCM Connection.                   */
               ProcessCSCMConnection(RemoteDeviceProperties);
            }
            else
            {
               /* Process a possible disconnection.                     */
               ProcessCSCMDisconnection(RemoteDeviceProperties);
      ...
      if(ChangedMemberMask & DEVM_REMOTE_DEVICE_PROPERTIES_CHANGED_LE_PAIRING_STATE)
      {
         ...
         if(RemoteDeviceProperties->RemoteDeviceFlags & DEVM_REMOTE_DEVICE_FLAGS_DEVICE_CURRENTLY_PAIRED_OVER_LE)
            ProcessCSCMDevicePaired(RemoteDeviceProperties);
         else
            ProcessCSCMDeviceUnPaired(RemoteDeviceProperties);
      }
         ...
         if(Success)
         {
            ...
            switch(TransactionEntry->TransactionType)
            {
               case ttEnableMeasurementCCCD:
                  ...
                  Handle                              = DeviceEntry->ClientInformation.CSC_Feature;
               ...
               case ttReadFeatures:
                  ...
                  else
                  {
                     ...
                     DispatchConfigurationStatusChangedEvent(DeviceEntry, CSCM_CONFIGURATION_STATUS_SUCCESS);
               ...
               case ttEnableControlCCCD:
                  ...
                  /* locations.                                         */
                  DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Control enabled: %08lX, %08lX\n", DeviceEntry->SupportedFeatures, DeviceEntry->Flags));
                  if((DeviceEntry->SupportedFeatures & CSCM_FEATURE_MULTIPLE_SENSOR_LOCATIONS_SUPPORTED) && !(DeviceEntry->Flags & DEVICE_ENTRY_FLAGS_SKIP_SUPPORTED_LOCATIONS))
                  {
                     /* Format the command to write to the control      */
                     /* point.                                          */
                     ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&OpCode, CSCS_SC_CONTROL_POINT_OPCODE_REQUEST_SUPPORTED_SENSOR_LOCATIONS);
               ...
               case ttDisableControlCCCD:
                  ...
                  Handle                              = DeviceEntry->ClientInformation.CSC_Measurement_Client_Configuration;
            ...
            if(Submit)
            {
               ...
               {
                  DeviceEntry->ConfigurationState = csNotConfigured;
                  DispatchConfigurationStatusChangedEvent(DeviceEntry, CSCM_CONFIGURATION_STATUS_GATT_OPERATION_FAILURE);
               }
            }
         }
         else
...
static void ProcessReadLocationResponse(GATM_Event_Data_t *EventData, Transaction_Entry_t *TransactionEntry, Boolean_t Success)
{
   ...
   if((EventData) && (TransactionEntry))
   {
      ...
      if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &TransactionEntry->BluetoothAddress)) != NULL)
      {
         ...
         if(Success)
         {
            if(EventData->EventData.ReadResponseEventData.ValueLength == CSCS_SENSOR_LOCATION_VALUE_LENGTH)
            {
               Location = READ_UNALIGNED_BYTE_LITTLE_ENDIAN(EventData->EventData.ReadResponseEventData.Value);

               if(CSCS_SENSOR_LOCATION_VALID(Location))
                  DispatchSensorLocationResponseEvent(TransactionEntry->ClientID, DeviceEntry, CSCM_GET_SENSOR_LOCATION_STATUS_SUCCESS, (CSCM_Sensor_Location_t)Location);
               else
                  DispatchSensorLocationResponseEvent(TransactionEntry->ClientID, DeviceEntry, CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_UNKNOWN, 0);
            }
            else
               DispatchSensorLocationResponseEvent(TransactionEntry->ClientID, DeviceEntry, CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_UNKNOWN, 0);
         }
         else
         {
            switch(EventData->EventData.ErrorResponseEventData.ErrorType)
            {
               case retProtocolTimeout:
                  Status = CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_TIMEOUT;
                  break;
               case retErrorResponse:
                  switch(EventData->EventData.ErrorResponseEventData.AttributeProtocolErrorCode)
                  {
                     ...
                     case ATT_PROTOCOL_ERROR_CODE_INSUFFICIENT_ENCRYPTION_KEY_SIZE:
                        Status = CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_SECURITY;
                        break;
                     case ATT_PROTOCOL_ERROR_CODE_INSUFFICIENT_RESOURCES:
                        Status = CSCM_GET_SENSOR_LOCATION_STATUS_INSUFFICIENT_RESOURCES;
                        break;
                     default:
                        Status = CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_UNKNOWN;
                  }
                  break;
               default:
                  Status = CSCM_GET_SENSOR_LOCATION_STATUS_FAILURE_UNKNOWN;
   ...
   if((EventData) && (TransactionEntry))
   {
      ...
      if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &TransactionEntry->BluetoothAddress)) != NULL)
      {
         ...
         if(DeviceEntry->ConfigurationState == csConfigured)
         {
            ...
            {
               case retProtocolTimeout:
                  Status = CSCM_PROCEDURE_STATUS_FAILURE_TIMEOUT;
                  break;
               case retErrorResponse:
                  switch(EventData->AttributeProtocolErrorCode)
                  {
                     ...
                     case ATT_PROTOCOL_ERROR_CODE_INSUFFICIENT_ENCRYPTION:
                     case ATT_PROTOCOL_ERROR_CODE_INSUFFICIENT_ENCRYPTION_KEY_SIZE:
                        Status = CSCM_PROCEDURE_STATUS_FAILURE_SECURITY;
                        break;
                     case ATT_PROTOCOL_ERROR_CODE_INSUFFICIENT_RESOURCES:
                        Status = CSCM_PROCEDURE_STATUS_INSUFFICIENT_RESOURCES;
                        break;
                     default:
                        Status = CSCM_PROCEDURE_STATUS_FAILURE_UNKNOWN;
                  }
                  break;
               default:
                  Status = CSCM_PROCEDURE_STATUS_FAILURE_UNKNOWN;
...
static void ProcessGATTNotification(GATM_Handle_Value_Data_Event_Data_t *EventData)
{
   unsigned long                 Flags;
   Device_Entry_t               *DeviceEntry;
   CSCM_Wheel_Revolution_Data_t  WheelData;
   CSCM_Crank_Revolution_Data_t  CrankData;
   CSCS_CSC_Measurement_Data_t   MeasurementData;
   ...
   if(EventData)
   {
      ...
      {
         if(CSCS_Decode_CSC_Measurement(EventData->AttributeValueLength, EventData->AttributeValue, &MeasurementData) == 0)
         {
            Flags                                = MeasurementData.Flags;

            WheelData.CumulativeWheelRevolutions = MeasurementData.WheelRevolutionData.CumulativeWheelRevolutions;
            WheelData.LastWheelEventTime         = MeasurementData.WheelRevolutionData.LastWheelEventTime;

            CrankData.CumulativeCrankRevolutions = MeasurementData.CrankRevolutionData.CumulativeCrankRevolutions;
            CrankData.LastCrankEventTime         = MeasurementData.CrankRevolutionData.LastCrankEventTime;

            DispatchMeasurementEvent(DeviceEntry, Flags, &WheelData, &CrankData);
         }
      }
   }
...
static void ProcessGATTIndication(GATM_Handle_Value_Data_Event_Data_t *EventData)
{
   int Result;
   Device_Entry_t *DeviceEntry;
   CSCS_SC_Control_Point_Response_Data_t ResponseData;
   ...
   if((DeviceEntry = SearchDeviceEntry(&DeviceEntryList, &EventData->RemoteDeviceAddress)) != NULL)
   {
      ...
      {
         /* First, stop the timer for this procedure since it has       */
         /* completed.                                                  */
         TMR_StopTimer(DeviceEntry->ProcedureTimerID);
         DeviceEntry->ProcedureTimerID = 0;

         /* Now try to decode the result.                               */
         if((Result = CSCS_Decode_SC_Control_Point_Response(EventData->AttributeValueLength, EventData->AttributeValue, &ResponseData)) == 0)
         {
            if(DeviceEntry->ConfigurationState == csConfigured)
            {
               /* Check if the response is successful.                  */
               if(ResponseData.ResponseCodeValue == CSCS_SC_CONTROL_POINT_RESPONSE_CODE_SUCCESS)
               {
                  ...
                  switch(DeviceEntry->CurrentProcedure)
                  {
                     case cpUpdateCumulative:
                        if(ResponseData.RequestOpCode == CSCS_SC_CONTROL_POINT_OPCODE_SET_CUMULATIVE_VALUE)
                        {
                           DispatchCumulativeValueUpdatedEvent(DeviceEntry);
                           Result = CSCM_PROCEDURE_STATUS_SUCCESS;
                        }
                        else
                           Result = CSCM_PROCEDURE_STATUS_FAILURE_INVALID_RESPONSE;
                        break;
                     case cpUpdateLocation:
                        if(ResponseData.RequestOpCode == CSCS_SC_CONTROL_POINT_OPCODE_UPDATE_SENSOR_LOCATION)
                        {
                        ...
                        break;
                     default:
                        DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unexpected procedure completed\n"));
                        Result = CSCM_PROCEDURE_STATUS_FAILURE_INVALID_RESPONSE;
                        break;
                  }
               ...
               else
               {
                  ...
                  DispatchProcedureCompleteEvent(DeviceEntry, CSCM_PROCEDURE_STATUS_FAILURE_ERROR_RESPONSE, ResponseData.ResponseCodeValue);
               }
            }
            else
            {
               if((DeviceEntry->ConfigurationState == csGettingSupportedLocations) && (ResponseData.RequestOpCode == CSCS_SC_CONTROL_POINT_OPCODE_REQUEST_SUPPORTED_SENSOR_LOCATIONS))
               {
                  if(ResponseData.ResponseCodeValue == CSCS_SC_CONTROL_POINT_RESPONSE_CODE_SUCCESS)
                  {
                     DeviceEntry->SupportedSensorLocations = ConvertLocationListToMask(&ResponseData.SupportedSensorLocations);
                     DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Supported Locations Known: %08X\n", DeviceEntry->SupportedSensorLocations));
                     DeviceEntry->ConfigurationState = csConfigured;
                     DispatchConfigurationStatusChangedEvent(DeviceEntry, CSCM_CONFIGURATION_STATUS_SUCCESS);
                  ...
                  {
                     DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Supported Locations Procedure Failed: %d\n", ResponseData.ResponseCodeValue));
                     DeviceEntry->ConfigurationState = csNotConfigured;
                     DispatchConfigurationStatusChangedEvent(DeviceEntry, CSCM_CONFIGURATION_STATUS_UNKNOWN_ERROR);
                  }
               }
               else
                  DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unexpected configuration state: %d\n", DeviceEntry->ConfigurationState));
            }
         }
         else
         {
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unable to parse control point response data: %d\n", Result));
            
            if(DeviceEntry->ConfigurationState == csConfigured)
            {
               /* Just dispatch the invalid response failure.           */
               DispatchProcedureCompleteEvent(DeviceEntry, CSCM_PROCEDURE_STATUS_FAILURE_INVALID_RESPONSE, 0);
            }
            else
            {
               DeviceEntry->ConfigurationState = csNotConfigured;
         ...
         DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Indication for unexpected handle: %04X\n", EventData->AttributeHandle));
   }
...
static Boolean_t BTPSAPI ProcedureTimerCallback(unsigned int TimerID, void *CallbackParameter)
{
   ...
   DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Enter (HFRM)\n"));
   ...
   DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("Exit (HFRM): %d\n", ret_val));
      ...
      if(DEVM_AcquireLock())
      {
         switch(EventData->EventType)
         {
            case getGATTWriteResponse:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("GATM Write Response\n"));
               ...
               if((TransactionEntry = DeleteTransactionEntryByGATMTransactionID(&TransactionEntryList, EventData->EventData.WriteResponseEventData.TransactionID)) != NULL)
               {
                  ...
                  switch(TransactionEntry->TransactionType)
                  {
                     ...
                     case ttWriteControlPoint:
                        ...
                        break;
                     default:
                        DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unknown Write Transaction Type\n"));
                        break;
                  }
            ...
            case getGATTReadResponse:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("GATM Read Response\n"));
               ...
               if((TransactionEntry = DeleteTransactionEntryByGATMTransactionID(&TransactionEntryList, EventData->EventData.ReadResponseEventData.TransactionID)) != NULL)
               {
                  ...
                  switch(TransactionEntry->TransactionType)
                  {
                     ...
                     case ttReadLocation:
                        ...
                        break;
                     default:
                        DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unknown Read Transaction Type\n"));
                  }
            ...
            case getGATTHandleValueData:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("GATM Handle Value Data\n"));
               ...
               if((TransactionEntry = DeleteTransactionEntryByGATMTransactionID(&TransactionEntryList, EventData->EventData.ErrorResponseEventData.TransactionID)) != NULL)
               {
                  switch(TransactionEntry->TransactionType)
                  {
                     ...
                     case ttReadLocation:
                        ...
                        break;
                     default:
                        DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unknown Transaction Type\n"));
                  }
                  FreeTransactionEntryMemory(TransactionEntry);
               }
               break;
            default:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("Unhandled GATM Event: %u\n", EventData->EventType));
               break;
         }
      ...
      if(Initialized)
      {
         if(DEVM_AcquireLock())
         {
            ...
            if((DeviceEntry = SearchDeviceEntryByProcedureID(&DeviceEntryList, (unsigned int)CallbackParameter)) != NULL)
               DispatchProcedureCompleteEvent(DeviceEntry, CSCM_PROCEDURE_STATUS_FAILURE_TIMEOUT, 0);
      ...
      if(!Initialized)
      {
         ...
         if(!MSG_RegisterMessageGroupHandler(BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER, CSCManagerGroupHandler, NULL))
         {
            if((Result = GATM_RegisterEventCallback(GATMEventCallback, NULL)) > 0)
            {
               ...
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("CSC Initialized: %08X\n", BTPM_MESSAGE_GROUP_CYCLING_SPEED_CADENCE_MANAGER));
         ...
         if(Result)
         {
            DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_VERBOSE), ("CSC Initialization Failed: %d\n", Result));
      ...
      if(DEVM_AcquireLock())
      {
         switch(EventData->EventType)
         {
            ...
            case detRemoteDevicePropertiesChanged:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("detRemoteDevicePropertiesChanged\n"));
            ...
            case detRemoteDeviceDeleted:
               DebugPrint((BTPM_DEBUG_ZONE_LE_CYCLING_SPEED | BTPM_DEBUG_LEVEL_FUNCTION), ("detRemoteDeviceDeleted\n"));
