...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXRSCM - Simple Linux application using Bluetopia Platform Manager     */
/*              Running Speed and Cadence (RSC - Client Role only) Manager    */
...
#include "LinuxRSCM_COL.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTRSCM.h"     /* RSC Manager Application Programming Interface.  */
...
#ifndef RSCS_ASSIGN_RSCS_SERVICE_UUID_16
...
static unsigned int        RSCMClientEventCallbackID;/* Variable which holds the       */
                                                    /* current RSCM Server Event       */
                                                    /* Callback ID that is assigned    */
                                                    /* from the RSC Manager when the   */
                                                    /* local client registers for RSC  */
                                                    /* Server Events.                  */
...
static int RegisterRSCMEvents(ParameterList_t *TempParam);
static int UnRegisterRSCMEvents(ParameterList_t *TempParam);
static int QueryConnectedSensors(ParameterList_t *TempParam);
static int ConfigureRemoteSensor(ParameterList_t *TempParam);
static int UnConfigureRemoteSensor(ParameterList_t *TempParam);
static int GetConnectedSensorInfo(ParameterList_t *TempParam);
static int GetSensorLocation(ParameterList_t *TempParam);
static int UpdateCumulativeValue(ParameterList_t *TempParam);
static int UpdateSensorLocation(ParameterList_t *TempParam);
static int StartSensorCalibration(ParameterList_t *TempParam);
...
static char *ConvertSensorLocationToString(RSCM_Sensor_Location_t Location);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM RSC Manager Callback function prototype.                     */
static void BTPSAPI RSCM_Event_Callback(RSCM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERRSCMEVENTS", RegisterRSCMEvents);
   AddCommand("UNREGISTERRSCMEVENTS", UnRegisterRSCMEvents);
   AddCommand("QUERYCONNECTEDSENSORS", QueryConnectedSensors);
   AddCommand("CONFIGUREREMOTESENSOR", ConfigureRemoteSensor);
   AddCommand("UNCONFIGUREREMOTESENSOR", UnConfigureRemoteSensor);
   AddCommand("GETCONNECTEDSENSORINFO", GetConnectedSensorInfo);
   AddCommand("GETSENSORLOCATION", GetSensorLocation);
   AddCommand("UPDATECUMULATIVEVALUE", UpdateCumulativeValue);
   AddCommand("UPDATESENSORLOCATION", UpdateSensorLocation);
   AddCommand("STARTSENSORCALIBRATION", StartSensorCalibration);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      RSCS_ASSIGN_RSCS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  39)RegisterRSCMEvents                         *\r\n");
   printf("*                  40)UnRegisterRSCMEvents                       *\r\n");
   printf("*                  41)QueryConnectedSensors                      *\r\n");
   printf("*                  42)ConfigureRemoteSensor                      *\r\n");
   printf("*                  43)UnConfigureRemoteSensor                    *\r\n");
   printf("*                  44)GetConnectedSensorInfo                     *\r\n");
   printf("*                  45)GetSensorLocation                          *\r\n");
   printf("*                  46)UpdateCumulativeValue                      *\r\n");
   printf("*                  47)UpdateSensorLocation                       *\r\n");
   printf("*                  48)StartSensorCalibration                     *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int RegisterRSCMEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      if(!RSCMClientEventCallbackID)
      {
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Register_Collector_Event_Callback(RSCM_Event_Callback, NULL)) > 0)
         {
            printf("RSCM_Register_Collector_Event_Callback() Success. Callback ID: %d\r\n", ret_val);

            RSCMClientEventCallbackID = (unsigned int)ret_val;
            ret_val                   = 0;
         }
         else
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Register_Collector_Event_Callback() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int UnRegisterRSCMEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Submit the Command.                                            */
         RSCM_Un_Register_Collector_Event_Callback(RSCMClientEventCallbackID);

         printf("RSCM_Un_Register_Collector_Event_Callback() Success.\r\n");

         RSCMClientEventCallbackID = 0;
         ret_val                   = 0;
      }
      else
      {
...
static int QueryConnectedSensors(ParameterList_t *TempParam)
{
   ...
   unsigned int             MaximumDevices;
   RSCM_Connected_Sensor_t *ConnectedSensorList = NULL;

   /* First, check to make sure that we have already been initialized.  */
   if(Initialized)
   {
      /* Make sure the parameters for this function appear valid.       */
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         MaximumDevices = TempParam->Params[0].intParam;

         /* If the user requested any devices, allocate the memory for  */
         /* them                                                        */
         if((!MaximumDevices) || ((ConnectedSensorList = (RSCM_Connected_Sensor_t *)BTPS_AllocateMemory(sizeof(RSCM_Connected_Sensor_t) * MaximumDevices)) != NULL))
         {
            if((ret_val = RSCM_Query_Connected_Sensors(MaximumDevices, ConnectedSensorList, &TotalDevices)) >= 0)
            {
               printf("RSCM_Query_Connected_Sensors() Success. Result: %d. Total Devices: %u.\r\n", ret_val, TotalDevices);

               for(Index=0;Index<(unsigned int)ret_val;Index++)
               {
                  BD_ADDRToStr(ConnectedSensorList[Index].RemoteDeviceAddress, BoardStr);
                  printf("\r\n\t%2u. %s:\r\n", Index+1, BoardStr);
                  printf("\t\tConfigured:                 %s.\r\n", ConnectedSensorList[Index].Configured?"TRUE":"FALSE");
                  printf("\t\tSuppoted Characteristics:   %08lX.\r\n", ConnectedSensorList[Index].SupportedOptionalCharateristics);
                  printf("\t\tSupported Features:         %08lX.\r\n", ConnectedSensorList[Index].SupportedFeatures);

                  if(ConnectedSensorList[Index].SupportedFeatures & RSCM_FEATURE_MULTIPLE_SENSOR_LOCATIONS_SUPPORTED)
                     printf("\t\tSupported Sensor Locations: %08lX.\r\n", (unsigned long)ConnectedSensorList[Index].SupportedSensorLocations);
               }
            ...
            {
               /* Error occurred, inform the user and flag an error.          */
               printf("RSCM_Query_Connected_Sensors() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            printf("Unable to allocate device list.\r\n");
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: QueryConnectedSensors [Maximum Number Devices].\r\n");
...
static int ConfigureRemoteSensor(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure the the parameters for this function seem valid.     */
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Configure_Remote_Sensor(BD_ADDR, Flags)) == 0)
         {
            printf("RSCM_Configure_Remote_Sensor() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Configure_Remote_Sensor() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: ConfigureRemoteSensor [BD_ADDR] [Flags (Optional) 1-Skip Supported Locations].\r\n");
...
static int UnConfigureRemoteSensor(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Un_Configure_Remote_Sensor(BD_ADDR)) == 0)
         {
            printf("RSCM_Un_Configure_Remote_Sensor() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Un_Configure_Remote_Sensor() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: UnConfigureRemoteSensor  [BD_ADDR].\r\n");
...
static int GetConnectedSensorInfo(ParameterList_t *TempParam)
{
   int                     ret_val;
   BD_ADDR_t               BD_ADDR;
   RSCM_Connected_Sensor_t ConnectedSensor;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Get_Connected_Sensor_Info(BD_ADDR, &ConnectedSensor)) == 0)
         {
            printf("RSCM_Get_Connected_Sensor_Info() Success. \r\n");
            printf("\r\nSensor Info:\r\n");
            printf("\tConfigured:                 %s.\r\n", ConnectedSensor.Configured?"TRUE":"FALSE");
            printf("\tSuppoted Characteristics:   %08lX.\r\n", ConnectedSensor.SupportedOptionalCharateristics);
            printf("\tSupported Features:         %08lX.\r\n", ConnectedSensor.SupportedFeatures);

            if(ConnectedSensor.SupportedFeatures & RSCM_FEATURE_MULTIPLE_SENSOR_LOCATIONS_SUPPORTED)
               printf("\tSupported Sensor Locations: %08lX.\r\n", (unsigned long)ConnectedSensor.SupportedSensorLocations);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Get_Connected_Sensor_Info() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: GetConnectedSensorInfo  [BD_ADDR].\r\n");
...
static int GetSensorLocation(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Get_Sensor_Location(BD_ADDR)) == 0)
         {
            printf("RSCM_Get_Sensor_Location() Success. \r\n");
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Get_Sensor_Location() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: GetSensorLocation  [BD_ADDR].\r\n");
...
static int UpdateCumulativeValue(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Update_Cumulative_Value(BD_ADDR, (DWord_t)TempParam->Params[1].intParam)) > 0)
         {
            printf("RSCM_Update_Cumulative_Value() Success. ProcedureID: %d.\r\n", ret_val);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Update_Cumulative_Value() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: UpdateCumulativeValue  [BD_ADDR] [Value].\r\n");
...
static int UpdateSensorLocation(ParameterList_t *TempParam)
{
   ...
   {
      /* Make sure the the parameters for this function seem valid.     */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (TempParam->Params[1].intParam >= rslOther) && (TempParam->Params[1].intParam <= rslChest))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

         /* Submit the Command.                                         */
         if((ret_val = RSCM_Update_Sensor_Location(BD_ADDR, (RSCM_Sensor_Location_t)TempParam->Params[1].intParam)) > 0)
         {
            printf("RSCM_Update_Sensor_Location() Success. Procedure ID: %d\r\n", ret_val);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Update_Sensor_Location() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: UpdateSensorLocation  [BD_ADDR] [Location].\r\n");
         printf("\r\nPossible Locations:\r\n");

         for(Index=rslOther; Index <= rslChest; Index++)
         {
            if((LocationStr = ConvertSensorLocationToString(Index)) != NULL)
               printf("\t%2u. %s\r\n", Index, LocationStr);
         }
...
static int StartSensorCalibration(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
      {
         ...
         /* Submit the Command.                                         */
         if((ret_val = RSCM_Start_Sensor_Calibration(BD_ADDR)) > 0)
         {
            printf("RSCM_Start_Sensor_Calibration() Success. ProcedureID: %d.\r\n", ret_val);
         ...
         {
            /* Error occurred, inform the user and flag an error.       */
            printf("RSCM_Start_Sensor_Calibration() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: StartSensorCalibration [BD_ADDR].\r\n");
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* Converts a Sensor Location to a string for display.               */
static char *ConvertSensorLocationToString(RSCM_Sensor_Location_t Location)
{
   char *String;
...
static void BTPSAPI RSCM_Event_Callback(RSCM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case retRSCConnected:
            BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, BoardStr);

            printf("RSCM Connected:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tConfigured:            %s.\r\n", EventData->EventData.ConnectedEventData.Configured?"TRUE":"FALSE");
            printf("\tSupported Optional Characteristics:\r\n");
            
            if(EventData->EventData.ConnectedEventData.SupportedOptionalCharateristics & RSCM_SUPPORTED_CHARACTERISTIC_SENSOR_LOCATION)
               printf("\t\tSensor Location.\r\n");
            if(EventData->EventData.ConnectedEventData.SupportedOptionalCharateristics & RSCM_SUPPORTED_CHARACTERISTIC_CONTROL_POINT)
               printf("\t\tControl Point.\r\n");
            break;
         case retRSCDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Disconnected:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            break;
         case retRSCConfigurationStatusChanged:
            BD_ADDRToStr(EventData->EventData.ConfigurationStatusChangedEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Configuration Status Changed:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tConfigured:            %s.\r\n", EventData->EventData.ConfigurationStatusChangedEventData.Configured?"TRUE":"FALSE");
            printf("\tStatus                 %u.\r\n", EventData->EventData.ConfigurationStatusChangedEventData.Status);
            break;
         case retRSCMeasurement:
            BD_ADDRToStr(EventData->EventData.MeasurementEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Measurement:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tFlags:                 %08lX.\r\n", EventData->EventData.MeasurementEventData.MeasurementData.Flags);
            printf("\tInstantaneous Speed:   %u.\r\n", EventData->EventData.MeasurementEventData.MeasurementData.Instantaneous_Speed);
            printf("\tInstantaneous Cadence: %u.\r\n", EventData->EventData.MeasurementEventData.MeasurementData.Instantaneous_Cadence);
            
            if(EventData->EventData.MeasurementEventData.MeasurementData.Flags & RSCM_MEASUREMENT_INSTANTANEOUS_STRIDE_LENGTH_PRESENT)
               printf("\tStride Length:         %u.\r\n", EventData->EventData.MeasurementEventData.MeasurementData.Instantaneous_Stride_Length);

            if(EventData->EventData.MeasurementEventData.MeasurementData.Flags & RSCM_MEASUREMENT_TOTAL_DISTANCE_PRESENT)
               printf("\tTotal Distance:        %u.\r\n", EventData->EventData.MeasurementEventData.MeasurementData.Total_Distance);

            if(EventData->EventData.MeasurementEventData.MeasurementData.Flags & RSCM_MEASUREMENT_WALKING_RUNNING_BIT_VALID)
               printf("\tRunning Status:        %s.\r\n", (EventData->EventData.MeasurementEventData.MeasurementData.Flags & RSCM_MEASUREMENT_STATUS_RUNNING)?"Running":"Walking");

            break;
         case retRSCSensorLocationResponse:
            BD_ADDRToStr(EventData->EventData.SensorLocationResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Sensor Location Response:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tStatus:                %u.\r\n", EventData->EventData.SensorLocationResponseEventData.Status);
            printf("\tSensor Location        %s.\r\n", ConvertSensorLocationToString(EventData->EventData.SensorLocationResponseEventData.SensorLocation));
            break;
         case retRSCCumulativeValueUpdated:
            BD_ADDRToStr(EventData->EventData.CumulativeValueUpdatedEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Cumulative Value Updated:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tValue:                 %u.\r\n", EventData->EventData.CumulativeValueUpdatedEventData.CumulativeValue);
            break;
         case retRSCSensorLocationUpdated:
            BD_ADDRToStr(EventData->EventData.SensorLocationUpdatedEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Sensor Location Updated:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tSensor Location        %s.\r\n", ConvertSensorLocationToString(EventData->EventData.SensorLocationUpdatedEventData.SensorLocation));
            break;
         case retRSCProcedureComplete:
            BD_ADDRToStr(EventData->EventData.ProcedureCompleteEventData.RemoteDeviceAddress, BoardStr);
            printf("RSCM Procedure Complete:\r\n");
            printf("\tRemote Device Address: %s.\r\n", BoardStr);
            printf("\tProcedure ID:          %u.\r\n", EventData->EventData.ProcedureCompleteEventData.ProcedureID);
            printf("\tStatus:                %u.\r\n", EventData->EventData.ProcedureCompleteEventData.Status);

            if(EventData->EventData.ProcedureCompleteEventData.Status == RSCM_PROCEDURE_STATUS_FAILURE_ERROR_RESPONSE)
               printf("\tResponse Error Code    %u.\r\n", EventData->EventData.ProcedureCompleteEventData.ResponseErrorCode);
            break;
         default:
            printf("Unknown RSC Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nRSCM Event Data is NULL.\r\n");
