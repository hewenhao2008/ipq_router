...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXHDPM - Simple Linux application using Bluetopia Platform Manager     */
/*              Human Interface Device (HDP) Manager Application Programming  */
/*              Interface (API).                                              */
/*                                                                            */
/*  Author:  Damon Lange                                                      */
/*                                                                            */
...
#include "LinuxHDPM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTHDPM.h"     /* HDP Manager Application Programming Interface.  */
...
static int HDPRegisterEndpoint(ParameterList_t *TempParam);
static int HDPUnRegisterEndpoint(ParameterList_t *TempParam);
static int HDPDataConnectionRequestResponse(ParameterList_t *TempParam);
static int HDPQueryRemoteDeviceInstances(ParameterList_t *TempParam);
static int HDPQueryRemoteDeviceInstanceEndpoints(ParameterList_t *TempParam);
static int HDPQueryRemoteDeviceEndpointDescription(ParameterList_t *TempParam);
static int HDPConnectRemoteDevice(ParameterList_t *TempParam);
static int HDPDisconnectRemoteDevice(ParameterList_t *TempParam);
static int HDPConnectRemoteDeviceEndpoint(ParameterList_t *TempParam);
static int HDPDisconnectRemoteDeviceEndpoint(ParameterList_t *TempParam);
static int HDPWriteData(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM HDP Manager Callback function prototype.                     */
static void BTPSAPI HDPM_Event_Callback(HDPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("HDPREGISTERENDPOINT"                       , HDPRegisterEndpoint);
   AddCommand("HDPUNREGISTERENDPOINT"                     , HDPUnRegisterEndpoint);
   AddCommand("HDPDATACONNECTIONREQUESTRESPONSE"          , HDPDataConnectionRequestResponse);
   AddCommand("HDPQUERYREMOTEDEVICEINSTANCES"             , HDPQueryRemoteDeviceInstances);
   AddCommand("HDPQUERYREMOTEDEVICEINSTANCEENDPOINTS"     , HDPQueryRemoteDeviceInstanceEndpoints);
   AddCommand("HDPQUERYREMOTEDEVICEENDPOINTDESCRIPTION"   , HDPQueryRemoteDeviceEndpointDescription);
   AddCommand("HDPCONNECTREMOTEDEVICE"                    , HDPConnectRemoteDevice);
   AddCommand("HDPDISCONNECTREMOTEDEVICE"                 , HDPDisconnectRemoteDevice);
   AddCommand("HDPCONNECTREMOTEDEVICEENDPOINT"            , HDPConnectRemoteDeviceEndpoint);
   AddCommand("HDPDISCONNECTREMOTEDEVICEENDPOINT"         , HDPDisconnectRemoteDeviceEndpoint);
   AddCommand("HDPWRITEDATA"                              , HDPWriteData);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  33)HDPRegisterEndpoint                        *\r\n");
   printf("*                  34)HDPUnregisterEndpoint                      *\r\n");
   printf("*                  35)HDPDataConnectionRequestResponse           *\r\n");
   printf("*                  36)HDPQueryRemoteDeviceInstances              *\r\n");
   printf("*                  37)HDPQueryRemoteDeviceInstanceEndpoints      *\r\n");
   printf("*                  38)HDPQueryRemoteDeviceEndpointDescription    *\r\n");
   printf("*                  39)HDPConnectRemoteDevice                     *\r\n");
   printf("*                  40)HDPDisconnectRemoteDevice                  *\r\n");
   printf("*                  41)HDPConnectRemoteDeviceEndpoint             *\r\n");
   printf("*                  42)HDPDisconnectRemoteDeviceEndpoint          *\r\n");
   printf("*                  43)HDPWriteData                               *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for Registering a local HDP */
   /* Endpoint. This function returns zero if successful and a negative */
   /* value if an error occurred.                                       */
static int HDPRegisterEndpoint(ParameterList_t *TempParam)
{
   int   Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         if((TempParam->NumberofParameters >= 3) && (TempParam->Params[2].strParam))
            Description = TempParam->Params[2].strParam;
         else
            Description = NULL;

         Result = HDPM_Register_Endpoint(TempParam->Params[0].intParam, (TempParam->Params[1].intParam ? drSource : drSink), Description, HDPM_Event_Callback, NULL);

         if(Result >= 0)
         {
            printf("HDPM_Register_Endpoint() Success.\r\n");
         ...
         else
         {
            ...
            printf("HDPM_Register_Endpoint() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPM_Register_Endpoint [Data Type] [Role (0: Sink, 1: Source)] [Description (Optional)].\r\n");
   ...
   /* The following function is responsible for UnRegistering a local   */
   /* HDP Endpoint. This function returns zero if successful and a      */
   /* negative value if an error occurred.                              */
static int HDPUnRegisterEndpoint(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 1))
      {
         Result = HDPM_Un_Register_Endpoint(TempParam->Params[0].intParam);

         if(!Result)
         {
            printf("HDPM_Un_Register_Endpoint() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Un_Register_Endpoint() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPM_Un_Register_Endpoint [Endpoint ID].\r\n");
   ...
   /* incoming data connection request. This function returns zero if   */
   /* successful and a negative value if an error occurred.             */
static int HDPDataConnectionRequestResponse(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      {
         Result = HDPM_Data_Connection_Request_Response(TempParam->Params[0].intParam, TempParam->Params[1].intParam, ((TempParam->Params[2].intParam == 0) ? cmNoPreference : ((TempParam->Params[2].intParam == 1) ? cmReliable : cmStreaming)));

         if(!Result)
         {
            printf("HDPM_Data_Connection_Request_Response() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Data_Connection_Request_Response() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPM_Data_Connection_Request_Response [DataLink ID] [MCAP Response Code] [Channel Mode (0: No Preference, 1: Reliable, 2: Streaming)].\r\n");
   ...
   /* The following function is responsible for Querying the cached     */
   /* SDP records of a remote device for HDP Instances. This function   */
   /* returns zero if successful and a negative value if an error       */
   /* occurred.                                                         */
static int HDPQueryRemoteDeviceInstances(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 1) && (TempParam->Params[0].strParam))
      {
         ...
         if((Result = HDPM_Query_Remote_Device_Instances(RemoteDeviceAddress, 0, NULL, &TotalNumberInstances)) >= 0)
         {
            if(TotalNumberInstances > 0)
            {
               if((InstanceList = (DWord_t *)BTPS_AllocateMemory(sizeof(DWord_t)*TotalNumberInstances)) != NULL)
               {
                  Result = HDPM_Query_Remote_Device_Instances(RemoteDeviceAddress, TotalNumberInstances, InstanceList, NULL);

                  if(Result >= 0)
                  {
                     printf("HDPM_Query_Remote_Device_Instances() Success: %d.  Total Instances: %d\r\n", Result, TotalNumberInstances);
                     printf("  Instances:\r\n");

                     for(Index = 0; Index < (unsigned int)Result; Index++)
                        printf("      0x%08X\r\n", (unsigned int)InstanceList[Index]);
                  ...
                  else
                  {
                     ...
                     /* an error.                                       */
                     printf("HDPM_Query_Remote_Device_Instances() Failure (Final Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

                     ret_val = FUNCTION_ERROR;
                  }

                  BTPS_FreeMemory(InstanceList);
               }
               else
               {
                  /* Unable to allocate memory for List.                */
         ...
         {
            printf("HDPM_Query_Remote_Device_Instances() Failure (Initial Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPQueryRemoteDeviceInstances [BD_ADDR].\r\n");
   ...
   /* The following function is responsible for Querying the cached     */
   /* SDP records of a remote device for the Endpoints of a particular  */
...
static int HDPQueryRemoteDeviceInstanceEndpoints(ParameterList_t *TempParam)
{
   ...
   HDPM_Endpoint_Info_t *EndpointList;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 2) && (TempParam->Params[0].strParam))
      {
         ...
         if((Result = HDPM_Query_Remote_Device_Instance_Endpoints(RemoteDeviceAddress, TempParam->Params[1].intParam, 0, NULL, &TotalNumberEndpoints)) >= 0)
         {
            if((EndpointList = (HDPM_Endpoint_Info_t *)BTPS_AllocateMemory(sizeof(HDPM_Endpoint_Info_t)*TotalNumberEndpoints)) != NULL)
            {
               Result = HDPM_Query_Remote_Device_Instance_Endpoints(RemoteDeviceAddress, TempParam->Params[1].intParam, TotalNumberEndpoints, EndpointList, NULL);

               if(Result >= 0)
               {
                  printf("HDPM_Query_Remote_Device_Instance_Endpoints() Success: %d.  Total Endpoints: %d\r\n", Result, TotalNumberEndpoints);

                  for(Index = 0; Index < (unsigned int)Result; Index++)
                  {
                     printf("  Endpoint %d:\r\n", EndpointList[Index].EndpointID);
                     printf("      Data Type: 0x%04X\r\n", EndpointList[Index].DataType);
                     printf("      Role:      %s\r\n", ((EndpointList[Index].Role == drSink) ? "Sink" : "Source"));
                  }
               ...
               else
               {
                  ...
                  /* error.                                             */
                  printf("HDPM_Query_Remote_Device_Instance_Endpoints() Failure (Final Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

                  ret_val = FUNCTION_ERROR;
               }

               BTPS_FreeMemory(EndpointList);
            }
            else
            {
               /* Unable to allocate memory for List.                   */
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Query_Remote_Device_Instance_Endpoints() Failure (Initial Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPQueryRemoteDeviceInstanceEndpoints [BD_ADDR] [Instance].\r\n");
   ...
   /* The following function is responsible for Querying the cached SDP */
   /* records of a remote device to determine the Description field a   */
   /* particular endpoint. This function returns zero if successful and */
   /* a negative value if an error occurred.                            */
static int HDPQueryRemoteDeviceEndpointDescription(ParameterList_t *TempParam)
{
   int                  Result;
   ...
   char                *Description;
   BD_ADDR_t            RemoteDeviceAddress;
   unsigned int         TotalDescriptionLength;
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters == 4) && (TempParam->Params[0].strParam))
      {
         StrToBD_ADDR(TempParam->Params[0].strParam, &RemoteDeviceAddress);

         EndpointInfo.EndpointID = TempParam->Params[2].intParam;
         EndpointInfo.DataType   = TempParam->Params[3].intParam;

         if((Result = HDPM_Query_Remote_Device_Endpoint_Description(RemoteDeviceAddress, TempParam->Params[1].intParam, &EndpointInfo, 0, NULL, &TotalDescriptionLength)) >= 0)
         {
            if((Description = (char *)BTPS_AllocateMemory(sizeof(char)*(TotalDescriptionLength + 1))) != NULL)
            {
               Result = HDPM_Query_Remote_Device_Endpoint_Description(RemoteDeviceAddress, TempParam->Params[1].intParam, &EndpointInfo, (TotalDescriptionLength + 1), Description, NULL);

               if(Result >= 0)
               {
                  Description[TotalDescriptionLength] = '\0';

                  printf("HDPM_Query_Remote_Device_Endpoint_Description() Success: %d.\r\n", Result);
                  printf("  Endpoint %d Description: %s\r\n", EndpointInfo.EndpointID, Description);
               ...
               else
               {
                  /* Error in the API call. Inform the user and flag an */
                  /* error.                                             */
                  printf("HDPM_Query_Remote_Device_Endpoint_Description() Failure (Final Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

                  ret_val = FUNCTION_ERROR;
               }

               BTPS_FreeMemory(Description);
            }
            else
            {
               /* Unable to allocate memory for List.                   */
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Query_Remote_Device_Endpoint_Description() Failure (Initial Query): %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPQueryRemoteDeviceEndpointDescription [BD_ADDR] [Instance] [Endpoint ID] [Data Type].\r\n");
   ...
   /* to an HDP Instance on a remote device. This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int HDPConnectRemoteDevice(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 2) && (TempParam->Params[0].strParam))
      {
         ...
         Result = HDPM_Connect_Remote_Device(RemoteDeviceAddress, TempParam->Params[1].intParam, HDPM_Event_Callback, NULL, NULL);

         if(!Result)
         {
            printf("HDPM_Connect_Remote_Device() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Connect_Remote_Device() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPConnectRemoteDevice [BD_ADDR] [Instance].\r\n");
   ...
   /* The following function is responsible for Disconnecting from an   */
   /* HDP Instance on a remote device. This function returns zero if    */
   /* successful and a negative value if an error occurred.             */
static int HDPDisconnectRemoteDevice(ParameterList_t *TempParam)
{
   int       Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 2) && (TempParam->Params[0].strParam))
      {
         ...
         Result = HDPM_Disconnect_Remote_Device(RemoteDeviceAddress, TempParam->Params[1].intParam);

         if(!Result)
         {
            printf("HDPM_Disconnect_Remote_Device() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Disconnect_Remote_Device() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPDisconnectRemoteDevice [BD_ADDR] [Instance].\r\n");
   ...
   /* The following function is responsible for Initiating a Data       */
   /* connection to an endpoint of an already-connected remote Instance.*/
...
static int HDPConnectRemoteDeviceEndpoint(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 4) && (TempParam->Params[0].strParam))
      {
         ...
         Result = HDPM_Connect_Remote_Device_Endpoint(RemoteDeviceAddress, TempParam->Params[1].intParam, TempParam->Params[2].intParam, ((TempParam->Params[3].intParam == 0) ? cmNoPreference : ((TempParam->Params[3].intParam == 1) ? cmReliable : cmStreaming)), HDPM_Event_Callback, NULL, NULL);

         if(Result >= 0)
         {
            printf("HDPM_Connect_Remote_Device_Endpoint() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Connect_Remote_Device_Endpoint() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPConnectRemoteDeviceEndpoint [BD_ADDR] [Instance] [Endpoint ID] [Channel Mode (0=No Preference, 1=Reliable, 2=Streaming)].\r\n");
   ...
   /* The following function is responsible for Disconnecting a Data    */
   /* Connection from a remote Instance. This function returns zero if  */
   /* successful and a negative value if an error occurred.             */
static int HDPDisconnectRemoteDeviceEndpoint(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 1))
      {
         Result = HDPM_Disconnect_Remote_Device_Endpoint(TempParam->Params[0].intParam);

         if(!Result)
         {
            printf("HDPM_Disconnect_Remote_Device_Endpoint() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Disconnect_Remote_Device_Endpoint() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPDisconnectRemoteDeviceEndpoint [Data Link ID].\r\n");
   ...
   /* The following function is responsible for Sending data via an     */
   /* existing data connection. This function returns zero if successful*/
...
static int HDPWriteData(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         Result = HDPM_Write_Data(TempParam->Params[0].intParam, BTPS_StringLength(TempParam->Params[1].strParam), (Byte_t *)TempParam->Params[1].strParam);

         if(Result >= 0)
         {
            printf("HDPM_Write_Data() Success: %d.\r\n", Result);
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("HDPM_Write_Data() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: HDPWriteData [Data Link ID] [Data String].\r\n");
...
static void BTPSAPI HDPM_Event_Callback(HDPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case hetHDPConnectionStatus:
            printf("HDPM: Connection Status:\r\n");

            BD_ADDRToStr(EventData->EventData.ConnectionStatusEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Instance:       0x%08X\r\n", (unsigned int)(EventData->EventData.ConnectionStatusEventData.Instance));
            printf("    Status:         %d\r\n", EventData->EventData.ConnectionStatusEventData.Status);
            break;
         case hetHDPDisconnected:
            printf("HDPM: Device Disconnected.\r\n");

            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Instance:       0x%08X\r\n", (unsigned int)(EventData->EventData.DisconnectedEventData.Instance));
            break;
         case hetHDPIncomingDataConnectionRequest:
            printf("HDPM: Incoming Data Connection Request.\r\n");

            BD_ADDRToStr(EventData->EventData.IncomingDataConnectionRequestEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Endpoint ID:    %u\r\n", EventData->EventData.IncomingDataConnectionRequestEventData.EndpointID);
            printf("    Channel Mode:   %s\r\n", ((EventData->EventData.IncomingDataConnectionRequestEventData.ChannelMode == cmReliable) ? "Reliable" : ((EventData->EventData.IncomingDataConnectionRequestEventData.ChannelMode == cmStreaming) ? "Streaming" : "No Preference")));
            printf("    DataLink ID:    %u\r\n", EventData->EventData.IncomingDataConnectionRequestEventData.DataLinkID);

            printf("Respond with HDPDataConnectionRequestResponse\r\n");
            break;
         case hetHDPDataConnected:
            printf("HDPM: Data Connected.\r\n");

            BD_ADDRToStr(EventData->EventData.DataConnectedEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Endpoint ID:    %u\r\n", EventData->EventData.DataConnectedEventData.EndpointID);
            printf("    DataLink ID:    %u\r\n", EventData->EventData.DataConnectedEventData.DataLinkID);
            break;
         case hetHDPDataDisconnected:
            printf("HDPM: Data Disconnected.\r\n");

            BD_ADDRToStr(EventData->EventData.DataDisconnectedEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    DataLink ID:    %u\r\n", EventData->EventData.DataDisconnectedEventData.DataLinkID);
            printf("    Reason:         %d\r\n", EventData->EventData.DataDisconnectedEventData.Reason);
            break;
         case hetHDPDataConnectionStatus:
            printf("HDPM: Data Connection Status.\r\n");

            BD_ADDRToStr(EventData->EventData.DataConnectionStatusEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    Instance:       0x%08X\r\n", EventData->EventData.DataConnectionStatusEventData.Instance);
            printf("    Endpoint ID:    %u\r\n", EventData->EventData.DataConnectionStatusEventData.EndpointID);
            printf("    DataLink ID:    %u\r\n", EventData->EventData.DataConnectionStatusEventData.DataLinkID);
            printf("    Status:         %d\r\n", EventData->EventData.DataConnectionStatusEventData.Status);
            break;
         case hetHDPDataReceived:
            printf("HDPM: Data Received.\r\n");

            BD_ADDRToStr(EventData->EventData.DataReceivedEventData.RemoteDeviceAddress, Buffer);

            printf("    Remote Address: %s\r\n", Buffer);
            printf("    DataLink ID:    %u\r\n", EventData->EventData.DataReceivedEventData.DataLinkID);
            printf("    Bytes:          %u\r\n", EventData->EventData.DataReceivedEventData.DataLength);
            printf("    String: ");

            for(Index = 0; Index < EventData->EventData.DataReceivedEventData.DataLength; Index++)
            {
               if(isprint(EventData->EventData.DataReceivedEventData.Data[Index]))
                  printf("%c", EventData->EventData.DataReceivedEventData.Data[Index]);
               else
                  printf(".");
            }
            printf("\r\n");
            break;
         default:
            printf("HDPM: Unknown HDP Manager Event Received: 0x%08X, Length: 0x%08X.\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nHDPM Event Data is NULL.\r\n");
