...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXTIPM_CLT - Simple Linux application using Bluetopia Platform Manager */
/*                  Time (TIP - Client Role only) Manager Application         */
...
/*   --------  -----------    ------------------------------------------------*/
/*   10/08/130 M. Seabold     Initial creation. (Based on LinuxTIPM_SRV)      */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxTIPM_SRV.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTTIPM.h"     /* TIP Manager Application Programming Interface.  */
...
#define PLATFORM_MANAGER_NOT_INITIALIZED_ERROR     (-7)  /* Denotes that an   */
   ...
   /* Time Service 16 bit UUID to the specified UUID_16_t               */
   /* variable.  This MACRO accepts one parameter which is a pointer to */
   /* a UUID_16_t variable that is to receive the CTS UUID Constant     */
   /* value.                                                            */
   /* * NOTE * The UUID will be assigned into the UUID_16_t variable in */
   /*          Little-Endian format.                                    */
#ifndef CTS_ASSIGN_CTS_SERVICE_UUID_16
...
static unsigned int        TIPMClientEventCallbackID;/* Variable which holds the       */
                                                    /* current TIPM Server Event       */
                                                    /* Callback ID that is assigned    */
                                                    /* from the TIP Manager when the   */
                                                    /* local client registers for TIP  */
                                                    /* Server Events.                  */
...
static char *DSTOffsetToString(TIPM_DST_Offset_Type_t DSTOffset);
static char *TimeZoneToString(TIPM_Time_Zone_Type_t TimeZone);
static char *TimeSourceToString(TIPM_Time_Source_Type_t TimeSource);
static char *MonthToString(TIPM_Month_Of_Year_Type_t Month);
static char *DayOfWeekToString(TIPM_Week_Day_Type_t DayOfWeek);
static char *TimeUpdateCurrentStateToString(TIPM_Time_Update_State_Current_State_t CurrentState);
static char *TimeUpdateStateResultToString(TIPM_Time_Update_State_Result_t Result);
...
static int TIPRegisterClientEvents(ParameterList_t *TempParam);
static int TIPUnRegisterClientEvents(ParameterList_t *TempParam);
static int TIPGetCurrentTime(ParameterList_t *TempParam);
static int TIPEnableTimeNotifications(ParameterList_t *TempParam);
static int TIPGetLocalTimeInformation(ParameterList_t *TempParam);
static int TIPGetTimeAccuracy(ParameterList_t *TempParam);
static int TIPGetNextDSTChangeInformation(ParameterList_t *TempParam);
static int TIPGetReferenceTimeUpdateState(ParameterList_t *TempParam);
static int TIPRequestReferenceTimeUpdate(ParameterList_t *TempParam);
static int TIPQueryConnectedDevices(ParameterList_t *TempParam);
...
static void DisplayCurrentTimeData(TIPM_Current_Time_Data_t *CurrentTimeData);
static void DisplayLocalTimeInformation(TIPM_Local_Time_Information_Data_t *LocalTimeInformation);
static void DisplayReferenceTimeInformation(TIPM_Reference_Time_Information_Data_t *ReferenceTimeInformation);
static void DisplayTimeWithDST(TIPM_Time_With_DST_Data_t *TimeWithDST);
static void DisplayTimeUpdateState(TIPM_Time_Update_State_Data_t *TimeUpdateState);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM TIP Manager Callback function prototype.                     */
static void BTPSAPI TIPM_Event_Callback(TIPM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("TIPREGISTERCLIENTEVENTS", TIPRegisterClientEvents);
   AddCommand("TIPUNREGISTERCLIENTEVENTS", TIPUnRegisterClientEvents);
   AddCommand("TIPGETCURRENTTIME", TIPGetCurrentTime);
   AddCommand("TIPENABLETIMENOTIFICATIONS", TIPEnableTimeNotifications);
   AddCommand("TIPGETLOCALTIMEINFORMATION", TIPGetLocalTimeInformation);
   AddCommand("TIPGETTIMEACCURACY", TIPGetTimeAccuracy);
   AddCommand("TIPGETNEXTDSTCHANGEINFORMATION", TIPGetNextDSTChangeInformation);
   AddCommand("TIPGETREFERENCETIMEUPDATESTATE", TIPGetReferenceTimeUpdateState);
   AddCommand("REQUESTREFERENCETIMEUPDATE", TIPRequestReferenceTimeUpdate);
   AddCommand("TIPQUERYCONNECTEDDEVICES", TIPQueryConnectedDevices);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(AdvertisingBuffer[NON_ALIGNED_BYTE_SIZE]), HCI_LE_ADVERTISING_REPORT_DATA_TYPE_16_BIT_SERVICE_UUID_PARTIAL);
      CTS_ASSIGN_CTS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
   ...
   /* The following function is a utility function that is used to      */
   /* convert a DST Offset into a string.                               */
static char *DSTOffsetToString(TIPM_DST_Offset_Type_t DSTOffset)
{
   char *ret_val;
   ...
   switch(DSTOffset)
   {
      case tmdStandardTime:
         ret_val = "Standard Time";
         break;
      case tmdHalfAnHourDaylightTime:
         ret_val = "Half-Hour Daylight (+0.5h) Time";
         break;
      case tmdDaylightTime:
         ret_val = "Daylight (+1h) Time";
         break;
      case tmdDoubleDaylightTime:
         ret_val = "Double Daylight (+2h) Time";
         break;
      default:
      case tmdUnknown:
         ret_val = "Unkown";
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Time Zone into a string.                                */
static char *TimeZoneToString(TIPM_Time_Zone_Type_t TimeZone)
{
   char *ret_val;
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Time Source into a string.                              */
static char *TimeSourceToString(TIPM_Time_Source_Type_t TimeSource)
{
   char *ret_val;
   ...
   switch(TimeSource)
   {
      ...
      case tmsNetworkTimeProtocol:
         ret_val = "Network Time Protocol (NTP)";
         break;
      case tmsGps:
         ret_val = "Global Positioning System (GPS)";
         break;
      case tmsRadioTimeSignal:
         ret_val = "Radio Time Signal";
         break;
      case tmsManual:
         ret_val = "Manual";
         break;
      case tmsAtomicClock:
         ret_val = "Atomic Clock";
         break;
      case tmsCellularNetwork:
         ret_val = "Cellular Network";
         break;
   }
...
static char *MonthToString(TIPM_Month_Of_Year_Type_t Month)
{
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Day of Week into a string.                              */
static char *DayOfWeekToString(TIPM_Week_Day_Type_t DayOfWeek)
{
   char *ret_val;
   ...
   /* The following function is a utility function that is used to      */
   /* convert a Time Update State Current State into a string.          */
static char *TimeUpdateCurrentStateToString(TIPM_Time_Update_State_Current_State_t CurrentState)
{
   char *ret_val;
   ...
   switch(CurrentState)
   {
      case tcsIdle:
         ret_val = "Idle";
         break;
      case tcsUpdatePending:
         ret_val = "Update Pending";
         break;
      default:
         ret_val = "Unknown State";
   }

   return(ret_val);
}
   /* The following function is a utility function that is used to      */
   /* convert a Time Update State Result into a string.                 */
static char *TimeUpdateStateResultToString(TIPM_Time_Update_State_Result_t Result)
{
   char *ret_val;
   ...
   switch(Result)
   {
      ...
      case treCanceled:
         ret_val = "Canceled";
         break;
      case treNoConnectionToReference:
         ret_val = "No Connection To Reference";
         break;
      case treReferenceRespondedWithError:
         ret_val = "Reference Responded With Error";
         break;
      case treTimeout:
         ret_val = "Timeout";
         break;
      case treUpdateNotAttemptedAfterReset:
         ret_val = "Update Not Attempted After Reset";
         break;
      default:
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)StartAdvertising                           *\r\n");
   printf("*                  36)StopAdvertising                            *\r\n");
   printf("*                  37)TIPRegisterClientEvents                    *\r\n");
   printf("*                  38)TIPUnRegisterClientEvents                  *\r\n");
   printf("*                  39)TIPGetCurrentTime                          *\r\n");
   printf("*                  40)TIPEnableTimeNotifications                 *\r\n");
   printf("*                  41)TIPGetLocalTimeInformation                 *\r\n");
   printf("*                  42)TIPGetTimeAccuracy                         *\r\n");
   printf("*                  43)TIPGetNextDSTChangeInformation             *\r\n");
   printf("*                  44)TIPGetReferenceTimeUpdateState             *\r\n");
   printf("*                  45)TIPRequestReferenceTimeUpdate              *\r\n");
   printf("*                  46)TIPQueryConnectedDevices                   *\r\n");
   printf("*                  47)StartObservationScan                       *\r\n");
   printf("*                  48)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the TIP*/
   /* Manager to receive TIP Client Events.  This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int TIPRegisterClientEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      /* If there is an Event Callback Registered, then we need to flag */
      /* an error.                                                      */
      if(!TIPMClientEventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = TIPM_Register_Client_Event_Callback(TIPM_Event_Callback, NULL)) > 0)
         {
            printf("TIPM_Register_Client_Event_Callback() Success: %d.\r\n", Result);

            /* Note the Callback ID and flag success.                   */
            TIPMClientEventCallbackID = (unsigned int)Result;
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("TIPM_Register_Client_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("TIP Manager Client Event Callback already registered.\r\n");
   ...
   /* previously registered TIP CLient Event Registration with the TIP  */
...
static int TIPUnRegisterClientEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         TIPM_Un_Register_Client_Event_Callback(TIPMClientEventCallbackID);

         printf("TIPM_Un_Register_Client_Event_Callback() Success.\r\n");

         /* Flag that there is no longer a Callback registered.         */
         TIPMClientEventCallbackID = 0;
      ...
      else
      {
         ...
         printf("TIP Manager Event Callback is not registered.\r\n");
   ...
   /* time.  This function returns zero if successful and a negative    */
...
static int TIPGetCurrentTime(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            /* Parameters are valid. Convert the BD_ADDR.               */
            StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

            /* Submit the command.                                      */
            ret_val = TIPM_Get_Current_Time(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val > 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Get_Current_Time: Function Succcessful. %d\r\n", ret_val);
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPGetCurrentTime [BD_ADDR]\n");
   ...
   /* The following function is responsible for Enabling Time           */
   /* Notifications. This function returns zero if successful and a     */
...
static int TIPEnableTimeNotifications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam))
         {
            /* Parameters are valid. Convert the BD_ADDR.               */
            StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

            /* Submit the command.                                      */
            ret_val = TIPM_Enable_Time_Notifications(TIPMClientEventCallbackID, BD_ADDR, TempParam->Params[1].intParam);

            if(ret_val == 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Enable_Time_Notifications(%d): Function Succcessful. %d\r\n", TempParam->Params[1].intParam, ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Enable_Time_Notifications(%d) Failure: %d (%s).\r\n", TempParam->Params[1].intParam, ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPEnableTimeNotifications [BD_ADDR] [Enable (0/1)]\n");
   ...
   /* The following function is responsible for getting the Local Time  */
   /* Information.  This function returns zero if successful and a      */
   /* negative value if there was an error.                             */
static int TIPGetLocalTimeInformation(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Submit the command.                                      */
            ret_val = TIPM_Get_Local_Time_Information(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val > 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Get_Local_Time_Information: Function Succcessful. %d\r\n", ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Get_Local_Time_Information: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPGetLocalTimeInformation [BD_ADDR]\n");
   ...
   /* The following function is responsible for getting the Time        */
   /* Accuracy. This function returns zero if successful and a negative */
   /* value if there was an error.                                      */
static int TIPGetTimeAccuracy(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Submit the command.                                      */
            ret_val = TIPM_Get_Time_Accuracy(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val > 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Get_Time_Accuracy: Function Succcessful. %d\r\n", ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Get_Time_Accuracy: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPGetTimeAccuracy [BD_ADDR]\n");
   ...
   /* The following function is responsible for getting the Next DST    */
   /* Change Information.  This function returns zero if successful and */
...
static int TIPGetNextDSTChangeInformation(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Submit the command.                                      */
            ret_val = TIPM_Get_Next_DST_Change_Information(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val > 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Get_Next_DST_Change_Information: Function Succcessful. %d\r\n", ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Get_Next_DST_Change_Information: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPGetNextDSTChangeInformation [BD_ADDR]\n");
   ...
   /* time update state.  This function returns zero if successful and a*/
   /* negative value if there was an error.                             */
static int TIPGetReferenceTimeUpdateState(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Submit the command.                                      */
            ret_val = TIPM_Get_Reference_Time_Update_State(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val > 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Get_Reference_Time_Update_State: Function Succcessful. %d\r\n", ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Get_Reference_Time_Update_State: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPGetReferenceTimeUpdateState [BD_ADDR]\n");
   ...
   /* time udpate.  This function returns zero if successful and a      */
   /* negative value if there was an error.                             */
static int TIPRequestReferenceTimeUpdate(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(Initialized)
   {
      ...
      if(TIPMClientEventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam))
         {
            ...
            /* Submit the command.                                      */
            ret_val = TIPM_Request_Reference_Time_Update(TIPMClientEventCallbackID, BD_ADDR);

            if(ret_val == 0)
            {
               /* The function was successful.                          */
               printf("TIPM_Request_Reference_Time_Update: Function Succcessful. %d\r\n", ret_val);
            ...
            {
               /* There was an error.                                   */
               printf("TIPM_Request_Reference_Time_Update: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: TIPRequestReferenceTimeUpdate [BD_ADDR]\n");
   ...
   /* connected devices (local client connections). This function       */
...
static int TIPQueryConnectedDevices(ParameterList_t *TempParam)
{
   ...
   int                  Index;
   char                 BoardStr[16];
   TIPM_Remote_Device_t Devices[20];

   /* First, check to make sure that we have already been initialized.  */
   if(Initialized)
   {
      /* Submit the command.                                            */
      ret_val = TIPM_Query_Connected_Devices(tctClient, 20, Devices, NULL);

      if(ret_val >= 0)
      {
         /* The function was successful.                                */
         printf("TIPM_Query_Connected_Devices: Function Succcessful. %d\r\n", ret_val);

         printf("Devices:\r\n");

         /* Print each connected device.                                */
         for(Index=0;Index<ret_val;Index++)
         {
            BD_ADDRToStr(Devices[Index].RemoteDeviceAddress, BoardStr);
            printf("   %s\t0x%08X\r\n", BoardStr, Devices[Index].SupportedServicesMask);
         }
      ...
      {
         /* There was an error.                                         */
         printf("TIPM_Query_Connected_Devices: Failure: %d (%s).\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
...
static int StopAdvertising(ParameterList_t *TempParam)
{
   ...
   /* The following function is a utility function to display the       */
   /* contents of a Current_Time_Data structure.                        */
static void DisplayCurrentTimeData(TIPM_Current_Time_Data_t *CurrentTimeData)
{
   if(CurrentTimeData)
   {
      /* Print the current Day/Date/Time.                               */
      printf("   Current Time:\r\n");
      printf("      %s %s %u, %u. %02u:%02u:%02u\r\n", 
            DayOfWeekToString(CurrentTimeData->ExactTime.DayDateTime.DayOfWeek), 
            MonthToString(CurrentTimeData->ExactTime.DayDateTime.DateTime.Month), 
            CurrentTimeData->ExactTime.DayDateTime.DateTime.Day,
            CurrentTimeData->ExactTime.DayDateTime.DateTime.Year,
            CurrentTimeData->ExactTime.DayDateTime.DateTime.Hours,
            CurrentTimeData->ExactTime.DayDateTime.DateTime.Minutes,
            CurrentTimeData->ExactTime.DayDateTime.DateTime.Seconds
            );

      printf("   Fractions256:          %u\r\n", CurrentTimeData->ExactTime.Fractions256);
      printf("   AdjustReasonMask:      0x%08X\r\n", (unsigned int)CurrentTimeData->AdjustReasonMask);
   }
   else
      printf("Invalid Current Time Data\r\n");
}

   /* The following function is a utility function to display the       */
   /* contents of a Local_Time_Data structure.                          */
static void DisplayLocalTimeInformation(TIPM_Local_Time_Information_Data_t *LocalTimeInformation)
{
   if(LocalTimeInformation)
   {
      printf("   Time Zone:             %s\r\n", TimeZoneToString(LocalTimeInformation->Time_Zone));
      printf("   DST Offset:            %s\r\n", DSTOffsetToString(LocalTimeInformation->Daylight_Saving_Time));
   }
   else
      printf("   Invalid Local Time Data\r\n");
}

   /* The following function is a utility function to display the       */
   /* contents of a Reference_Time_Data structure.                      */
static void DisplayReferenceTimeInformation(TIPM_Reference_Time_Information_Data_t *ReferenceTimeInformation)
{
   if(ReferenceTimeInformation)
   {
      printf("   Flags:                 %lu\r\n", ReferenceTimeInformation->Flags);
      printf("   Source:                %s\r\n", TimeSourceToString(ReferenceTimeInformation->Source)); 
      printf("   Accuracy:              %u\r\n", ReferenceTimeInformation->Accuracy);
      printf("   Days Since Update:     %u\r\n", ReferenceTimeInformation->Days_Since_Update);
      printf("   Hours Since Update:    %u\r\n", ReferenceTimeInformation->Hours_Since_Update);
   }
   else
      printf("   Invalid Reference Time Data\r\n");
}

   /* The following function is a utility function to display the       */
   /* contents of a Time_With_DST structure.                            */
static void DisplayTimeWithDST(TIPM_Time_With_DST_Data_t *TimeWithDST)
{
   if(TimeWithDST)
   {
      /* Display the date of the next DST change.                       */
      printf("   Date:\r\n");
      printf("      %s %u, %u. %02u:%02u:%02u\r\n",
            MonthToString(TimeWithDST->DateTime.Month),
            TimeWithDST->DateTime.Day,
            TimeWithDST->DateTime.Year,
            TimeWithDST->DateTime.Hours,
            TimeWithDST->DateTime.Minutes,
            TimeWithDST->DateTime.Seconds
            );

      printf("   DST Offset:            %s\r\n", DSTOffsetToString(TimeWithDST->DSTOffset));

   }
   else
      printf("   Invalid Time With DST Data\r\n");
}
...
static void DisplayTimeUpdateState(TIPM_Time_Update_State_Data_t *TimeUpdateState)
{
   if(TimeUpdateState)
   {
      printf("   Current State:         %s\r\n", TimeUpdateCurrentStateToString(TimeUpdateState->CurrentState));
      printf("   Result:                %s\r\n", TimeUpdateStateResultToString(TimeUpdateState->Result));
   }
   else
      printf("Invalid Time Update State Data\r\n");
}
...
void BTPSAPI ServerUnRegistrationCallback(void *CallbackParameter)
{
   ...
   if(TIPMClientEventCallbackID)
   {
      TIPM_Un_Register_Server_Event_Callback(TIPMClientEventCallbackID);
...
static void BTPSAPI TIPM_Event_Callback(TIPM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case aetTIPConnected:
            BD_ADDRToStr(EventData->EventData.ConnectedEventData.RemoteDeviceAddress, BoardStr);

            printf("TIPM Connected:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Connection Type:       %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == tctServer) ? "Server" : "Client");
            if(EventData->EventData.ConnectedEventData.ConnectionType == tctClient)
               printf("   Supported Services Mask: 0x%08X\r\n", (unsigned int)EventData->EventData.ConnectedEventData.SupportedServicesMask);
            break;
         case aetTIPDisconnected:
            BD_ADDRToStr(EventData->EventData.DisconnectedEventData.RemoteDeviceAddress, BoardStr);

            printf("TIPM Disconnected:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Connection Type:       %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == tctServer) ? "Server" : "Client");
            break;
         case aetTIPGetCurrentTimeResponse:
            BD_ADDRToStr(EventData->EventData.GetCurrentTimeResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Current Time Response:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Status:                %u\r\n", EventData->EventData.GetCurrentTimeResponseEventData.Status);
            DisplayCurrentTimeData(&(EventData->EventData.GetCurrentTimeResponseEventData.CurrentTimeData));
            break;
         case aetTIPCurrentTimeNotification:
            BD_ADDRToStr(EventData->EventData.CurrentTimeNotificationEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Current Time Notification:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            DisplayCurrentTimeData(&(EventData->EventData.CurrentTimeNotificationEventData.CurrentTimeData));
            break;
         case aetTIPLocalTimeInformationResponse: 
            BD_ADDRToStr(EventData->EventData.LocalTimeInformationResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Local Time Information Response:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Status:                %u\r\n", EventData->EventData.LocalTimeInformationResponseEventData.Status);
            DisplayLocalTimeInformation(&(EventData->EventData.LocalTimeInformationResponseEventData.LocalTimeInformation));
            break;
         case aetTIPTimeAccuracyResponse:
            BD_ADDRToStr(EventData->EventData.TimeAccuracyResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Time Accuracy Response:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Status:                %u\r\n", EventData->EventData.TimeAccuracyResponseEventData.Status);
            DisplayReferenceTimeInformation(&(EventData->EventData.TimeAccuracyResponseEventData.ReferenceTimeInformation));
            break;
         case aetTIPNextDSTChangeResponse:
            BD_ADDRToStr(EventData->EventData.NextDSTChangeResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Next DST Change Response:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Status:                %u\r\n", EventData->EventData.NextDSTChangeResponseEventData.Status);
            DisplayTimeWithDST(&(EventData->EventData.NextDSTChangeResponseEventData.TimeWithDST));
            break;
         case aetTIPTimeUpdateStateResponse: 
            BD_ADDRToStr(EventData->EventData.TimeUpdateStateResponseEventData.RemoteDeviceAddress, BoardStr);
            printf("TIPM Time Update State Response:\r\n");
            printf("   Bluetooth Address:     %s\r\n", BoardStr);
            printf("   Status:                %u\r\n", EventData->EventData.TimeUpdateStateResponseEventData.Status);
            DisplayTimeUpdateState(&(EventData->EventData.TimeUpdateStateResponseEventData.TimeUpdateStateData));
            break;
         default:
            printf("Unknown TIP Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nTIPM Event Data is NULL.\r\n");
