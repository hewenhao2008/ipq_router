...
/*                  Generic Access Profile (GATT) Manager Application         */
...
#include "LinuxGATM_CLT.h"    /* Main Application Prototypes and Constants.   */
...
static int GATTReadValueRequest(ParameterList_t *TempParam);
static int GATTWriteValueRequest(ParameterList_t *TempParam);
static int GATTWriteValueCommand(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("READVALUE", GATTReadValueRequest);
   AddCommand("WRITEVALUE", GATTWriteValueRequest);
   AddCommand("WRITEVALUECOMMAND", GATTWriteValueCommand);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  51)ReadValue                                  *\r\n");
   printf("*                  52)WriteValue                                 *\r\n");
   printf("*                  53)WriteValueCommand                          *\r\n");
   printf("*                  54)SetLocalDeviceAppearance                   *\r\n");
   printf("*                  55)StartAdvertising                           *\r\n");
   printf("*                  56)StopAdvertising                            *\r\n");
   printf("*                  57)StartObservationScan                       *\r\n");
   printf("*                  58)StopObservationScan                        *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int GATTQueryConnectedDevices(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for reading the value of a  */
   /* specified attribute from a specified GATT connection.  This       */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int GATTReadValueRequest(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(GATMCallbackID)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 4) && (TempParam->Params[1].strParam))
         {
            /* Convert the parameter to a Bluetooth Device Address.     */
            StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

            /* Check to see if we should read it as a string.           */
            if(TempParam->NumberofParameters >= 5)
               ReadAsString = (Boolean_t)TempParam->Params[4].intParam;
            else
               ReadAsString = FALSE;

            if((Result = GATM_ReadValue(GATMCallbackID, BD_ADDR, TempParam->Params[1].intParam, TempParam->Params[2].intParam, (Boolean_t)TempParam->Params[3].intParam)) >= 0)
            {
               printf("GATM_ReadValue() Success: Transaction ID = %d.\r\n", Result);
            ...
            {
               /* Error Connecting With Remote Device, inform the user  */
               /* and flag an error.                                    */
               printf("GATM_ReadValue() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: ReadValue [BD_ADDR] [AttributeHandle] [Offset] [ReadAll (Boolean)] [DisplayAsString (Optional)].\r\n");
   ...
   /* The following function is responsible for writing the value of a  */
   /* specified attribute from a specified GATT connection.  This       */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int GATTWriteValueRequest(ParameterList_t *TempParam)
{
      ...
      if(GATMCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 4) && (TempParam->Params[1].strParam))
         {
            ...
            if(TempParam->Params[2].intParam == 1)
               ValueLength = ExtractHexString(TempParam->Params[3].strParam, &Value);
            else
            {
               ValueLength = (Word_t)BTPS_StringLength(TempParam->Params[3].strParam);
               Value       = (Byte_t *)TempParam->Params[3].strParam;
            }

            /* Submit the request.                                      */
            if((Result = GATM_WriteValue(GATMCallbackID, BD_ADDR, TempParam->Params[1].intParam, ValueLength, Value)) >= 0)
            {
               printf("GATM_WriteValue() Success: Transaction ID = %d.\r\n", Result);
            ...
            else
            {
               /* Error Connecting With Remote Device, inform the user  */
               /* and flag an error.                                    */
               printf("GATM_WriteValue() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));

               ret_val = FUNCTION_ERROR;
            }

            /* Free the memory allocated for the buffer (if any was     */
            /* allocated).                                              */
            if(TempParam->Params[2].intParam == 1)
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: WriteValue [BD_ADDR] [AttributeHandle] [Format (0 = ASCII String, 1 = Hex String)] [Value].\r\n");
   ...
   /* The following function is responsible for writing the value of a  */
   /* specified attribute from a specified GATT connection (using the   */
   /* write command procedure).  This function returns zero if          */
   /* successful and a negative value if an error occurred.             */
static int GATTWriteValueCommand(ParameterList_t *TempParam)
{
   int           Result;
   ...
   if(Initialized)
   {
      ...
      if(GATMCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 4) && (TempParam->Params[1].strParam))
         {
            ...
            /* Check to see if a signed write is to be performed.       */
            if(TempParam->NumberofParameters >= 5)
            {
               SignedWrite = (Boolean_t)TempParam->Params[4].intParam;
            }
            else
            {
               SignedWrite = FALSE;
            ...
            /* Submit the command.                                      */
            if((Result = GATM_WriteValueWithoutResponse(GATMCallbackID, BD_ADDR, TempParam->Params[1].intParam, SignedWrite, ValueLength, Value)) >= 0)
            {
               printf("GATM_WriteValueWithoutResponse() Success: %d.\r\n", Result);
            ...
            else
            {
               /* Error Connecting With Remote Device, inform the user  */
               /* and flag an error.                                    */
               printf("GATM_WriteValueWithoutResponse() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: WriteValueCommand [BD_ADDR] [AttributeHandle] [Format (0 = ASCII String, 1 = Hex String)] [Value] [Write Without Response(0 = No Response, 1 = Signed, No Response )].\r\n");
   ...
   if(EventData)
   {
      switch(EventData->EventType)
      {
         ...
         case getGATTReadResponse:
            ...
            BD_ADDRToStr(EventData->EventData.ReadResponseEventData.RemoteDeviceAddress, Buffer);

            printf("    Connection Type:  %s\r\n", (EventData->EventData.ReadResponseEventData.ConnectionType == gctLE)?"LE":"BR/EDR");
            printf("    Remote Address:   %s\r\n", Buffer);
            printf("    Transaction ID:   %u\r\n", EventData->EventData.ReadResponseEventData.TransactionID);
            printf("    Attribute Handle: 0x%04X (%u)\r\n", EventData->EventData.ReadResponseEventData.Handle, EventData->EventData.ReadResponseEventData.Handle);
            printf("    Final?:           %s\r\n", (EventData->EventData.ReadResponseEventData.Final?"YES":"MAYBE"));
            printf("    Value Length:     %u\r\n", EventData->EventData.ReadResponseEventData.ValueLength);
            printf("    Value:            \r\n");

            DumpData(ReadAsString, EventData->EventData.ReadResponseEventData.ValueLength, EventData->EventData.ReadResponseEventData.Value);
            break;
         case getGATTWriteResponse:
            printf("\r\nGATT Write Response Data\r\n");

            BD_ADDRToStr(EventData->EventData.WriteResponseEventData.RemoteDeviceAddress, Buffer);

            printf("    Connection Type:  %s\r\n", (EventData->EventData.WriteResponseEventData.ConnectionType == gctLE)?"LE":"BR/EDR");
            printf("    Remote Address:   %s\r\n", Buffer);
            printf("    Transaction ID:   %u\r\n", EventData->EventData.WriteResponseEventData.TransactionID);
            printf("    Attribute Handle: 0x%04X (%u)\r\n", EventData->EventData.WriteResponseEventData.Handle, EventData->EventData.WriteResponseEventData.Handle);
            break;
