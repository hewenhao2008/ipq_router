...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXBASM - Simple Linux application using Bluetopia Platform Manager     */
/*              Battery Service (BAS) Manager Application Programming         */
/*              Interface (API).                                              */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   11/28/12  R. Byrne       Initial creation. (Based on LinuxHRPM)          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxBASM.h"     /* Main Application Prototypes and Constants.      */

#include "SS1BTBASM.h"     /* BAS Manager Application Programming Interface.  */
...
#ifndef BAS_ASSIGN_BAS_SERVICE_UUID_16
...
static int BASRegisterClientEvents(ParameterList_t *TempParam);
static int BASUnRegisterClientEvents(ParameterList_t *TempParam);
static int BASEnableNotifications(ParameterList_t *TempParam);
static int BASDisableNotifications(ParameterList_t *TempParam);
static int BASGetBatteryLevel(ParameterList_t *TempParam);
static int BASGetBatteryIdentification(ParameterList_t *TempParam);
static int BASCancelTransaction(ParameterList_t *TempParam);
...
static void BTPSAPI BASM_Event_Callback(BASM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("BASREGISTERCLIENTEVENTS", BASRegisterClientEvents);
   AddCommand("BASUNREGISTERCLIENTEVENTS", BASUnRegisterClientEvents);
   AddCommand("BASENABLENOTIFICATIONS", BASEnableNotifications);
   AddCommand("BASDISABLENOTIFICATIONS", BASDisableNotifications);
   AddCommand("BASGETBATTERYLEVEL", BASGetBatteryLevel);
   AddCommand("BASGETBATTERYIDENTIFICATION", BASGetBatteryIdentification);
   AddCommand("BASCANCELTRANSACTION", BASCancelTransaction);
...
static void FormatAdvertisingData(DEVM_Advertising_Information_t *AdvertisingInformation, Byte_t *AdvertisingBuffer)
{
   ...
   if((AdvertisingInformation) && (AdvertisingBuffer))
   {
      ...
      BAS_ASSIGN_BAS_SERVICE_UUID_16(&(AdvertisingBuffer[NON_ALIGNED_WORD_SIZE]));
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  35)BASRegisterClientEvents                    *\r\n");
   printf("*                  36)BASUnRegisterClientEvents                  *\r\n");
   printf("*                  37)BASEnableNotifications                     *\r\n");
   printf("*                  38)BASDisableNotifications                    *\r\n");
   printf("*                  39)BASGetBatteryLevel                         *\r\n");
   printf("*                  40)BASGetBatteryIdentification                *\r\n");
   printf("*                  41)BASCancelTransaction                       *\r\n");
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering with the BAS*/
   /* Manager to receive BAS Events.  This function returns zero if     */
   /* successful and a negative value if an error occurred.             */
static int BASRegisterClientEvents(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(Initialized)
   {
      ...
      if(!EventCallbackID)
      {
         /* Callback has not been registered, go ahead and attempt to   */
         /* register it.                                                */
         if((Result = BASM_Register_Client_Event_Callback(BASM_Event_Callback, NULL)) > 0)
         {
            printf("BASM_Register_Client_Event_Callback() Success: %d.\r\n", Result);
         ...
         else
         {
            /* Error registering the Callback, inform user and flag an  */
            /* error.                                                   */
            printf("BASM_Register_Client_Event_Callback() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* Callback already registered, go ahead and notify the user.  */
         printf("BAS Manager Event Callback already registered.\r\n");
   ...
   /* previously registered BAS Event Registration with the BAS Manager.*/
...
static int BASUnRegisterClientEvents(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         /* Callback has been registered, go ahead and attempt to       */
         /* un-register it.                                             */
         BASM_Un_Register_Client_Event_Callback(EventCallbackID);

         printf("BASM_Un_Register_Client_Event_Callback() Success.\r\n");
      ...
      else
      {
         ...
         printf("BAS Manager Event Callback is not registered.\r\n");
   ...
   /* registered with the BAS Module before calling this command.  This */
   /* function returns zero if successful and a negative value if an    */
   /* error occurred.                                                   */
static int BASEnableNotifications(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BASM_Enable_Notifications(EventCallbackID, BluetoothAddress, TempParam->Params[1].intParam)) == 0)
            {
               printf("BASM_Enable_Notifications() Success.\r\n");
            }
            else
            {
               printf("BASM_Enable_Notifications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BASEnableNotifications [BD_ADDR] [InstanceID].\r\n");
         }
      }
      else
      {
         ...
         printf("An event callback must be registered with the BASM Manager. Use BASRegisterClientEvents.\r\n");
      }
   }
   else
   {
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BASM_Disable_Notifications(EventCallbackID, BluetoothAddress, TempParam->Params[1].intParam)) == 0)
            {
               printf("BASM_Disable_Notifications() Success.\r\n");
            }
            else
            {
               printf("BASM_Disable_Notifications() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BASDisableNotifications [BD_ADDR] [InstanceID].\r\n");
         }
      }
      else
      {
...
static int BASGetBatteryLevel(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BASM_Get_Battery_Level(EventCallbackID, BluetoothAddress, TempParam->Params[1].intParam)) > 0)
            {
               printf("BASM_Get_Battery_Level() Success.\r\n\n");
            ...
            {
               printf("BASM_Get_Battery_Level() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BASGetBatteryLevel [BD_ADDR] [InstanceID].\r\n");
         }
      }
      else
      {
   ...
   /* must be registered with the BAS Module before calling this        */
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            ...
            /* Submit the command.                                      */
            if((ret_val = BASM_Get_Battery_Identification(EventCallbackID, BluetoothAddress, TempParam->Params[1].intParam)) > 0)
            {
               printf("BASM_Get_Battery_Identification() Success.\r\n\n");
               printf("TransactionID = %d.\r\n", ret_val);
            ...
            {
               printf("BASM_Get_Battery_Identification() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BASGetBatteryIdentification [BD_ADDR] [InstanceID].\r\n");
         }
      }
      else
      {
...
static int BASCancelTransaction(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(EventCallbackID)
      {
         ...
         {
            /* Submit the command.                                      */
            if((ret_val = BASM_Cancel_Transaction(EventCallbackID, TempParam->Params[0].intParam)) == 0)
            {
               printf("BASM_Cancel_Transaction() Success.\r\n");
            }
            else
            {
               printf("BASM_Cancel_Transaction() Failure: %d, %s.\r\n", ret_val, ERR_ConvertErrorCodeToString(ret_val));
         ...
         else
         {
            ...
            printf("Usage: BASCancelTransaction [TransactionID].\r\n");
         }
      }
      else
      {
...
static void BTPSAPI BASM_Event_Callback(BASM_Event_Data_t *EventData, void *CallbackParameter)
{
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case betBASConnected:
            printf("betBASConnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BASM_CONNECTED_EVENT_DATA_SIZE)
            {
               ...
               printf("   Connection Type:     %s\r\n", (EventData->EventData.ConnectedEventData.ConnectionType == bctServer) ? "Server" : "Client");
               printf("   Number of Instances: %u\r\n", EventData->EventData.ConnectedEventData.NumberOfInstances);
            }
            break;
         case betBASDisconnected:
            printf("betBASDisconnected with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BASM_DISCONNECTED_EVENT_DATA_SIZE)
            {
               ...
               printf("   Connection Type:    %s\r\n", (EventData->EventData.DisconnectedEventData.ConnectionType == bctServer) ? "Server" : "Client");
            }
            break;
         case betBASBatteryLevel:
            printf("betBASBatteryLevel with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BASM_BATTERY_LEVEL_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.BatteryLevelEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n", BoardStr);
               printf("   InstanceID:        %u\r\n", EventData->EventData.BatteryLevelEventData.InstanceID);
               printf("   TransactionID:     %u\r\n", EventData->EventData.BatteryLevelEventData.TransactionID);
               printf("   Status:            %d\r\n", EventData->EventData.BatteryLevelEventData.Status);

               /* Check for a successful status.                        */
               if(!EventData->EventData.BatteryLevelEventData.Status)
               {
                  printf("\n");
                  printf("   Battery Level:     %u%%\r\n", EventData->EventData.BatteryLevelEventData.BatteryLevel);
               }
            }
            break;
         case betBASBatteryLevelNotification:
            printf("betBASBatteryLevelNotification with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BASM_BATTERY_LEVEL_NOTIFICATION_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.BatteryLevelNotificationEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n", BoardStr);
               printf("   InstanceID:        %u\r\n\n", EventData->EventData.BatteryLevelNotificationEventData.InstanceID);
               printf("   Battery Level:     %u%%\r\n", EventData->EventData.BatteryLevelNotificationEventData.BatteryLevel);
            }
            break;
         case betBASBatteryIdentification:
            printf("betBASBatteryIdentification with size %u:\r\n", EventData->EventLength);
            if(EventData->EventLength >= BASM_BATTERY_IDENTIFICATION_EVENT_DATA_SIZE)
            {
               BD_ADDRToStr(EventData->EventData.BatteryIdentificationEventData.RemoteDeviceAddress, BoardStr);

               printf("   Event Callback ID: %u\r\n", EventData->EventCallbackID);
               printf("   Bluetooth Address: %s\r\n", BoardStr);
               printf("   InstanceID:        %u\r\n", EventData->EventData.BatteryIdentificationEventData.InstanceID);
               printf("   TransactionID:     %u\r\n", EventData->EventData.BatteryIdentificationEventData.TransactionID);
               printf("   Status:            %d\r\n", EventData->EventData.BatteryIdentificationEventData.Status);

               /* Check for a successful status.                        */
               if(!EventData->EventData.BatteryIdentificationEventData.Status)
               {
                  printf("\n");
                  printf("   Namespace:         0x%02X\r\n", EventData->EventData.BatteryIdentificationEventData.Namespace);
                  printf("   Description:       0x%04X\r\n", EventData->EventData.BatteryIdentificationEventData.Description);
               }
            }
            break;
         default:
            printf("Unknown BAS Manager Event Received: 0x%08X, Length: 0x%08X\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nBASM Event Data is NULL.\r\n");
