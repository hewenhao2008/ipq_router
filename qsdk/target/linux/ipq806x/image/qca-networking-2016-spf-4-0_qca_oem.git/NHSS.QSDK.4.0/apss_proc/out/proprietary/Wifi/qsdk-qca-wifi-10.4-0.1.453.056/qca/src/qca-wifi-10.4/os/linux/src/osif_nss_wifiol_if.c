...
static int osif_nss_ol_ce_raw_send(void *nss_wifiol_ctx, uint32_t radio_id, qdf_nbuf_t netbuf, uint32_t  len);
...
void update_ring_info(
    ...
    printk("src -> NSS:nentries %d mask %x swindex %d write_index %d hw_index %d CE_space %x ownerspace %x \n",
            sring->nentries,
            sring->nentries_mask,
            sring->sw_index,
            sring->write_index,
    ...
    printk("dest -> NSS:nentries %d mask %x swindex %d write_index %d hw_index %d CE_space %x ownerspace %x \n",
            dring->nentries,
            dring->nentries_mask,
            dring->sw_index,
            dring->write_index,
...
void osif_nss_ol_event_receive(struct ol_ath_softc_net80211 *scn, struct nss_cmn_msg *wifimsg)
{
    ...
    switch (msg_type) {
        case NSS_WIFI_POST_RECV_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    printk("NSS WIFI OFFLOAD - FAILED TO INITIALIZE CE for NSS radio %d\n", scn->nss_wifiol_id);
                    scn->nss_wifiol_ce_enabled = 0;
                    return;
                }
                scn->nss_wifiol_ce_enabled = 1;
                printk("NSS CE init succeeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
        ...
        case NSS_WIFI_INIT_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    printk("NSS WIFI INIT MSG Failed for NSS radio %d\n", scn->nss_wifiol_id);
                }
                printk("NSS WIFI init succeeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
        ...
        case NSS_WIFI_TX_INIT_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    printk("NSS WIFI TX INIT MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
                }
                printk("NSS WIFI TX init succeeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
        ...
        case NSS_WIFI_RAW_SEND_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    printk("NSS WIFI RAW SEND MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
                }
                printk("NSS WIFI RAW SEND succeeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
        ...
        case NSS_WIFI_MGMT_SEND_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    ...
                    printk("Mgmt send failed for NSS radio %d\n", scn->nss_wifiol_id);
        ...
        case NSS_WIFI_STATS_MSG:
            {
                ...
                pdev_txrxstats->mcast_enhance.num_me_rcvd += nss_mestats->rcvd;
                pdev_txrxstats->mcast_enhance.num_me_ucast += nss_mestats->ucast_converted;
                pdev_txrxstats->mcast_enhance.num_me_dropped_a += nss_mestats->alloc_fail;
                pdev_txrxstats->mcast_enhance.num_me_dropped_i += nss_mestats->enqueue_fail + nss_mestats->copy_fail + nss_mestats->peer_flow_ctrl_send_fail;
                pdev_txrxstats->mcast_enhance.num_me_dropped_s += nss_mestats->loopback_err + nss_mestats->dst_addr_err;
        ...
        case NSS_WIFI_HTT_INIT_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("HTT RX init configuration failed with error for NSS radio %d\n", scn->nss_wifiol_id);
                scn->osif_nss_ol_wifi_cfg_complete.response = NSS_TX_FAILURE;
                complete(&scn->osif_nss_ol_wifi_cfg_complete.complete);
                return;
            }

            printk("HTT RX INIT configuration success for NSS radio %d\n", scn->nss_wifiol_id);
        ...
        case NSS_WIFI_PEER_STATS_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("Peer Stats Message - send to NSS failed %d\n", scn->nss_wifiol_id);
                scn->osif_nss_ol_wifi_cfg_complete.response = NSS_TX_FAILURE;
                complete(&scn->osif_nss_ol_wifi_cfg_complete.complete);
                return;
            }
            ...
            scn->osif_nss_ol_wifi_cfg_complete.response = NSS_TX_SUCCESS;
            complete(&scn->osif_nss_ol_wifi_cfg_complete.complete);
            printk("Peer Stats Message - success %d\n", scn->nss_wifiol_id);
	    break;

        case NSS_WIFI_ME_SYNC_MSG:
            {
                struct MC_LIST_UPDATE list;
                struct nss_wifi_tx_me_host_sync_msg *me_sync_msg =
                   (struct nss_wifi_tx_me_host_sync_msg *)&((struct nss_wifi_msg *)wifimsg)->msg.wmehsync;
                struct nss_wifi_tx_me_host_sync_grp_entry *me_sync =
                   (struct nss_wifi_tx_me_host_sync_grp_entry *)&me_sync_msg->grp_entry[0];
                struct ieee80211vap *vap = NULL;
                int idx = 0;
                ...
                while(1) {
                    ...
                    vap = ol_ath_vap_get(scn, me_sync_msg->vdev_id);
                    list.vap = vap;
        ...
        case NSS_WIFI_WDS_PEER_ADD_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI PEER ADD MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
                return;
            }
            break;
        ...
        case NSS_WIFI_WDS_PEER_DEL_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI PEER DEL MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_PEER_FREELIST_APPEND_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI PEER FREELIST APPEND MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_RX_REORDER_ARRAY_FREELIST_APPEND_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI RX REORDER ARRAY FREELIST APPEND MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_WDS_VENDOR_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI WDS VENDOR MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_DBDC_PROCESS_ENABLE_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI DBDC Process Enable failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_PRIMARY_RADIO_SET_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI Primary Radio Set failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_FORCE_CLIENT_MCAST_TRAFFIC_SET_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS Client MCast Traffic Set failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_STORE_OTHER_PDEV_STAVAP_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS Store Other Pdev STAVAP MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_ENABLE_PERPKT_TXSTATS_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS Perpacket TX Stats MSG failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_IGMP_MLD_TOS_OVERRIDE_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS IGMP MLD TOS OVERRIDE failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_MSDU_TTL_SET_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS MSDU_TTL configure msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_MONITOR_FILTER_SET_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS Monitor Filter set msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_OL_STATS_CFG_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS OL_Stats configure msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_PKTLOG_CFG_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS PCKTLOG configure msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_STOP_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI STOP msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_TX_QUEUE_CFG_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI TX QUEUE config failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_TX_MIN_THRESHOLD_CFG_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI TX MIN THRESHOLD config failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;

        case NSS_WIFI_RESET_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                printk("NSS WIFI RESET config failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
...
int osif_nss_ol_wds_extn_peer_cfg_send(struct ol_txrx_pdev_t *pdev, struct ol_txrx_peer_t *peer)
{
    ...
    printk("%s %p %d Peer CFG ol stats enable \n",__func__, nss_wifiol_ctx, radio_id);
     ...
     */
    printk("peer->wds_enabled %d\n", peer->wds_enabled);
    printk("peer->wds_tx_mcast_4addr %d\n", peer->wds_tx_mcast_4addr);
    printk("peer->wds_tx_ucast_4addr %d\n", peer->wds_tx_ucast_4addr);
    printk("peer->wds_rx_filter %d\n", peer->wds_rx_filter);
    printk("peer->wds_rx_ucast_4addr %d\n", peer->wds_rx_ucast_4addr);
    printk("peer->wds_rx_mcast_4addr %d\n", peer->wds_rx_mcast_4addr);
 ...
 */
int osif_nss_ol_mgmt_frame_send(void *nss_wifiol_ctx, int32_t radio_id, uint32_t desc_id, qdf_nbuf_t netbuf)
{
    int32_t len;
    int32_t ifnum;
    struct nss_wifi_msg wifimsg;
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        printk("%s: WIFI Management send failed %d \n",__FUNCTION__, status);
        return 1;
    }
 ...
 */
int osif_nss_ol_stats_frame_send(void *nss_wifiol_ctx, int32_t radio_id, qdf_nbuf_t netbuf)
{
    int32_t len = 0;
    int32_t ifnum = 0;
    struct nss_wifi_msg wifimsg;
...
int osif_nss_peer_stats_frame_send(struct ol_txrx_pdev_t *pdev, uint16_t peer_id)
{
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        printk("%s: WiFi peer stats send request %d \n",__FUNCTION__, status);
        return 1;
    }
    ...
    if (ret == 0) {
        printk("Waiting for peer stats req msg ack timed out %d\n", radio_id);
        return 1;
    }
    ...
    if (NSS_TX_FAILURE == scn->osif_nss_ol_wifi_cfg_complete.response) {
        printk("NACK for peer_stats_send request from NSS FW %d\n", radio_id);
        return 1;
    }
...
int osif_nss_ol_wifi_pause(struct ol_ath_softc_net80211 *scn)
{
    ...
    {
        retrycnt++;
        printk("WIFI Pause message send failed  %d Retrying %d \n", status, retrycnt);
        if (retrycnt >= OSIF_NSS_OL_MAX_RETRY_CNT){
            printk("Max retry reached : Unregistering with NSS\n");
            return -1;
        }
    }
...
int osif_nss_ol_wifi_tx_capture_set(struct ol_txrx_pdev_t *pdev, uint8_t tx_capture_enable)
{
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        printk("WIFI TX Data Capture message send failed  %d \n", status);
        return -1;
    }
...
int osif_nss_ol_wifi_reset(struct ol_ath_softc_net80211 *scn, uint16_t delay)
{
    ...
    {
        retrycnt++;
        printk("WIFI reset message send failed  %d Retrying %d \n", status, retrycnt);
        if (retrycnt >= OSIF_NSS_OL_MAX_RETRY_CNT){
            printk("Max retry reached : Unregistering with NSS\n");
            break;
        }
...
#define CE_HTT_TX_CE            4
    ...
    ifnum = osif_nss_ol_get_ifnum(scn->nss_wifiol_id);
    if(!ifnum){
        printk("no if found return ");
        return -1;
    }
...
void osif_nss_ol_post_recv_buffer(struct ol_ath_softc_net80211 *scn)
{
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        printk("WIFI CE configure failed %d \n", status);
        return;
    }
...
int osif_ol_nss_htc_send(void *nss_wifiol_ctx, int nssid, qdf_nbuf_t netbuf, uint32_t transfer_id)
{
...
int osif_nss_ol_htt_rx_init(void *htt_handle)
{
    ...
    ret = wait_for_completion_timeout(&scn->osif_nss_ol_wifi_cfg_complete.complete, msecs_to_jiffies(OSIF_NSS_WIFI_CFG_TIMEOUT_MS));
    if (ret == 0) {
        printk("Waiting for htt rx init config msg ack timed out\n");
        return 1;
    }
    ...
    if (NSS_TX_FAILURE == scn->osif_nss_ol_wifi_cfg_complete.response) {
        ...
        printk("nack for htt_rx_init config msg\n");
        return 1;
    }
...
static int osif_nss_ol_ce_raw_send(void *nss_wifiol_ctx, uint32_t radio_id, qdf_nbuf_t netbuf, uint32_t  len )
{
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        printk("htt config message failed %d \n", status);
        return 1;
    }
    printk("delay 100ms \n");
    mdelay(100);
