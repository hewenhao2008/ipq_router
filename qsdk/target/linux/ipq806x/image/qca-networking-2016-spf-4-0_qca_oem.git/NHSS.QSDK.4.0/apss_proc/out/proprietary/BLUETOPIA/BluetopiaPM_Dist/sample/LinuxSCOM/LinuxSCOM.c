...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXSCOM - Simple Linux sample application using Bluetopia Platform      */
...
/*   --------  -----------    ------------------------------------------------*/
/*   06/02/11  M. Hundley     Initial creation.                               */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#include "LinuxSCOM.h"     /* Main Application Prototypes and Constants.      */
...
static int RegisterServerConnection(ParameterList_t *TempParam);
static int UnRegisterServerConnection(ParameterList_t *TempParam);
static int EnableServerConnection(ParameterList_t *TempParam);
static int OpenRemoteConnection(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* BTPM SCO Manager Callback function prototype.                     */
static void BTPSAPI SCOM_Event_Callback(SCOM_Event_Data_t *EventData, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERSERVERCONNECTION", RegisterServerConnection);
   AddCommand("UNREGISTERSERVERCONNECTION", UnRegisterServerConnection);
   AddCommand("ENABLESERVERCONNECTION", EnableServerConnection);
   AddCommand("OPENREMOTECONNECTION", OpenRemoteConnection);
   ...
   /* interpreter.  After the function has been run it then checks the  */
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  26)ConnectWithRemoteDevice                    *\r\n");
   printf("*                  27)DisconnectRemoteDevice                     *\r\n");
   ...
   printf("*                  35)RegisterServerConnection                   *\r\n");
   printf("*                  36)UnRegisterServerConnection                 *\r\n");
   printf("*                  37)EnableServerConnection                     *\r\n");
   printf("*                  38)OpenRemoteConnection                       *\r\n");
   printf("*                  39)CloseConnection                            *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
...
static int DisconnectRemoteDevice(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters))
      {
         ...
         {
            printf("DEVM_DisconnectRemoteDevice() Success: %lu.\r\n", DisconnectFlags);
...
static int ChangeSimplePairingParameters(ParameterList_t *TempParam)
{
   ...
   /* modules to register a local SCO/eSCO Manager server.  This allows */
   /* the ability for the client to accept/reject incoming SCO/eSCO     */
   /* requests.                                                         */
static int RegisterServerConnection(ParameterList_t *TempParam)
{
   int       Result;
   ...
   /* First, check that we have already been initialized                */
   if(Initialized)
   {
      /* Initialized, check the specified parameters.                   */
      if((TempParam) && (TempParam->NumberofParameters > 0))
      {
         EnableConnection = (Boolean_t)TempParam->Params[0].intParam;

         /* Convert parameter from string to Bluetooth Device Address.  */
         /* If a specific device is not required, a NULL device address */
         /* (all zeros) should be used.                                 */
         if((TempParam->NumberofParameters > 1) && (TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
            StrToBD_ADDR(TempParam->Params[1].strParam, &BD_ADDR);
         else
         {
            /* No Bluetooth device specified, default to empty.         */
            ASSIGN_BD_ADDR(BD_ADDR, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
         }

         /* Parameters parsed, go ahead and attempt to register the     */
         /* server.                                                     */
         if((Result = SCOM_RegisterServerConnection(EnableConnection, BD_ADDR, SCOM_Event_Callback, NULL)) > 0)
         {
            printf("SCOM_RegisterServerConnection() Success: %d.\r\n", Result);
         ...
         {
            /* The Server was not successfully registered.              */
            printf("SCOM_RegisterServerConnection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters was/were invalid.   */
         printf("Usage: RegisterServerConnection [EnableConnection - 0/1] [BD_ADDR (optional)].\r\n");
...
static int UnRegisterServerConnection(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         /* Parameter appears to be semi-valid, so go ahead and         */
         /* un-register the server.                                     */
         if((Result = SCOM_UnRegisterServerConnection((unsigned int)(TempParam->Params[0].intParam))) == 0)
         {
            printf("SCOM_UnRegisterServerConnection() Success: %d.\r\n", Result);
         ...
         {
            printf("SCOM_UnRegisterServerConnection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* There was an error with the parameter.                      */
         printf("Usage: UnRegisterServerConnection [ConnectionID].\r\n");
   ...
   /* The following function allows local modules to enable/disable     */
   /* incoming connections for a previously registered SCO/eSCO server. */
static int EnableServerConnection(ParameterList_t *TempParam)
{
   int       ret_val;
   int       Result;
   ...
   if(Initialized)
   {
      /* Initialized, check the parameters.                             */
      if((TempParam) && (TempParam->NumberofParameters >= 2))
      {
         EnableConnection = (Boolean_t)TempParam->Params[1].intParam;

         /* Convert parameter from string to Bluetooth Device Address.  */
         /* If a specific device is not required, a NULL device address */
         /* (all zeros) should be used.                                 */
         if((TempParam->NumberofParameters > 2) && (TempParam->Params[2].strParam) && (strlen(TempParam->Params[2].strParam)))
            StrToBD_ADDR(TempParam->Params[2].strParam, &RemoteDevice);
         else
         {
            /* No Bluetooth device specified, default to empty.         */
            ASSIGN_BD_ADDR(RemoteDevice, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
         }

         /* Parameters parsed, attempt to enable the connection.        */
         if((Result = SCOM_EnableServerConnection((unsigned int)(TempParam->Params[0].intParam), EnableConnection, RemoteDevice)) == 0)
         {
            printf("SCOM_EnableServerConnection() Success: %d.\r\n", Result);
            if(EnableConnection)
         ...
         {
            printf("SCOM_EnableServerConnection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      else
      {
         ...
         printf("Usage: EnableServerConnection [ConnectionID] [Enable Connection - 0/1] [BD_ADDR (optional)].\r\n");
   ...
   /* The following function allows local modules to open remote SCO/   */
   /* eSCO connctions to a specified remote device.                     */
static int OpenRemoteConnection(ParameterList_t *TempParam)
{
   int       ret_val;
   int       Result;
   ...
   if(Initialized)
   {
      ...
      {
         /* Convert the parameter from a string to Bluetooth device     */
         /* address.                                                    */
         StrToBD_ADDR(TempParam->Params[0].strParam, &RemoteDevice);

         if((Result = SCOM_OpenRemoteConnection(RemoteDevice, SCOM_Event_Callback, NULL, NULL)) > 0)
         {
            printf("SCOM_OpenRemoteConnection() Success.\r\n");
         ...
         {
            printf("SCOM_OpenRemoteConnection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the parameters was invalid.                  */
         printf("Usage: OpenRemoteConnection [Remote Device].\r\n");
   ...
   /* The following function allows local modules to close an active    */
   /* SCO/eSCO connection.  If this function is specified with a server */
   /* connection ID, it will NOT un-register the local SCO/eSCO server  */
   /* connection, it will just disconnect any currently connected remote*/
   /* device.                                                           */
static int CloseConnection(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   /* First, check that we have already been initialzed.                */
   if(Initialized)
   {
      /* Next, check to make sure we have the required input parameters.*/
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
         /* Parameters appear semi-valid, attempt to close the          */
         /* connection.                                                 */
         if((Result = SCOM_CloseConnection((unsigned int)TempParam->Params[0].intParam)) >= 0)
         {
            printf("SCOM_CloseConnection() Success.\r\n");
         ...
         {
            printf("SCOM_CloseConnection() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* There was an error parsing the parameters.                  */
         printf("Usage: CloseConnection [ConnectionID].\r\n");
   ...
   if(EventData)
   {
      ...
      switch(EventData->EventType)
      {
         case setServerConnectionOpen:
            printf("SCO Connection Open.\r\n");

            BD_ADDRToStr(EventData->EventData.ServerConnectionOpenEventData.RemoteDeviceAddress, Buffer);

            printf("    Connection ID:  %u\r\n", EventData->EventData.ServerConnectionOpenEventData.ConnectionID);
            printf("    Remote Address: %s\r\n", Buffer);
            break;
         case setConnectionClose:
            printf("SCO Connection Closed.\r\n");

            printf("    Connection ID: %u\r\n", EventData->EventData.ConnectionCloseEventData.ConnectionID);
            break;
         case setRemoteConnectionOpenStatus:
            printf("SCO Outgoing Connection Status.\r\n");

            printf("    Connection ID: %u\r\n", EventData->EventData.RemoteConnectionOpenStatusEventData.ConnectionID);
            printf("    Status:        %d, %s\r\n", EventData->EventData.RemoteConnectionOpenStatusEventData.Status, ERR_ConvertErrorCodeToString(EventData->EventData.RemoteConnectionOpenStatusEventData.Status));
            break;
         default:
            printf("Unknown SCO Manager Event Received: 0x%08X, Length: 0x%08X.\r\n", (unsigned int)EventData->EventType, EventData->EventLength);
            break;
      }
   }
   else
      printf("\r\nSCOM Event Data is NULL.\r\n");
