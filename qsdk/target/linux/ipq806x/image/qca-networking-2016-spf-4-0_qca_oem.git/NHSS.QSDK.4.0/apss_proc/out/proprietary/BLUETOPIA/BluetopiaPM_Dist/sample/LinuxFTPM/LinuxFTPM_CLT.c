...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  LINUXFTPM_SRV - Simple Linux application using Bluetopia Platform Manager */
/*                  File Transfer Protocol Manager Application Programming    */
/*                  (API) Interface - Client Role only.                       */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
#include "LinuxFTPM_CLT.h" /* Main Application Prototypes and Constants.      */

#include "SS1BTPM.h"       /* BTPM Application Programming Interface.         */
#include "SS1BTFTPM.h"     /* FTP Manager Application Programming Interface.  */
...
static unsigned int        FTPConnectionID;         /* Variable used to hold the       */
                                                    /* FTP Client ID of the currently  */
                                                    /* opened Remote FTP Server (or    */
                                                    /* Local FTP Client).              */

static Boolean_t           Connected;               /* Variable which flags whether or */
                                                    /* not there is currently an       */
                                                    /* active connection.              */

static unsigned int        InRootDirectory;         /* Variable used to flag whether   */
                                                    /* or not we are currently in the  */
                                                    /* Root Directory.                 */

static char                CurrentPath[2048+1];     /* Variable used to hold the       */
...
static int Directory(ParameterList_t *TempParam);
static int ChangeDirectory(ParameterList_t *TempParam);
static int MakeDirectory(ParameterList_t *TempParam);
static int DeleteDirectory(ParameterList_t *TempParam);
static int DeleteFile(ParameterList_t *TempParam);
static int PutFile(ParameterList_t *TempParam);
static int GetFile(ParameterList_t *TempParam);
static int MakeFile(ParameterList_t *TempParam);
static int Abort(ParameterList_t *TempParam);
static int LocalDirectory(ParameterList_t *TempParam);
...
static void BTPSAPI DEVM_Authentication_Callback(DEVM_Authentication_Information_t *AuthenticationRequestInformation, void *CallbackParameter);
   ...
   /* FTPM Client Event Callback function prototype.                    */
static void BTPSAPI FTPM_Event_Callback(FTPM_Event_Data_t *FTPM_Event_Data, void *CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("DISCONNECT", CloseRemoteServer);
   AddCommand("DIR", Directory);
   AddCommand("CD", ChangeDirectory);
   AddCommand("MKDIR", MakeDirectory);
   AddCommand("RMDIR", DeleteDirectory);
   AddCommand("DEL", DeleteFile);
   AddCommand("PUT", PutFile);
   AddCommand("GET", GetFile);
   AddCommand("MKFILE", MakeFile);
   AddCommand("ABORT", Abort);
   AddCommand("LDIR", LocalDirectory);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  48)SendRawHCICommand                          *\r\n");
   printf("*                  49)Connect                                    *\r\n");
   printf("*                  50)Disconnect                                 *\r\n");
   printf("*                  51)dir                                        *\r\n");
   printf("*                  52)cd                                         *\r\n");
   printf("*                  53)mkdir                                      *\r\n");
   printf("*                  54)rmdir                                      *\r\n");
...
static int SendRawHCICommand(ParameterList_t *TempParam)
{
   ...
   /* with a Remote FTP Server.  This function returns zero if          */
   /* successful and a negative value if an error occurred.             */
static int OpenRemoteServer(ParameterList_t *TempParam)
{
   int       ret_val;
   ...
   if(Initialized)
   {
      /* Next, let's make sure that there is not an FTP Client already  */
      /* open.                                                          */
      if(!FTPConnectionID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 2) && (TempParam->Params[1].intParam))
         {
            ...
            /* Now let's attempt to open the Remote FTP Server.         */
            Result = FTPM_Open_Remote_File_Server(BD_ADDR, (unsigned int)TempParam->Params[1].intParam, (unsigned long)TempParam->Params[2].intParam, FTPM_Event_Callback, NULL, NULL);

            if(Result > 0)
            {
               /* Inform the user that the call to open the Remote FTP  */
               /* Server was successful.                                */
               printf("FTPM_Open_Remote_File_Server() was successful.\r\n");

               /* Note the FTP Client ID.                               */
               FTPConnectionID = Result;

               /* Flag that we are in the Root Directory.               */
            ...
            {
               /* Inform the user that the call to Open the Remote File */
               /* Server failed.                                        */
               printf("Error calling FTPM_Open_Remote_File_Server(): %d.\r\n", Result);
         ...
         {
            printf("Usage: Connect [Remote Device Address] [RFCOMM Server Port] [Open Flags: Bit Mask (1: Authorization, 2: Authentication, 4: Encryption)].\r\n");
      ...
      {
         printf("Unable to open FTP Client.\r\nFTP Client is already open.\r\n");
...
static int CloseRemoteServer(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Next, let's make sure that a Remote FTP Server is indeed Open. */
      if(FTPConnectionID)
      {
         printf("FTP Client closed successfully.\r\n");

         /* Simply close the FTP Client.                                */
         FTPM_Close_Client(FTPConnectionID);

         /* Flag that there is no longer an FTP Client connected.       */
         FTPConnectionID = 0;
      ...
      {
         /* Display an error to the user informing them that no FTP     */
         /* Client is open.                                             */
         printf("You must open an FTP Client before you can close an FTP Client.\r\n");
   ...
   /* The following function is responsible for issuing a Directory     */
   /* Request on a Remote FTP Server.  This function returns zero if    */
   /* successful and a negative value if an error occurred.             */
static int Directory(ParameterList_t *TempParam)
{
   int   ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's determine if the user has specified a valid     */
         /* Bluetooth Device Address to connect with.                   */
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (BTPS_StringLength(TempParam->Params[0].strParam)))
            DirectoryName = TempParam->Params[0].strParam;
         else
            DirectoryName = NULL;

         /* Simply issue the FTP Get Directory Requst Command.          */
         Result = FTPM_Get_Directory(FTPConnectionID, DirectoryName);

         if(Result >= 0)
         {
            /* Inform the user that the Get Directory Request was issued*/
            /* successfully.                                            */
            printf("FTPM_Get_Directory() issued successfully.\r\n");

            ret_val = 0;
         }
         else
         {
            /* Inform the user that there was an error submitting the   */
            /* Get Directory Request.                                   */
            printf("Error calling FTPM_Get_Directory().\r\n");
      ...
      {
         /* Inform the user that the command cannot be completed because*/
         /* there is no FTP Client currently open.                      */
         printf("You must open an FTP Client before issuing this command.\r\n");
   ...
   /* The following function is responsible for issuing a Directory     */
   /* Change Request on a Remote FTP Server.  This function returns zero*/
   /* if successful and a negative value if an error occurred.          */
static int ChangeDirectory(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's determine if the user has requested to either   */
         /* change into a specific directory, or has requested to go up */
         /* a directory.                                                */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            if(!strcmp(TempParam->Params[0].strParam, ".."))
            {
               /* The user has requested to go up to the parent         */
               /* directory.                                            */
               if(!InRootDirectory)
               {
                  /* We are not already in the Root Directory, so issue */
                  /* the Change Directory Request.                      */
                  Result = FTPM_Set_Directory(FTPConnectionID, NULL);

                  /* Inform the user of the result of the Set Directory */
                  /* Command that was issued.                           */
                  if(Result >= 0)
                  {
                     printf("FTPM_Set_Directory() successful.\r\n");

                     ret_val = 0;
                  }
                  else
                  {
                     printf("FTPM_Set_Directory() unsuccessful: %d.\r\n", Result);
               ...
               {
                  /* Inform the user that we are already in the Root    */
                  /* Directory.                                         */
            ...
            {
               /* The user has requested to change into a directory.    */
               /* Simply issue the Change Directory Command with this   */
               /* Directory.                                            */
               Result = FTPM_Set_Directory(FTPConnectionID, TempParam->Params[0].strParam);
      ...
      {
         /* Inform the user that the command cannot be completed because*/
         /* there is FTP Client currently open.                         */
         printf("You must open an FTP Client before issuing this command.\r\n");
   ...
   /* The following function is responsible for creating a Directory    */
   /* on a Remote FTP Server.  This function returns zero if successful */
   /* and a negative value if an error occurred.                        */
static int MakeDirectory(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's make sure that a directory was specified as a   */
         /* parameter to this command.                                  */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* The user has requested to create a directory.  Simply    */
            /* issue the Create Directory Command with the Directory    */
            /* Name.                                                    */
            Result = FTPM_Create_Directory(FTPConnectionID, TempParam->Params[0].strParam);

            /* Inform the user of the result of the Create Directory    */
            /* Command that was issued.                                 */
            if(Result >= 0)
            {
               printf("FTPM_Create_Directory() successful.\r\n");

               ret_val = 0;
            }
            else
            {
               printf("FTPM_Create_Directory() unsuccessful: %d.\r\n", Result);
   ...
   /* The following function is responsible for Deleting a Directory    */
   /* from the Remote FTP Server.  This function returns zero if        */
   /* successful and a negative value if an error occurred.             */
static int DeleteDirectory(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's make sure that a remote directory was specified */
         /* as a parameter to this command.                             */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* The user has requested to delete a directory.  Simply    */
            /* issue the Delete Directory Command with the Directory    */
            /* Name.                                                    */
            Result = FTPM_Delete_Directory(FTPConnectionID, TempParam->Params[0].strParam);

            /* Inform the user of the result of the Delete Directory    */
            /* Command that was issued.                                 */
            if(Result >= 0)
            {
               printf("FTPM_Delete_Directory() successful.\r\n");

               ret_val = 0;
            }
            else
            {
               printf("FTPM_Delete_Directory() unsuccessful: %d.\r\n", Result);
         ...
         {
            /* Invalid parameters, so inform the user.                  */
            printf("Usage: RMDIR [Remote Directory].\r\n");
   ...
   /* Remote FTP Server.  This function returns zero if successful and a*/
   /* negative value if an error occurred.                              */
static int DeleteFile(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's make sure that a remote file was specified as a */
         /* parameter to this command.                                  */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* The user has requested to delete a file.  Simply issue   */
            /* the Delete File Command with the File Name.              */
            Result = FTPM_Delete_File(FTPConnectionID, TempParam->Params[0].strParam);

            /* Inform the user of the result of the Delete File Command */
            /* that was issued.                                         */
            if(Result >= 0)
            {
               printf("FTPM_Delete_File() successful.\r\n");

               ret_val = 0;
            }
            else
            {
               printf("FTPM_Delete_File() unsuccessful: %d.\r\n", Result);
   ...
   /* The following function is responsible for transferring a File to  */
   /* a Remote FTP Server.  This function returns zero if successful    */
   /* and a negative value if an error occurred.                        */
static int PutFile(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's make sure that a local file was specified as a  */
         /* parameter to this command.                                  */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* Next, let's check to see if the user has requested to put*/
            /* the File to the Remote Device with a different name.     */
            if(TempParam->NumberofParameters > 1)
            {
               /* More than one parameter was specified, so let's make  */
               /* sure that it is valid.                                */
               if((TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
               {
                  /* Simply put the Local File to the Remote Device with*/
                  /* the specified Remote Device File Name.             */
                  Result = FTPM_Put_File(FTPConnectionID, NULL, TempParam->Params[0].strParam, TempParam->Params[1].strParam);

                  /* Inform the user of the result of the Put File      */
                  /* Command that was issued.                           */
                  if(Result >= 0)
                  {
                     printf("FTPM_Put_File() successful.\r\n");

                     ret_val = 0;
                  }
                  else
                  {
                     printf("FTPM_Put_File() unsuccessful: %d.\r\n", Result);
            ...
            {
               /* The user has requested to put a file.  Simply issue   */
               /* the Put File Command with the File Name.              */

               /* Since the user did not specify a File Name for the    */
               /* Remote File, we will have to use the File name of the */
               /* Local File, but we have to make sure we strip out all */
               /* of the path information (if present).                 */

               /* Simply loop from the end of the File Name until we    */
               /* find a '/' character.  If we do find one we need to   */
               /* note the location to use.                             */
               Index = strlen(TempParam->Params[0].strParam) - 1;
               while((Index >= 0) && (TempParam->Params[0].strParam[Index] != '/'))
                  Index--;

               /* We are either one less than the array (Index is       */
               /* negative) or we are sitting on a '/' character.       */
               /* Either way we need to increment the Index by one to   */
               /* make sure that Index is located at the first location */
               /* of the File Name.                                     */
               Index++;

               Result = FTPM_Put_File(FTPConnectionID, NULL, TempParam->Params[0].strParam, &(TempParam->Params[0].strParam[Index]));

               /* Inform the user of the result of the Put File Command */
               /* that was issued.                                      */
               if(Result >= 0)
               {
                  printf("FTPM_Put_File() successful.\r\n");
   ...
   /* The following function is responsible for transferring a File from*/
   /* a Remote FTP Server.  This function returns zero if successful    */
   /* and a negative value if an error occurred.                        */
static int GetFile(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* Next, let's check to see if the user has requested to get*/
            /* the File to the Local Device with a different name.      */
            if(TempParam->NumberofParameters > 1)
            {
               /* More than one parameter was specified, so let's make  */
               /* sure that it is valid.                                */
               if((TempParam->Params[1].strParam) && (strlen(TempParam->Params[1].strParam)))
               {
                  /* Simply get the Local File from the Remote Device   */
                  /* with the specified Local File Name.                */
                  /* * NOTE * We are passing in 0 for the length of the */
                  /*          file.  This has no meaning other than not */
                  /*          displaying the file size while the        */
                  /*          transfer is occurring.                    */
                  Result = FTPM_Get_File(FTPConnectionID, TempParam->Params[0].strParam, 0, NULL, TempParam->Params[1].strParam);

                  /* Inform the user of the result of the Get File      */
                  /* Command that was issued.                           */
                  if(Result >= 0)
                  {
                     printf("FTPM_Get_File() successful.\r\n");

                     ret_val = 0;
                  }
                  else
                  {
                     printf("FTPM_Get_File() unsuccessful: %d.\r\n", Result);
            ...
            {
               /* The user has requested to get a file.  Simply issue   */
               /* the Get File Command with the File Name.              */
               /* * NOTE * We are passing in 0 for the length of the    */
               /*          file.  This has no meaning other than not    */
               /*          displaying the file size while the transfer  */
               /*          is occurring.                                */
               Result = FTPM_Get_File(FTPConnectionID, TempParam->Params[0].strParam, 0, NULL, TempParam->Params[0].strParam);

               /* Inform the user of the result of the Get File Command */
               /* that was issued.                                      */
               if(Result >= 0)
               {
                  printf("FTPM_Get_File() successful.\r\n");
   ...
   /* The following function is responsible for creating an empty File  */
   /* on a Remote FTP Server.  This function returns zero if successful */
   /* and a negative value if an error occurred.                        */
static int MakeFile(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Next, let's make sure that a file was specified as a        */
         /* parameter to this command.                                  */
         if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
         {
            /* The user has requested to create a file.  Simply issue   */
            /* the Create File Command with the File Name.              */
            Result = FTPM_Create_File(FTPConnectionID, TempParam->Params[0].strParam);

            /* Inform the user of the result of the Create File Command */
            /* that was issued.                                         */
            if(Result >= 0)
            {
               printf("FTPM_Create_File() successful.\r\n");

               ret_val = 0;
            }
            else
            {
               printf("FTPM_Create_File() unsuccessful: %d.\r\n", Result);
         ...
         {
            /* Invalid parameters, so inform the user.                  */
            printf("Usage: MKFILE [Remote File].\r\n");
   ...
   /* FTP Action on a Remote FTP Server.  This function returns zero if */
   /* successful and a negative value if an error occurred.             */
static int Abort(ParameterList_t *TempParam)
{
   int ret_val;
   ...
   if(Initialized)
   {
      ...
      if(FTPConnectionID)
      {
         /* Simply issue the FTP Abort Request Command.                 */
         Result = FTPM_Abort(FTPConnectionID);

         if(Result >= 0)
         {
            /* Inform the user that the Abort was issued successfully.  */
            printf("FTPM_Abort() issued successfully.\r\n");
         ...
         else
         {
            /* Inform the user that there was an error submitting the   */
            /* Abort Request.                                           */
            printf("Error calling FTPM_Abort().\r\n");
   ...
   /* of the Local Device on the Display.  This function returns zero if*/
   /* successful and a negative value if an error occurred.             */
static int LocalDirectory(ParameterList_t *TempParam)
{
   int  ret_val;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam) && (strlen(TempParam->Params[0].strParam)))
      {
         /* If the user has requested a specific type/name to be        */
         /* displayed then we need to append it to the List Command.    */
         sprintf(Buffer, "ls -al ");

         /* Note that if we do not have enough space for the requested  */
         /* Directory Listing then we will simply list all of the       */
         /* directory information.                                      */
         if((strlen(Buffer) + strlen(TempParam->Params[0].strParam)) < sizeof(Buffer))
            strcat(Buffer, TempParam->Params[0].strParam);
         else
            strcat(Buffer, "*");
      ...
      else
      {
         ...
         {
            /* Simply display the contents of the current directory.    */
            ret_val = system("ls -al");
            if(ret_val)
               printf("system(%s) returned %d.\r\n", Buffer, ret_val);
...
//xxx Here check if this will default to "just works" or auto accept.
   ...
   /* The following function is the FTPM Client Event Callback function */
   /* that is Registered with the FTP Manager Module.  This callback is */
   /* responsible for processing all FTPM Client Events.                */
static void BTPSAPI FTPM_Event_Callback(FTPM_Event_Data_t *EventData, void *CallbackParameter)
{
   int                     AM_Flag;
   int                     i;
   char                    BoardStr[13];
   char                    temp[512];
   char                    size[132];
   FTPM_Directory_Entry_t *Entry;

   if(EventData)
   {
      switch(EventData->EventType)
      {
         case etFTPMConnectConfirmation:
            /* The Client received a Connect Confirmation, display      */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMConnectConfirmation, FTPConnectionID: 0x%04X, Status 0x%04X\r\n", EventData->EventData.ClientConnectConfirmationEventData.FTPConnectionID,
                                                                                   EventData->EventData.ClientConnectConfirmationEventData.FTPConnectStatus);
            BD_ADDRToStr(EventData->EventData.ClientConnectConfirmationEventData.RemoteDeviceAddress, BoardStr);
            printf("BD_ADDR: %s\r\n", BoardStr);

            /* Check to see if the Client Port was opened successfully. */
            if(EventData->EventData.ClientConnectConfirmationEventData.FTPConnectStatus)
            {
               /* There was an error while trying to open a Client port.*/
               /* invalidate the FTP Client ID set during the call to   */
               /* the OpenFTPClient() function.                         */
               FTPConnectionID = 0;
            ...
            else
            {
               ...
               /* Initialize the Current Directory String to be the     */
               /* root directory.                                       */
               strcpy(CurrentPath, "/");
            ...
            printf("\r\nFTPM>");
            break;
         case etFTPMDisconnectIndication:
            /* The Client was disconnected, display the appropriate     */
            /* information and clean up the information relevant to this*/
            /* connection.                                              */
            printf("\r\n");
            printf("etFTPMDisconnectIndication, FTPConnectionID: 0x%04X\r\n", EventData->EventData.DisconnectIndicationEventData.FTPConnectionID);

            /* Flag that there is no longer a Client connected.         */
            FTPConnectionID = 0;
            ...
            printf("\r\nFTPM>");
            break;
         case etFTPMAbortConfirmation:
            /* An Abort was executed.  Display any relevant information.*/
            printf("\r\n");
            printf("etFTPMAbortConfirmation, FTPConnectionID: 0x%04X\r\n", EventData->EventData.ClientAbortConfirmationEventData.FTPConnectionID);
            printf("\r\nFTPM>");
            break;
         case etFTPMDirectoryRequestConfirmation:
            /* A Directory Request Confirmation was received, display   */
            /* any relevant information and update the Directory.       */
            printf("\r\n");
            printf("etFTPMDirectoryRequestConfirmation, FTPConnectionID: 0x%04X, Request Complete: %s, Parent Directory: %s\r\n", EventData->EventData.ClientDirectoryRequestConfirmationEventData.FTPConnectionID,
                                                                                                                         (EventData->EventData.ClientDirectoryRequestConfirmationEventData.RequestCompleted?"TRUE":"FALSE"),
                                                                                                                         (EventData->EventData.ClientDirectoryRequestConfirmationEventData.ParentDirectory?"TRUE":"FALSE"));

            /* Set variables, Entry and i, to point into the structure  */
            /* to increase readability.                                 */
            Entry = EventData->EventData.ClientDirectoryRequestConfirmationEventData.FTPDirectoryEntries;
            i     = EventData->EventData.ClientDirectoryRequestConfirmationEventData.NumberDirectoryEntries;

            /* While there are more directory entries...                */
            while(i--)
            {
               /* First make sure that the entry contains the           */
               /* information we are looking for via the bit flags mask.*/
               if((Entry->FieldMask & (FTPM_OBJECT_INFO_MASK_NAME | FTPM_OBJECT_INFO_MASK_SIZE | FTPM_OBJECT_INFO_MASK_MODIFIED)))
               {
                  /* The entry contains information we are looking for, */
                  /* now check to see if this is a File or Directory    */
                  /* Type.                                              */
                  if(!Entry->DirectoryEntry)
                  {
                     /* This is a File so create an application specific*/
                     /* string to display the number of bytes.          */
                     sprintf(size, "%u Bytes", Entry->Size);

                     /* Now retrieve the modified date and time         */
                     /* information.                                    */
                     AM_Flag = (int)(Entry->Modified.Hour < 12);
                     if(AM_Flag == FALSE)
                     {
                        Entry->Modified.Hour -= ((Byte_t)12);
                     }

                     sprintf(temp, "%u/%02u/%04u %u:%02u %s", Entry->Modified.Month,
                                                              Entry->Modified.Day,
                                                              Entry->Modified.Year,
                                                              Entry->Modified.Hour,
                                                              Entry->Modified.Minute,
                                                              (AM_Flag)?"AM":"PM");

                     /* Add this File to the display list.              */
                     printf("(File) %20s %15s   %s\r\n", temp, size, Entry->Name);
                  }
                  else
                  {
                     /* This entry is a Directory so just add it to the */
                     /* display list.                                   */
                     printf("(Dir.) %20s %15s   %s\r\n", "", "", Entry->Name);
                  }
               }
            ...
            printf("\r\nFTPM>");
            break;
         case etFTPMChangeDirectoryConfirmation:
            /* A Change Directory Confirmation was received, display the*/
            /* appropriate information.                                 */
            printf("\r\n");
            printf("etFTPMChangeDirectoryConfirmation, FTPConnectionID: 0x%04X, Success: %s\r\n", EventData->EventData.ClientChangeDirectoryConfirmationEventData.FTPConnectionID,
                                                                                         (EventData->EventData.ClientChangeDirectoryConfirmationEventData.Success?"TRUE":"FALSE"));

            /* Check to see if the command was successfully executed.   */
            if(EventData->EventData.ClientChangeDirectoryConfirmationEventData.Success)
            {
               /* The command was successfully executed, now determine  */
               /* if the Change Directory was up to the Parent Directory*/
               /* or if it was to a Sub-Directory.                      */
               if(EventData->EventData.ClientChangeDirectoryConfirmationEventData.ChangedDirectoryName)
               {
                  /* The Directory was changed to a Sub-Directory.  Add */
                  /* the Path information to the Current Path array and */
                  /* update the Path Edit Box.                          */
                  i = strlen(CurrentPath);
                  if(CurrentPath[i - 1] != '/')
                     strcat(CurrentPath, "/");

                  strcat(CurrentPath, EventData->EventData.ClientChangeDirectoryConfirmationEventData.ChangedDirectoryName);

                  InRootDirectory = 0;
               }
               else
               {
                  /* The Directory was changed to the Parent Directory. */
                  i = strlen(CurrentPath) - 1;

                  /* We need to remove the the current sub-directory    */
                  /* from the Current Path String.                      */
                  while((i >= 0) && (CurrentPath[i] != '/'))
                     CurrentPath[i--] = '\0';

                  /* If we do not have any Path Information left then   */
                  /* we *MUST* be at the Root Directory.                */
                  if(i)
                     CurrentPath[i] = '\0';
                  else
                     InRootDirectory = 1;
            ...
            printf("\r\nFTPM>");
            break;
         case etFTPMDirectoryDeleteConfirmation:
            /* A Directory Delete Confirmation was received, display any*/
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMDirectoryDeleteConfirmation, FTPConnectionID: 0x%04X, Success: %s, Directory Not Empty: %s\r\n", EventData->EventData.ClientDirectoryDeleteConfirmationEventData.FTPConnectionID,
                                                                                                                  (EventData->EventData.ClientDirectoryDeleteConfirmationEventData.Success?"TRUE":"FALSE"),
                                                                                                                  (EventData->EventData.ClientDirectoryDeleteConfirmationEventData.DirectoryNotEmpty?"TRUE":"FALSE"));
            printf("   Deleted Directory: ");
            printf("%s", EventData->EventData.ClientDirectoryDeleteConfirmationEventData.DeletedDirectoryName);
            printf("\r\n");
            printf("\r\nFTPM>");
            break;
         case etFTPMDirectoryCreateConfirmation:
            /* A Directory Create Confirmation was received, display any*/
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMDirectoryCreateConfirmation, FTPConnectionID: 0x%04X, Success: %s\r\n", EventData->EventData.ClientDirectoryCreateConfirmationEventData.FTPConnectionID,
                                                                                         (EventData->EventData.ClientDirectoryCreateConfirmationEventData.Success?"TRUE":"FALSE"));
            printf("   Created Directory: ");
            printf("%s", EventData->EventData.ClientDirectoryCreateConfirmationEventData.CreatedDirectoryName);
            printf("\r\n");

            /* Check to see if the command was issued successfully.     */
            if(EventData->EventData.ClientDirectoryCreateConfirmationEventData.Success)
            {
               /* The command was issued successfully.  When a Directory*/
               /* is Created the Current Directory is automatically     */
               /* changed to the new directory so update the Current    */
               /* Path.                                                 */
               i = strlen(CurrentPath);
               if(CurrentPath[i - 1] != '/')
                  strcat(CurrentPath, "/");

               strcat(CurrentPath, EventData->EventData.ClientDirectoryCreateConfirmationEventData.CreatedDirectoryName);
            ...
            printf("\r\nFTPM>");
            break;
         case etFTPMFileDeleteConfirmation:
            /* A File Delete Confirmation was received, display any     */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMFileDeleteConfirmation, FTPConnectionID: 0x%04X, Success: %s\r\n", EventData->EventData.ClientFileDeleteConfirmationEventData.FTPConnectionID,
                                                                                    (EventData->EventData.ClientFileDeleteConfirmationEventData.Success?"TRUE":"FALSE"));
            printf("   Deleted File: ");
            printf("%s", EventData->EventData.ClientFileDeleteConfirmationEventData.DeletedFileName);
            printf("\r\n");

            printf("\r\nFTPM>");
            break;
         case etFTPMFileCreateConfirmation:
            /* A File Create Confirmation was received, display any     */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMFileCreateConfirmation, FTPConnectionID: 0x%04X, Success: %s\r\n", EventData->EventData.ClientFileCreateConfirmationEventData.FTPConnectionID,
                                                                                    (EventData->EventData.ClientFileCreateConfirmationEventData.Success?"TRUE":"FALSE"));
            printf("   Created File: ");
            printf("%s", EventData->EventData.ClientFileCreateConfirmationEventData.CreatedFileName);
            printf("\r\n");

            printf("\r\nFTPM>");
            break;
         case etFTPMFilePutConfirmation:
            /* A Put File Confirmation was received, display any        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMFilePutConfirmation, FTPConnectionID: 0x%04X, Success: %s\r\n", EventData->EventData.ClientFilePutConfirmationEventData.FTPConnectionID,
                                                                                 (EventData->EventData.ClientFilePutConfirmationEventData.Success?"TRUE":"FALSE"));
            printf("   Transfer Complete: %s, Total Length: 0x%08X, Transferred Length: 0x%08X\r\n", (EventData->EventData.ClientFilePutConfirmationEventData.TransferComplete?"TRUE":"FALSE"),
                                                                                                     EventData->EventData.ClientFilePutConfirmationEventData.TotalLength,
                                                                                                     EventData->EventData.ClientFilePutConfirmationEventData.TransferredLength);
            printf("   File: ");
            printf("%s", EventData->EventData.ClientFilePutConfirmationEventData.FileName);
            printf("\r\n");

            printf("\r\nFTPM>");
            break;
         case etFTPMFileGetConfirmation:
            /* A Get File Confirmation was received, display any        */
            /* relevant information.                                    */
            printf("\r\n");
            printf("etFTPMFileGetConfirmation, FTPConnectionID: 0x%08X, Success: %s\r\n", EventData->EventData.ClientFileGetConfirmationEventData.FTPConnectionID, (EventData->EventData.ClientFileGetConfirmationEventData.Success?"TRUE":"FALSE"));

            /* Note that since we passed in 0 for Total Length we will  */
            /* not display the Total Length so that the user doesn't    */
            /* have to keep seeing zero for this value.                 */
            printf("   Transfer Complete: %s, Transferred Length: 0x%08X", (EventData->EventData.ClientFileGetConfirmationEventData.TransferComplete?"TRUE":"FALSE"),
                                                                           EventData->EventData.ClientFileGetConfirmationEventData.TransferredLength);
            printf("\r\n");
            printf("   File: ");
            printf("%s", EventData->EventData.ClientFileGetConfirmationEventData.FileName);
            printf("\r\n");
