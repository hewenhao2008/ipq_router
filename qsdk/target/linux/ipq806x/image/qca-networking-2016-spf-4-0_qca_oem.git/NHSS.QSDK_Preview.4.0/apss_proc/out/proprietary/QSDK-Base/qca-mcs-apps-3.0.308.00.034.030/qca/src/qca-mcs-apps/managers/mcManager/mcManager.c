	...
	struct mcManagerEncapTable **encapT)
{
	...
	if (!encapEntry) {
		...
		if (mcManagerUpdateEncapEntry(QCADevMAC, encapEntry, mdbEntry) == MCS_NOK) {
			mcManagerDebug(DBGERR, "%s: Update Encap Entry failed\n", __func__);
			return MCS_NOK;
		}
	}
	...
	struct mcManagerFloodTable **floodT)
{
	...
	if (!floodEntry) {
		...
		if (mcManagerUpdateFloodEntry(floodEntry, mdbEntry) == MCS_NOK) {
			mcManagerDebug(DBGERR, "%s: Update Flood Entry failed\n", __func__);
			return MCS_NOK;
		}
	}
...
MCS_STATUS mcManagerIFTUpdate(struct __mc_mdb_entry *mdbEntry, struct mcManagerIFTable **ifT)
{
	...
	if (!ifEntry) {
		...
		if (mcManagerUpdateIFEntry(ifEntry, mdbEntry) == MCS_NOK) {
			mcManagerDebug(DBGERR, "%s: Update IF Entry failed\n", __func__);
			return MCS_NOK;
		}
	}
