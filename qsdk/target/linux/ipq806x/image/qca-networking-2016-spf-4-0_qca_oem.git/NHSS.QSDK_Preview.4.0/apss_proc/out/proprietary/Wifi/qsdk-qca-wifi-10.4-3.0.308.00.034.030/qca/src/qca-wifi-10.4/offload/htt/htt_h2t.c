...
htt_h2t_frag_desc_bank_cfg_msg(struct htt_pdev_t *pdev)
{
    ...
    *msg_word = 0;
    ...
    if (pdev->nss_wifiol_ctx) {
        if (osif_ol_nss_htc_send(pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, msg, pdev->htc_endpoint))
            return A_ERROR;
    } else
#endif
    {
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    if (pdev->nss_wifiol_ctx) {
        status = osif_nss_ol_stats_frame_send(pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, msg);
    } else
#endif
    {
...
#define MAX_MGMT_BUF_PKTS 512 /*TODO: This needs fine tuning */
    ...
    tx_params += HTT_MGMT_FRM_HDR_DOWNLOAD_LEN/4;
    ...
    if (tid == HTT_TX_EXT_TID_NONPAUSE) {
        ...
        if (subtype_index <= (IEEE80211_FC0_SUBTYPE_DEAUTH >> IEEE80211_FC0_SUBTYPE_SHIFT)){
            if(vdev->txpow_mgt_frm[subtype_index] != 0xFF){
                power = (power <= vdev->txpow_mgt_frm[subtype_index])? power : vdev->txpow_mgt_frm[subtype_index];
            } else{
                power = (power <= pdev->txpow_mgt_frm[subtype_index])? power : pdev->txpow_mgt_frm[subtype_index];
            }
        }
        if(power == 0){
        ...
        if (subtype_index <= (IEEE80211_FC0_SUBTYPE_DEAUTH >> IEEE80211_FC0_SUBTYPE_SHIFT)) {
            if(vdev->txpow_mgt_frm[subtype_index] != 0xff) {
                ...
                power = pdev->txpow_mgt_frm[subtype_index];
            }
            if (power != 0xFF) {
                HTT_MGMT_TX_DESC_PARAMS_POWER_SET(*tx_params, power&0xFF);
                HTT_MGMT_TX_DESC_PARAMS_EXTENSION_SET(*tx_params, 1);
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    if (pdev->nss_wifiol_ctx) {
        status = osif_nss_ol_mgmt_frame_send(pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, desc_id, msg);
    } else {
