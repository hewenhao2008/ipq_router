    ...
    switch (wifi_metadata->pkt_type) {
        ...
        case NSS_WIFI_VDEV_EXT_TX_COMPL_PKT_TYPE: {
            ...
            if (!vdev->pdev->htt_pdev->htt_process_data_tx_frames){
                printk("Drop frame !!.. HTT Function pointer NULL\n");
                dev_kfree_skb_any(skb);
                break;
            }
            ...
            if (qdf_nbuf_headroom(skb) < sizeof(struct tx_capture_hdr)) {
                printk("Less Head room : Drop Frame!!\n");
                dev_kfree_skb_any(skb);
                break;
            }
...
int32_t osif_nss_ol_vap_create(struct ol_txrx_vdev_t * vdev, struct ieee80211vap *vap, osif_dev *os_dev, uint32_t if_num)
{
    ...
    if (di_type == NSS_DYNAMIC_INTERFACE_TYPE_NONE) {
        printk("%s() di returned invalid type : %d\n",__FUNCTION__, di_type);
        return -1;
    }
    ...
    if (status != NSS_TX_SUCCESS) {
        printk("%s() Unable to register the vdev with nss\n", __FUNCTION__);
        goto dealloc;
    }
    ...
    if (status != NSS_TX_SUCCESS) {
        printk("%s() Unable to send the vdev config message to NSS\n",__FUNCTION__);
        goto unregister;
    }
    ...
    if (ret == 0) {
        printk("%s() Waiting for vdev config msg ack timed out\n", __FUNCTION__);
        goto unregister;
    }
    ...
    if (NSS_TX_FAILURE == osif_nss_vdev_cfgp.response) {
        ...
        printk("%s() Received nack for vdev config msg\n", __FUNCTION__);
        goto unregister;
    }
    os_dev->nss_ifnum = if_num;
...
void osif_nss_vdev_dealloc(osif_dev *osif, int32_t if_num) {
    ...
    printk("Dealloc Dynamic interface Node: %d of type: %d radio_id: %d\n", if_num, di_type, radio_id);
...
int  osif_nss_ol_vap_down(osif_dev *osif) {
    ...
    if ((nss_ifnum == -1) || (nss_ifnum == NSS_PROXY_VAP_IF_NUMBER)) {
        printk("%s() vap down called on invalid interface nss_ifnum %d\n", __FUNCTION__, nss_ifnum);
        return -1;
    }
