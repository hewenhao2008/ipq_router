...
ath_set_config(ath_dev_t dev, ath_param_ID_t ID, void *buff)
{
    ...
    u_long bt_weight;
#endif
    u_int32_t framesubt_txpow = *(u_int32_t *)buff;
    int frame_subtype = 0;
    ...
    switch(ID)
    {
        ...
        case ATH_PARAM_DELAY_STAVAP_UP:
            ...
            break;
#endif
        case ATH_PARAM_TXPOW_MGMT:
            {
                frame_subtype = (framesubt_txpow >> 8);
                transmit_power = (framesubt_txpow &  0x00FF);
                sc->txpow_mgt_frm[(frame_subtype >> IEEE80211_FC0_SUBTYPE_SHIFT)] = transmit_power;
            }
            break;
...
#define ANI_CCK_ERR_OFFSET 11
       ...
       case ATH_PARAM_DIAG_ENABLE:
        ...
        case ATH_PARAM_TXPOW_MGMT:
          {
              *(int*)buff = sc->txpow_mgt_frm[(value[1] >> IEEE80211_FC0_SUBTYPE_SHIFT)];
          }
          break;
        case ATH_PARAM_TID_OVERRIDE_QUEUE_MAPPING:
